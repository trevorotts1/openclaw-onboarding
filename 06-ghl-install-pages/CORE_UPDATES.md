...
- Viewport minimum: 1440x900. Builder loads inside nested iframes — enter the builder frame with agent-browser `frame @ref` / `frame main` (snapshot-scoped, GATE #12); for cross-origin in-frame drag/click/edit call the frame-scoped entrypoints in tools/ghl_iframe_drag.py (Playwright over the agent-browser CDP), always behind the tools/browser_manager.sh gateway.
