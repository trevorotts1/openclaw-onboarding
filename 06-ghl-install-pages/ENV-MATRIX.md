...
| Docker env-file / workdir | n/a | `docker compose restart` **skips** `env_file` re-read — a changed `.env` needs `docker compose up -d --force-recreate`; a pushed workdir needs `chown -R node` after the push (container runs as `node`, not root) | Operational doctrine, not code inside this skill — recorded here so a VPS build-unit operator does not hit the two documented traps (`docker-compose-restart-vs-up`, `vps-container-workdir-chown-node`) while running a Skill-6 live build inside a container. |
| Session persistence | `~/.agent-browser/` profile + `state save` files persist across runs | Same paths inside the container; persist only if the workdir is on a **mounted volume** | `AB_REAPER_PLAYWRIGHT_DIR` scopes the reaper's cleanup; a VPS build MUST confirm `~/.agent-browser` is on a persistent (not ephemeral-overlay) mount before relying on `state save/load` across container restarts — **[ASSUMED, spec-carried]**: confirm per-box on first VPS live run (§9.4 step 10), not assumed fleet-wide. |
...
5. **The first-hour ground truth (spec §9.4) is run on BOTH one Mac and one
   VPS** before any fix in this family is declared fleet-ready — this unit's
   own gates (below) are the offline proxy for that; a live VPS run is a W2/W4
   concern, not this branch-only unit's.
