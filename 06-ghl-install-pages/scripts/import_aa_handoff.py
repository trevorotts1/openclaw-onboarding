#!/usr/bin/env python3
"""import_aa_handoff.py — U061: checksum-verified import adapter for Skill 6."""
import argparse, hashlib, json, shutil, sys
from pathlib import Path

SKILL_NUMBER=6; SKILL_NAME="ghl-install-pages"
REQUIRED_BASES=["Landing_Page"]
...
def _map(target, deliver_dir):
    ...
    for inp in target.get("inputs",[]):
        ...
        if base=="Landing_Page": lp={"name":base.replace("_"," "),"file":inp["file"],"sha256":inp["sha256"]}
    return {"source":"avatar-alchemist-handoff","skill":6,"skill_name":"ghl-install-pages",
            "purpose":target.get("purpose",""),"page_type":"funnel","landing_page_doc":lp,
            "design_direction_source":"Landing Page intelligence from AA brand intelligence",
            "build_note":"This brief feeds into the GHL browser builder.","client_label":""}

def import_handoff(handoff_path, deliver_dir, out_dir):
    h,hv=_load_handoff(handoff_path)
    if h is None: return None,hv
    t=_find_target(h)
    if t is None: return None,[("AF-IMPORT-NO-TARGET","no target for skill 6")]
    ...
    r["files_copied"]=cp
    (out_dir/"aa-landing-page-brief.json").write_text(json.dumps(r,indent=2)+"\n",encoding="utf-8")
    return r,[]

_FIXT={"Landing_Page":"# Landing Page Intelligence\n\n## Design Direction\nClean layout.\n\n## Copy Framework\nHook -> Solution -> CTA.\n\n## Page Structure\n1. Hero\n2. Social proof\n3. Features\n4. Testimonials\n5. Footer CTA"}
...
def _mk_fixture(tmp, corrupt=False):
    ...
    for base,content in _FIXT.items():
        ...
        if corrupt and base=="Landing_Page": sha="0"*64
    ...
    handoff={"handoff":"avatar-alchemist-downstream","skill":"52-avatar-alchemist","client_label":label,
        "targets":[{"skill_number":6,"skill_dir":"06-ghl-install-pages","skill_name":"ghl-install-pages",
        "purpose":"GHL landing-page install.","inputs":inputs,"supporting":[]}],"notes":[]}
...
def run_self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        elif r.get("landing_page_doc") is None: ok=False; print("FAIL: landing_page_doc missing")
        elif not (o/"aa-landing-page-brief.json").is_file(): ok=False; print("FAIL: output missing")
        else: print("6 OK valid: "+str(len(r["files_copied"]))+" files")
        hp2,dd2=_mk_fixture(tmp/"c",corrupt=True); _,v2=import_handoff(hp2,dd2,tmp/"o2")
        if "AF-IMPORT-CHECKSUM-MISMATCH" in {c for c,_ in v2}: print("6 OK corrupt")
        else: ok=False; print("6 FAIL: corrupt not caught")
        ...
        if "AF-IMPORT-NO-TARGET" in {c for c,_ in v3}: print("6 OK no target")
        else: ok=False; print("6 FAIL: missing target")
        hpa,dda=_mk_fixture(tmp/"da"); import_handoff(hpa,dda,tmp/"oa")
        hpb,ddb=_mk_fixture(tmp/"db"); import_handoff(hpb,ddb,tmp/"ob")
        oa=json.loads((tmp/"oa"/"aa-landing-page-brief.json").read_text())
        ob=json.loads((tmp/"ob"/"aa-landing-page-brief.json").read_text())
        for o in (oa,ob): o.pop("files_copied",None)
        if oa==ob: print("6 OK deterministic")
        else: ok=False; print("6 FAIL: non-deterministic")
    print("6 RESULT: "+("PASS" if ok else "FAIL")); return 0 if ok else 1

def main(argv):
    if "--self-test" in argv: return run_self_test()
    ap=argparse.ArgumentParser(description="Import AA handoff into Skill 6.")
    ...
    if v:
        ...
        return 2
    print("PASS: landing page brief -> "+str(od/"aa-landing-page-brief.json")); return 0
