...
```python
from tools.ghl_image_stage import run_image_pipeline

manifest = run_image_pipeline(
    prompts=page_copy_prompts,   # list of copy-derived image prompts
    run_dir=RUN_DIR,             # evidence root — writes images/ subtree
    location_id=GHL_LOCATION_ID,
    pit_token=location_pit,
)
# manifest is a list of {id, prompt, file, cdn_url, cdn_http_status, used_on_page_id}
# run_image_pipeline raises ImagePipelineError on any failure — never returns silently
```

**The dispatcher MUST call `run_image_pipeline`; it MUST NOT write its own image
loop, call `kie_generate.py` directly, or hand-roll any of the four stages below.**
Ban hand-rolled build scripts for these runs.

Internally `run_image_pipeline` (in `tools/ghl_image_stage.py`) executes four
stages in order:

1. **Generate** — submits each prompt to `api.kie.ai`
   (`gpt-image-2-image-to-image`, 16:9 2K) via `KIE_API_KEY` from the client env
   store; polls until the job completes; downloads the result; **verifies PNG
   magic bytes** (`b"\x89PNG"` — raises `ImagePipelineError` on non-PNG, NEVER
   silently continues or substitutes an SVG stub).
2. **Upload** — pushes each verified PNG to the GoHighLevel media library via
   `tools/ghl_media.py` (`POST services.leadconnectorhq.com/medias/upload-file`,
   Bearer LOCATION PIT, `Version: 2021-07-28`) → captures `{fileId, public url}`.
   Auth-model split: media upload uses the `services.*` origin with a Bearer PIT —
   bare Python is fine (NOT the Cloudflare-1010-gated funnels-builder origin).
3. **Re-verify CDN** — confirms each upload URL returns genuine HTTP 200; logs to
   `logs/asset-cdn.log` (`status=200 | <https url> | OK`). A non-200 CDN URL
   raises `ImagePipelineError`.
4. **Reference in-page** — returns the manifest so the §2.1–2.6 edit splice can
   embed `<img src="<CDN url>">` in the page blob.
...
If `KIE_API_KEY` is absent → `run_image_pipeline` raises `ImagePipelineError`
with `reason:"missing_kie_api_key"` — the build records honest FAIL for images;
it NEVER substitutes an SVG stub or a local placeholder.

(T2 owns `ghl_media.py` + the manifest contract; T4 adds `ghl_image_stage.py`
as the dispatcher-facing entrypoint.)
