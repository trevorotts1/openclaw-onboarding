...
## 4. Key Detection
- [ ] Search all standard secret locations in order: `~/clawd/secrets/.env`, `~/.openclaw/openclaw.json` `env.vars`, `~/.openclaw/.env`, `~/.openclaw/openclaw.json` `env.vars`, `~/.openclaw/.env`, live environment, and `~/.env`.
