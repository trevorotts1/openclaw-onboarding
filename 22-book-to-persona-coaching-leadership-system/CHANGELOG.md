...
- **`persona-categories.json` schema 1.3 -> 1.4:** every one of the 99 personas gains `emotional_register` (string), `audience_resonance` (string), `conversion_style` (string, one of `story-close`/`stack-close`/`logic-close`/`invitation-close`/`challenge-close`) — synthesized from each persona's own already-catalogued `voice_style.tone[]/devices[]/cadence/avoid[]` and `domain[]`, never invented from the author's name (see `pipeline/schema14_register_assignments.py`'s docstring for the per-field rationale). Adds top-level controlled-vocabulary arrays `emotionalRegisterTags` (14), `audienceResonanceTags` (12), `conversionStyleTags` (5 — the operator-pinned canonical closing-style set). `lastUpdated` bumped.
- **New one-time backfill script `pipeline/backfill-schema14-registers.py`** (+ data module `pipeline/schema14_register_assignments.py`): additive, idempotent (re-run is a no-op unless `--force`), fails loud and writes nothing if the post-write catalog doesn't validate clean under `persona_blend.validate_catalog_tags`. Mirrors the v6.17.0 one-time 1.2->1.3 backfill's shape for schema 1.3->1.4.
- **`23-ai-workforce-blueprint/scripts/persona_blend.py`:** `validate_catalog_tags` extended with three new vocab checks — `emotional_register`/`audience_resonance`/`conversion_style` each required-non-empty-string-when-present, membership-checked against `emotionalRegisterTags`/`audienceResonanceTags`/`conversionStyleTags` when that vocab is populated — same subset-of-controlled-vocab pattern `audiences[]`/`topics[]` already use, degrading to a no-op on a pre-1.4 catalog exactly like the v1.3 fields do on a pre-1.3 catalog.
- **`pipeline/orchestrator.py`:** the D6 (`ONB22-DUALITY-TAGS`) `## Duality Tags` block parser/gate/stamp (`_parse_duality_tags_block`/`_validate_duality_tags`/`_register_duality_tags`) widened to carry the three new scalar fields through the SAME all-or-nothing write (a rejection on any one of the seven fields — old or new — omits the whole block, never a half-valid persona entry). `_synthesis_system()` now also dynamically injects the LIVE `emotionalRegisterTags`/`audienceResonanceTags`/`conversionStyleTags` vocabulary into the Phase-3 prompt (vocab-first, omitted when empty — same contract as the existing `audienceTags`/`topicTags` injection).
- **`agent-prompts/synthesis-agent-prompt.md`:** Duality Tags section documents the three new OPTIONAL scalar fields + a VOCAB-FIRST paragraph + updated worked JSON example.
- **VERIFIED GAP CLOSED — `pipeline/persona_fleet.py`'s publish path:** contrary to a naive "the publish path already carries whole-persona fields" assumption, `_ENRICHMENT_FIELDS` (and therefore `CANONICAL_ENTRY_FIELDS`) did NOT include the three new fields — a `publish-personas-to-fleet.sh` run would have silently STRIPPED them on workspace→repo sync even though the D6 pipeline stamps them workspace-side. Fixed: `_ENRICHMENT_FIELDS` extended; `_validate_entry`/`sync_categories` gained the matching vocab-membership checks (`register_vocab`/`resonance_vocab`/`closer_vocab` params), same pattern as the existing `audience_vocab`/`topic_vocab`. Proven fail-first in `tests/unit/persona-fleet-schema14-sync.test.py` (2 of its 4 cases fail against the pre-fix `_ENRICHMENT_FIELDS` tuple).
- **`QC.md` Section 4D:** widened to the seven-field set + the schema-1.4 vocab in the automated check's mini-catalog.
- **`PIPELINE.md`:** Post-Categorization section widened to document all seven fields and the all-or-nothing gate.
- **New tests:** `tests/unit/schema14-enrichment-complete.test.py` (A-U3 ACCEPT (a) — the actual validation script: N/N coverage, `validate_catalog_tags().ok==true` with the extended vocab populated and enforced, `emotional_register` occurrence count >= persona count). `tests/unit/persona-fleet-schema14-sync.test.py` (4 tests: happy-path survive-the-sync, two vocab-first rejections, repo-side-enrichment-preserved). `tests/unit/p4-01-book-to-persona-matcher-selectable-e2e.test.py` gained 2 cases (schema-1.4 happy path + out-of-vocab all-or-nothing rejection) — A-U3 ACCEPT (b), the CI end-to-end synthesis test. `tests/unit/persona-duality-tags-pipeline.test.sh` gained 15 assertions (56 total, was 42) — scalar fields land on Case 2, a new Case 2b (vocab-first scalar rejection), Case 11b (`_synthesis_system()` schema-1.4 vocab injection). All pre-existing tests pass UNMODIFIED (46-test blend-matcher suite, 20-case regression corpus at 100%, P4-02 synergy, A-U2 directive, match-score-log, publish-to-fleet 12/12, asset-freshness, asset-rebuild-gate) — persona-SET count triad still agrees at 99/99/99/99/99 (no persona added/removed, additive fields only).

## v6.18.0 - 2026-07-11 - fix(D6/ONB22-DUALITY-TAGS): pipeline now stamps audience/topic/voice_style/usable_as duality tags on every newly-synthesized persona

Closes the gap left by the v6.17.0 one-time backfill: nothing in the pipeline ever wrote the v1.3 additive `audiences[]`/`topics[]`/`voice_style{}`/`usable_as[]` fields for a persona built AFTER that backfill, freezing the Skill-23 voice-first AUDIENCE+TOPIC blend matcher's (`persona_blend.py`) candidate universe at those 99 personas forever. New personas were fully usable in Coaching Mode / Task Mode but permanently invisible to that matcher.

- **`agent-prompts/synthesis-agent-prompt.md`:** new OPTIONAL `## Duality Tags` section (appended after the 14 mandatory sections) instructing the Phase-3 synthesis model to emit a fenced `json` object carrying `audiences[]`, `topics[]`, `voice_style{summary(required),tone[],devices[],cadence,signature_moves[],avoid[]}`, `usable_as[]` — vocab-first (choose from the live controlled vocabulary, never invent tags) and with an explicit "leave `audiences[]` empty unless genuinely confident" guardrail against fabricated audience-voice claims.
- **`pipeline/orchestrator.py`:** `_synthesis_system()` now dynamically appends the LIVE `audienceTags[]`/`topicTags[]` vocabulary (read fresh from the canonical `persona-categories.json`) to the Phase-3 system prompt so the model can actually see what "vocab-first" means at synthesis time; omitted entirely when the vocab is empty (pre-enrichment 1.2 catalog). New `_parse_duality_tags_block` / `_validate_duality_tags` / `_register_duality_tags` parse the model's block out of the finished blueprint and gate it through `persona_blend.validate_catalog_tags` — the SAME authoritative validator the Skill-23 matcher enforces at read-time (imported directly from `23-ai-workforce-blueprint/scripts/persona_blend.py`, with a defensive local structural fallback when skill 23 isn't installed alongside skill 22). Wired into the existing Phase-6 `_append_persona_to_categories` write, immediately before the P13-2 schema-lint gate.
- **Additive, never-to-zero contract:** an absent block is a NO-OP (matches `persona_blend.py`'s own pre-enrichment semantics); a malformed/rejected block (bad JSON, an audience/topic tag not already in a populated vocab, a non-enum `usable_as`, a `voice_style` missing its required `summary`) is reported LOUD (`_DUALITY_TAG_WRITE_FAILURES` / `pipeline_had_duality_tag_failures()`) and its fields are OMITTED — never written half-valid, and never blocking the load-bearing `domain[]`/`perspective[]` registration (kept deliberately separate from the F1.4 `PHASE6_CATEGORIES_EXIT_CODE` accumulator, which governs core routing, not this optional enrichment layer).
- **`QC.md`:** new SECTION 4D — Duality Tag Verification (WARN-level automated + manual check calling the same `persona_blend.validate_catalog_tags` gate) + Anti-Pattern 9 (Fabricated Duality Tags) + Section 6 pass-criteria bullets.
- **`PIPELINE.md`:** documents the new Post-Categorization sub-step, the entry-format additions, and the additive/never-to-zero contract.
- **`pipeline/persona_fleet.py`'s publish path is UNCHANGED** — it already carried these four fields workspace→repo (v6.17.0); this release is what actually starts populating them for new personas.
- **New test:** `tests/unit/persona-duality-tags-pipeline.test.sh` (42 assertions: no-op on absence, vocab-first accept/reject, malformed-JSON/no-fence/bad-enum/missing-summary rejection, never-to-zero core registration, defensive fallback when `persona_blend` isn't importable, `_synthesis_system()` dynamic vocab injection). Existing `phase6-categories-fail-loud.test.sh` (21/21), `orchestrator-embed-fail-exit8.test.py` (2/2), `persona-fallback-invariant.test.py` (8/8), `publish-personas-to-fleet.test.sh` (12/12), and skill 23's `test-persona-blend-matcher.py` (42/42) all still pass unmodified — no regressions.
