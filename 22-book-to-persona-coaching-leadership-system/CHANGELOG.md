...
## v6.16.0 - 2026-07-05 - feat(F3.8/Q3, DEP-6): CODE + IMAGE craft-slot library coverage — first `software-craft` persona + IMAGE-slot surfacing

Slot-based work (a website/app build decomposed into CODE / IMAGE / CONTENT sub-tasks — DEP-4 matcher slots, DEP-5 dispatch render) had no true CODE or IMAGE craft specialist: the selector's `engineering` dept resolved only to nearest-domain proxies (`productivity-systems` / `strategy-innovation` / `operations`), and `design`/image work was carried solely by the sketchnote/visual-thinking specialist. A decomposed CODE or IMAGE slot could not resolve to a genuine craft persona. This adds the missing coverage, embedding the CODE source + controlled-vocab tag BEFORE the slot-consuming matcher (DEP-4) and dispatch (DEP-5) trains so their slots resolve to a real specialist, not a proxy.

- **CODE — new `software-craft` controlled-vocab domain tag, added in ONE change** across the four tables that must move in lockstep: `persona-categories.json` `domainTags` (the vocabulary `pipeline/persona_fleet.py sync-categories` validates against — data-driven, no code change there), and the selector's `DEPT_DOMAIN_TAGS['engineering']` + `_CATEGORY_DOMAINS['code']` + `CRAFT_PRIMARY_DOMAINS['code']` (in `23-ai-workforce-blueprint/scripts/persona-selector-v2.py`, tracked at the repo version — its half is logged there, not here), with the `build-workforce.py` `dept_to_domains['engineering']` mirror kept byte-identical so the matrix and the selector pre-qualify the same pool. `software-craft` is listed FIRST as the craft-defining primary; the pre-DEP-6 build/ops/idea-framing proxies remain so existing engineering personas still qualify (never-to-zero).
- **CODE — new persona `hunt-thomas-pragmatic-programmer`** (Andrew Hunt & David Thomas, *The Pragmatic Programmer*), the library's first true software-craft specialist, built via the normal Skill-22 dual-mode blueprint (Section 3 developer coaching + Section 4 engineering governance) and registered with the `software-craft` domain. A decomposed CODE slot now earns the full craft-domain bonus on the real engineering specialist instead of a proxy.
- **IMAGE — surface/tag only, NO new source.** `budelmann-brand-identity-essentials` is designated the IMAGE-craft slot specialist (form/color/imagery/typography) via its existing `visual-communication` + `visual-storytelling` domains, now that `visual-communication` joins `visual-storytelling` as a `design` craft-primary in the selector. `opara-color-works` is unioned in via specialty (custom-tag) recall for colour-heavy image tasks. Both carry `_craft_slot` annotations; the incumbent design specialist (`rohde-the-sketchnote-workbook`) is NOT displaced — it still wins on `visual-storytelling` task-fit.
- **Triad re-baselined 81 → 82** for the one CODE add: `INDEX-MANIFEST.json` `persona_count`/`canonical_persona_count` → 82 with `asset_rebuild_required: true` (the published v2.3.0 asset still embeds 81; the +1 delta must be embedded via `shared-utils/prebuilt-index/build-and-publish.sh` HASH-SKIP before the fleet points at a v2.4.x asset — `base_tag`/`base_sha256`/`base_asset_url` pin the still-served base). The N38 count triad (blueprint dirs == categories keys == persona_count == canonical_persona_count) holds at 82 in this single mergeable change.
- **Test:** `tests/unit/dep6-craft-slot-coverage.test.py` — offline, stdlib-only contract test: `software-craft` in vocab; hunt-thomas built + registered + tagged; the four selector tables carry the tag in one change; the CODE specialist earns a craft bonus and every non-`software-craft` persona earns **zero** (inertness); budelmann + opara earn the IMAGE craft bonus while rohde is not displaced; the N38 triad agrees at 82; and the `build-workforce.py` mirror carries `software-craft`.
