...
- [ ] Section includes `python3 ~/.openclaw/scripts/gemini-search.py "<task keywords>"` as the runtime query pattern
...
- [ ] Entry references `python3 ~/.openclaw/scripts/gemini-indexer.py --status` for live count
...
- Expected: Before any professional task, run `python3 ~/.openclaw/scripts/gemini-search.py "<task keywords>"`, load the returned persona's Task Mode, and execute through that methodology. Skip ONLY if the user explicitly says so.
...
- Expected: `python3 ~/.openclaw/scripts/gemini-indexer.py` then `# Handled by gemini-indexer.py`
...
**Expected behavior:**
1. Agent runs `python3 ~/.openclaw/scripts/gemini-search.py "sales email review outreach quality standard"` BEFORE writing any output
...
**Run:** `python3 ~/.openclaw/scripts/gemini-indexer.py --status` 
...
**Run:** `python3 ~/.openclaw/scripts/gemini-search.py "habit building systems behavior change consistency"`
...
- [ ] Confirmed: Agent queries `python3 ~/.openclaw/scripts/gemini-search.py` before executing professional tasks
...
[ ] PARTIAL — File structure complete, but Gemini Engine not yet embedded or core files not yet updated
    Action: Complete Gemini Engine setup (python3 ~/.openclaw/scripts/gemini-indexer.py) and apply CORE_UPDATES.md
...
```bash
python3 ~/.openclaw/scripts/gemini-indexer.py --rebuild && python3 ~/.openclaw/scripts/gemini-indexer.py
```
