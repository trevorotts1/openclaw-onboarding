...
S22="$SKILLS_DIR_DEFAULT/22-book-to-persona-coaching-leadership-system"
assert "Skill 22 folder present"                        "[ -d \"$S22\" ]"
assert "INSTRUCTIONS.md present (TYP read-order req)"   "[ -f \"$S22/INSTRUCTIONS.md\" ]"
assert "pipeline/orchestrator.py present"               "[ -f \"$S22/pipeline/orchestrator.py\" ]"
assert "extraction-agent-prompt.md present"             "[ -f \"$S22/agent-prompts/extraction-agent-prompt.md\" ]"
assert "analysis-agent-prompt.md present"               "[ -f \"$S22/agent-prompts/analysis-agent-prompt.md\" ]"
assert "synthesis-agent-prompt.md present"              "[ -f \"$S22/agent-prompts/synthesis-agent-prompt.md\" ]"
assert "playbook-appendix-prompt.md present (Phase 3b)" "[ -f \"$S22/agent-prompts/playbook-appendix-prompt.md\" ]"
assert "orchestrator wires Phase 3b appendix"           "grep -q 'run_playbook_appendix' \"$S22/pipeline/orchestrator.py\""
assert "orchestrator enforces appendix floor"           "grep -q '_validate_appendix' \"$S22/pipeline/orchestrator.py\""
