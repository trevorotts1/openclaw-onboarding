...
def _match_key(name):
    ...
    ("SDR (Sales Development Rep)") collapse to the same key. Drops
    parentheticals + punctuation, lowercases, collapses spaces."""
    n = re.sub(r"\([^)]*\)", " ", name or "")     # drop "(Sales Development Rep)"
...
def _first_sentence(text, cap=160):
    """First sentence of `text`, capped at `cap` chars on a WORD boundary.

    Never cuts mid-word and never leaves a "..." dangling after a partial word;
    when it has to shorten, it trims back to the last whole word and ends with a
    period so the line reads as a complete thought.
    """
    t = (text or "").replace("\n", " ").strip()
    if not t:
        return ""
