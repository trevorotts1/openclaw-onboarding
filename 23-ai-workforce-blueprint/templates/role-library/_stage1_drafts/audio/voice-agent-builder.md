...
| **OpenAI API / Anthropic API** | LLM reasoning layer for agents (if using bring-your-own-model instead of the platform's built-in LLM) | platform.openai.com or console.anthropic.com, API access | System prompt engineering is the most critical skill. Voice agent prompts differ from chat prompts: they must be concise (shorter responses = lower latency and cost), include explicit error recovery instructions, define interruption handling, and specify conversation-closing logic. |
...
- **OpenAI API Documentation** (platform.openai.com/docs) — GPT-4o and GPT-4o Realtime API documentation for voice-optimized LLM usage.
- **Anthropic API Documentation** (docs.anthropic.com) — Claude API documentation for LLM prompt engineering.
...
10. {{COMPANY_NAME}} adopts a new primary LLM for voice agents (e.g., switching from GPT-4o to Claude, or adopting a new voice-optimized model). Prompt engineering practices in SOP 9.2 and all agent system prompts must be updated.
