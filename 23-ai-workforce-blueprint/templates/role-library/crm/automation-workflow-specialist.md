...
| **Automation template library + matcher (REUSE-FIRST)** | The 28 proven sequences you match against in SOP 9.6 STEP 0.4 — `sequence`, `copy_persona`, `ghl_build`, `flexibility` per template | `44-convert-and-flow-operator/automation-templates/` + `_matcher/cli.py --match` | Template-first / reuse-before-reinvent; guide-not-rule (honor explicit spec; CREATE_NEW + save_new_template only when nothing fits) |
| **Funnel→automation link map** | When the task carries `funnel_template_id`, names the recommended primary/supporting automations for that funnel | `44-convert-and-flow-operator/automation-templates/_links/funnel-to-automation.json` | Recommended defaults, never mandatory; expand via `automation_matcher.expand_funnel_to_automations` |
| **Email Engine + superlibrary (Skill 50)** | Deploy APPROVED email copy as DRAFT-ONLY GHL workflow email steps (this role wires what the engine authored) | `50-email-engine/` -- `tools/email_matcher_cli.py --match` selects framework/buyer-type/objective/persona/sequence; `tools/prove-email.py` QCs (fail-closed); draft-only deploy via Skill 44 | Client runtime uses the CLIENT's own provider (never Anthropic); the deterministic gates (email_matcher.py, prove-email.py) are provider-neutral; nothing sends without human approval. |
| **Social Media in a Box (Skill 57)** | When a GHL workflow must fire from a social comment/DM keyword — wire the automation to the `commentKeyword`/`ctaLink` the engine posts (I12); never fork a second poster to "trigger" anything. | `bash 57-social-media-in-a-box/social-media-entry.sh --run-dir DIR --mode <mode>` — the ONE front door (same DEPS/BYPASS-SCAN/HASH-PIN gates + run-scoped nonce) | Fail-closed provers + signed certificate; client providers only (never Anthropic); GHL-direct posting; never hand-roll a poster (BYPASS-SCAN blocks it). |
| **GHL Form Builder dependency (Skill 6 + Skill 44)** | **Skill 6 is the ONE GHL delivery rail — FUNNELS / WEBSITES / SURVEYS / FORMS.** Build the on-submit **`Form Submitted → Add Contact Tag` workflow** AFTER the form ID exists (the form builder has NO native add-tag control) | Skill 44 `caf` WF gates (WF-1..21 + rubric ≥ 8.5): trigger `Form Submitted` (this form) → action `Add Contact Tag` → `zhc_<slug>`. The `zhc_` fields/tags are pre-created (Skill 44, GET-first) before the browser build. Shared procedure: `universal-sops/form-craft/` | The workflow is DRAFT unless approved; never create tags on the fly; never duplicate an existing `zhc_` object. |
...
|---|---|---|---|
| **44** convert-and-flow-operator | "build a workflow" · "automate this" · "an order-bump" | `~/.openclaw/skills/44-convert-and-flow-operator/` | `universal-sops/funnel-automation-build-quality-rubric.md/` |
...
1. **STEP 0 — Model pre-flight (before you touch anything that writes).**
   ...
   - Verify: If the active model is a lighter/faster model (mini, flash, deepseek-flash, or any non-high-reasoning model) OR thinking is not HIGH, send the owner this exact recommendation ONCE: "GHL workflow builds are complex and error-prone on lighter models. It is HIGHLY RECOMMENDED to switch to a high-reasoning model (e.g. deepseek-v4-pro or GLM 5.2) with thinking set to HIGH before proceeding. Say 'proceed anyway' to continue, or switch first and re-run." Do not repeat it the same session after acknowledgement.
...
1.5. **STEP 0.4 — Template-first automation match — reuse before reinvent (MANDATORY; flexibility = guide-not-rule).** Before PLAN MODE builds the strategist's outline from scratch, consult the shipped **automation template library** — 28 proven email/SMS/multichannel sequences in `44-convert-and-flow-operator/automation-templates/` (categories: welcome-indoctrination, sales-close-sequences, engagement-broadcast, funnel-specific-followups, multichannel-automation), each carrying the `sequence`, `copy_persona`, `ghl_build`, and `flexibility` contract.
   - Run the matcher (this is Skill-44 `INSTRUCTIONS.md` **Step 0.4**): `python3 44-convert-and-flow-operator/automation-templates/_matcher/cli.py --match "<outcome + channel + audience>" --json` (reads the committed `_matcher/catalog-index.json`).
   - **If the task carries a `funnel_template_id`** (from the Funnel Strategist's funnel-spec.json / the P4→P5 handoff), read `_links/funnel-to-automation.json` for the recommended primary + supporting automations for that funnel and expand them (`automation_matcher.expand_funnel_to_automations`).
   - **Honor the flexibility contract:** the template is a GUIDE/RESOURCE, never a rule. EXPLICIT user spec → build THAT (template = optional reference). UNSURE → suggest + await confirm. HANDS-OFF → build from the template. Nothing fits → CREATE_NEW, then `save_new_template` so the library grows. Never blocks a build; never imposes a template over an explicit spec.
   - When a template is matched, instantiate the workflow plan FROM it (`instantiate_workflow`) and carry it into PLAN MODE as the proven skeleton — adapt copy/timing to the client's pinned A2 values rather than re-deriving the node graph.
   - Failure/abort: matching is advisory; a matcher miss never blocks the build — proceed to PLAN MODE net-new.
