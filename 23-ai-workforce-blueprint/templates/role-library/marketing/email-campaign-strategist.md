...
0c. Select the email-copy persona **from the ranked search result** (do not default by habit). The email-copy pool includes: `bly-copywriters-handbook` (direct-response sequences), `wiebe-copy-hackers` (voice-of-customer subject lines), `jim-edwards-copywriting-secrets` (subject-line hooks, headline/CTA formulas, short-form sales copy), `hormozi-100m-offers` (value-stack nurture and offer emails), `russell-brunson-lead-funnels` (welcome / lead-nurture email sequences tied to a funnel), and `russell-brunson-marketing-secrets-blackbook` (offer-narrative broadcast emails). Ground the subject line approach, opening hook, and CTA framing in the **selected** (top-ranked) persona's methodology.
