...
| **Social Media in a Box (Skill 57)** | When the weekly social-week digest email ships (`--mode newsletter`, C4 — GHL Campaigns, Tue 9 AM client-timezone, subject ≤60 / preview ≤120 bands). Boundary: Skill 50 remains your general marketing-email superlibrary; 57's `newsletter` is ONLY the social-week digest (§6.5). | `bash 57-social-media-in-a-box/social-media-entry.sh --run-dir DIR --mode <mode>` — the ONE front door (same DEPS/BYPASS-SCAN/HASH-PIN gates + run-scoped nonce) | Fail-closed provers + signed certificate; client providers only (never Anthropic); GHL-direct posting; never hand-roll a poster (BYPASS-SCAN blocks it). |
...
|---|---|---|---|
| **50** email-engine | "write my email sequence" · "nurture emails" · "a welcome series" | `~/.openclaw/skills/50-email-engine/` | `universal-sops/email-craft/` |
...
0c. Select the email-copy persona **from the ranked search result** (do not default by habit). The email-copy pool includes: `bly-copywriters-handbook` (direct-response sequences), `wiebe-copy-hackers` (voice-of-customer subject lines), `edwards-copywriting-secrets` (subject-line hooks, headline/CTA formulas, short-form sales copy), `hormozi-100m-offers` (value-stack nurture and offer emails), `russell-brunson-lead-funnels` (welcome / lead-nurture email sequences tied to a funnel), `brunson-marketing-secrets-blackbook` (offer-narrative broadcast emails), and `pedro-adao-challenge-secrets-masterclass` (challenge reminder + "we're live" + daily-recap + offer-window promo + cart-close follow-up sequences across email/SMS/Messenger for a 5-day challenge launch). Ground the subject line approach, opening hook, and CTA framing in the **selected** (top-ranked) persona's methodology.
