...
The enforcement pipeline for this is the Loop Protection System (Skill 61), which you OPERATE — you no longer hand-roll the detection. Its D1 restart-velocity detector catches F5-class restart storms (Loop class LP-B1) and F4-class kill-spawn races (LP-B2); its D4 detector catches the gateway deferral deadlock / orphan-`:18789` handoff (LP-B3) and the invalid-config engine freeze (LP-B5); its **process circuit breaker** trips at 10 restarts / 15 min (or 40 / day) and PARKS the unit visible-red instead of letting it respawn 4,528 times — the structural fix for the storm the old status quo could not stop. The kill cards you reach for: LF-6 (process-unit park + boot-log capture) for F4/F5, LF-3 (orphan-gateway clear → kickstart → pid-stability verify) for the deferral deadlock, LF-7 (config restore from last-good snapshot) for the freeze. Skill 61's watchdog lives at host level OUTSIDE every OpenClaw session — the same Box B law your own watchdog cron follows — so it survives the very wedges it treats. Run `bash ~/.openclaw/skills/61-loop-protection-system/loop-companion.sh audit --local` to see the current restart-velocity and breaker state for every supervised unit.
