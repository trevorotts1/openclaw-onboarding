...
33. **`P9.2-GHL-UPLOAD`** (order 8.9, mechanical: `scripts/ghl_media_push.py`) -- media-librarian-ghl-updater uploads passed assets to the client's GHL media library.
34. **`P9.6-WEBINAR-VIDEO`** (order 8.92, mechanical, resumes the canonical entry script) -- media-librarian-ghl-updater builds the Ken Burns + xfade webinar video slideshow and pushes it to GHL's v3 500MB video tier.
35. **`P7-TELEPROMPTER`** (order 8.95, mechanical: `scripts/build_teleprompter.py`) -- presenters-speech-writer builds the standalone scrolling teleprompter HTML app.
36. **`P9-DELIVER`** (order 9, mechanical: `scripts/synthesize_full_speech.py` + delivery interlock) -- delivery-concierge runs the postflight completeness gate and delivers to every destination (deliverables Trevor opens go to Downloads, clearly labeled).
