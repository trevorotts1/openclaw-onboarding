...
>
> **Executed counts (out of 62 declared):** the count now depends on BOTH deck type AND the two upsell elections — see `DEPARTMENT-COUNTS-CANONICAL.md` for the full mechanically-derived table (deck type x sales/checkout x VSL). With both upsells declined and no signature/content-conversion signals: **39 on a standard from-scratch deck, 43 on a signature deck, 40 on a content-conversion deck**. Do not restate the full matrix here — `DEPARTMENT-COUNTS-CANONICAL.md` is the single source; link to it.

<!-- BEGIN GENERATED: pipeline-sequence -- regenerate with: python3 scripts/ci/phase_doc_sync.py --write -->

### Upsell (P-U) and other conditional phases

All 62 declared phases are listed once, in the sequence below, in the manifest's own `order`. 22 of them are P-U upsell phases. 18 phases declare a `defers_unless` predicate in the manifest; each is marked CONDITIONAL below with that exact predicate, and defers (returns empty, no-op) when it is not met. A decline is a logged client waiver, never silence. The phases that defer through the runner's own filters rather than a `defers_unless` predicate -- `P-CONVERTER`, the four `P-SP-*` signature phases and `P-SPEECH-QC` -- are named in the note above. The sales/checkout and VSL upsell contracts are in [`sops/SALES-CHECKOUT-BUILDER-SOP.md`](sops/SALES-CHECKOUT-BUILDER-SOP.md) and [`sops/VSL-BUILDER-SOP.md`](sops/VSL-BUILDER-SOP.md).
...
10. **`P-U-SALES-COPY`** (order 3.6, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Sales Page Copy (8-section main structure). Owned by `slide-copywriter`. Produces `working/upsell/copy/sales.fragment.md`, `working/upsell/copy/copy_ledger.json`. Full contract: the `P-U-SALES-COPY` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
11. **`P-U-CHECKOUT-COPY`** (order 3.7, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Checkout Page Copy (lean order-form structure). Owned by `slide-copywriter`. Produces `working/upsell/copy/checkout.fragment.md`. Full contract: the `P-U-CHECKOUT-COPY` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
12. **`P-U-VSL-RESEARCH`** (order 3.8, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Research (length, hook-to-reveal timing, gating practice). Owned by `deep-research-specialist-presentations`. Produces `working/upsell/vsl-research.md`. Full contract: the `P-U-VSL-RESEARCH` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
13. **`P-U-VSL-COPY`** (order 3.9, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Page Copy (headline, body, CTA). Owned by `slide-copywriter`. Produces `working/upsell/copy/vsl.fragment.md`. Full contract: the `P-U-VSL-COPY` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
14. **`P4-COPY`** (order 4, fanout by section, max None units) -- slide-copywriter (concurrent: offer-price-strategist) writes `slides_copy.md` + `price_ladder.json`, loading `research_map.json` and weaving each assigned anchor into the copy.
15. **`P-SP-STRUCTURE`** (order 4.1) -- SIGNATURE-ONLY -- signature-presentation-architect locks the SACRED 4-phase arc (Avatar >=11 -> Story >=13 -> Teaching >=36 -> Pitch >=40, >=100 slides floor) with contiguous phase labels, image suggestions on every slide, and the N.E.E.I.T. / 4-Quadrant markers.
16. **`P-SP-P3-HYGIENE`** (order 4.15) -- SIGNATURE-ONLY -- qc-specialist-signature-presentations enforces no-pitch hygiene on the Teaching band: no offer name, price, or enroll/buy/scarcity CTA before the Phase-3-to-Phase-4 bridge.
17. **`P-U-DESIGN-SALES`** (order 4.2, fanout by slide, max None units, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Sales Page Design (5,000–19,000 prompt band). Owned by `slide-image-creator`. Produces `prompts/sales.design.txt`. Full contract: the `P-U-DESIGN-SALES` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
18. **`P1Q-COPY-QC`** (order 4.2) -- qc-specialist-presentations runs the copy QC gate (score >= 8.5). This is the current "Phase 1Q" -- it is sequenced AFTER Slide Copy (order 4.2, not before it); a QC now follows the artifact it grades, which is the defect the O1 re-sequence fixed.
19. **`P-U-DESIGN-RENDER-SALES`** (order 4.21, script: `scripts/build_infographic.py`, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Sales Page Design Render (canonical Kie path, 16:9 2560x1440, 102,400-byte PNG floor) — FIX 28: the PNG producer; the agent phase above authors the prompt, this script phase renders it. Owned by `slide-image-creator`. Produces `design/sales-design.png`. Full contract: the `P-U-DESIGN-RENDER-SALES` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
20. **`P-U-DESIGN-CHECKOUT`** (order 4.3, fanout by slide, max None units, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Checkout Page Design (includes order/payment form design). Owned by `slide-image-creator`. Produces `prompts/checkout.design.txt`. Full contract: the `P-U-DESIGN-CHECKOUT` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
21. **`P-U-DESIGN-RENDER-CHECKOUT`** (order 4.31, script: `scripts/build_infographic.py`, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Checkout Page Design Render (canonical Kie path, 16:9 2560x1440, 102,400-byte PNG floor) — FIX 28: the PNG producer; the agent phase above authors the prompt, this script phase renders it. Owned by `slide-image-creator`. Produces `design/checkout-design.png`. Full contract: the `P-U-DESIGN-RENDER-CHECKOUT` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
22. **`P-U-DESIGN-VSL`** (order 4.4, fanout by slide, max None units, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Page Design (includes email/phone gate form design). Owned by `slide-image-creator`. Produces `prompts/vsl.design.txt`. Full contract: the `P-U-DESIGN-VSL` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
23. **`P-U-DESIGN-RENDER-VSL`** (order 4.41, script: `scripts/build_infographic.py`, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Page Design Render (canonical Kie path, 16:9 2560x1440, 102,400-byte PNG floor) — FIX 28: the PNG producer; the agent phase above authors the prompt, this script phase renders it. Owned by `slide-image-creator`. Produces `design/vsl-design.png`. Full contract: the `P-U-DESIGN-RENDER-VSL` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
24. **`PF-DESIGN`** (order 4.5) -- typography-architect locks `type_system` + `layout_map` + `treatment_table` and runs the self-audit before any prompt is written.
25. **`P-TYPO-QC`** (order 4.6) -- qc-specialist-typography-presentations independently reviews the design brief.
26. **`P4-PROMPT`** (order 4.7, fanout by slide, max None units) -- prompt-author-presentations writes each slide's image prompt to the treatment table, to the 9,000-character standard.
27. **`P-PROMPT-QC`** (order 4.8, fanout by slide, max None units) -- qc-specialist-prompt-presentations independently (dual-scored) reviews the prompts.
28. **`P-STYLE-SPEC`** (order 4.84, fanout by slide, max None units) -- Style Preview Spec (fanout unit: 3 slide-bound units each author one attention-grade variant + representative slide, aggregated to the spec before the 9 samples render). Owned by `brand-steward`. Produces `working/copy/style_preview_spec.json`. Full contract: [`sops/brand-steward-sops.md`](sops/brand-steward-sops.md) and the `P-STYLE-SPEC` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
29. **`P-STYLE-PREVIEW`** (order 4.85, script: `scripts/build_deck.py`) -- slide-image-creator renders 9 samples across 3 style variants; the owner picks 1 via their OWN gateway before the full render, and the winner's 3 representative renders are LOCKED + reused so Kie never double-charges.
30. **`P-STYLE-PICK`** (order 4.86) -- Owner Style Pick (human gateway: owner approves ONE of the 3 variants; choice logged with a verified owner_msg_id). Owned by `brand-steward`. Produces `working/copy/style_preview_choice.json`. Gates: AF-STYLE-UNPICKED. Full contract: [`sops/brand-steward-sops.md`](sops/brand-steward-sops.md) and the `P-STYLE-PICK` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
31. **`P4-RENDER`** (order 4.9, script: `scripts/build_deck.py`) -- slide-image-creator's deterministic render via `build_deck.py` (this script; the ONLY renderer -- Logo via image-to-image on Mode B; hook slides pure-type; no footer hook).
32. **`P-IMAGE-QC`** (order 4.95, fanout by slide, max None units) -- qc-specialist-image-presentations runs the independent multimodal vision QC gate on every rendered slide.
33. **`P-U-HTML-SALES`** (order 5.2, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Sales Page HTML (mirrors design; GHL code-block ready). Owned by `pptx-assembly-specialist`. Produces `pages/sales.fragment.html`. Full contract: the `P-U-HTML-SALES` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
34. **`P-U-HTML-CHECKOUT`** (order 5.3, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Checkout Page HTML (mirrors design; SKILL44_WIDGET form embed). Owned by `pptx-assembly-specialist`. Produces `pages/checkout.fragment.html`. Full contract: the `P-U-HTML-CHECKOUT` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
35. **`P-U-HTML-VSL`** (order 5.4, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Page HTML (mirrors design; gate overlay with Skill-44 form embed). Owned by `pptx-assembly-specialist`. Produces `pages/vsl.fragment.html`. Full contract: the `P-U-HTML-VSL` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
36. **`P-U-FORM-GATE`** (order 5.6, script: `scripts/ghl_external_installer.py`, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL Email/Phone Gate Form + Workflow (Skill 44). Owned by `media-librarian-ghl-updater`. Produces `ecosystem/gate-form.json`, `workflows/gate-workflow.json`. Full contract: the `P-U-FORM-GATE` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
37. **`P-U-GHL-SALES`** (order 6.2, script: `scripts/ghl_external_installer.py`, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes"`) -- Upsell Sales + Checkout GHL Funnel Build (Skill 6). Owned by `media-librarian-ghl-updater`. Produces `working/sales-checkout/ghl_build_receipt.json`. Full contract: the `P-U-GHL-SALES` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
38. **`P-U-GHL-VSL`** (order 6.4, script: `scripts/ghl_external_installer.py`, CONDITIONAL -- defers unless `intake.want_vsl_page == "yes"`) -- Upsell VSL GHL Funnel Build (Skill 6; gate form SKILL44_WIDGET). Owned by `media-librarian-ghl-updater`. Produces `working/vsl/ghl_build_receipt.json`. Full contract: the `P-U-GHL-VSL` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
39. **`P-SHIFT-QC`** (order 7.5) -- qc-specialist-presentations runs the Priority-Shift Ship Gate -- the ONLY phase where copy, design, and rendered images all coexist; the framework's 14-item pre-output checklist (item 0 = the North Star: attention is the #1 job).
40. **`P8-ASSEMBLE`** (order 8) -- pptx-assembly-specialist assembles the final deck PPTX.
41. **`P8.1-PDF-EXPORT`** (order 8.1, script: `scripts/pdf_export.py`) -- exports the deck PDF (portable-document export, required delivery output + QC render).
42. **`P8.2-GUIDE`** (order 8.2, script: `scripts/presenter_guide.py`) -- presenters-guide-specialist builds the speaker-facing outline PDF (font >= 12) + Notion.
43. **`P8.25-WORKBOOK`** (order 8.25, script: `scripts/workbook_builder.py`) -- pptx-assembly-specialist builds the branded content-rich Workbook (regular PDF + fillable AcroForm PDF).
44. **`P8.3-INFOGRAPHIC`** (order 8.3, script: `scripts/build_infographic.py`) -- Infographic Render (canonical Kie path, 9:16 1440x2560, 102,400-byte PNG floor). Owned by `slide-image-creator`. Produces `working/deliverables/infographic.png`. Full contract: [`sops/slide-image-creator-sops.md`](sops/slide-image-creator-sops.md) and the `P8.3-INFOGRAPHIC` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
45. **`P9-SPEECH`** (order 8.5, script: `scripts/speech_build_harness.py`, fanout by slide, max None units) -- presenters-speech-writer writes the word-for-word Presenter's Speech at 130 wpm (length-gated).
46. **`P8.4-FISH-TAG`** (order 8.52, script: `scripts/speech_fish_tag.py`) -- fish-audio-expression-specialist expression-tags the speech for Fish Audio synthesis.
47. **`P9-SPEECH-WEBINAR-INTRO`** (order 8.54, script: `scripts/synthesize_full_speech.py`) -- audio-demonstration-specialist renders the webinarized speech audio (welcome + Q&A + crescendo close framing).
48. **`P9.1-SPEECH-PDF`** (order 8.55, script: `scripts/speech_pdf_chain.py`) -- presenters-speech-writer renders the teleprompter-print Presenter's Speech PDF.
...
54. **`P-U-FORM-CHECKOUT`** (order 8.77, script: `scripts/checkout_form_builder.py`) -- CONDITIONAL, upsell branch, same flag/waiver as `P-U-CHECKOUT-BUILD` -- INTERIM PLACEHOLDER: re-verifies the same `build_receipt.json` P-U-SALES-BUILD produced; no dedicated payment/lead-capture form-wiring implementation exists yet (see `SALES-CHECKOUT-BUILDER-SOP.md` SS4 "THE CHECKOUT FORM" for the proposed follow-up).
...
56. **`P9.2-GHL-UPLOAD`** (order 8.9, script: `scripts/ghl_media_push.py`) -- media-librarian-ghl-updater uploads passed assets to the client's GHL media library.
57. **`P9.6-WEBINAR-VIDEO`** (order 8.92, script: `scripts/build_webinar_video.py`) -- media-librarian-ghl-updater builds the Ken Burns + xfade webinar video slideshow and pushes it to GHL's v3 500MB video tier.
58. **`P-U-VSL-BUILD`** (order 8.93, script: `scripts/vsl_builder.py`) -- CONDITIONAL, upsell branch -- DEFERS unless the client elects it (`want_vsl_page`, default NO, opt-in only; a decline is a logged client waiver) -- media-librarian-ghl-updater builds the video-sales-letter page (kie design -> HTML -> GHL funnel push). MUST run after `P9.6-WEBINAR-VIDEO` (order 8.92) -- `verify_video_dependency()` fails closed (`AF-VSL-NO-VIDEO`) if `{deck_slug}-WEBINAR.mp4` does not exist yet. Template: LOOP2C-VSL-SALES-CHECKOUT-WEBSITE.md.
59. **`P7-TELEPROMPTER`** (order 8.95, script: `scripts/build_teleprompter.py`) -- presenters-speech-writer builds the standalone scrolling teleprompter HTML app.
60. **`P9-DELIVER`** (order 9, script: `scripts/synthesize_full_speech.py`) -- delivery-concierge runs the postflight completeness gate and delivers to every destination (deliverables Trevor opens go to Downloads, clearly labeled).
61. **`P-U-QC`** (order 9.05, CONDITIONAL -- defers unless `intake.want_sales_checkout == "yes" or intake.want_vsl_page == "yes"`) -- Upsell QC + Playwright Verify (brand, logo, mirror, band, copy). Owned by `qc-specialist-presentations`. Produces `qc/upsell-scorecard.json`. Full contract: the `P-U-QC` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
62. **`P-BUNDLE-GATE`** (order 9.95, script: `scripts/bundle_gate.py`) -- Bundle-Completeness Gate (terminal; run_postflight_gate body via bundle_gate.py — exit 5 naming any missing/under-threshold deliverable). Owned by `pptx-assembly-specialist`. Produces `working/checkpoints/bundle_gate.json`. Gates: AF-BUNDLE-COMPLETE. Full contract: [`sops/pptx-assembly-specialist-sops.md`](sops/pptx-assembly-specialist-sops.md) and the `P-BUNDLE-GATE` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).

<!-- END GENERATED: pipeline-sequence -->
