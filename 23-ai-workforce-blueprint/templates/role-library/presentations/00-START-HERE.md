...
14. **`P4-COPY`** (order 4, fanout by section, max 5 units) -- slide-copywriter (concurrent: offer-price-strategist) writes `slides_copy.md` + `price_ladder.json`, loading `research_map.json` and weaving each assigned anchor into the copy.
...
26. **`P4-PROMPT`** (order 4.7, fanout by slide, max 12 units) -- prompt-author-presentations writes each slide's image prompt to the treatment table, to the 9,000-character standard.
27. **`P-PROMPT-QC`** (order 4.8, fanout by slide, max 12 units) -- qc-specialist-prompt-presentations independently (dual-scored) reviews the prompts.
28. **`P-STYLE-SPEC`** (order 4.84, fanout by slide, max 3 units) -- Style Preview Spec (fanout unit: 3 slide-bound units each author one attention-grade variant + representative slide, aggregated to the spec before the 9 samples render). Owned by `brand-steward`. Produces `working/copy/style_preview_spec.json`. Full contract: [`sops/brand-steward-sops.md`](sops/brand-steward-sops.md) and the `P-STYLE-SPEC` row in [`PIPELINE-MANIFEST.json`](../../../../universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json).
...
32. **`P-IMAGE-QC`** (order 4.95, fanout by slide, max 12 units) -- qc-specialist-image-presentations runs the independent multimodal vision QC gate on every rendered slide.
...
45. **`P9-SPEECH`** (order 8.5, script: `scripts/speech_build_harness.py`, fanout by slide, max 12 units) -- presenters-speech-writer writes the word-for-word Presenter's Speech at 130 wpm (length-gated).
