...
| Slide math accuracy | SLIDE_COUNT_FINAL = max(duration_target, source_slide_count) on 100% of runs; Mode A within the duration-derived default target/cap (applied only when the owner gave a duration and no count, per ruling 9.5), Mode B never below source_slide_count; content-driven count kept in full whenever the source carries more substantive points |
...
This department's older documents use NUMERIC short codes ("Phase 1", "Phase 3", "Phase 6") for pipeline stages. Every such short code used anywhere in a role file resolves to exactly one `phases[].id` in `universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json` (manifest_version 67, 62 phases). When dispatching, gating, or reading a manifest row, the MANIFEST ID is the canonical key; the numeric short code is prose shorthand only. This table is the single mapping — if a stage is not in it (owner approval gates, the capacity probe), it is NOT a manifest phase and has no manifest row.

| Legacy short code | Manifest `phases[].id` (order) | Manifest row |
|---|---|---|
| Phase -0.5 (Deep Research dispatch) | `P-0.5-RESEARCH` (order -0.5) | deep-research-specialist-presentations; produces `working/research/brief-[DECK_SLUG].md` |
| Step 0.5 (capacity probe) | NOT a manifest phase | pre-run readiness; writes `capacity_plan.json` (Gate 1 input) |
| Phase A (pre-run readiness) | NOT a manifest phase | Gate 1 condition, before `P0A-INTAKE` |
| Phase B+ (Hook Strategist onward) | any id at order >= `P0B-PRIORITY` (0.2) | dispatch block on `P-0.5-RESEARCH` completion |
| Intake / interview confirm | `P0A-INTAKE` (order 0.1) | director-of-presentations; produces `intake.json` |
| Phase 1 (copy write) | `P4-COPY` (order 4) | slide-copywriter; produces `working/copy/slides_copy.md` |
...
| Upsell waves | `P-U-SALES-COPY` (3.6), `P-U-CHECKOUT-COPY` (3.7), `P-U-VSL-RESEARCH` (3.8), `P-U-VSL-COPY` (3.9), `P-U-DESIGN-SALES` (4.2), `P-U-DESIGN-RENDER-SALES` (4.21), `P-U-DESIGN-CHECKOUT` (4.3), `P-U-DESIGN-RENDER-CHECKOUT` (4.31), `P-U-DESIGN-VSL` (4.4), `P-U-DESIGN-RENDER-VSL` (4.41), `P-U-HTML-SALES` (5.2), `P-U-HTML-CHECKOUT` (5.3), `P-U-HTML-VSL` (5.4), `P-U-FORM-GATE` (5.6), `P-U-GHL-SALES` (6.2), `P-U-GHL-VSL` (6.4), `P-U-SALES-BUILD` (8.75), `P-U-CHECKOUT-BUILD` (8.76), `P-U-FORM-CHECKOUT` (8.77), `P-U-COLLATERAL` (8.8), `P-U-VSL-BUILD` (8.93), `P-U-QC` (9.05) | conditional upsell branch (P-U), defers per its own preflight |
...
| Phases with no legacy short code | `P-STYLE-SPEC` (4.84), `P-STYLE-PICK` (4.86), `P8.3-INFOGRAPHIC` (8.3), `P-BUNDLE-GATE` (9.95) | added after the numeric short-code era; the manifest row is the only key |
...
3. Allocate the arc using the SOP-STORY-01-VILLAIN-HERO-ARC + SOP-PRIORITY-02-EIGHT-MOVE-BUILD-SEQUENCE + director-of-presentations SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) worked allocation table. Percentages from the arc produce fractions; use this pre-reconciled allocation for the common counts. For other counts, allocate proportionally, round, then add or remove slides from the Secrets sections (NEVER from the offer section) until the total matches `SLIDE_COUNT`:
...
4. Apply the SOP-STORY-01-VILLAIN-HERO-ARC + SOP-PRIORITY-02-EIGHT-MOVE-BUILD-SEQUENCE (PRESENTATION-MASTER-DOCTRINE.md §4) proven flow. The proven deck runs SEVEN sections with on-screen progress labels ("SECTION 3 OF 7"). This is the narrative the allocation table serves, and it is how the ten canonical arc sections (step 3a) collapse onto the slide-count math:
   ...
   Concrete-value examples from one proven reference run (prices and numbers are that run's, not a template):
...
5. Write the arc_allocation.json to working/copy/arc_allocation.json with the reserved slots tagged: HOOK_SLIDE (x3-4), PROMISES, VALUE_STACK, RE_PITCH, WALL_OF_WINS, and the LADDER tags (ANCHOR/DROP1/DROP2/DROP3/FINAL with their BUILDUP slides), plus each one-big-idea split. Every slot also carries its canonical arc-section label (A through J per SOP-STORY-01-VILLAIN-HERO-ARC + slide-copywriter SOP 9.x (PRESENTATION-MASTER-DOCTRINE.md §4)) alongside its proven-section label, so each slide is traceable to its beat for the Copywriter and QC.
...
7. Verify against the canonical arc (SOP-STORY-01-VILLAIN-HERO-ARC + slide-copywriter SOP 9.x (PRESENTATION-MASTER-DOCTRINE.md §4)): the deck includes a Hook Open (A) inside the first 10 to 15%, a Care / See-Yourself beat (B) before any credentials, a dedicated A4 hook slide, the hook sung on its 3 to 4 DEDICATED slides (ruling 9.2: exactly 3 to 4 dedicated hook slides; the "~10x" attention density is satisfied by the spoken band, which carries the hook refrain 5 to 20 times across the deck -- it is a spoken-verse measure, not a slide count, so no slide-level hook floor or raw-count target is checked), and a Hook Callback (J) reprise on the final substantive slide. If any is missing, add the slot before handing off (hook-refrain slots go at the section boundaries that lack one).
...
**When to run:** At the start of Phase 2 (prompt authoring, manifest id `P4-PROMPT`, order 4.7) and Phase 4 (image generation, manifest id `P4-RENDER`, order 4.9), and whenever dispatching multiple sub-agents. (Short codes per the Phase-Code Map above; the manifest id is the canonical key.)
...
### Gate 3 -- Copy QC (Phase 1Q; manifest id `P1Q-COPY-QC`, order 4.2)
copy_qc_report.json average >= 8.5. If not, loop back to Slide Copywriter.

### Gate 4 -- Owner Copy Approval (Phase 1A; an interlock between `P1Q-COPY-QC` and `P4-PROMPT`, not a manifest phase)
approval_record.json has copy_approved_by and copy_approved_at. No Phase 2 (`P4-PROMPT`) without this.

### Gate 5 -- Prompt QC (Phase 3; manifest id `P-PROMPT-QC`, order 4.8)
prompt_qc_report.json average >= 8.5 per prompt. If not, loop back to Slide Image Creator. **No Phase 4 (image generation, manifest id `P4-RENDER`, order 4.9) may begin until prompt_qc_report.json exists on disk with a passing status. This is a hard interlock: the Slide Submitter is not dispatched, and no createTask call is made, until this file exists and every prompt shows a passing score. A missing or failing prompt_qc_report.json is not a soft warning; it is a generation stop.**

### Gate 6 -- Image QC (Phase 5; manifest id `P-IMAGE-QC`, order 4.95)
image_qc_report.json: each image >= 8.5 or re-generated (max 3 attempts, then escalate).

### Gate 7 -- Final Deck QC (Phase 6 assembly + final aggregation; manifest ids `P8-ASSEMBLE` order 8 and `P-QC-AGGREGATE` order 8.65)
...
- Slide Submitter (Phase 4 image generation) -- receives model_manifest.json (operator-confirmed at echo) before any generation job; the manifest is the authorization token for all Kie.ai calls
...
- Running discovery without collecting TONE from the seven named styles -- tone governs every line of copy; a missing tone produces a copy QC cascade.
- Collecting REPRESENTATION_MIX without percentages -- a vague "diverse audience" is not enforceable in Phase 2 and produces mismatched slides.
- Stating VIP_TIER exists without capturing the real spot count -- fabricated scarcity is a copy QC auto-fail.
- Skipping the HOOK SEED question when the client has signature language -- the hook should be theirs whenever possible; agents derive it only when the client has none.
...
- Using the old (wrong) slide-math table -- the SOP-PITCH-01..06 cluster (PRESENTATION-MASTER-DOCTRINE.md §4) table governs; the 90 figure is the Mode A target/cap, and in Mode B it yields to source_slide_count (add-only, never trimmed).
