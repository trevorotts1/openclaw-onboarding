...
export class R2Shim {
  ...
  async put(key, value, options = {}) {
    ...
    if (options.onlyIf?.etagDoesNotMatch === "*" && old !== undefined) return null;
    if (options.onlyIf?.etagMatches !== undefined && options.onlyIf.etagMatches !== old) return null;
    this.map.set(key, typeof value === "string" ? value : String(value));
    return { key, etag: this.map.get(key) };
  }
  async get(key) {
    const v = this.map.get(key);
    if (v === undefined) return null;
    return { key, etag: v, async text() { return v; } };
  }
