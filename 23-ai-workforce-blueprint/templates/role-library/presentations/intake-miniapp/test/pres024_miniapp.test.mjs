...
test("QC-PRES-024.1b: renew by run_id works and never resets answers", async () => {
  ...
  const r = await h.handle("POST", "/api/sessions/renew", { token: "admin-tok", body: { company_id: "acme", installation_id: "install-test", presentation_id: "presentation-test", run_id: s.run_id, company_id: "acme" } });
...
test("QC-PRES-024.8: legacy single-grant semantics preserved — pre-renewal flow unchanged (reuse, order gate, poll cursor)", async () => {
  ...
  const again = await h.handle("POST", "/api/sessions", {
    token: "admin-tok", body: { company_id: "acme", installation_id: "install-test", presentation_id: "presentation-test", run_id: "run-legacy", box_id: "box1", questions_payload: PAYLOAD },
  });
