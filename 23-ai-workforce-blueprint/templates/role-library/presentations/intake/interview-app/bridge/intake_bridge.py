...
def _fetch_intake(args) -> dict:
    ...
    PRES-008: raises instead of sys.exit — a per-session failure inside the
    poll loop must be contained by that loop's poison-session handler
    (SystemExit is not an Exception and would otherwise kill the whole poll
    batch behind one bad session)."""
    admin = os.environ.get("INTAKE_ADMIN_TOKEN", "")
    if not admin:
...
def _list_intakes(args) -> list:
    ...
    PRES-008: raises instead of sys.exit (see _fetch_intake) — cmd_poll maps
    a list failure onto the whole-batch rc rather than escaping its handlers."""
    admin = os.environ.get("INTAKE_ADMIN_TOKEN", "")
    if not admin:
        raise RuntimeError("INTAKE_ADMIN_TOKEN not set in env")
...
def cmd_poll(args) -> int:
    ...
    print(json.dumps({"status": "poll_done", "discovered": len(intakes),
                      ...
                      "blocked": blocked, "crashed": crashes}))
    return 0
...
def main(argv=None) -> int:
    ...
    i.add_argument("--run-dir", required=True, help="deck run directory root; the submission stamps under <run-dir>/<session-id>/ by default")
    ...
    p.add_argument("--run-dir", required=True, help="deck run directory root; each intake stamps under <run-dir>/<session-id>/")
    p.add_argument("--no-per-session-dirs", action="store_true",
                   help="LEGACY opt-out: stamp every intake directly under --run-dir "
                        "(refused when more than one intake is discovered — a shared "
                        "target directory for multiple submissions is forbidden)")
