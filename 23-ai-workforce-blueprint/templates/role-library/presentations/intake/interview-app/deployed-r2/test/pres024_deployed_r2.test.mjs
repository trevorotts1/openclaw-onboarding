...
test("PRES-024 [deployed-r2]: two deck sessions renew independently; answers never bleed", async () => {
  ...
  assert.equal(v1.body.revision, 1);
  assert.equal(v2.body.revision, 1);
});
...
test("PRES-024 [deployed-r2]: legacy contract intact — reuse, ordered answers, intake list", async () => {
  ...
  const again = await h.handle("POST", "/api/sessions", { token: "admin-tok", body: { run_id: "run-r2-legacy", box_id: "box1", questions_payload: PAYLOAD } });
