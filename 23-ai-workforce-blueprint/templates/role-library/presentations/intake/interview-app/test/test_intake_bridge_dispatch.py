...
What is proven (offline, no engine spawned, no network):
  1. _dispatch_launch ACQUIRES the lease (working/.lease.json is written with
     ...
     lease at call time.
  2. dispatch_new receives the engine presentation_type from the intake record;
     the separately-derived deck_type remains a display/delivery axis.
...
class TestDispatchLaunch(unittest.TestCase):
    ...
    def test_dispatch_new_receives_engine_presentation_type_not_display_deck_type(self):
        """A signature intake hands the launcher's `signature` vocabulary,
        never the derived display deck type `signature_presentation`."""
    ...
    def test_from_scratch_webinar_passes_from_scratch_to_launcher(self):
        intake = _grounded_intake()
        intake.update(deck_type="webinar", presentation_type="from_scratch")
    ...
    def test_shared_launcher_accepts_presentation_type_and_dispatch_new_forwards_it(self):
        ...
        transport boundary, so no engine/provider process is spawned."""
        import presentation_job.launcher as real_launcher
        assert real_launcher.normalize_presentation_type("from_scratch") == "from_scratch"
        with self.assertRaises(real_launcher.UnknownPresentationType):
            real_launcher.normalize_presentation_type("webinar")
        seen = {}
        original = real_launcher.dispatch
        def transport_boundary(run_dir, **kwargs):
            seen.update(run_dir=run_dir, **kwargs)
            return 4242
