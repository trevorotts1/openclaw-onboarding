...
- `presentations/scripts/build_teleprompter.py` is a no-AI generator that reads the finished `PRESENTERS-SPEECH.md` (parsing the `## Slide N -- Headline (STAGE)` + `> ... SECONDS: Ns` contract) and emits the self-contained `presenter-teleprompter.html` (inline CSS+JS+speech JSON, no external assets) with THREE selectable scroll modes — **Manual** (no auto-motion: wheel/arrows/rail/clicker, Space = next slide), **Auto** (fixed-speed scroll with the speed slider seeded from the WPM, Space = play/pause), and **Voice** (voice-tracking via the browser's Web Speech API: a fuzzy sliding-window aligner follows what the presenter ACTUALLY says — paraphrase/skip/ad-lib tolerant, an adjustable sensitivity slider, spoken-word highlight as the tracked-position indicator, manual-jump resync, hardened auto-restart, and graceful fallback to Auto when the mic is denied or recognition is unsupported; the Voice option is hidden where the API is absent). Plus: big adjustable font, horizontal + vertical mirror modes, eye-line reading guide, progress bar, slide rail/jump, per-slide pacing countdown from the SECONDS metadata, fullscreen, localStorage settings (legacy saved modes migrate), dark high-contrast theme, the locked brand accent via `--design-system`, and the brand name from intake.json. Run with `--sample` to render a stub, or `--emit-sample-speech PATH` to see the input contract.
...
3. Run `python3 presentations/scripts/build_teleprompter.py --speech working/presenter-speech/PRESENTERS-SPEECH.md --out working/delivery/presenter-teleprompter.html --intake working/copy/intake.json --design-system working/typography/design_system.json`. The `--design-system` flag injects the locked brand accent (`brand.accent`, falling back to `brand.accent_hex` then `brand.primary`) into the teleprompter `:root` CSS so the cue/accent color matches the deck — never the house amber. Confirm the printed `accent:` line shows the locked brand hex (not `#f2b134` unless the brand IS amber), that it parses every slide, and that it opens in a browser.
