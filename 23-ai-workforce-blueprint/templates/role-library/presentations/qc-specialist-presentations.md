...
**Signature Presentations (Skill 51) cross-reference.** For a deck whose `working/copy/intake.json` records `deck_type: signature_presentation`, the SACRED Trevor Otts 4-phase methodology carries its own AUTO-FAIL battery, enforced fail-closed by three deterministic provers wired as manifest phases P-SP-INTAKE / P-SP-STRUCTURE / P-SP-P3-HYGIENE (`build_deck._chk_sp_intake` / `_chk_sp_structure` / `_chk_sp_no_pitch`, which DEFER — return empty — for every non-signature deck, so nothing here changes for a normal deck): **AF-SP-8Q-MISSING, AF-SP-8Q-SPLIT, AF-SP-FRAME-UNSET, AF-SP-TYPE-MISMATCH, AF-SP-OFFER-UNDECLARED, AF-SP-SLIDE-FLOOR, AF-SP-PHASE-RANGE, AF-SP-PHASE-ORDER, AF-SP-PHASE-LABEL, AF-SP-IMG-SUGGESTION, AF-SP-CASESTUDY-CAP, AF-SP-TEACH-STEPS, AF-SP-HOOK, AF-SP-QUADRANT, AF-SP-P3-PITCH** (registered in `MASTER-QC-AUTOFAIL-RULESET.md` Section 5). The independent SEMANTIC layer for signature decks is owned by the QC Specialist (Signature Presentations) (`qc-specialist-signature-presentations.md`); you do not re-grade those decks, but you treat any AF-SP-* code you see as a first-class auto-fail that vetoes scoring, exactly like the AF-HOOK / AF-AUD / AF-OBI / AF-DEN battery.
