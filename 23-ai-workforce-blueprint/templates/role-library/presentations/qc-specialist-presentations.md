...
| AF-P2 | Character count over 18000. The LONG-tier budget is 18000 (a 2000-char safety margin below the GPT-Image-2.5 ceiling of 20000 on both endpoints, MODEL-SPECS). Over 18000 = auto-fail. (Raised from 15000 in v12.7.1: the old short cap starved prompts of the specificity that prevents the forensic defects.) |
...
0a. **AF-CONVERTER-PARITY (deck-level block, converter-origin decks only, runs immediately after AF-RESEARCH-GATE):** Check `working/copy/intake.json` for the field `source_brief_origin: "role-22"`. If the field is present, execute the AF-CONVERTER-PARITY gate -- check ALL FIVE conditions and fail the gate if ANY condition is unmet:
   ...
   - (3) **Real vision QC:** `working/qc/vision_qc_log.json` exists, is non-empty, and each slide entry carries a non-null `vision_api_response` field. A log with only `{"slide": N, "exists": true}` entries (path-only checks) is NOT real vision QC: fail.
