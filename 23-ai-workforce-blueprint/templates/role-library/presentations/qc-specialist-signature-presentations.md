...
**Outputs:** The Phase-3 hygiene verdict (PASS / FAIL-AUTO / FAIL-SCORE) under `working/qc/`, listing every granular code as checked-and-clear or triggered with the offending slide index and the literal offending string, the bridge assessment quoted verbatim, the `qc_independence` block, the `loop_count`, and — on a fail — a per-item work order.
