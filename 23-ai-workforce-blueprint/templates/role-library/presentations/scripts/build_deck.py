    ...
    --logo <png-or-URL>  — the client's REAL, official logo. When supplied
        ...
        wears the client's actual mark, not an AI-hallucinated wordmark:
          (1) When --logo is a URL: KIE composites the REAL logo via image-to-image
              (gpt-image-2-5-sunburst-image-to-image, input_urls) in the SAME single generation
...
# Heavy gpt-image-2.5 renders (e.g. a split then-vs-now band with two photoreal scenes
...
PER_SLIDE_CREDIT_ESTIMATE = 4      # AF-KIE-BALANCE: estimated Kie credits consumed per slide render (gpt-image-2.5 2K)
...
# the single composed gpt-image-2.5 image; the only legitimate PPTX text part is the
...
# gpt-image-2.5 (text-to-image, or image-to-image for the logo composite). There is
...
# gpt-image-2.5 render carries a photographic/illustrative subject (and model noise),
...
def load_rich_prompt(slide: dict, run_dir: Path) -> str:
    ...
    (gpt-image-2.5) renders the WHOLE slide in ONE generation.
...
def _chk_assets_manifest(run_dir: Path, slides_path: Optional[Path] = None) -> str:
    ...
    gpt-image-2.5 as input_urls). Provided-but-unconsumed assets are the defect this
    ...
    if manifest == "__absent__":
        if intake_says_provided:
            return ("AF-MANIFEST-UNREFERENCED: intake.json declares the client provided "
                    ...
                    "it as gpt-image-2.5 input_urls.")
    ...
    if unconsumed:
        return ("AF-MANIFEST-UNREFERENCED: one or more provided assets are not provably "
                ...
                "gpt-image-2.5 as input_urls, not collected and ignored.")
    return ""
...
#      native PPTX text run instead of a single composed gpt-image-2.5 image, and the
...
def _delivered_pptx_native_text(pptx_path: Path) -> str:
    ...
    if offenders:
        return ("delivered slides carry NATIVE on-slide text instead of a single composed "
                "gpt-image-2.5 image — " + "; ".join(offenders[:8]))
    return ""
...
def _chk_no_overlay(run_dir: Path, slides_path: Optional[Path] = None) -> str:
    ...
    only legitimate slide text is baked into the single composed gpt-image-2.5 image;
    ...
    for rel in OVERLAY_FORBIDDEN_FILES:
        if (run_dir / rel).exists():
            return ("AF-OVERLAY-DELIVERED: the eliminated native-text-overlay file "
                    ...
                    "re-render the affected slide as a single composed gpt-image-2.5 image.")
    ...
    for pptx in sorted(run_dir.glob("**/*.pptx")):
        ...
        if reason:
            return ("AF-OVERLAY-DELIVERED: " + reason + ". Every delivered slide must be a "
                    "SINGLE composed gpt-image-2.5 image with its text baked in; the native "
...
def _owner_skip_approved(run_dir: Path, af_code: str):
    ...
    try:
        ...
        # SEALED mode, not a live re-read: facts.enforcing was resolved once
        # for this run_dir (on its first seal() touch, forced or not — see
        # presentation_job/runfacts.py's _resolve_enforcing_mode()), so it
        # cannot drift mid-run even if PRES_TRUST_BOUNDARY_ENFORCE changes in
        # the environment after this run was already admitted.
        if facts.enforcing:
...
def check_image_qc_vision(run_dir: Path, slides_path: Optional[Path] = None) -> str:
    ...
    for png in pngs:
        ...
        if size < PLACEHOLDER_MIN_BYTES:
            problems.append(
                ...
                f"kie-bake floor — a flat/native-rendered card, not a real gpt-image-2.5 "
    ...
    if problems:
        return ("AF-IMAGE-QC-VISION: the image-QC gate did not actually inspect the "
                ...
                "card through kie.ai gpt-image-2.5 and re-run the Image QC Specialist with a "
...
def check_canonical_render_path(run_dir: Path, slides_path: Optional[Path] = None) -> str:
    ...
    for py in candidates:
        ...
        if "Image.new" in text and "2048" in text and "1152" in text:
            offenders.append((rel, AF_LOCAL_CANVAS,
                              ...
                              "of a kie.ai gpt-image-2.5 render"))
    ...
    if offenders:
        ...
        return (f"{code}: non-canonical renderer/assembler {rel!r} {why}. The canonical "
                ...
                f"WORDS and VISUAL are generated together in ONE kie.ai gpt-image-2.5 image, "
...
PREFLIGHT_REQUIRED = [
    ...
    (None,
     ...
     "public_url + non-empty consumed_by (fed to gpt-image-2.5 as input_urls)",
    ...
    # gpt-image-2.5 image; only off-slide speaker notes are allowed). Run-dir-scoped.
    (None,
     "no native overlay — no pptx_text_overlays.json present and no delivered slide ships "
     "native on-slide text instead of a composed gpt-image-2.5 image",
...
def _cc_registration_linkage_matches(manifest: dict) -> bool:
    """T2 gate teeth (honesty fix, 2026-08-18 — renamed from
    ...
    round-trip — the old name and the old docstring claimed that, and the
    construction cannot deliver it: it is an HMAC keyed on fully
    attacker-computable data, over an EMPTY message. The receipt
    cc_board.stamp_task_id writes on a successful ingest is
    ``cc_registration`` = registration_linkage_tag(task_id, idempotency_key,
    deck_slug) — see that function's docstring for the full explanation of
    what this does and does not establish. Because both the "key" material
    ...
    WIRE FORMAT: reads the SAME dict key (``"hmac"``) and recomputes with the
    SAME formula as before the rename — deliberately unchanged so every
    process_manifest.json already stamped by production (and every one
    stamped going forward, since cc_board's producer side is equally
    unchanged) verifies identically. Only the symbol name and the docstring's
    honesty about what a match proves changed.
    ...
    task_id, wrong digest, non-dict receipt) is False — could-not-confirm-
    consistency NEVER reads as a pass. Never raises (a malformed manifest is
    just False)."""
    try:
...
# Deprecated alias — kept so any external caller/test still importing the old
# symbol name does not break. Prefer _cc_registration_linkage_matches(); the
# old name claimed a security property ("verified") this check never had.
_cc_registration_verified = _cc_registration_linkage_matches
...
def _chk_cc_registered(run_dir: Optional[Path], deck_slug: str) -> str:
    ...
    Center registration. THREE outcomes (T2 gate teeth):
      ...
      VERIFIED    — cc_task_id present AND the cc_registration linkage tag
                    matches (honesty fix, 2026-08-18: this is a same-file
                    ...
                    registration_linkage_tag for exactly what it does and
                    does not establish). Treat "VERIFIED" here as "no
      ...
      UNVERIFIED  — cc_register_attempted=True but NO matching tag: either
                    ...
                    NEVER prints as verified. Fail-soft per
    ...
    if attempted or (isinstance(tid, str) and tid.strip()):
        ...
        return (
            ...
            "receipt does not match (missing, mismatched, or a tampered tag) — "
            "this task id was NOT provably produced by a real "
...
def run_postflight_gate(bundle_dir: Path, ledger_path: Path, deck_slug: str,
    ...
    # native on-slide text run (only a composed gpt-image-2.5 image + off-slide notes).
    overlay_reason = ""
    if run_dir is not None:
        ...
        for pptx in sorted(bundle_dir.glob("*.pptx")):
            ...
            if r:
                overlay_reason = ("AF-OVERLAY-DELIVERED: " + r + ". Every delivered slide "
                                  "must be a single composed gpt-image-2.5 image; the native "
    ...
    if overlay_reason:
        missing_or_short.append((
            "deck_pptx",
            _expand_filename("deck.pptx", deck_slug),
            "no native overlay (every slide a composed gpt-image-2.5 image; "
    ...
    if missing_or_short or not all_verified:
        ...
        for (key, fname, label, min_b, actual_b, reason) in missing_or_short:
            if reason == "ABSENT":
                ...
                print(f"           is present) instead of a single composed gpt-image-2.5 "
                ...
                print(f"           Re-render any flat card through kie.ai gpt-image-2.5 and "
    ...
    # matching cc_registration linkage tag -> pass. NOTE (honesty fix,
...
def main():
    ...
    # input_urls (verified gpt-image-2-5-sunburst-image-to-image). A local path falls back to a
