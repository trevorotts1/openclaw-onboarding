...
#
# The old run_signature_deck.py call is retained as FALLBACK only: when
# the engine is unavailable, the door falls back to the 2-of-20 runner so a
# deck render is never blocked on a missing engine component.
# ===========================================================================
ENGINE_ENTRY="$SCRIPTS_DIR/presentation_job.py"
INTAKE_LEDGER="$RUN_DIR/working/interview/intake_ledger.json"

# Resolve client + deck_type from the intake ledger for the engine
_RESOLVED_CLIENT="operator"
_RESOLVED_PRESENTATION_TYPE="from_scratch"
if [ -f "$INTAKE_LEDGER" ] && command -v python3 >/dev/null 2>&1; then
    _INTAKE_INFO="$(python3 -c "
import json, sys
try:
    d = json.load(open('$INTAKE_LEDGER'))
    client = str(d.get('client_name') or d.get('client') or d.get('requester_name') or 'operator')
    ptype = str(d.get('presentation_type') or d.get('deck_type') or 'from_scratch')
    # Normalize to the engine's legal set: from_scratch, content_personal, content_general, signature
    legal = {'from_scratch', 'content_personal', 'content_general', 'signature',
             'standard', 'signature_presentation'}
    if ptype not in legal:
        # Map common aliases
        alias = {'standard': 'from_scratch',
                 'signature_presentation': 'signature'}
        ptype = alias.get(ptype, 'from_scratch')
    chat_id = str(d.get('requester_chat_id') or d.get('chat_id') or '')
    print(f'{client}|{ptype}|{chat_id}')
except Exception:
    print('operator|from_scratch|')
" 2>/dev/null || true)"
    if [ -n "$_INTAKE_INFO" ]; then
        _RESOLVED_CLIENT="${_INTAKE_INFO%%|*}"
        _REST="${_INTAKE_INFO#*|}"
        _RESOLVED_PRESENTATION_TYPE="${_REST%%|*}"
        _RESOLVED_CHAT_ID="${_REST##*|}"
    fi
fi
...
if [ -f "$ENGINE_ENTRY" ] && command -v python3 >/dev/null 2>&1; then
    ...
    # Step 1: Build a temporary intake JSON the engine's --new path can consume.
    # The engine's cmd_new() requires intake.presentation_type and intake.requester.chat_id.
...
intake = {
    'presentation_type': '$_RESOLVED_PRESENTATION_TYPE',
    'requester': {
        'chat_id': '${_RESOLVED_CHAT_ID:-}',
        'client_name': '$_RESOLVED_CLIENT',
    },
    'client': '$_RESOLVED_CLIENT',
    'deck_type': '$_RESOLVED_PRESENTATION_TYPE',
    'source': 'canonical-entry',
}
# Preserve signature_source for signature decks
if intake['presentation_type'] == 'signature':
    intake['signature_source'] = ledger.get('signature_source', 'from_scratch')

with open('$_ENGINE_INTAKE_TMP', 'w') as f:
...
" 2>/dev/null || true
    ...
    _CREATE_RC=0
    python3 "$ENGINE_ENTRY" --new --run-dir "$RUN_DIR" --intake "$_ENGINE_INTAKE_TMP" 2>&1 || _CREATE_RC=$?
    if [ "$_CREATE_RC" -ne 0 ]; then
        # state.json may already exist from a prior run -- that's OK.
        # Only fall back if --new genuinely failed for a reason other than pre-existing state.
        if [ -f "$RUN_DIR/state.json" ]; then
            note "Engine state already exists in $RUN_DIR (reusing existing job)"
        else
            echo "!! [$PROG] engine --new failed (exit $_CREATE_RC) -- falling back to run_signature_deck.py" >&2
    ...
    note "Engine entry not found at $ENGINE_ENTRY -- falling back to run_signature_deck.py"
fi
