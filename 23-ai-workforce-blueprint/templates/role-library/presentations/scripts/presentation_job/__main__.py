...
#
# F11 (2026-09-06): the five helpers this section used to define in-line now
# live in presentation_job/autospawn.py, VERBATIM, together with the full F07
# design note (concurrency, the getppid survival argument, termination, the
# no-double-spawn lock). Nothing about their behaviour changed.
#
# WHY THEY MOVED: phases.Engine._run_agent_phase must be able to respawn a dead
# dispatcher MID-RUN (F11: measured runs last 22 h, a dispatcher lives 6 h), and
# the Engine cannot import this module to reach them -- this module imports
# phases.Engine at module scope, and under `python3 -m presentation_job` the
# entry module registers as `__main__`, not `presentation_job.__main__`, so a
# `from . import __main__` would execute a second, independent copy.
#
# The private names are re-exported here so every existing caller -- main()
# below, and tests/test_dispatcher_autospawn.py, which calls
# pj_main._spawn_dispatcher_if_available / pj_main._stop_auto_dispatcher
# directly -- keeps working unchanged.
# ---------------------------------------------------------------------------
from .autospawn import (  # noqa: F401  -- re-exported for callers and tests
    _pid_is_alive,
    _auto_dispatch_lock_path,
    _auto_dispatch_disabled,
    _spawn_dispatcher_if_available,
