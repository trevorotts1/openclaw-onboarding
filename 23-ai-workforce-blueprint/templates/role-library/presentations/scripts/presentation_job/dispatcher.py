...
def _dispatch_phase_fanout_units(
    ...
    pending_items = [it for it in wanted_items if it["key"] not in _banked_keys]
    _append_sidecar(run_dir, phase_id, {
    ...
    all_pending_units = [fanout.Unit(key=it["key"], payload=it)
    ...
    if failed:
        ...
        return DispatchResult(phase_id, "partial_failure",
