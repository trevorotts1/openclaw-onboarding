...
class Engine:
    ...
    def run_phase(self, phase: Phase) -> int:
        ...
        with self._state_lock:
            ...
            # FIX 109 engine door #2: intake provenance gate. A phase runs only
            # when the run's current intake.json sha is provenanced — an
            # out-of-band rewrite blocks here (and again in the verifier
            # funnel) until the approval path re-writes it.
            gate_rc = self._check_intake_provenance(phase)
            if gate_rc is not None:
                return gate_rc
    ...
    def _check_intake_provenance(self, phase: Phase) -> Optional[int]:
        """FIX 109 engine door #2: the pre-phase provenance gate. Before ANY
        phase runs, the run's current working/copy/intake.json sha must be
        provenanced (appear as some row's sha_after in
        working/checkpoints/intake.provenance.jsonl) — an out-of-band rewrite
        (a leftover worker, a shell write) refuses the phase here and in
        phase_verifiers' funnel (the same runfacts.check_intake_provenance
        oracle on both doors; both fail CLOSED when runfacts cannot import).
        The block message names the sha (the QC proof's requirement).

        Producer phases are exempt via the same rule as _intake_gate_applies:
        a phase that itself (re)writes intake.json must be allowed to run to
        re-arm the run. The oracle itself already tolerates a run with no
        provenance log (pre-FIX-109 runs keep running) and a run with no
        intake.json yet."""
        if _INTAKE_ARTIFACT in phase.produces_artifact:
            return None  # producer exemption: the writer re-arms the run
        try:
            from presentation_job import runfacts as _rf109
        except Exception as exc:  # noqa: BLE001 -- fail CLOSED (FIX 17 pattern)
            return self._block(
                phase,
                f"AF-INTAKE-PROVENANCE: runfacts oracle unavailable ({exc}) — "
                "refusing to run without the intake provenance check.")
        try:
            ok, why, _invalidated = _rf109.check_intake_provenance(
                self.run_dir, manifest_path=str(getattr(self.manifest, "path", "") or ""))
        except Exception as exc:  # noqa: BLE001 -- a failing oracle is a refusal, never a pass
            return self._block(
                phase,
                f"AF-INTAKE-PROVENANCE: provenance check failed ({exc}) — "
                "refusing to run; resolve via resolve_intake.py.")
        if not ok:
