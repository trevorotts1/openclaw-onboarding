...
class Engine:
    ...
    def close(self) -> int:
        ...
        try:
            ...
            # A rejected waiver must be readable (U013 step 7, B3 con :872-873).
            print(f"\nFATAL: waiver rejected", file=sys.stderr)
            print(f"  reason: {exc}", file=sys.stderr)
            print(f"  schema: {{rule, source, client_request_quote, "
                  f"intake_field?, captured_at, captured_from}}",
                  file=sys.stderr)
            print(f"  source: 'intake_field' or 'transcript'", file=sys.stderr)
            print(f"  resume: python3 presentation_job.py --resume "
                  f"--run-dir {self.run_dir}", file=sys.stderr)
        ...
        for k in ALL_GATE_KEYS:
            ...
            if k in waived and k not in NON_WAIVABLE_GATES:
                g["state"] = "waived"
                continue
            # Warn-mode: report, count, record, do not block.
            if g.get("warn_only"):
                gate_warnings.append((k, g.get("reason") or "failed"))
                continue
            failures.append((k, g.get("reason") or "failed"))

        if gate_warnings:
            self.state["gate_warnings"] = [
                {"gate": k, "reason": r} for k, r in gate_warnings
            ]
            lines = "\n".join(f"    - {k}: {r}" for k, r in gate_warnings)
            print(f"\nWARN-MODE GATES -- {len(gate_warnings)} gate(s) in warn-mode "
                  f"did not pass -- see state.json gate_warnings:\n{lines}",
                  file=sys.stderr)
            print("  These gates are counted but do not block delivery. "
                  "The exit criterion for removing them from WARN_ONLY_GATES "
                  "is a warn count of zero across the golden corpus.",
                  file=sys.stderr)
