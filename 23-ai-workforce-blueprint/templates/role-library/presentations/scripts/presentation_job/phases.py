...
class Engine:
    ...
    def _revalidate_banked(self, phase: Phase, ps: Dict[str, Any]) -> List[str]:
        ...
        for rel in ps.get("artifacts", []) or []:
            ok, why = validate_artifact(self.run_dir, rel, self.manifest, recorded_sha=shas.get(rel))
            if not ok:
    ...
    def _run_script_phase(self, phase: Phase) -> int:
        ...
        # Also record the pre-run state of the phase's outputs so a resume can tell
        ...
        while time.time() < deadline:
            ...
            if now - last_cp >= checkpoint_every:
                ...
                self._checkpoint(phase.id, status="running",
                                 ...
                                 waited_seconds=int(now - (deadline - phase.budget_minutes * 60)))
            time.sleep(15)
        return self._block(
            phase,
    ...
    def _block(self, phase: Phase, reason: str) -> int:
        ...
        safe = f"{len(banked)} file(s) already produced are saved — nothing is lost."
        if lost:
            safe = (f"{len(banked)} file(s) are saved and {len(lost)} will be rebuilt on resume "
                    "— nothing you sent us is lost.")
        self.report.to_requester(
            "blocked",
            f"Your presentation is paused at {phase.id}. {reason} {safe} "
