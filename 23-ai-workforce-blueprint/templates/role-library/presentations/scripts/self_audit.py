...
# ---------------------------------------------------------------------------
# SINGLE SOURCE OF TRUTH (U05) — the 10-deliverable audit list is imported
# from presentation_job/deliverables.py, the one place the whitelist is
# defined. No other file (including this one) may hardcode a deliverable
# list of its own.
#
# There is deliberately NO inline fallback list here. A deployed self-audit
# that cannot resolve the canonical whitelist must fail loudly and refuse to
# run rather than silently auditing against a stale hand-copied list -- a
# quiet fallback is exactly the drift that produced the original bundle-
# completeness errors this script exists to catch.
# ---------------------------------------------------------------------------
...
try:
    from presentation_job.deliverables import DELIVERABLE_AUDIT_SPEC as _AUDIT_SPEC
except ImportError as exc:
    raise ImportError(
        "self_audit.py requires presentation_job/deliverables.py (the single-source "
        "deliverable whitelist) to be importable. Refusing to run: auditing a "
        "handoff against anything other than the canonical whitelist would be "
