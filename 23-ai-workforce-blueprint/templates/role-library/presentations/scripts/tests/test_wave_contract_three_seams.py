...
# ---------------------------------------------------------------------------
# QC.md proof item 2: "a stubbed 12-slide P4-PROMPT wave completes on attempt 1".
# Drives the REAL dispatcher parallel path (dispatcher._dispatch_prompt_phase_parallel)
# with the worker's provider transport stubbed at the module seam
# (parallel_prompt_worker.provider_call) and the REAL deterministic prompt gate
# (dispatcher._verify_single_prompt -> build_deck.check_prompt_qc_deterministic)
# deciding success -- the same gates a live run uses, zero provider spend.
# Inline mode (workers_requested=1 -> no spawn pool) so pytest needs no __main__ guard.
# ---------------------------------------------------------------------------
def test_qc_stubbed_12slide_wave_completes_attempt_1(tmp_path, monkeypatch):
    from presentation_job import parallel_prompt_worker as _ppw_mod
    from presentation_job.manifest import Phase

    run_dir = tmp_path / "runs" / "pres-12"
    (run_dir / "working" / "copy").mkdir(parents=True)
    slides = [{"slide": i, "copy": [f"copy {i}"], "archetype": "hero",
               "research_anchors": [], "design_tokens": {},
               "negative_requirements": []} for i in range(1, 13)]
    (run_dir / "working" / "copy" / "slides.json").write_text(
        json.dumps(slides), encoding="utf-8")

    # Transport stub: a gate-passing rich prompt, no provider call. The stub
    # body is proven to clear the REAL gate above (11,587 chars, all engines).
    # Spawn children cannot inherit an in-process monkeypatch, so the stub is
    # resolved through the worker's DESIGNED spawn seam -- the
    # PRESENTATION_PROMPT_PROVIDER_STUB env spec (a JSON file, no code exec) --
    # which _resolve_provider consults in every child before the production
    # dispatcher transport.
    stub_spec = tmp_path / "provider-stub.json"
    stub_spec.write_text(json.dumps({"default": "succeed"}), encoding="utf-8")
    monkeypatch.setenv(_ppw_mod._STUB_ENV, str(stub_spec))
    monkeypatch.setattr(dispatcher, "_ppw", _ppw_mod)
    ...
    # Attempt 1: exactly one parallel_wave_started + one finish row, no retry
    # waves, no serial fallback, no error rows.
    sidecar = run_dir / "working" / "work-orders" / "P4-PROMPT.dispatcher-log.jsonl"
    assert sidecar.is_file(), f"no dispatcher sidecar at {sidecar}"
    rows = [json.loads(line) for line in
            sidecar.read_text(encoding="utf-8").splitlines() if line.strip()]
    started = [r for r in rows if r.get("status") == "parallel_wave_started"]
    finished = [r for r in rows if r.get("status") == "parallel_wave_finished"]
    errors = [r for r in rows if r.get("status") == "error"]
    fallbacks = [r for r in rows if r.get("status") == "serial_fallback"]
    assert len(started) == 1, rows
    assert len(finished) == 1, rows
    assert not errors, rows
    assert not fallbacks, rows
    assert finished[0].get("worker_exit_code") == 0, finished[0]
    assert finished[0].get("succeeded") == 12, finished[0]
    assert finished[0].get("failed") == 0, finished[0]

    # The phase verified through the REAL verifier and returned ok.
    verified = [r for r in rows if r.get("status") == "verified"]
    assert verified, rows
    assert verified[0].get("verifier_ok") is True, verified[0]
    assert result.status == "ok", (result.status, result.reasons[:5])

    # All 12 canonical prompt files exist on disk and clear the gate.
    prompts = sorted(p.name for p in (run_dir / "working" / "prompts").glob("*.txt"))
    assert prompts == [f"slide-{i:02d}.txt" for i in range(1, 13)], prompts
    for i in range(1, 13):
        ok, reasons = dispatcher._verify_single_prompt(run_dir, i)
        assert ok, (i, reasons[:3])
