...
**THE 9,000-CHARACTER FLOOR ONLY COUNTS WHEN THE WORDS ARE BAKED IN.** A prompt must be >= 9,000 characters (`PROMPT_CHAR_FLOOR = 9000`, hard, fail-loud) AND those characters must include the slide's verbatim words with their spelling-locks. A prompt that reaches 9,000 characters by padding scene description while deferring the words to an overlay does NOT satisfy the floor — it is a words-deferred stub and is rejected (AF-P1 / AF-PROMPT-FLOOR; the canonical-render-bypass and local-canvas auto-fails `AF-CANONICAL-RENDER-BYPASS`, `AF-LOCAL-CANVAS`, and the vision image-QC `AF-IMAGE-QC-VISION` fire downstream in `build_deck.py` if anything routes around this).
...
1. Read BOTH full-density gold-standard exemplars in Appendix A of this file in full (Exemplar 1, the A4 type-dominant slide; Exemplar 2, the A2 people slide). Do not write a single prompt before reading both (SOP 9.1 step 0).
...
| Prompts that reach 9,000 chars by padding scene while omitting the verbatim words | 0 (AUTO-FAIL, AF-P1) |
...
- this file's SOP 9.1 (the 15-element spec), SOP 9.2 (composition rules / the five archetypes), and SOP 9.8 (the mandatory paired negative block)
- Appendix A of this file -- the two full-density gold-standard exemplars (Exemplar 1 A4 type-dominant slide + Exemplar 2 A2 people slide), read in full before writing any prompt -- the density and structure reference
- SOP 9.2 of this file (the five proven archetypes A1-A5) and SOP 9.1 element 11 of this file (the three engines)
...
**Steps:**
0. **Read BOTH full-density gold-standard exemplars in Appendix A of this file in full before writing your first prompt.** Exemplar 1 is an A4 type-dominant slide; Exemplar 2 is an A2 people slide. Study their anatomy: the header block (title, ARCHETYPE / SECTION / LADDER tags, ONE BIG IDEA line), zone percentages, emotionally precise photo direction, exact verbatim copy with per-line font/size/color and per-string spelling-lock, the gold rule devices, the logo chip spec, the three-engine people spec with explicit REPRESENTATION_MIX, MOOD + LIGHTING, the COLOR VERIFICATION and COLOR GRADING blocks, and the closing eight-class DO-NOT block. Your prompts must match their density (9,000-18,000 chars) and structure, adapted to each slide's own archetype and brand variables. Do not write a single prompt before you have read both.
1. For each slide N, create working/prompts/slide-NN-prompt.txt (zero-padded number, e.g., slide-01-prompt.txt). **Line 1 of every prompt declares its archetype**, in the form `[ARCHETYPE A1] [SECTION: ...] [LADDER: ...]` followed by a ONE BIG IDEA line, exactly as the Appendix A exemplars do. The archetype is taken from the slide's ARCHETYPE field in slides_copy.md (A1-A5 per SOP 9.2).
2. Write the 15-element prompt in this exact order. Each element must be present:
   ...
   11. **PEOPLE (driven by the THREE ENGINES, defined here and detailed in SOP 9.2 strengthening + SOP 9.3 strengthening + SOP 9.4 of this file; all three required on every people-slide, missing any one is an auto-fail):**
...
3. Verify character count of the completed prompt. Target: 9,000-18,000 characters. Hard floor: 9,000. Hard maximum: 18,000 (the LONG-tier budget; GPT-Image 2 accepts up to 20,000 on both endpoints per MODEL-SPECS, and 18,000 keeps a 2,000-character safety margin). If under 9,000, the prompt is starved -- expand with genuine specificity (the per-line spelling-lock, the full paired negative block, exhaustive logo-placement language, complete people-anatomy direction, deeper world/scene detail), never with boilerplate. If over 18,000, the prompt is too long for the LONG tier -- tighten redundant phrasing, never by deleting the negative block or any spelling-lock. Spend the expanded budget ONLY on defect-preventing specificity (density-calibration rule, SOP 9.4-strengthening).
...
7. After all slides are written, run a batch self-check:
   ...
   n. WORDS BAKED IN + NO SKIP/OVERLAY LANGUAGE (FIX-4): every verbatim word that must appear on the slide is written INTO this prompt's copy elements (so kie.ai bakes it in the same generation), not deferred to any later step. The prompt body contains NONE of the banned skip/overlay phrases ("kie.ai SKIPPED", "skip kie.ai", "post-production overlay", "applied in post", "typography overlay readiness", "overlay the headline", "typography system renders the slide", "render locally", "Pillow slide canvas", "native typography card", "PowerPoint text on top"). Pure-typography hook/section slides are authored as full kie.ai renders (cream + display type baked in), never as a skipped or locally-rendered card. A prompt that defers any word, or carries any banned phrase, or pads to 9,000 chars while omitting the verbatim words, is an AUTO-FAIL (AF-P1 with the canonical-render-bypass / local-canvas auto-fails AF-CANONICAL-RENDER-BYPASS / AF-LOCAL-CANVAS firing downstream).

**Outputs:**
- working/prompts/slide-NN-prompt.txt (one file per slide, zero-padded)

**RENDERED VERBATIM (build_deck.py contract -- ADDITIVE, does not change the authoring rules above):** the deterministic renderer (`scripts/build_deck.py`) loads THIS rich prompt file and sends it to KIE (gpt-image-2) **VERBATIM** -- the WHOLE slide (typography sizes and per-line weights, placement, usage, the logo(s) via image-to-image `input_urls`, the scene, the verbatim copy, and the negative block) is rendered in ONE generation. `scripts/build_deck.py` / `scripts/run_signature_deck.py` is the ONLY sanctioned render path; a deck produced by any hand-rolled `working/*.py` renderer, a local Pillow canvas, or a native-text assembler is a canonical-render bypass (AF-CANONICAL-RENDER-BYPASS / AF-LOCAL-CANVAS) and cannot ship. The renderer NEVER composes its own thin scene+copy prompt and NEVER silently falls back to one. The renderer enforces a HARD floor of **9,000 characters** (AF-P1 / AF-PROMPT-FLOOR; `build_deck.py` constant `PROMPT_CHAR_FLOOR = 9000`): a slide whose prompt file is absent or under 9,000 chars, OR whose 9,000 chars omit the verbatim baked words, is NOT run, NOT rendered, and NOT updated -- it fails loud. (This HARD renderer-side floor equals the upstream Phase-3 QC minimum of 9,000 -- they are the same reconciled standard; the authoring band of 9,000-18,000 above is unchanged.) The renderer also reads `working/prompts/slide-NN.txt`; either name (`slide-NN.txt` or `slide-NN-prompt.txt`) is accepted.
...
**THE FIVE PROVEN ARCHETYPES (primary, defined here in SOP 9.2; the deck is built on exactly these five, rotated):**
...
2. When LOGO_ON_SLIDES = true, write element 10 to PASS the locked LOGO_URL as the first reference in `input.input_urls` and name it in the prompt: "The first reference image is the company logo: place it [per the rule], do not redraw, recolor, or restyle it." NEVER text-to-image the mark. One locked asset, composited identically on every slide (the anti-mutation path; SOP-IMG-01 Mode B, design-system/05). If the logo carries text and it garbles twice on render, composite the REAL logo IMAGE onto the rendered slide PNG via the PIL image-composite path (SOP-IMG-05), baked into the image before assembly — this is an IMAGE composite of the real mark, NEVER a native PPTX text run (AF-OVERLAY-DELIVERED). Critical slide TEXT that garbles is fixed by the re-prompt/re-seed loop then human escalation (Decision 5C), never an overlay.
...
- The two full-density gold-standard exemplars in Appendix A of this file (the density reference)

**Steps:**
1. Every prompt TARGETS 9,000 to 18,000 characters, calibrated against the two full-density gold-standard exemplars in Appendix A of this file (Exemplar 1 A4 type-dominant + Exemplar 2 A2 people slide) as the density reference, then enriched to the higher band with the defect-preventing specificity this SOP adds (the per-line spelling-lock on every text string, the full eight-class paired negative block, the exhaustive image-to-image logo-placement language, the complete people-anatomy direction, deeper world/scene and grade detail). GPT-Image 2 accepts up to 20,000 characters on both endpoints and is strongest on long structured prompts (MODEL-SPECS, the authoritative source); the old, narrower band used only a fraction of the available budget and STARVED the prompt of the specificity that prevents the forensic defects. The exemplars show the art-direction floor; the higher band is reached by garble-proofing and negative-prompting every line, not by padding.
...
4. Hard limits still apply: under 9,000 characters is an auto-fail at prompt QC (AF-P1); over 18,000 is an auto-fail (AF-P2, the LONG-tier budget, a 2,000-char safety margin below the 20,000 API ceiling). Never trim below the limit by deleting the negative block or any spelling-lock; tighten redundant phrasing instead.

**Outputs:**
- Every prompt in the 9,000 to 18,000 band (9,000 hard floor, 18,000 hard ceiling)

**Hand to:** SOP 9.1 step 3 verification

**Failure mode:** If a prompt cannot reach the 9,000-character hard floor with genuine specificity even after the spelling-lock, the full paired negative block, the image-to-image logo language, and the complete anatomy direction are present, the slide's spec is probably too thin (a near-empty transition slide). Confirm the slide truly needs that little; if so, document it. Never inflate with filler to pass the count, and never reach the count by padding boilerplate.
...
**When to run:** As the structural reference for SOP 9.1 -- the anatomy of a complete, defect-proofed, long-budget prompt. Use it alongside the two full-density exemplars in Appendix A of this file; this template adds the spelling-lock, the image-to-image logo directive, and the SOP 9.8 negative block that the bare 15-element order does not show. Adapt every bracketed value from slides_copy.md and the STYLE BLOCK; never ship the bracket tokens themselves (they are an AF-P16 / AF-F10 defect on render).
...
Every prompt: hard floor 9,000, hard maximum 18,000 characters (the LONG-tier budget; GPT-Image 2 accepts up to 20,000, MODEL-SPECS). Band 9,000-18,000. Over 18,000 is an auto-fail (AF-P2). The budget is spent on defect-preventing specificity, never boilerplate.
...
- A 400-character prompt with only "make a professional slide about enrollment" -- far below the 9,000-char hard floor, starved of the spelling-lock, negative block, and logo direction.
...
- A prompt that does not declare its archetype (A1-A5) on line 1. Every prompt declares its archetype in the first line, per the Appendix A exemplars.
...
- Writing a prompt before reading BOTH gold-standard exemplars in Appendix A (Exemplar 1 A4 type-dominant and Exemplar 2 A2 people slide).
...
| 13 | Missing or unpaired negative block; under-budget starved prompt | Author the full eight-class SOP 9.8 block as the final paragraph, each negative paired with a positive twin; spend the 9,000-18,000 budget on defect-preventing specificity, not padding (Gate 11). |
...
**Tier 1:**
- This file -- SOP 9.1 (the 15-element spec) and Appendix A (the two full-density gold-standard exemplar prompts)
- universal-sops/CLIENT-WEBINAR-DECK-SOP.md (the deterministic-pipeline master authority: the build/registration/postflight contract)
...
1. This file's SOP 9.1 (the 15-element spec) or SOP 9.8 (the negative block) is updated.
...
## Appendix A -- THE TWO GOLD-STANDARD EXEMPLARS (full-density worked prompts)

**This appendix is the density-and-structure reference for this whole role.** There is no external "master SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4)" exemplar -- the master SOP (`universal-sops/CLIENT-WEBINAR-DECK-SOP.md`) is the deterministic-pipeline contract (build, registration, postflight), not a prompt library; both gold-standard exemplars live HERE. Read BOTH in full before writing your first prompt (SOP 9.1 step 0):

- **Exemplar 1 -- A4 TYPE-DOMINANT (price / offer reveal).** The archetype that failed worst in the field (thin price-slide prompts let the model free-associate a wedding invitation onto the money slide). It demonstrates the giant-number hero, the price-tag motif with the drawn-line strike, the per-string spelling-lock on every numeral, the no-people declaration, the color-grading block, and the eight-class DO-NOT block.
- **Exemplar 2 -- A2 PEOPLE SLIDE (photo one side, text opposite).** It demonstrates the three engines (Facial Expression, Audience, World) with an EXPLICIT REPRESENTATION_MIX drawn from the casting ledger (the single worst-failed axis in the field), the 45/55 split, the depth layers, the base-color text devices, the COLOR VERIFICATION block, and the eight-class DO-NOT block.

Both prompt bodies below are full-density (each ~12,000-13,000 characters, squarely in the 9,000-18,000 target band) and carry the complete anatomy: typography spec (per-line weight + large pt size), facial intelligence with explicit REPRESENTATION_MIX, the composition grid (thirds + zone percentages), lighting, color/brand with a grading block, the eight-class paired negative block, and every verbatim word baked into the copy elements. Both use generic brand grammar -- substitute the client's intake hexes, fonts, logo, REPRESENTATION_MIX, and copy; the bracketed values are the only placeholders and must be resolved before render (they are AF-P16 / AF-F10 if shipped).

### Exemplar 1 -- A4 TYPE-DOMINANT PRICE/OFFER REVEAL (full density)

```
[ARCHETYPE A4] [SECTION: THE OFFER / PRICE REVEAL] [LADDER: DROP 3 of 3 -- the final price]
ONE BIG IDEA: the full program that was anchored at $1,997 is, today only, $497 -- and the number itself is the hero of the slide.

// MODE: image-to-image (gpt-image-2-image-to-image). input_urls[0] = LOGO_URL <https URL of the locked company mark>. This is the ONLY reference image; there is no founder portrait on this slide.

FORMAT: Create a 16:9 presentation slide image at 2K resolution (2560x1440 pixels). One single composed image: the typography, the price tag, the struck price, the brand rules, and the logo are all baked into the pixels of THIS one generation. No second pass, no overlay, no native text added later.

BACKGROUND: Warm off-white #FBF7F4 base background filling the entire frame. Brand gold #C4A44D is used only as accent elements -- the price-tag border, the kicker rule, the glow on the final number, and the divider rules -- and never covers more than 20% of the visual area. No dark, navy, charcoal, or pure-black background anywhere in the frame; this is a bright, premium, confident money slide, not a moody one.

HEADLINE VERBATIM + SPELLING-LOCK (every quoted string carries its own letter-for-letter lock, written immediately after the string):
KICKER reads exactly: "TODAY ONLY". Render this exact string, letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "TODAY ONLY". Do not alter, misspell, duplicate, abbreviate, translate, or garble any character of it.
HEADLINE reads exactly, on two tight lines: "Everything You Need" / "For One Number". Render each line letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "Everything You Need" and "For One Number". Do not alter, misspell, duplicate, abbreviate, reorder, or split these strings.
ANCHOR (struck) PRICE reads exactly: "$1,997". Render this exact string, letter-for-letter, including the dollar sign and the comma, with no added, dropped, doubled, or substituted characters: "$1,997". Do not alter, misspell, duplicate, or drop any character of it.
FINAL PRICE reads exactly: "$497". Render this exact string, letter-for-letter, including the dollar sign, with no added, dropped, doubled, or substituted characters: "$497". Do not alter, misspell, duplicate, or drop any character of it.
PAYMENT LINE reads exactly: "or 3 payments of $179". Render this exact string, letter-for-letter, including the dollar sign and the numerals, with no added, dropped, doubled, or substituted characters: "or 3 payments of $179". Do not alter, misspell, duplicate, or drop any character of it.
SUB-LINE reads exactly: "Enrollment closes when the timer hits zero". Render this exact string, letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "Enrollment closes when the timer hits zero". Do not alter, misspell, duplicate, abbreviate, or garble any character of it.
The ONLY text baked onto this slide is the verbatim copy listed above (kicker, headline, anchor price, final price, payment line, sub-line). Do NOT bake any scene description, presenter narration, build note, the word "webinar", or any text not in this list.

TYPOGRAPHY: One typeface family (Montserrat); hierarchy is created by WEIGHT and SIZE, never by mixing typefaces. "TODAY ONLY" kicker in Montserrat Bold, approximately 22pt, gold #C4A44D, all-caps, generously letter-spaced. The two-line headline "Everything You Need / For One Number" in Montserrat Black, approximately 70-78pt, charcoal #231F20, tight leading so the two lines stack as one dominant block, with the single word "One" swapped to gold #C4A44D for emphasis. The anchor price "$1,997" in Montserrat ExtraBold, approximately 54pt, muted charcoal #6E6A66 (visually subordinate). The final price "$497" is the GIANT NUMBER hero of the slide: Montserrat Black, approximately 150pt, flat gold #C4A44D, roughly 2.7x the height of the struck price. The payment line "or 3 payments of $179" in Montserrat Medium, approximately 26pt, charcoal #231F20. The sub-line "Enrollment closes when the timer hits zero" in Montserrat Regular italic, approximately 17pt, charcoal #231F20. The typography is DESIGNED INTO the image as rendered designed type, baked into the pixels; it is never a basic font dropped on top. Basic or platform-default fonts (Calibri, Arial, Times, system default) are forbidden, and no text line appears without an exact weight AND a large pt size.

FONT PLACEMENT (per the Typography Architect type-layout card for the A4 type-dominant archetype; the canonical stack is the fallback only): the kicker "TODAY ONLY" with a 40px gold rule beneath it sits in the upper-left third; the two-line charcoal Black headline sits directly below the kicker across the upper-middle thirds and dominates the left and center of the frame; the price-tag motif carrying the struck anchor and the giant final price sits in the right third; the payment line sits just below the price tag; the italic sub-line sits in the lower margin. The hook refrain and any italic tertiary breathing line are NOT default rungs and do not appear on this slide. No text appears within 5% of any slide edge.

THIRDS GRID: the giant final-price number "$497" is the primary visual element and is anchored at the right-third vertical line, its optical center sitting at the upper-right thirds intersection so the eye lands on the number first. The headline text group occupies the left and center-left zones. The logo chip sits at the lower-right thirds intersection. The price tag and the headline do not overlap; a clear band of negative space separates them.

OBJECT PLACEMENT: the price-tag motif is a single large white hang-tag shape with a 2px gold #C4A44D border and a small punched hole at the top, positioned in the right third. On the tag: the anchor price "$1,997" appears in muted charcoal with a single clean DRAWN straight line in gold #C4A44D through the center of the numerals -- the line is a drawn object slightly wider than the text, NOT a font strikethrough style and NOT a diagonal scribble. Directly below and larger, the final price "$497" glows in gold at the bottom of the tag (a soft outer glow on the number only, never a gradient fill on the glyphs). Size contrast: the final price is approximately 2.7x the struck price. Objects must not overlap the headline text.

NEGATIVE SPACE AS A FEATURE: the breathing room around the giant number and between the headline block and the price tag is intentional and generous -- this is a confident, uncrowded money slide where the single number commands the frame. State the empty space as a deliberate compositional choice, not as an unfinished area to fill.

OVERLAYS: No text overlays on this slide beyond the verbatim copy above. No hook footer band. No bottom strip. No second idea.

BRAND PALETTE: Primary accent gold #C4A44D (money, value, the final number, dividers, the tag border, the kicker rule). Secondary accent raspberry #B23A66 (reserved -- not used on this slide so the gold reads as the single value color). Tertiary charcoal #231F20 (headline body, payment line, sub-line). All backgrounds remain warm off-white #FBF7F4. Headline contrast declaration: charcoal #231F20 on off-white #FBF7F4, contrast ratio 16.5:1, PASS (WCAG AA minimum 4.5:1 normal text, 3:1 large text). Final-price contrast declaration: gold #C4A44D on off-white #FBF7F4 at 150pt is large text, ratio approximately 2.1:1 -- BELOW AA for normal text but the number is reinforced by the gold tag border and the soft glow for separation; the surrounding charcoal headline carries the legibility load. No dark backgrounds.

LOGO (image-to-image): The first reference image is the company logo: place it in the bottom-right corner on a clean crisp white rectangular chip approximately 9% of slide width, with a subtle 1px gold #C4A44D border, at least 5% from any edge; do not redraw, recolor, restyle, reinterpret, or invent it -- reproduce the supplied mark pixel-for-pixel. One locked mark, identical to every other slide in the deck.

PEOPLE: none on this slide -- this is a type-dominant price reveal; the number is the hero and there is no human subject. Do not invent a person, a hand, or a crowd.

MOOD: confident, decisive, celebratory-but-grounded -- the relief and clarity of "this is finally within reach", readable in 2 seconds without narration. The visual energy should feel premium and trustworthy to a results-driven buyer who has just seen the value stacked. TEMPERATURE LOCK: WARM -- golden-hour light temperature, slightly lifted shadows, warm midtones, to match the deck's warm grade.

PROFESSIONALISM (standalone-art gate): this slide must read as a finished, gallery-grade STANDALONE PIECE OF ART, complete on its own with no other slide for context. Intentional art direction (the giant number as focal hero, generous negative space, a clear typographic hierarchy), the large creative typography composed INTO the image as part of the composition (not pasted on top), and a premium magazine-grade finish. No amateur stock-template look. No watermarks. No blur on the type. This is one image you could frame and hang. A slide that is "just a background with a price typed on it" is a defect.

// COLOR GRADING: TEMPERATURE=WARM, SATURATION=slightly elevated on the gold accent, desaturated warm neutral on the off-white field, TONAL CONTRAST=LOW (soft, even, premium), GRADE=match the deck's warm profile from the STYLE BLOCK. Every image in this deck must feel like it was shot in the same light.

DO-NOT BLOCK (final paragraph; inline, because GPT-Image 2 has no negative-prompt field):
Do not misspell, garble, duplicate, drop, add, substitute, or invent any letter, numeral, dollar sign, or comma; render every quoted string -- "TODAY ONLY", "Everything You Need", "For One Number", "$1,997", "$497", "or 3 payments of $179", "Enrollment closes when the timer hits zero" -- exactly as written, letter-for-letter.
Do not draw, invent, redesign, recolor, restyle, or substitute any logo, monogram, icon, leaf, sprout, tree, mountain, badge, roundel, or tagline lockup; reproduce only the supplied reference mark exactly as placed in the bottom-right white chip.
Do not render any bracketed token, any square brackets, or any of "owner to confirm", "insert", "tbd", "placeholder", "client win", "endorsement", "real result", "to supply", "pending", or any build note; only the quoted audience-facing copy appears.
Do not render any description of the picture, any spoken-script or presenter line, any stage direction, any telegraphing kicker ("before you decide", "one last proof", "this is not just a webinar"), any internal build note, or the literal word "webinar" as on-slide text.
Do not render any person, hand, face, or crowd on this type-dominant slide; there is no human subject here, so there are no fingers, limbs, or faces to deform.
Do not place a busy, cluttered, or high-detail background behind the price tag or the headline; keep the field clean off-white negative space so the type and the number stay legible at presentation distance.
Do not render the struck price as a font strikethrough style or a diagonal scribble; the strike is a single clean DRAWN gold line through the numerals. Do not apply any gradient, metallic, or radial-glow FILL to the glyphs of the price; the final number is flat gold with only a soft outer glow for separation.
Do not render any watermark, signature, UI artifact, emoji or clipart glyph, em dash, dark or pure-black background, grainy or banded texture, basic or platform-default font (Calibri, Arial, Times, system default), any text without a designed weight and large size, any text within 5% of any slide edge, or any plain background with text dropped on top that fails the standalone-art test.

COLOR VERIFICATION: off-white #FBF7F4 base confirmed across the full frame. Gold #C4A44D on the kicker, the kicker rule, the tag border, the drawn strike, the divider rules, the glow on "$497", and the logo chip border. Charcoal #231F20 on the headline body, the payment line, and the sub-line, with the single word "One" in gold. Muted charcoal #6E6A66 on the struck anchor price. Zero black backgrounds anywhere in the frame.
```

### Exemplar 2 -- A2 PEOPLE SLIDE (photo one side + text opposite, full density)

This exemplar demonstrates the three engines with an EXPLICIT REPRESENTATION_MIX drawn from the casting ledger (SOP-CAST-01), the 45/55 split, the depth layers, the base-color text devices, the COLOR VERIFICATION block, and the eight-class DO-NOT block. Substitute the client's captured REPRESENTATION_MIX, hexes, fonts, logo, and copy.

```
### SLIDE 09: I'm Not A Coach Who Read About It
[ARCHETYPE A2] [SECTION: AUTHORITY & STORY] [LADDER: none]
ONE BIG IDEA: the founder built and runs the exact thing she teaches; she is one of you, not an outsider who studied it.

// MODE: image-to-image (gpt-image-2-image-to-image). input_urls[0] = LOGO_URL <https URL of the locked company mark>. input_urls[1] = FOUNDER_PORTRAIT_URL is NOT used here (this is a cast audience-member, not the A5 founder-selfie path); the person is generated to the REPRESENTATION_MIX, not from a supplied portrait.

FORMAT: Create a 16:9 presentation slide image at 2K resolution (2560x1440 pixels). One single composed image: the photographed person, the typography, the brand rules, the divider, and the logo are all baked into the pixels of THIS one generation.

BACKGROUND / BASE: warm off-white #FBF7F4 across the full frame and across the entire right-hand text zone. NO black, navy, charcoal, or pure-black backgrounds anywhere in the frame.

LAYOUT: vertical 45/55 split. The LEFT 45% of the slide is a full-height photo panel carrying the person. The RIGHT 55% is the clean off-white #FBF7F4 text zone carrying the full text group. The person is placed on the LEFT because the eye reads the human first, lands on the face and the proof, then travels right into the promise text; the split is a deliberate 45/55, not a centered halve.

PHOTO PANEL (left 45%, full height) -- the person via the THREE ENGINES:

AUDIENCE ENGINE (explicit REPRESENTATION_MIX): the deck-wide REPRESENTATION_MIX for this client, drawn from the casting ledger (SOP-CAST-01), is [substitute the client's CAPTURED mix from the STYLE BLOCK, e.g. 70% African American women / 20% African American men / 10% multiracial, ages 35-55, dressed in the successful-owner code of the niche]. THIS slide is a single-subject shot, so it realizes ONE draw from that ledger: one African American woman in her late 40s, natural coiled hair worn in a shoulder-length style, dressed in a deep raspberry #B23A66 structured blazer over a cream shell -- professional but warm, the "successful owner" dress code of the niche, never corporate-stiff. This is a LEDGER DRAW for one slide, NOT a hardcoded per-slide demographic split: do not encode "60/30/10" or any fixed percentage into the scene itself; the deck-wide ratio is honored across all slides, never forced onto this one frame (the AF-R3 guard). Render rich, warm, dimensional skin true to the captured audience; do not lighten, ashen, grey, or desaturate deep skin tones.

FACIAL EXPRESSION ENGINE: AUTHORITY expression -- she looks direct to camera, settled, certain, no grin; the calm of someone who has done the thing and has nothing to prove. Shoulders square, chin level, a slight half-step lean toward the viewer that reads as "I am talking to you." Brow relaxed, eyes steady and warm, mouth closed in a composed, confident set -- not a sales smile, not a stock grin. The expression matches what the slide SAYS: she built it and she runs it. (Never write "smiling" alone; this is the AUTHORITY row of the Expression Vocabulary Table.)

WORLD ENGINE (real-world setting + justification + grounding): the setting is the founder's OWN working space or facility, not a studio cyclorama -- she stands inside or immediately adjacent to the real thing she built: [a justified real-world setting pulled from the client's niche and the GROUNDED_CONTENT variable -- for a strategy-consulting niche, her own bright office with a whiteboard of a real framework behind her; for a wellness niche, her own studio floor]. The setting is justified because the one idea is "I built it, I run it" -- she must be photographed inside the thing she built, not against a seamless backdrop. The props and the moment depict a concrete moment from THIS client's actual method (the GROUNDED_CONTENT), never an interchangeable stock scene. Premium lifestyle-documentary photography, real and warm.

LIGHTING: bright editorial interior, golden-hour daylight through real windows wrapping the face in soft warm key light, a gentle fill on the shadow side, and a subtle skin-tone-matched rim light separating her from the background. Clean, premium, aspirational. No institutional overhead fluorescents, no moody crushed shadow, no desaturation of the skin.

SHOT (taxonomy beneath the engines): three-quarter shot (Engine A, Single Subject), framed from mid-thigh up, the subject occupying the inner two-thirds of the photo panel with breathing room above the head. Her eye line falls at the upper-left thirds intersection of the photo panel.

DEPTH (all three layers explicit): FOREGROUND -- no near prop; the subject is the near plane. MIDGROUND -- the founder, sharp focus, separated from the background by the rim light and a shallow depth of field producing a soft background. BACKGROUND -- her real facility in softer focus and lower contrast, reading as environmental context, not a seamless studio backdrop.

The photo panel meets the text zone along a clean vertical line in gold #C4A44D (3px, full height), functioning as the premium divider between the two zones.

TEXT ZONE (right 55%, off-white #FBF7F4 background):

KICKER label -- top-left of the text zone, upper third, all-caps, generously letter-spaced, Montserrat SemiBold, approximately 16-18pt, gold #C4A44D. Reads exactly: "WHO'S TALKING TO YOU". Render this exact string, letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "WHO'S TALKING TO YOU". Do not alter, misspell, duplicate, or garble any character of it. A short 40px gold #C4A44D rule sits directly beneath the kicker.

HEADLINE -- directly below the kicker, left-aligned, Montserrat Black, very large (approximately 52-60pt relative to slide height), charcoal #231F20, three tight lines:
Line 1 reads exactly: "I'm Not A Coach". Render this exact string, letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "I'm Not A Coach". Do not alter, misspell, duplicate, or split it.
Line 2 reads exactly: "Who Read About It.". Render this exact string, letter-for-letter, including the period, with no added, dropped, doubled, or substituted characters: "Who Read About It.". Do not alter, misspell, duplicate, or split it.
Line 3 reads exactly: "I Built It. I Run It.". Render this exact string, letter-for-letter, including both periods, with no added, dropped, doubled, or substituted characters: "I Built It. I Run It.". Do not alter, misspell, duplicate, or split it -- and render the words "Built It" and "Run It" in raspberry #B23A66 for emphasis while the rest of the line stays charcoal.
The headline dominates the upper and middle thirds of the text zone and is the second thing the eye reads after the face.

A thin horizontal rule in gold #C4A44D (approximately 45% of the text-zone width, left-aligned) sits between the headline and the sub-copy below -- a premium breathing line.

SUB-COPY -- lower-middle third of the text zone, left-aligned, Montserrat Medium, approximately 20-22pt, charcoal #231F20, one line. Reads exactly: "Same long nights. Same payroll fear. I'm you.". Render this exact string, letter-for-letter, correctly spelled, with no added, dropped, doubled, or substituted characters: "Same long nights. Same payroll fear. I'm you.". Do not alter, misspell, duplicate, abbreviate, or garble any character of it. (Substitute the client's real peer-identity line from the niche; this is the illustrative form.)

The ONLY text baked onto this slide is the verbatim copy above (kicker, three headline lines, sub-copy). Do NOT bake any scene description, presenter narration, build note, the word "webinar", or any text not in this list. In particular, do NOT bake "she built and runs the exact thing she teaches" -- that is photo-brief direction for the imagery, never on-slide copy.

TYPOGRAPHY LAW: one typeface family (Montserrat); hierarchy by weight, never by mixing typefaces. Kicker in SemiBold; headline in Black; sub-copy in Medium; every line declares its exact weight AND a large pt size as stated above. The type is designed INTO the image as rendered designed type, not a basic font dropped on top. Basic or platform-default fonts (Calibri, Arial, Times, system default) are forbidden.
...
LOGO (image-to-image): the first reference image is the company logo: place it in the bottom-right corner of the text zone, approximately 9% of slide width, on a clean crisp white rectangular chip with a subtle 1px gold #C4A44D border, at least 5% from the slide edge and clear of the sub-copy; do not redraw, recolor, restyle, reinterpret, or invent it -- reproduce the supplied mark pixel-for-pixel. Never recolored, never distorted, never clipped.

MOOD: grounded, credible, peer-to-peer authority -- the image says "she is one of us and she actually did this", readable in 2 seconds without narration. Warm, bright, aspirational, premium documentary realism; not motivational-poster vague, not cold corporate headshot; a real owner in her real space, talking straight to one person. TEMPERATURE LOCK: WARM to match the deck grade.

PROFESSIONALISM (standalone-art gate): finished gallery-grade standalone piece of art, complete on its own with no other slide for context. Intentional art direction (focal hierarchy on the face, negative space in the text zone, depth of field), premium lifestyle-documentary photography (never stock, clipart, or cartoon), directional warm lighting, a clear hero subject, and the large creative typography composed INTO the image. Magazine-grade. Sharp focus on the human subject. One image you could frame and hang.

// COLOR GRADING: TEMPERATURE=WARM, SATURATION=slightly elevated on the subject and the raspberry wardrobe, desaturated warm neutral on the off-white field, TONAL CONTRAST=LOW-to-MEDIUM (soft, premium, dimensional skin), GRADE=match the deck's warm profile from the STYLE BLOCK. Every image in this deck must feel like it was shot in the same light.

DO-NOT BLOCK (final paragraph; inline, because GPT-Image 2 has no negative-prompt field):
Do not misspell, garble, duplicate, drop, add, substitute, or invent any letter or word; render every quoted string -- "WHO'S TALKING TO YOU", "I'm Not A Coach", "Who Read About It.", "I Built It. I Run It.", "Same long nights. Same payroll fear. I'm you." -- exactly as written, letter-for-letter.
Do not draw, invent, redesign, recolor, restyle, or substitute any logo, monogram, icon, leaf, sprout, tree, mountain, badge, roundel, or tagline lockup; reproduce only the supplied reference mark exactly as placed in the bottom-right white chip.
Do not render any bracketed token, any square brackets, or any of "owner to confirm", "insert", "tbd", "placeholder", "client win", "endorsement", "real result", "to supply", "pending", or any build note; only the quoted audience-facing copy appears.
Do not render any description of the picture, any spoken-script or presenter line, any stage direction, any telegraphing kicker, any internal build note, or the literal word "webinar" as on-slide text; the slide never narrates the scene it shows.
Do not render extra or missing fingers, malformed or fused hands, warped or distorted facial features, mismatched or asymmetric eyes, distorted teeth, plastic over-smoothed skin, or unnatural body proportions; her hands, face, and limbs are anatomically correct and natural.
Do not place a busy, cluttered, or high-detail background directly behind any text zone; keep the right-hand text zone clean off-white negative space so the type stays legible at presentation distance.
Do not render a demographic other than the slide's REPRESENTATION_MIX assignment; do not lighten, ashen, grey, or desaturate deep skin tones; do not mono-cast the deck; render rich, warm, dimensional skin true to the captured audience.
Do not render any watermark, signature, UI artifact, emoji or clipart glyph, em dash, dark or pure-black background, grainy or banded texture, basic or platform-default font (Calibri, Arial, Times, system default), any text without a designed weight and large size, any text within 5% of any slide edge, any text overlapping her face, a generic seamless studio backdrop behind the founder (she must be in her real facility), a salesy or grinning expression where calm authority belongs, or any plain background with text dropped on top that fails the standalone-art test.

COLOR VERIFICATION: off-white #FBF7F4 base confirmed across the full frame and the entire text zone. Raspberry #B23A66 on the wardrobe blazer and on the emphasis words "Built It" and "Run It". Gold #C4A44D on the kicker, the kicker tick, the vertical divider, the breathing rule, and the logo chip border. Charcoal #231F20 on the headline body and the sub-copy. Zero black backgrounds anywhere in the frame.
```

---

*End of how-to.md. All 19 sections present and filled. Appendix A (the two full-density gold-standard exemplars: Exemplar 1 A4 type-dominant price reveal, Exemplar 2 A2 people slide) follows Section 19.*
