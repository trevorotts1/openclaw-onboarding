...
1. Read the SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) gold-standard exemplar in full, and the Appendix A second exemplar (A2 people slide) in full. Do not write a single prompt before reading both (SOP 9.1 step 0).
...
- SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) gold-standard exemplar + Appendix A second exemplar (read in full before writing any prompt -- the density and structure reference)
- SOP-DESIGN-03-VARIABLE-LAYOUT-ANTI-TEMPLATE + brand-steward SOP (the five proven archetypes A1-A5) and SOP-ENGINE-00-INTELLIGENCE-ENGINES-FRAMEWORK element 11 (PRESENTATION-MASTER-DOCTRINE.md §4) (the three engines)
...
- The legacy native-text overlay ledger file (the eliminated working/checkpoints/ overlays JSON -- its former name is recorded in the RETIRED appendix of this file): this role NEVER writes it; the re-prompt/re-seed loop (then human escalation) is the only garble remedy, and its presence at assembly is the AF-OVERLAY-DELIVERED auto-fail
...
> **Phase-Code Map (per FIX of the short-code reconciliation):** the numeric short codes below resolve to manifest ids in `universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json` (manifest_version 60, 59 phases) exactly per the Director's Phase-Code Map (director-of-presentations.md Section 9). This role's own phases: prompt authoring `P4-PROMPT` (order 4.7), style preview `P-STYLE-PREVIEW` (4.85, fed by brand-steward's `P-STYLE-SPEC` 4.84 and closed by the human pick `P-STYLE-PICK` 4.86), deterministic render `P4-RENDER` (4.9, the `build_deck.py` render this department's QC consumes), upsell design `P-U-DESIGN-SALES`/`P-U-DESIGN-CHECKOUT`/`P-U-DESIGN-VSL` (4.2/4.3/4.4); the QC gates around them are `P1Q-COPY-QC` (4.2), `P-PROMPT-QC` (4.8), `P-IMAGE-QC` (4.95), and `P-SHIFT-QC` (7.5); typography lock is `PF-DESIGN` (4.5); copy is `P4-COPY` (4). The manifest id is the canonical key when reading a manifest row.

### SOP 9.1 -- Per-Slide Prompt Authoring (15-Element Spec; manifest id `P4-PROMPT`, order 4.7)

**When to run:** Phase 2 -- after slides_copy.md is approved by the owner (Phase 1A; an interlock between `P1Q-COPY-QC` and `P4-PROMPT`, not a manifest phase) and STYLE BLOCK is confirmed.
...
2. Write the 15-element prompt in this exact order. Each element must be present:
   ...
   10. **LOGO (ONE locked mark, composited image-to-image -- the logo-mutation defect fix; SOP-IMG-01 + SOP-DESIGN-04):** "[Logo placement per STYLE BLOCK rule -- typically lower-right corner on a clean white chip with a 1px gold border, approximately 9% of slide width, at least 5% from any edge, consistent size and position on every slide.]" **When LOGO_ON_SLIDES = true, the slide is generated in image-to-image mode (Mode B, model `gpt-image-2-image-to-image`) with the single locked `LOGO_URL` as the FIRST entry of `input_urls`, and the prompt MUST carry this verbatim sentence:** "The first reference image is the company logo: place it exactly as specified (bottom-right white chip, approximately 9% slide width, 1px gold border, at least 5% from any edge); do not redraw, recolor, restyle, reinterpret, or invent it -- reproduce the supplied mark pixel-for-pixel." A logo described only in words with no reference image, or any text-to-image (Mode A) call on a logo slide, is forbidden and is an AUTO-FAIL at QC (AF-P15; SOP-IMG-01 check 1 and check 3). The matching negative twin lives in the SOP 9.8 negative block: "Do not invent, redesign, or substitute any logo, monogram, icon, leaf, sprout, tree, mountain, badge, roundel, or tagline lockup; the only mark on the slide is the supplied reference logo." This is the closing of the forensic logo-mutation loop: passing the logo via image-to-image is the write-time guard, and the rendered-logo-matches-locked-asset diff (AF-F7) is the read-time guard. The logo reference is NEVER given the style-reference-only directive (that would erase its shape); reserve that directive for any optional style frame only.
   11. **PEOPLE (driven by the THREE ENGINES, transplanted from SOP-ENGINE-00-INTELLIGENCE-ENGINES-FRAMEWORK (PRESENTATION-MASTER-DOCTRINE.md §4) element 11; all three required on every people-slide, missing any one is an auto-fail):**
...
3. Verify character count of the completed prompt. Target: 9,000-14,000 characters. Hard floor: 9000 (AF-P1; build_deck.py PROMPT_CHAR_FLOOR=9000 refuses a sub-floor prompt). Hard maximum: 18,000 (the LONG-tier budget; GPT-Image 2 accepts up to 20,000 on both endpoints per MODEL-SPECS, and 18,000 keeps a 2,000-character safety margin). If under 9,000, the prompt is starved -- expand with genuine specificity (the per-line spelling-lock, the full paired negative block, exhaustive logo-placement language, complete people-anatomy direction, deeper world/scene detail), never with boilerplate. If over 18,000, the prompt is too long for the LONG tier -- tighten redundant phrasing, never by deleting the negative block or any spelling-lock. Spend the expanded budget ONLY on defect-preventing specificity (density-calibration rule, SOP 9.4-strengthening).
...
**RENDERED VERBATIM (build_deck.py contract -- ADDITIVE, does not change the authoring rules above):** the deterministic renderer (`scripts/build_deck.py`) loads THIS rich prompt file and sends it to KIE (gpt-image-2) **VERBATIM** -- the WHOLE slide (typography sizes and per-line weights, placement, usage, the logo(s) via image-to-image `input_urls`, the scene, the verbatim copy, and the negative block) is rendered in ONE generation. The renderer NEVER composes its own thin scene+copy prompt and NEVER silently falls back to one. The renderer enforces a HARD floor of **9,000 characters** (AF-P1, `build_deck.py` PROMPT_CHAR_FLOOR=9000, mirrored in the shared `prompt_gate.py` every image-API path imports): a slide whose prompt file is absent or under 9,000 chars is NOT run, NOT rendered, and NOT updated -- it fails loud. (The authoring target of 9,000-14,000 sits inside this band; the renderer-side floor and the upstream Phase-3 QC standard are the SAME 9,000 number, so a prompt that clears authoring cannot be refused at render.) The renderer also reads `working/prompts/slide-NN.txt`; either name (`slide-NN.txt` or `slide-NN-prompt.txt`) is accepted.
...
**THE FIVE PROVEN ARCHETYPES (primary, transplanted from SOP-DESIGN-03-VARIABLE-LAYOUT-ANTI-TEMPLATE + brand-steward SOP (PRESENTATION-MASTER-DOCTRINE.md §4); the deck is built on exactly these five, rotated):**
...
2. When LOGO_ON_SLIDES = true, write element 10 to PASS the locked LOGO_URL as the first reference in `input.input_urls` and name it in the prompt: "The first reference image is the company logo: place it [per the rule], do not redraw, recolor, or restyle it." NEVER text-to-image the mark. One locked asset, composited identically on every slide (the anti-mutation path; SOP-IMG-01 Mode B, design-system/05). If the logo garbles twice on render, the real logo IMAGE is composited onto the slide PNG via the PIL image-composite path (SOP-IMG-05) BEFORE assembly -- an IMAGE composite of the locked mark, NOT a native PPTX text run or overlay (the native-overlay path is eliminated, Decision 5C; AF-OVERLAY-DELIVERED).
...
2. On a dedicated hook slide, write the hook as the hero: "The verbatim hook line '[CANONICAL HOOK]' set very large in BLACK weight, centered, over a single low-opacity image at 15% opacity or lower (HOOK_IMAGE_OPACITY_CEILING_PCT = 15 in build_deck.py); '[emphasis word(s)]' in [accent]; no other copy, no footer band, no second idea, printed exactly once."
...
- The SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) gold-standard exemplar (the density reference)

**Steps:**
1. Every prompt TARGETS 9,000 to 14,000 characters, calibrated against the SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) exemplar (and the second A2 exemplar in Section 9.5 strengthening) as the density reference, then enriched to the higher band with the defect-preventing specificity this SOP adds (the per-line spelling-lock on every text string, the full eight-class paired negative block, the exhaustive image-to-image logo-placement language, the complete people-anatomy direction, deeper world/scene and grade detail). GPT-Image 2 accepts up to 20,000 characters on both endpoints and is strongest on long structured prompts (MODEL-SPECS, the authoritative source); the old short floor used only a fraction of the available budget and STARVED the prompt of the specificity that prevents the forensic defects. The exemplars show the art-direction floor; the higher band is reached by garble-proofing and negative-prompting every line, not by padding.
...
4. Hard limits still apply: under 9,000 characters is an auto-fail (AF-P1, the HARD floor -- the renderer refuses a sub-floor prompt outright); over 18,000 is an auto-fail (AF-P2, the LONG-tier budget, a 2,000-char safety margin below the 20,000 API ceiling). Never trim below the limit by deleting the negative block or any spelling-lock; tighten redundant phrasing instead.

**Outputs:**
- Every prompt in the 9,000 to 14,000 target band (or a documented reason it sits outside, within the 9000 to 18,000 working range)

**Hand to:** SOP 9.1 step 3 verification

**Failure mode:** If a prompt cannot reach the 9,000-character HARD floor with genuine specificity even after the spelling-lock, the full paired negative block, the image-to-image logo language, and the complete anatomy direction are present, the slide's spec is probably too thin (a near-empty transition slide). Deepen the art direction until it genuinely clears the floor -- the floor is not waivable, and a sub-floor prompt is refused at render (AF-P1). Never inflate with filler to pass the count, and never reach the count by padding boilerplate (AF-P-DENSITY requires 220+ distinct words).
...
7. **Garbled-text remedy = RE-PROMPT / RE-SEED loop, then HUMAN ESCALATION (Decision 5C -- the native-text overlay path is ELIMINATED, AF-OVERLAY-DELIVERED).** The legacy "two-failed-attempts native-text fallback" is REMOVED; it was NOT scoped to price text only -- it nominally applied to EVERY critical verbatim string (every headline, sub-headline, supporting line, kicker label, price, struck price, and any logo wordmark) that garbled, misspelled, or duplicated twice at image QC (the "hclarity" / "IDEHNOTTY DEVELOPMENT" defect class). That whole mechanism is retired. Today: (a) RE-PROMPT the slide (tighten the element-3 spelling-lock, strengthen the paired negative block) AND RE-SEED it (use a new seed) and re-render the SINGLE composed gpt-image-2 image; (b) if it still garbles, re-prompt/re-seed again within the loop; (c) if the garble PERSISTS after the loop, ESCALATE TO A HUMAN -- never paper it over with a native PPTX text box, and NEVER write the legacy native-text overlay ledger file: its mere presence at assembly is AF-OVERLAY-DELIVERED. For a struck price that garbles, the same loop applies -- re-prompt the drawn-line strike + re-seed, then escalate. (The LOGO is the only image-composite exception and it is NOT native text: when the model cannot bake the locked logo cleanly, the real logo IMAGE is composited onto the PNG via the PIL image-composite path SOP-IMG-05, baked into the image before assembly.) The full retired fallback specification is preserved in the RETIRED appendix at the end of this file.

**Outputs:**
- Price-drop slide prompts with the price-tag motif, the drawn-line strike, and new-price formatting instructions
- A re-prompt/re-seed render record (and, on persistent garble, a human-escalation note) -- NEVER a legacy native-text overlay ledger entry (Decision 5C; AF-OVERLAY-DELIVERED)

**Hand to:** QC Specialist (for Phase 3 prompt QC, which checks price-drop slides against price_ladder.json); PPTX Assembly Specialist receives re-prompt/re-seed render records only -- there is NO native-overlay handoff (Decision 5C; a legacy overlay ledger entry would be AF-OVERLAY-DELIVERED)
...
3. The size scale is applied: giant numbers 110-150pt, hero 2-line headline 62-86pt, secondary headline 42-62pt, sub-headline 24-32pt, body beat 18-22pt (BODY_BEAT_PT_RANGE = (18, 22) in build_deck.py), tertiary italic 16-19pt, kicker label ~13pt, footnote 11-13pt.
...
**When to run:** As the structural reference for SOP 9.1 -- the anatomy of a complete, defect-proofed, long-budget prompt. Use it alongside the SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) exemplar; this template adds the spelling-lock, the image-to-image logo directive, and the SOP 9.8 negative block that the bare 15-element order does not show. Adapt every bracketed value from slides_copy.md and the STYLE BLOCK; never ship the bracket tokens themselves (they are an AF-P16 / AF-F10 defect on render).
...
Every prompt: hard floor 9000 (AF-P1), hard maximum 18,000 characters (the LONG-tier budget; GPT-Image 2 accepts up to 20,000, MODEL-SPECS). Target 9,000-14,000. Over 18,000 is an auto-fail (AF-P2). The budget is spent on defect-preventing specificity, never boilerplate.
...
- PPTX Assembly Specialist -- receives re-prompt/re-seed render records (and, on persistent garble, human-escalation notes) for any slide whose text garbled at image QC; the legacy native-text overlay handoff is eliminated (Decision 5C; AF-OVERLAY-DELIVERED)
...
- A prompt that mentions the brand name literally (e.g., naming a real company) -- use BlackCEO token or the client_slug from intake.json only.
...
- A verbatim text string with no letter-for-letter spelling-lock instruction (the garbled-text path: "hclarity", "IDEHNOTTY DEVELOPMENT"). Every string carries its spelling-lock. AUTO-FAIL (AF-P14).
...
- A prompt that does not declare its archetype (A1-A5) on line 1. Every prompt declares its archetype in the first line, per the SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) exemplar.
...
- Writing a prompt before reading BOTH gold-standard exemplars (SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4), A4, and Appendix A, A2).
...
| 10 | Garbled / misspelled rendered text ("hclarity", "IDEHNOTTY DEVELOPMENT") | Wrap EVERY verbatim string in the element-3 spelling-lock; add negative-block class 1; on garble, run the re-prompt/re-seed loop and, on persistent garble, escalate to a human (Decision 5C -- the native-text overlay route is eliminated, AF-OVERLAY-DELIVERED; SOP 9.5 step 7). |
...
**SOP 9.5 strengthening.** The SOP-IMG-01-KIE-CALL-MECHANICS + prompt-author-presentations SOP + brand-steward SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) exemplar is an A4 type-dominant slide. This is the SECOND gold-standard exemplar, authored for an A2 people slide (photo one side, text opposite) at full density. **Read BOTH exemplars before writing your first prompt (SOP 9.1 step 0):** the master 7.5 A4 exemplar AND this A2 exemplar. Together they show the density and structure your prompts must match across the two most common archetypes. This exemplar uses generic brand grammar (substitute the client's intake hexes, fonts, logo, representation mix, and copy); it demonstrates the three engines (Facial Expression, Audience, World), the 45/55 split, the base-color text devices, the COLOR VERIFICATION block, and the AVOID block.
...
## RETIRED — Legacy Native-Text Overlay Fallback (Decision 5C; archived by FIX 80)

> Everything below this heading is RETIRED doctrine, preserved for historical reference only. Decision 5C (AF-OVERLAY-DELIVERED) eliminated the native-text overlay fallback: a garbled slide is remedied by the re-prompt/re-seed loop, then human escalation -- never by compositing native PPTX text. DO NOT re-introduce any mechanism below; the archive exists so the design history is not lost. This appendix is the only place in this file where the retired tokens appear.

**RETIRED: former Tools entry (overlay ledger write)**
...
**RETIRED: former logo garble remedy sentence (Element 10 step 2, pre-PIL path)**
...
**RETIRED: former SOP 9.5 step 7 (the two-failed-attempts native-text fallback)**
...
**RETIRED: former outputs bullet (overlay ledger entries)**
...
**RETIRED: former Hand-to line (overlay ledger handoff)**
...
**RETIRED: former Section 11 handoff line (overlay ledger handoff)**
...
**RETIRED: former Section 15 mistake-table row 10 prevention (native-text overlay route)**

| 10 | Garbled / misspelled rendered text ("hclarity", "IDEHNOTTY DEVELOPMENT") | Wrap EVERY verbatim string in the element-3 spelling-lock; add negative-block class 1; route to native-text overlay after two failed renders (SOP 9.5 step 7, generalized to all critical text). |

*End of RETIRED appendix. Nothing above this appendix in this file, and nothing inside it, is live doctrine; the live doctrine is the Decision-5C re-prompt/re-seed loop then human escalation.*
