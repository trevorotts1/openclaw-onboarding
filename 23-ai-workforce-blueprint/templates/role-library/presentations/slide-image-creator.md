...
- 45-design-intelligence-library/library/_system/MODEL-SPECS.md (read -- the authoritative model limits: GPT-Image-2.5 accepts up to 20,000 chars on both endpoints, has no negative-prompt field so negatives go inline, LONG tier is up to 18,000 chars)
...
> **Phase-Code Map (per FIX of the short-code reconciliation):** the numeric short codes below resolve to manifest ids in `universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json` (manifest_version 68, 62 phases) exactly per the Director's Phase-Code Map (director-of-presentations.md Section 9). This role's own phases: prompt authoring `P4-PROMPT` (order 4.7), style preview `P-STYLE-PREVIEW` (4.85), deterministic render `P4-RENDER` (4.9, the `build_deck.py` render this department's QC consumes), upsell design `P-U-DESIGN-SALES`/`P-U-DESIGN-CHECKOUT`/`P-U-DESIGN-VSL` (4.2/4.3/4.4); the QC gates around them are `P1Q-COPY-QC` (4.2), `P-PROMPT-QC` (4.8), `P-IMAGE-QC` (4.95), and `P-SHIFT-QC` (7.5); typography lock is `PF-DESIGN` (4.5); copy is `P4-COPY` (4). The manifest id is the canonical key when reading a manifest row.
...
**RENDERED VERBATIM (build_deck.py contract -- ADDITIVE, does not change the authoring rules above):** the deterministic renderer (`scripts/build_deck.py`) loads THIS rich prompt file and sends it to KIE (gpt-image-2.5) **VERBATIM** -- the WHOLE slide (typography sizes and per-line weights, placement, usage, the logo(s) via image-to-image `input_urls`, the scene, the verbatim copy, and the negative block) is rendered in ONE generation. The renderer NEVER composes its own thin scene+copy prompt and NEVER silently falls back to one. The renderer enforces a HARD floor of **9,000 characters** (AF-P1, `build_deck.py` PROMPT_CHAR_FLOOR=9000, mirrored in the shared `prompt_gate.py` every image-API path imports): a slide whose prompt file is absent or under 9,000 chars is NOT run, NOT rendered, and NOT updated -- it fails loud. (The authoring target of 9,000-14,000 sits inside this band; the renderer-side floor and the upstream Phase-3 QC standard are the SAME 9,000 number, so a prompt that clears authoring cannot be refused at render.) The renderer also reads `working/prompts/slide-NN.txt`; either name (`slide-NN.txt` or `slide-NN-prompt.txt`) is accepted.
...
| A5 | PORTRAIT / SELFIE (image-to-image) | The client's REAL founder portrait (supplied photo passed as an additional input image via gpt-image-2-5-sunburst-image-to-image) drives the slide; text group beside or below | Host intro, "I'm you" slides, guarantee (founder holding certificate), final push direct-to-camera |
...
1  Line 1 tags + ONE BIG IDEA line.
2  Canvas + base + NO black backgrounds.
3  Reference-image instruction (CRITICAL): state explicitly "the second reference image is the founder; her likeness drives the portrait" (the first reference is the logo). This is the gpt-image-2-5-sunburst-image-to-image path; the founder portrait URL is passed in input.input_urls.
...
```
DO-NOT BLOCK (final paragraph; inline, because GPT-Image-2.5 has no negative-prompt field):
...
// MODE: image-to-image (gpt-image-2-5-sunburst-image-to-image). input_urls[0] = LOGO_URL <https URL>.
...
Every prompt: hard floor 9000 (AF-P1), hard maximum 18,000 characters (the LONG-tier budget; GPT-Image-2.5 accepts up to 20,000, MODEL-SPECS). Target 9,000-14,000. Over 18,000 is an auto-fail (AF-P2). The budget is spent on defect-preventing specificity, never boilerplate.
...
- (c) LOGO IMAGE-TO-IMAGE DECLARED: on every LOGO_ON_SLIDES = true slide, the prompt declares Mode B (`gpt-image-2-5-sunburst-image-to-image`) with LOGO_URL as the first reference and the verbatim "place, do not redraw" sentence. A logo-in-words-only or text-to-image logo prompt = AUTO-FAIL (QC AF-P15; SOP-IMG-01 check 1/3).
...
**Tier 2:**
- Kie.ai GPT-Image-2.5 model documentation (prompt engineering best practices for this specific model)
