...
**Master authority:** universal-sops/CLIENT-WEBINAR-DECK-SOP.md (SOP-DESIGN-01-CREATIVE-TYPOGRAPHY-GUIDE (strikethrough/price-typography handling) + typography-architect SOP 9.3 (PRESENTATION-MASTER-DOCTRINE.md §4) the image engines, SOP-PITCH-01..06 cluster (PRESENTATION-MASTER-DOCTRINE.md §4) the pitch choreography)
...
- a VISION VERDICT half -- the perceptual call, graded at Image-QC and LOGGED to the per-slide rows of `working/qc/image_qc_report.json` (which AF-IMAGE-QC-VISION requires to carry a declared vision engine and a per-slide observation for every rendered slide). Codes: AF-FACE-MOOD, AF-WORD-IMAGE-MISMATCH/world-grounding, AF-LIGHT-SKINTONE, the plastic-hair read.
