...
**Enforced at the gate by:** QC Specialist - Presentations + QC Specialist - Image; the deterministic gate `build_deck.check_deck_harmony` (deck-level, the only harmony function in `build_deck.py`, defined at line 5726), and the per-slide harmony dimension inside `check_prompt_qc_deterministic`
...
|---|---|---|---|---|
| **Narrative harmony** | COPY-QC (Phase 1Q), **before any prompt is authored** | the arc coheres in the COPY — villain→hero ordering holds, the promise/stakes/proof/CTA beats sequence into one story, the pitch cadence is unbroken | **BLOCKED-HUMAN:** no separate copy-level harmony gate exists in `build_deck.py`; `check_deck_harmony` (the only harmony function, at line 5726) runs deck-level after render. See `CONTROL/QUESTIONS-FOR-HUMAN.md` U010. | Slide Copywriter / Offer-Price Strategist |
...
| **Deck-level visual harmony** | PRE-ASSEMBLY, **after render, before assembly/delivery** | across ALL rendered PNGs the four dimensions hold — recurring character, palette, world, archetype rhythm | `build_deck.check_deck_harmony` (deck-level), alongside `check_brand_consistency` + `check_visual_variety` | Director of Presentations (re-render ONLY the inconsistent slides) |
