...
**Detection script:** `scripts/intelligence_engines_check.py check_copy` (PRIORITY_STACK_NAMED before first ladder beat; explicit re-rank beat after PRICE; time-bound CTA token in CTA slide) + `scripts/build_deck.py` (PEAK tag in `arc_allocation.json`; peak+closing in top creativity-budget quartile; non-flat ending). REGISTERED — all four codes live in PIPELINE-MANIFEST.json (v20+) and enforced by `intelligence_engines_check.check_copy` (moves 1/7/8) + `build_deck` AF-PEAK-END (move-3/4 peak).
**Registered:** REGISTERED — all four codes live in PIPELINE-MANIFEST.json (v20+) and enforced by `intelligence_engines_check.check_copy` (moves 1/7/8) + `build_deck` AF-PEAK-END (move-3/4 peak). The eight-move map is recorded in `priority_shift_spec.json.eight_move_map[]`.
...
**Status:** **DOCTRINE — WIRED.** REGISTERED — all four codes live in PIPELINE-MANIFEST.json (v20+) and enforced by `intelligence_engines_check.check_copy` (moves 1/7/8) + `build_deck` AF-PEAK-END (move-3/4 peak). This SOP is wired into the existing arc allocation and pitch machinery rather than duplicating it.
