...
**Owning roles at write time:** Signature Presentation Architect (runs SOP 9.1 / 9.2); Brainstorming Buddy (signature mode) is the front door that runs the choice-first, one-question-at-a-time intake via `deck-intake-driver.py --signature`, then assembles the answers into the one atomic record.
...
2. **RECORD LAYER (one atomic block).** Once the answers are gathered one at a time, they are **assembled into ONE atomic intake RECORD** (`working/copy/sp_intake.json`). The record fields `asked_all_at_once: true` / `mode: one_block` / `one_question_per_turn: false` describe **that assembled ledger being committed as one atomic block** — they are NOT a licence to dump the 8 Questions at the owner. This assembled record is what `prove_sp_intake.py` validates; until it passes, slide authoring (`P4-COPY` and everything downstream) is locked.
...
**Record proof (`AF-SP-8Q-SPLIT`).** After the one-at-a-time conversation, the answers are committed as ONE atomic ledger. The runtime record must prove single-block COMMIT: `asked_all_at_once: true`, `one_question_per_turn: false`, a delivery `mode` of `one_block`, the frame question's `asked_in_same_block: true` (when present), and a `question_block_msg_id` that references EXACTLY ONE record commit. This is deterministic (`prove_sp_intake.py`) and gates the machine RECORD, not the conversation — `AF-SP-8Q-SPLIT` fires when the assembled ledger was not committed as one atomic block (`asked_all_at_once != true`, `one_question_per_turn == true` on the record, or more than one commit id). It is NOT tripped by asking the owner one question at a time — that is the REQUIRED conversation behavior.
...
`deck-intake-driver.py --signature` emits the intake plan — the choice-first `conversation_contract` (offer QUICK vs IN-DEPTH, then ask ONE question at a time), all 8 Questions, and the frame question — reading the SACRED spec `51-signature-presentation/intake/sp-8-questions.json`. The front-door agent runs the conversation one question at a time (never a batch — `AF-INTAKE-BATCH`). After the client answers, `--signature --record <answers.json>` **assembles the answers into ONE atomic record** `working/copy/sp_intake.json` (stamping `asked_all_at_once` / `one_block` on the COMMITTED ledger) and **runs `prove_sp_intake.py` against it** — the driver is wired straight to the `AF-SP-8Q-SPLIT` gate and exits non-zero (fail-closed) if the record is not a single atomic commit, is missing a question, or has no frame set. The driver never marks an intake verified when the gate cannot be run.
