...
`deck-intake-driver.py --signature` emits the intake plan — the choice-first `conversation_contract` (offer QUICK vs IN-DEPTH, then ask ONE question at a time), all 8 Questions, and the frame question — reading the SACRED spec `51-signature-presentation/intake/sp-8-questions.json`. The front-door agent runs the conversation one question at a time (never a batch — `AF-INTAKE-BATCH`). After the client answers, `--signature --record <answers.json>` **assembles the answers into ONE atomic record** `working/copy/sp_intake.json` (stamping `record_committed_atomically` / `one_block` on the COMMITTED ledger) and **runs `prove_sp_intake.py` against it** — the driver is wired straight to the `AF-SP-8Q-SPLIT` gate and exits non-zero (fail-closed) if the record is not a single atomic commit, is missing a question, or has no frame set. The driver never marks an intake verified when the gate cannot be run.
