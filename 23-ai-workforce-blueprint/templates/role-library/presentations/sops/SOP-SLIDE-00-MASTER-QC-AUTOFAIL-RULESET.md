...
| AF-CADENCE | 1Q/6 | DECK | a price-drop rung's window does not run VALUE_ADD->PROMISE->REPITCH_MINI->COST_OF_INACTION in order | pitch_engines_check.chk_cadence: per adjacent DROP pair in price_ladder.json.rungs, walk the slides_copy.md arc-tag window and assert the four beats appear in monotonic slide order; missing/misordered beat = fail (names the rung). Layers on AF-C7's global floor (SOP-PITCH-06 1.1) |
| AF-NO-COST-OF-INACTION | 1Q/6 | DECK | deck never states the cost of NOT acting | pitch_engines_check.chk_cost_of_inaction: price_ladder.json.cost_of_inaction_slide non-null AND a COST_OF_INACTION arc beat tagged in slides_copy.md; promotes offer SOP 9.6 to a hard gate (SOP-PITCH-06 1.2) |
| AF-GUARANTEE-GENERIC | 1Q/6 | slide | guarantee is a bare refund template with no felt frame | pitch_engines_check.chk_guarantee_generic: strip generic phrases (money-back/30-day/refund/satisfaction-guaranteed/no-questions) from price_ladder.json.guarantee.statement; nothing substantive left AND no guarantee_felt_frame = fail; felt-vs-generic verdict logged to copy_qc_report.json (SOP-PITCH-06 1.3) |
| AF-NO-BRANDED-METHOD | 1Q | DECK | generic intake content but zero named-method beat | pitch_engines_check.chk_branded_method: no NAMED_METHOD arc beat AND no intake.json.named_methodology = fail (propose a name for owner approval; de-inverts the retired ban) (SOP-PITCH-06 1.4) |
| AF-METHOD-FABRICATED | 1Q | slide | a NAMED_METHOD beat with no client source and no owner approval (silent fabrication) | pitch_engines_check.chk_branded_method: NAMED_METHOD arc beat present AND neither intake.json.named_methodology nor method_name_approval.json owner_approved:true = fail (SOP-PITCH-06 1.4) |
| AF-NO-TIME-TO-RESULT | 1Q | DECK | no EXPECTATION beat stating how long the result takes | pitch_engines_check.chk_time_to_result: an EXPECTATION arc beat with a nearby duration token AND intake.json.time_to_result non-null; absent beat / no duration / no intake source = fail (SOP-PITCH-06 1.5) |
| AF-SPEECH-HOOK-COUNT | Speech-QC | DECK | hook sung fewer than 5 or more than 20 times in the speech | pitch_engines_check.chk_speech_hook_count: count char-exact intake.json.hook occurrences in PRESENTERS-SPEECH.md; count<5 or count>20 = fail (defers when speech absent). Slide ceiling AF-HOOK-1 unchanged (SOP-PITCH-06 1.8) |
...
| AF-PROMPT-FLOOR | Phase Prompt-QC | slide | Image prompt under the reconciled 9,000-char standard OR over the ceiling OR missing required structural blocks | len(prompt) < 9000 (PROMPT_CHAR_FLOOR) OR len(prompt) > 18000 (PROMPT_CHAR_CEILING) OR prompt missing [ARCHETYPE, the negative-block header "DO-NOT BLOCK" (canonical; the legacy header "NEGATIVE BLOCK" is an accepted alias), "Do not " imperatives] |
...
| AF-OVERLAY-DELIVERED | 5/6/Postflight | slide | A delivered slide ships native on-slide text (or a pptx_text_overlays.json is present) instead of a single composed gpt-image-2.5 image | _chk_no_overlay(run_dir): fails if any eliminated pptx_text_overlays.json file is present OR the delivered PPTX carries any native (non-notes) on-slide text run; the native-overlay path is eliminated by construction; persistent garble re-prompts/re-seeds then escalates to a human, never a native overlay; strengthens AF-BAKED/AF-I14 (Decision 5C) |
...
| AF-NO-RUN-DIR | Delivery-Boundary | DECK | A deck artifact (.pptx/.pdf) is presented for delivery with NO governed run dir (no working/checkpoints/process_manifest.json) — hand-built OUTSIDE run_signature_deck.py | gate_delivered_artifact(artifact, run_dir): the OUT-OF-BAND delivery boundary gate inspects the SHIPPED ARTIFACT regardless of how it was produced. FAIL-CLOSED: if no governed run dir resolves from the artifact (walk-up) or an explicit run dir, the deck never passed through the governed pipeline and is REJECTED. Flips the legacy delivery_gate no-run-dir DEFER to a REJECT at the boundary. Skippable ONLY by a logged owner_skip_approval token (gate=AF-NO-RUN-DIR) in process_manifest.json or an owner_skip_approval.json adjacent to the artifact |
| AF-NOT-KIE-RENDERED | Delivery-Boundary | DECK | The shipped artifact cannot be proven kie.ai-baked: no real kie taskId per slide in the render record, OR the .pptx/.pdf is unreadable / a renamed-image or hand-built decoy | gate_delivered_artifact(artifact, run_dir) + check_kie_provenance + inspect_pptx_artifact/inspect_pdf_artifact: FAIL if process_manifest.json's render record maps no slide (or any slide) to a REAL kie taskId (taskId in {None,'',native,placeholder,none,null,n/a}), OR the delivered deck is not a readable OOXML/PDF deck. Stdlib-only (zipfile/zlib/re) so it bites on any box with no python-pptx dependency. Skippable ONLY by a logged owner_skip_approval token (gate=AF-NOT-KIE-RENDERED) in process_manifest.json |
...
| AF-HOOK-IMG | Prompt-QC/5 | DECK+slide | IMAGE-side hook: the canonical hook is not baked verbatim on 3-4 dedicated beats, OR is baked on > 4 slides, OR appears as a footer-band/over-stamp, OR a per-slide baked count >= 2, OR a pure-typography hook slide is excluded from scope | build_deck.check_hook_image: deterministic verbatim-count + footer-band scan + per-slide-count over the prompts' baked text and the render record. Mirrors the AF-HOOK-1 / AF-HOOK-4 / AF-C2 / AF-OBI-3 thresholds on the IMAGE side (the copy-side battery stays unchanged). A hook slide is NEVER out of scope. (v15.0.0 G3; resolves the AF-P12 collision + the hook_package/hook_variants artifact split) |
| AF-HARMONY | 1Q + Prompt-QC + Pre-Assembly/6 | DECK | The engines do not COHERE across the deck even though each slide passes in isolation: recurring-character discontinuity, palette/brand incoherence, world/scene discontinuity, or a broken archetype rhythm | build_deck.check_harmony (deck-level) + check_narrative_harmony (copy) + per-slide harmony (prompt). The orchestration layer ABOVE the per-engine checks: recurring-character continuity (Story) + palette coherence (cross-checked with check_brand_consistency) + world/scene continuity + archetype rhythm. Realized at THREE shift-left placements — narrative harmony at COPY-QC, per-slide harmony at PROMPT-QC, deck-level visual harmony at PRE-ASSEMBLY (SOP-HARMONY-01). Distinct from AF-CREATIVITY (which only rejects template SAMENESS) |
| AF-EXCELLENCE | Prompt-QC | slide + DECK | A prompt clears the 9,000-char LENGTH floor but spent the budget on boilerplate padding rather than defect-preventing specificity — floor-grazing but soulless. The QUALITY floor, INDEPENDENT of length (a 9,000-char prompt that is off-engine or off-harmony still routes back; §1.1a TWO FLOORS) | build_deck.check_prompt_qc_deterministic EXCELLENCE dimension: scores density spent on per-line spelling-lock coverage, the full 8-class negative block, complete people-anatomy + world-grounding, and grade detail. Below the EXCELLENCE floor routes back even at >= 9,000 chars. Length never buys a pass; engines never buy a pass; BOTH gates are required |
...
**v15.0.0 — the copy + pitch engine codes now ALSO fire at the render preflight (un-bypassable backstop).** `AF-NO-VILLAIN`, `AF-NO-FELT-STAKES`, `AF-CADENCE`, `AF-NO-COST-OF-INACTION`, `AF-GUARANTEE-GENERIC`, `AF-NO-BRANDED-METHOD` / `AF-METHOD-FABRICATED`, and `AF-NO-TIME-TO-RESULT` now ALSO fire un-bypassably inside the render preflight via `build_deck.check_intelligence_engines_copy` and `build_deck.check_pitch_engines` (registered in `PREFLIGHT_REQUIRED`), so a deck cannot reach kie.ai with a missing narrative or pitch engine even if an agent skipped Phase-1Q copy-QC. The gate phase shown ("1Q") is the EARLIEST (shift-left) catch where the defect costs one re-write; the render preflight is the backstop where it would otherwise cost 50 paid renders. The image-side perceptual codes (`AF-FACE-PROMPT-MISSING`, `AF-WORLD-SCALE`, `AF-LIGHT-PROMPT-MISSING`, `AF-HAIR-INAUTHENTIC`) already fire at the preflight via `check_intelligence_engines_prompt` (shipped v14.28.x — verify only, do not rebuild).

---

## 5.5 THE SEND-BACK-THROUGH RULE (v15.0.0 — below-standard routes BACK, it never renders)

**Doctrine (binding).** A prompt or a script that fails QC is **NOT passed, NOT waved through, and NOT rendered.** It is **routed BACK to the author with a per-slide, per-deficiency work order**, re-authored (only the failing items), and re-QC'd — a bounded author -> QC -> fail -> routeback -> re-author-only-the-failing-items -> re-QC loop that **physically prevents a thin or soulless prompt from reaching `submit_task` / kie.ai.** This is the runtime form of Trevor's law: *"A thin or off prompt must be sent BACK THROUGH to be re-authored — it must NOT reach kie.ai."*

**Two loops, two phases (shift-left — catch at the earliest detectable phase):**

| Loop | Phase | Owner re-authors | Work order | Source-of-truth measurer | Cap |
|---|---|---|---|---|---|
| **Copy-QC loop** (`run_copy_qc_loop`) | COPY-QC (Phase 1Q), **before any image prompt is authored** | Slide Copywriter (narrative) / Offer-Price Strategist (offer) | `working/qc/copy_qc_routeback-<attempt>.json` | `build_deck.check_copy_qc_deterministic` (NOT the agent's self-score) | `PROMPT_QC_MAX_ATTEMPTS` (default 4) |
| **Prompt-QC loop** (`run_prompt_qc_loop`) | PROMPT-QC, **before any kie.ai render — the money step** | Prompt Author (SOP 9.4) | `working/qc/prompt_qc_routeback-<attempt>.json` | `build_deck.check_prompt_qc_deterministic` (NOT the agent's self-score) | `PROMPT_QC_MAX_ATTEMPTS` (default 4) |

**The work order is targeted, not a flat "fail".** Each `*_routeback-<attempt>.json` carries, per slide and per deficiency: the code, the severity, the **measured-vs-required** delta, the **missing intelligence/engine name**, and an actionable `reauthor_directive` (e.g. *"Re-author slide-07 to 9,000–18,000; ADD an explicit facial-expression token; key/fill/rim light; spelling-lock on '…'. Do NOT pad."*). The author re-authors **only the failing slides** and knows exactly which engine is absent.

**Loop properties (the contract):**
1. **Exit on the MEASURER, never the self-score.** A self-typed `pass:true` over thin prompts cannot exit the loop — `check_*_qc_deterministic` re-opens every on-disk artifact and is the gate. This is the explicit fix for the field failure where images were generated one minute after QC "passed."
2. **Termination guaranteed** — bounded by `PROMPT_QC_MAX_ATTEMPTS` (default 4, env-overridable).
3. **Owner override only AFTER the cap.** Reuse the existing audited `_owner_skip_approved`; it never short-circuits a first-pass fail and is double-logged in `process_manifest.json`.
4. **Physically blocks render.** On the no-override path the loop `fatal()`s **before** the render precondition is attested, so a thin/off prompt cannot reach `submit_task` / kie.ai. Below-standard NEVER renders.
5. **Both floors gate the exit (§1.1a).** A prompt passes only when `length >= 9,000` **AND** every engine is present **AND** harmony holds **AND** the EXCELLENCE quality bar is cleared. A 9,000-char prompt missing one engine (or off-harmony) routes back exactly as a 500-char stub does — length alone is never a pass.

**Escalation when the cap exhausts.** Loop count > 4 with no owner override → the **Director of Presentations** is the escalation owner (then the human owner per QC SOP 9.4). The Director never overrides a first-pass fail; the deck simply does not ship while a slide is below standard.
...
**Producing-role requirement (Slide Image Creator):** Every non-pure-typography slide must specify a real generated raster (Kie / GPT-Image-2.5) at >=1920px on the long edge, full-bleed or designed-zone, sourced from the Category E grounded anchor. Decorative icon-font glyphs, single-color clip-art PNGs <=256px, and emoji-as-iconography are forbidden as slide content art.
...
2. **Model pin held on a converter run:** the latest `process_manifest.json` render record `model_used` == `intake.json model_pin` with no undocumented fallback (re-uses AF-MODEL-SOVEREIGNTY logic). The gpt-image-2.5 pin applies to converter runs exactly as it applies to regular runs.
...
**What failed:** Two of four decks submitted to `nano-banana-pro` instead of `gpt-image-2-5-sunburst-text-to-image`. The wrong model was copy-pasted from example payloads without reading the client's config.
...
## 9. NEW AUTO-FAIL CODES -- 2026-06-20 "MAKE-THE-PROMISE-TRUE" ENGINE ENFORCEMENT (the make-it-true pass)

These codes close the engine gaps the June-19 breakdown exposed: the ordered PITCH CADENCE loop, cost-of-inaction, felt-stakes, villain→hero ordering, time-to-result, branded-methodology (de-inverted), audience-demographic, the speech-side hook floor, and the perceptual-engine PROMPT-SIDE mechanical halves. Each is authored as doctrine in its owning SOP and registered here with (a) a binary trigger, (b) an EXACT detection method against a named data file / tag / count / arithmetic / vision-log record, (c) an owning role that produces the checkable artifact, and (d) a QC specialist + gate phase that runs the check at the 8.5 loop.

**Honest mechanical/vision split (the Slide-00 §0 principle held true).** For an irreducibly perceptual engine, the GATEABLE half is the mechanical prompt-token assertion (a deterministic string-presence test the Prompt-QC specialist runs); the match VERDICT stays vision-subjective but is LOGGED to the per-slide rows of `working/qc/image_qc_report.json`, which `AF-IMAGE-QC-VISION` requires to carry a declared vision engine and a per-slide observation for every rendered slide, so it is auditable. "Vision QC is our checker for the perceptual half; the gateable half of every perceptual engine is the mechanical prompt-token assertion."

**LOCKSTEP / REGISTRATION STATUS (read before wiring — this is the honest state, verified 2026-06-20).** The codes below are AUTHORED DOCTRINE; they are NOT yet wired. `PIPELINE-MANIFEST.json` DOES exist — at the cluster path `universal-sops/presentation-slide-craft/PIPELINE-MANIFEST.json` (`manifest_version` 12, 59 autofails, 34 roles), and `sync_check.py` (AF-SYNC) runs and currently reports IN SYNC (exit 0). The Section-5 wireable table that AF-SYNC actually parses lives in the CLUSTER copy `universal-sops/presentation-slide-craft/MASTER-QC-AUTOFAIL-RULESET.md` (the `_first_existing` source of truth), which is a separate file from this role-library mirror. None of the 17 codes below are in the manifest yet — that is correct and intentional: they are registration-pending. The final wiring step for EVERY code is identical: author a NEW `_chk_` function (separate check logic — NEVER inside the live `build_deck.py` render path), then register the code in (1) `PIPELINE-MANIFEST.autofails` with `enforced_by` + `py_symbol` (+ bump `manifest_version`), (2) the CLUSTER ruleset's Section-5 table (and this mirror), (3) the producing-role SOP, and (4) the QC-specialist role — or AF-SYNC will (correctly) fail once a code is half-wired. Placing these in Section 9 (not Section 5) of THIS mirror keeps them out of the AF-SYNC parse while their `py_symbol`s are still pending, so this consolidation pass creates ZERO lockstep drift (re-verified: `sync_check.py` exit 0 after these edits).

### Code Index (one row per new auto-fail; the registration-pending list)

| Code | Engine / doctrine | Level | Gate phase | Owning role | Mechanical detection method (exact) |
|------|-------------------|-------|-----------|-------------|-------------------------------------|
| `AF-CADENCE` | Pitch cadence (ordered loop) | DECK | 1Q + 6 | Director + Offer Price Strategist + Slide Copywriter | For each adjacent DROP pair in `price_ladder.json.rungs`, walk the slide window and assert, IN MONOTONIC SLIDE ORDER, the four beats `[VALUE_ADD → PROMISE → REPITCH_MINI → COST_OF_INACTION]` as arc tags in `slides_copy.md`, cross-checked to `offer_stack.json.value_additions_by_drop`. Missing / out-of-order beat = fail, naming the rung + beat. (Layers on top of `AF-C7`'s global-property floor; does not replace it.) |
| `AF-NO-COST-OF-INACTION` | Cost-of-inaction | DECK | 1Q + 6 | Offer Price Strategist (SOP 9.6) | Assert `price_ladder.json.cost_of_inaction_slide` present, non-null, references a real slide whose copy carries a `COST_OF_INACTION` beat tag. (Per-rung firing is added by `AF-CADENCE`.) Promotes the SOP 9.6 "flag as doctrine gap" verify-step into a hard gate. |
| `AF-GUARANTEE-GENERIC` | Guarantee-they-can-feel | slide | 1Q + 5/6 | Offer Price Strategist + Slide Copywriter | MECHANICAL half (hard trigger): the guarantee slide copy must NOT be framed ENTIRELY by the generic-template blocklist regex `(money[- ]back|30[- ]day(s)?|refund|satisfaction guaranteed)`; it must carry a named, felt, client-specific stake string `guarantee_felt_frame` from intake. SUBJECTIVE half (logged): Copy/Vision-QC scores felt-vs-generic to `copy_qc_report.json`. |
| `AF-NO-BRANDED-METHOD` | Branded methodology (de-inverted) | DECK | 1Q | Slide Copywriter + Director + Brainstorming Buddy | A named-method beat exists in `slides_copy.md` AND either `intake.json.named_methodology` is non-null OR `working/copy/method_name_approval.json` carries `owner_approved:true`. Zero method beat on a generic-content deck = fail. |
| `AF-METHOD-FABRICATED` | Branded methodology (anti-silent-fabrication) | slide | 1Q | Slide Copywriter | A method name on a slide with NEITHER a client-supplied `intake.json.named_methodology` NOR an `owner_approved:true` record = fail. Catches silent invention; the de-inverted ban (see slide-copywriter rule-7 failure mode) keeps silent fabrication illegal while requiring owner-approved naming. |
| `AF-NO-TIME-TO-RESULT` | Expectation (time-to-result) | DECK | 1Q | Slide Copywriter + Offer Price Strategist | An `EXPECTATION`/timeframe beat tag present in the offer section of `slides_copy.md` carrying a duration token (regex `\b\d+\s*(day|week|month|session)s?\b`) sourced from `intake.json.time_to_result` (never fabricated). |
| `AF-AUDIENCE-MISMATCH` | Audience (demographic adaptation) | DECK | 1Q + Image-QC | Brand Steward + Hook Strategist | Assert `intake.json.audience_demographic` non-null; assert `representation_audit.json` cast composition within the ±10% band of the declared demographic (reuses the existing band machinery). |
| `AF-NO-PROBLEM` | Promise+Problem co-presence | DECK | 1Q | Slide Copywriter | Assert an explicit problem/`stakes` beat tag AND a `promise` beat are BOTH present in `slides_copy.md`. (Only add if `AF-C11`'s `stakes` assertion does not already gate co-presence; otherwise this is REDUNDANT with `AF-C11` and is NOT added — confirm before wiring.) |
| `AF-SPEECH-HOOK-COUNT` | Hook spoken floor (de-inverted vs slide ceiling) | DECK | Speech-QC | Presenters Speech Writer | In `PRESENTERS-SPEECH.md`, count verbatim hook-string occurrences (char-exact to `intake.json.hook`); assert `5 ≤ count ≤ 20`. Below 5 = under-sung fail. The slide-side ceiling (`AF-HOOK-1`/`AF-C2`, 3-4 dedicated slides) is UNCHANGED: visual ceiling vs spoken floor, reconciled in SOP-SLIDE-03. |
| `AF-NO-FELT-STAKES` | Emotional intelligence (felt-stakes) | DECK | 1Q | Slide Copywriter + Deep Research Specialist | A `FELT_STAKES` beat tag present in `slides_copy.md` BEFORE the first ladder beat, whose copy pairs a concrete number (regex `\b\d[\d,]*\b`) with a personal-loss frame token from a `felt_stakes_frame` vocabulary (the "3,285 mornings left" device). Presence + structure = mechanical; "does it land" = logged copy-QC note. |
| `AF-NO-VILLAIN` | Story intelligence (villain→hero ordering) | DECK | 1Q + 6 | Director + Slide Copywriter | Scan `slides_copy.md` arc tags in slide order: a `VILLAIN`/antagonist beat must exist AND precede the `HERO`/promise/solution beat. Absent villain, or villain after hero, = fail. (Orthogonal to `AF-STORY-CHARACTER-DRIFT` visual continuity, which is unchanged.) |
| `AF-FACE-PROMPT-MISSING` | Facial (prompt-side mechanical half) | slide | Prompt-QC | Prompt Author / Slide Image Creator | Every people-prompt MUST carry an explicit expression string from the Expression Vocabulary Table; assert string-presence at Prompt-QC. Verdict stays `AF-FACE-MOOD` (vision, logged to `vision_qc_log.json`). |
| `AF-LIGHT-PROMPT-MISSING` | Lighting (prompt-side mechanical half) | slide | Prompt-QC | Prompt Author / Slide Image Creator | Every people-prompt MUST NAME a key/fill/rim direction string AND a rim/hair-light token appropriate to the cast member's skin tone; assert string-presence at Prompt-QC. Verdict stays `AF-LIGHT-SKINTONE` (vision, logged). |
| `AF-WORLD-SCALE` | World (prompt-side mechanical half + named code) | slide | Prompt-QC + Image-QC | Prompt Author / Slide Image Creator | Named alias of the mismatch check so the World engine has its own code. Every scene-prompt MUST STATE a setting + a one-line believability justification string ("a normal teenager's room: poster, a few trophies — NOT a luxury condo"); assert string-presence at Prompt-QC. "Would this person be in this room" verdict stays vision (`AF-WORD-IMAGE-MISMATCH`, logged). |
| `AF-HAIR-INAUTHENTIC` | Representation (hair authenticity) | slide | Prompt-QC + Image-QC | Brand Steward + Prompt Author | MECHANICAL half: the people-prompt carries a hairstyle token that is a member of the age-banded, by-gender hairstyle reference catalog (asset shipped by Brand Steward), age-appropriate to the cast member's stated age band (Prompt-QC string-membership). SUBJECTIVE half (logged): vision check for the "AI plastic hair" failure. Representation ratio `AF-R*`/`AF-CAST` unchanged. |
| `AF-DECK-TYPE-MISSING` | Deck-type declaration (training-vs-pitch) | DECK | 1Q | Director + Content-to-Presentation Architect | Assert `intake.json.deck_type` is declared. The pitch-machinery gates (`AF-PITCH-MISSING`, `AF-DEN-1..8`, `AF-C7`, `AF-CADENCE`, `AF-NO-COST-OF-INACTION`, `AF-C11` CTA, `AF-NO-VILLAIN`) become CONDITIONAL on `deck_type == pitch`; image/graphics QC weight lowers when `deck_type == training`. This is a conditional-skip wired into the existing gates, not a parallel fail. |
| `AF-TYPE-BEAT-MISSING` | The four presentation types (per-type beat map) | DECK | 1Q | Director | Load the per-type required-beat map from `SOP-TYPES-00-PRESENTATION-TYPES.md`; assert the deck carries exactly the beats its `deck_type` requires (the existing `AF-NO-*` beat checks become type-conditional). NEEDS TREVOR INPUT: enumerate/define the three types beyond "general" before this map can be authored. |

**WIRING PREREQUISITE (per code, before any of the above executes as a gate): register into the EXISTING manifest.** `PIPELINE-MANIFEST.json` already exists and is in sync (cluster path, v12). Each code above is dormant doctrine until its `_chk_` function is authored and the code is added to `PIPELINE-MANIFEST.autofails` (+ the cluster Section-5 table + producing role + QC role), per the LOCKSTEP / REGISTRATION STATUS note above. AF-SYNC is functional today; it will (correctly) start FAILING the instant a code is added to the cluster Section-5 table without a matching manifest entry and `py_symbol`, so always wire all four surfaces together and bump `manifest_version`. Owner of the per-code wiring: the check-author + Director; the new `_chk_` logic goes in a separate gate script (e.g. `gate_integrity_check.py` / a new `cadence_check.py` / `beats_check.py`), never the `build_deck.py` render core.

**EXECUTABLE CHECK SHIPPED for the SEVEN INTELLIGENCE engines (Facial / World / Lighting / Emotional / Story-villain→hero / Typography / Representation-hair).** The mechanical half of the six perceptual/structural INTELLIGENCE codes above — `AF-FACE-PROMPT-MISSING`, `AF-WORLD-SCALE`, `AF-LIGHT-PROMPT-MISSING`, `AF-HAIR-INAUTHENTIC`, `AF-NO-FELT-STAKES`, `AF-NO-VILLAIN` — is now a real, separate, deterministic check: `scripts/intelligence_engines_check.py` (zero third-party deps; exit 4 on drift, 0 on pass, 2 if it cannot run). It does NOT touch `build_deck.py`'s render path. It scans `working/prompts/slide-*.txt` for the Facial/World/Lighting/hair prompt tokens, and `working/copy/slides_copy.md` (+ `working/copy/price_ladder.json`) for the FELT_STAKES-before-ladder and VILLAIN-before-HERO beat ordering. The QC specialists invoke it as: Prompt QC → `python3 scripts/intelligence_engines_check.py working --phase prompt` (the four prompt-side codes); Deck QC at Phase 1Q → `--phase copy` (the two copy-beat deck codes). Typography (Engine 3) is already mechanical (`AF-F12`/`AF-F13`/`AF-PROMPT-FLOOR`/`AF-P14`/`AF-TYPE-8THROW`) and adds no new code; the vision VERDICT halves (`AF-FACE-MOOD`, world-grounding, `AF-LIGHT-SKINTONE`, the plastic-hair read) stay logged to `vision_qc_log.json` and are graded at Image-QC. **REGISTRATION COMPLETE (2026-06-26).** All six codes (`AF-FACE-PROMPT-MISSING`, `AF-WORLD-SCALE`, `AF-LIGHT-PROMPT-MISSING`, `AF-HAIR-INAUTHENTIC`, `AF-NO-FELT-STAKES`, `AF-NO-VILLAIN`) are now registered in `PIPELINE-MANIFEST.autofails` as `enforced_by: qc_check` with `check_script: scripts/intelligence_engines_check.py::check_prompts|check_copy` (`py_symbol: null` — Guard A audits only `build_deck`-enforced codes, so a `qc_check` code is governed by sync_check + the QC rubric), and they appear in the Section-5 machine-checkable table of BOTH ruleset copies. AF-SYNC stays green. They run as a QC-invoked gate (not a `build_deck` gate) and are exercised by `test_engine_checks()` in `test_preflight.py` so they cannot silently no-op.

### Decisions flagged for Trevor (do NOT silently resolve)
1. **Branded-Methodology** (`AF-NO-BRANDED-METHOD`/`AF-METHOD-FABRICATED`): confirm auto-naming-WITH-owner-approval replaces the retired hard "never fabricate a methodology name" ban (the de-inversion is already authored in the slide-copywriter rule-7 failure mode).
2. **Lighting role** (`AF-LIGHT-PROMPT-MISSING`): create a standalone Lighting specialist, or keep lighting owned by the Slide Image Creator (recommended — do not fragment ownership). The transcript's "there's literally a lighting agent" maps to the engine, not a separate role.
...
Trevor's HARD RULE these enforce: every slide's WORDS + VISUAL are generated TOGETHER in ONE image by kie.ai gpt-image-2.5 ONLY (no Pillow slide canvas, no other model, NO PowerPoint text overlaid on top), and the process is IMPOSSIBLE to skip unless the OWNER explicitly approves.

### Code Index

| Code | Successor of | Gate phase | Scope | py_symbol (build_deck.py) | What it blocks |
|------|--------------|-----------|-------|---------------------------|----------------|
| `AF-CANONICAL-RENDER-BYPASS` | AF-RENDERER (closeout-only, py_symbol null) | Phase 4/6/Postflight | DECK | `check_canonical_render_path` | a deck/slide produced by any path other than `build_deck.py` + `run_signature_deck.py` (hand-rolled `working/*.py` renderer/assembler, direct kie `createTask`, or a slide untraceable to a canonical kie `taskId`) |
| `AF-LOCAL-CANVAS` | AF-BAKED / AF-I14 | Phase 4/5/6 | slide | `check_canonical_render_path` | a slide image fabricated locally — a PIL `Image.new`/`ImageDraw` slide canvas, a sub-51,200-byte flat-cream card, or a pure-typography hook with no real kie `taskId` |
| `AF-IMAGE-QC-VISION` | AF-NO-VISION-QC (RETIRED 2026-07-26 — was closeout-only, py_symbol null) | Image-QC/Postflight | DECK | `check_image_qc_vision` | a pixel-blind image-QC pass — a self-typed score, filename/prompt reasoning, a missing per-slide `vision_api_response`, an out-of-scope slide exclusion, or an overlay-blessing rubric |
...
**Failure message:** `AF-LOCAL-CANVAS: slide {N} was fabricated locally (PIL Image.new/ImageDraw slide canvas, a sub-51,200-byte flat-cream card, or a pure-type hook with no kie taskId). Every slide is a kie.ai gpt-image-2.5 bake; Pillow is permitted ONLY for the locked-logo composite (SOP-IMG-05). Re-render via kie.ai, or record a logged owner_skip_approval.`
