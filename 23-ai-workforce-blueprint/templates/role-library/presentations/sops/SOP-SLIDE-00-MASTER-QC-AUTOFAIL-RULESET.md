...
| AF-INTAKE-BATCH | INTAKE | INTAKE | INTAKE-SCOPED -- never gates the deterministic build. The client intake conversation dumped a batch: a single assistant turn emitted two or more intake questions before the owner answered the current one (numbered, bulleted, or joined by "and"), OR it opened with no quick-vs-in-depth CHOICE first. Doctrine: CLIENT-WEBINAR-DECK-SOP section 0.5 Client Intake Conversation Contract. The banned reference anti-pattern is one message enumerating Topic + Audience + Tone + Deadline + Existing assets + Content together with "give me whatever you have got and I will get moving" | qc_check: the QC/Healer intake-trace scan counts intake questions per assistant turn before an owner answer; two or more before an answer (or a missing choice-first opening) trips AF-INTAKE-BATCH. Scoped to the INTAKE conversation ONLY -- it never inspects, runs inside, or gates build_deck.py / run_signature_deck.py, so it touches no build phase. enforced_by qc_check; py_symbol null |
