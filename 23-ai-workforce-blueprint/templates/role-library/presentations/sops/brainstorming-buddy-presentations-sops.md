...
**Version:** 2.2 (regenerated after surgery v2.2 -- SOP 9.1/9.2/9.3 add EVENT/OFFER PRICE SEPARATION: EVENT_PRICE (free/paid to attend) is a distinct axis from FINAL_PRICE (the offer); a free event NEVER implies a free offer and defaults to expect_paid_offer:true assumed; free-only close needs explicit owner sign-off. Prior: v2.1 SOP 9.0 adds DELIVERABLE_SET / WANT_AUDIO_DEMO / TARGET_WPM(140) + the style branch)
...
3b. **(Decision 2A) Ask the PITCH BRANCH verbatim, once, early:** *"Does this presentation end with an offer or pitch — a price, package, or call to buy — or is it a teaching and content-only presentation?"* Record an explicit boolean `pitch_included` (true = ends with an offer/pitch; false = teaching/content-only). NEVER default it and NEVER force a pitch: a pitchless deck is first-class — BUT a free EVENT (free webinar/workshop/challenge) must NEVER be read as `pitch_included:false`. Event access and the offer are SEPARATE axes (see 3c): a free event is normally the front of a paid funnel, so if the event is free and the owner has not explicitly chosen a teaching/content-only deck, default `expect_paid_offer:true` and `pitch_included:true` marked `assumed:true` (the Devil's-Advocate ALWAYS-PITCH-SOMETHING doctrine); `pitch_included:false` is set only when the owner explicitly states it. The gate **AF-PITCH-FLAG-UNSET** fails any deck whose intake has no boolean `pitch_included`; downstream, `pitch_included:false` SUPPRESSES the Offer Price Strategist + price ladder and arms **AF-PITCH-LEAK** (no pitch/price content may appear), while `pitch_included:true` requires the offer arc (**AF-PITCH-MISSING**).
3c. **(Decision 2B) EVENT/OFFER PRICE SEPARATION (ask as two separate questions; never collapse them):** Capture the EVENT/ACCESS price — free or paid to ATTEND — as `EVENT_PRICE` / `EVENT_ACCESS_MODE` (question 4a), AND the OFFER price — what is sold at the end — as `OFFER_STACK` / `FINAL_PRICE` / `PRICE_MODE` (question 4b), as two distinct questions. NEVER infer, zero out, or skip `FINAL_PRICE` from `EVENT_PRICE`: a free event does NOT mean a free offer. A free event with the offer price left blank defaults `expect_paid_offer:true` (`assumed:true`) and is surfaced at the SOP 9.3 read-back; a free-only close (free event AND no paid offer) is locked ONLY on the owner's explicit sign-off, never inferred from the event being free. Every price/pitch assumption is read back to the owner (`assumed:true`), never silently defaulted or denied.
...
2b. **(Decision 2A) PITCH BRANCH (verbatim, once, early):** *"Does this presentation end with an offer or pitch — a price, package, or call to buy — or is it a teaching and content-only presentation?"* Record the explicit boolean `pitch_included` (never default; never force a pitch — a pitchless deck is first-class). A free EVENT must NEVER be read as `pitch_included:false`; if the event is free and the owner has not explicitly chosen teaching/content-only, default `expect_paid_offer:true` / `pitch_included:true` (`assumed:true`) per 2c. Gate **AF-PITCH-FLAG-UNSET**.
2c. **(Decision 2B) EVENT/OFFER PRICE SEPARATION (two separate questions; never collapse):** Capture the EVENT/ACCESS price (free or paid to attend) as `EVENT_PRICE` / `EVENT_ACCESS_MODE`, AND the OFFER price as `OFFER_STACK` / `FINAL_PRICE` / `PRICE_MODE`, separately. NEVER infer or skip `FINAL_PRICE` from `EVENT_PRICE` — a free event is the front of a paid funnel, not a free offer. Free event + blank offer price defaults `expect_paid_offer:true` (`assumed:true`, surfaced at lock); a free-only close needs explicit owner sign-off.
...
1. Compose the READ-BACK: a short plain-language summary of the brief in the owner's
   ...
   state the assumed paid offer explicitly so the owner can confirm or correct it.
2. List every `assumed: true` field explicitly so the owner can correct defaults — including
   any price/pitch assumption (e.g. a free event defaulting to `expect_paid_offer:true` /
   `pitch_included:true`, or a defaulted `FINAL_PRICE`). A free-only close (free event, no
   paid offer) is locked ONLY on explicit owner sign-off, never inferred from the free event.
