...
2. This role: SOP 9.1 audience-mode (one-person / general, asked first) -> SOP 9.2 ingest (mode-aware privacy rule on recordings) -> SOP 9.3 signal-vs-fluff -> SOP 9.4 analysis + theme + arc -> SOP 9.5 devices + simplify -> SOP 9.6 micro-vs-full -> SOP 9.7 deliverable bundle (deck + Presenter guide + infographic checklist; cover/closing in one-person mode) -> SOP 9.8 hand `source_brief.json` to the Director.
3. Director: validates, propagates mode and bundle, collects audience/representation/style via the Buddy's SOP 9.0 if not on file (theme/arc/hook/mode pre-seeded), dispatches ROLE-04 only if proof flags want it.
