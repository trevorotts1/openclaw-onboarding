...
> **BINDING -- how GHL is touched.** Any GHL upload goes EXCLUSIVELY through `POST https://services.leadconnectorhq.com/medias/upload-file` (Version `2021-07-28`, multipart, optional `parentId`) with the CLIENT's GHL **LOCATION** PIT (the agency token 401s for media). Token: `GOHIGHLEVEL_API_KEY` (preferred) / legacy `GHL_API_KEY`; location id: `GOHIGHLEVEL_LOCATION_ID` (preferred) / `GHL_LOCATION_ID`. NEVER create a GHL folder (folder-create returns 404; the Media Librarian resolved `ghl_folder_id` to a human-made folder id or `"root"`). Agent-browser / Playwright / any GHL UI automation is FORBIDDEN. Reference: `29-ghl-convert-and-flow/references/medias.md`.
