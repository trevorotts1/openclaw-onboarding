...
4b. **Capture the definitive `audience_mode` and `target_talk_minutes` into intake.json (ADDITIVE -- both are non-defaultable build-stopping fields, enforced by `build_deck.py` `_chk_intake`).** These are in ADDITION to the existing `presentation_mode` axis (one-person / general), not a replacement for it.
   ...
   - **`client_requested_slide_count` (OPTIONAL; positive integer):** the EXACT number of slides the client explicitly asked for ("make it 25 slides", "I want a 50-slide deck"). **When the client states a number, capture it here verbatim and HONOR IT EXACTLY: 25 -> 25, 50 -> 50, 500 -> 500.** It is NEVER floored up, capped down, defaulted, or substituted by the duration/content/source heuristic, and the client is NEVER asked to accept a different number (no "this is 20 slides, you said 25 — does 20 work?"). When set, it IS the deck length and OVERRIDES every heuristic floor/ceiling (the duration pacing floor, the Mode-A target/cap, AND the Mode-B anti-compression coverage floor). It is enforced at build time by `build_deck.py` `_chk_slide_count_exact` (`AF-SLIDE-COUNT-EXACT`): a deck whose length differs from the requested count — in either direction — AUTO-FAILS. When the client gives NO number, OMIT the field and the duration/source sizing below governs. (When present, `_chk_intake` requires it be a positive integer, so a malformed value halts intake rather than silently disabling the gate.)
   - **DURATION-SIZING DOCTRINE (sizing follows the talk length, not the reverse):** the slide count, the on-slide copy density, AND the presenter speech are all sized to `target_talk_minutes`, paced at the verified **130 words per minute** standard (the Presenter Speech Writer's exposed, tunable rate). `slide_count_final = client_requested_slide_count` **when the client stated an explicit count (honored EXACTLY)**; otherwise `slide_count_final = max(target-sized slide count, source_slide_count)` (the anti-compression coverage floor still applies, Mode B add-only). The presenter speech is budgeted at `target_talk_minutes x 130 wpm` minus the mandatory pause budget, exactly as the Presenter Speech Writer SOP 9.1 specifies. The on-slide copy density still adapts to the talk length even when the client fixed the slide count — the count is the client's, the per-slide pacing is the Director's.
...
1. If the client provided a duration in minutes (not a slide count), convert using the master SOP table (SOP-PITCH-01..06 cluster (PRESENTATION-MASTER-DOCTRINE.md §4)). This table governs and cannot be overridden by agent judgment:
   ...
   - **CLIENT'S EXPLICIT COUNT WINS — FIRST AND ABSOLUTE.** If the client stated an exact number of slides (`client_requested_slide_count`), that number IS the deck length and is honored EXACTLY (25 -> 25, 50 -> 50, 500 -> 500). Skip the table-derived target entirely: the duration table below is only the FALLBACK used when the client gave no number. Never floor it up to a pacing minimum, never cap it down to 90, never default it, and never ask the client to accept a different number. (`AF-SLIDE-COUNT-EXACT` enforces this at build time.)
   - The rate is roughly 1.3 to 1.5 slides per minute, tapering as duration grows. A three-hour presentation does NOT mean 300 slides. In Mode A (net-new) with no explicit client count, 90 is the target/cap for any deck; in Mode B this cap yields to `source_slide_count` (add-only -- see step 2).
   ...
   - Propose the slide count from this table ONLY when the client gave no explicit number; the client confirms it during the intake echo. Record `SLIDE_COUNT`. (When the client DID give a number, that number is `SLIDE_COUNT` — there is nothing to propose or negotiate.)
