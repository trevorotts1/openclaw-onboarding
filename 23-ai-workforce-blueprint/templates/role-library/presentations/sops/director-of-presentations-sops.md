...
5a. **Dispatch ROLE-04 Deep Research Specialist as Phase -0.5 (MANDATORY).** After the brief lock (step 5) and after SOP 9.2 PRD approval, dispatch ROLE-04 as Phase -0.5. Supply the locked `working/copy/deck_brief.json` (or `source_brief.json` when originating from ROLE-22). BLOCK: do not dispatch Phase B+ (Hook Strategist) until `working/research/brief-[DECK_SLUG].md` exists on disk and its header records `research_complete: true`. On receipt of the completed Research Brief, route ALL twelve categories (A-L) as follows:
