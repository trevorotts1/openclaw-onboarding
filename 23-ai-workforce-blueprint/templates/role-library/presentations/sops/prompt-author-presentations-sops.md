...
### SOP 9.1 — Author each slide prompt to the 9,000-char standard
...
3. The prompt MUST be >= 9,000 non-whitespace characters (`PROMPT_CHAR_FLOOR`) and <= 18,000 (`PROMPT_CHAR_CEILING`). A prompt below the floor is NOT run, NOT rendered, NOT updated (AF-P1 / AF-PROMPT-FLOOR). Re-author until every slide clears the floor AND fills the 9,000-18,000 band with real specificity (the EXCELLENCE quality gate, not boilerplate padding).
...
**Output:** `working/prompts/slide-*.txt`, one >=9,000-char prompt per slide with the verbatim words baked into the prompt body, rendered VERBATIM by `build_deck.py`.
