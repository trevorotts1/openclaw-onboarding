...
22. **Felt-stakes before the offer (Emotional Intelligence; AF-NO-FELT-STAKES).** Write at least one FELT_STAKES slide that makes the audience FEEL the cost of inaction in concrete human terms BEFORE the first ladder beat -- the "3,285 mornings left" device: a concrete NUMBER paired with a personal-loss frame ("mornings left", "days you will never get back", "every day you wait costs you"). Tag the slide `FELT_STAKES` in the slide block so QC can assert it mechanically. The NUMBER comes from the Deep Research brief or intake, NEVER fabricated. A deck whose stakes are only abstract ("things could be better") fails AF-NO-FELT-STAKES; the felt-stakes must land before the offer so the audience feels the cost first. (Distinct from the cost-of-inaction beat in the offer ladder -- this is the EMOTIONAL pre-offer slide; that is the arithmetic answer inside the close.)
23. **Villain before hero (Story Intelligence; AF-NO-VILLAIN).** "No one cares about the hero until they meet the villain." Write an explicit VILLAIN/antagonist beat -- name the real enemy (the broken system, the old way, the lie they've been told, the thing actually stopping them) -- and place it BEFORE the HERO/solution/promise beat in slide order. Tag the antagonist slide `VILLAIN` and the solution slide `HERO`. A deck with no antagonist beat, or one where the solution arrives before the villain is named, fails AF-NO-VILLAIN. This is the narrative ordering rule; it is ORTHOGONAL to the visual same-character continuity rule (AF-STORY-CHARACTER-DRIFT) and to the old-way-vs-new-way belief-shift contrast (step 5) -- the villain is the named antagonist of the story, written as its own beat. See SOP-STORY-01.
...
**Failure mode (PITCH-ENGINE arc tags the copywriter MUST emit; gated by `scripts/pitch_engines_check.py`, SOP-PITCH-06).** The pitch-engine gates read arc-beat tags from `slides_copy.md` (syntax `<!-- ARC: TAG -->` or `[ARC: TAG]`). The copywriter must tag and write these beats or the deck auto-fails:
- **`EXPECTATION` (gated AF-NO-TIME-TO-RESULT, SOP-PITCH-06 1.5).** Tag a slide in the offer section that states how long the result takes, carrying a real duration token (e.g. "8 weeks to final, a shift in 2") sourced from `intake.json.time_to_result` -- never fabricated. Absent beat / no duration token / no intake source = fail.
- **The four CADENCE beats per price-drop rung (gated AF-CADENCE, SOP-PITCH-06 1.1).** Between each DROP and the next, tag, IN ORDER, `VALUE_ADD` -> `PROMISE` -> `REPITCH_MINI` -> `COST_OF_INACTION`. The Director reserves the four-beat block per rung in `arc_allocation.json`; the copywriter writes and tags the copy in that order. Missing or out-of-sequence beat = fail (names the rung).
- **`COST_OF_INACTION` (also gated deck-wide by AF-NO-COST-OF-INACTION, SOP-PITCH-06 1.2).** At least one cost-of-inaction beat tied to `price_ladder.json.cost_of_inaction_slide`, plus the per-rung recurrences above.
- **`FELT_STAKES` and `VILLAIN`/`HERO`** beats are tagged here too but their gates (AF-NO-FELT-STAKES, AF-NO-VILLAIN) are owned by `scripts/intelligence_engines_check.py` (Emotional + Story engines); see SOP-SLIDE-00 Section 9. Tag `FELT_STAKES` before the first ladder beat (a concrete number paired with a personal-loss frame) and `VILLAIN` before `HERO`.
