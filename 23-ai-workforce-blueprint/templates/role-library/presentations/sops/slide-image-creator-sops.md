...
- **(Decision 1C) working/copy/assets_manifest.json** when present — client-provided photo / product / logo assets to CONSUME as gpt-image-2.5 `input_urls`.
...
3. Verify character count of the completed prompt. Target: 9,000-18,000 characters. Hard floor: 9,000. Hard maximum: 18,000 (the LONG-tier budget; GPT-Image-2.5 accepts up to 20,000 on both endpoints per MODEL-SPECS, and 18,000 keeps a 2,000-character safety margin). If under 9,000, the prompt is starved -- expand with genuine specificity (the per-line spelling-lock, the full paired negative block, exhaustive logo-placement language, complete people-anatomy direction, deeper world/scene detail), never with boilerplate. If over 18,000, the prompt is too long for the LONG tier -- tighten redundant phrasing, never by deleting the negative block or any spelling-lock. Spend the expanded budget ONLY on defect-preventing specificity (density-calibration rule, SOP 9.4-strengthening).
