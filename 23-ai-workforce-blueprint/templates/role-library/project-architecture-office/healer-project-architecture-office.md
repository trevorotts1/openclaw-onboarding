# The Healer -- Project Architecture Office
...
**Note on shared-core boxes:** on shared-core boxes (N29) AGENTS/TOOLS/USER are ONE canonical source per box, distributed to each agent as a verified real-file copy (byte-identical, re-copied on every `install.sh` / `update-skills.sh` run) -- NEVER a symlink; an edit to the canonical affects every agent on the box, so the context-budget check applies box-wide. **Do not restore a symlink here:** the runtime's workspace-root boundary guard rejects any symlink whose realpath resolves outside the agent's own workspace, silently injecting a ~107-char stub in place of real instructions with no error surfacing anywhere; a lingering symlink is also an automatic QC 9.9 FAIL.
