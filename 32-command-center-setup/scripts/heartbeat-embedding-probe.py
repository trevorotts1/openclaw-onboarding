#!/usr/bin/env python3
"""Skill 32 — Fleet Embedding Health Probe  (heartbeat-embedding-probe.py  v1.1.0)
...
Renamed from `heartbeat-canary-probe.py` (D20, operator-box-proof vocabulary);
a one-release compatibility shim at the old filename execs this file unchanged.

Triggered by:
  - HEARTBEAT.md "Fleet Embedding Health Probe" check (every 6 h, wired as an
    OpenClaw cron — see HEARTBEAT.md for the exact openclaw cron create command)
  - Manual: python3 heartbeat-embedding-probe.py [--db <path>] [--dry-run]
...
PROBE_PHRASE = "onboard"
PROBE_PHRASE_SEMANTIC = "how do I bring a new team member up to speed"
...
def _keyword_hits(db: sqlite3.Connection, phrase: str) -> int:
    """Count SOPs matching the probe phrase via keyword search."""
...
def _semantic_hits(sop_embeddings_count: int, keyword_count: int) -> int:
    """
    Estimate semantic search hit count for the probe phrase.
...
def _send_rescue_alert(
    ...
    msg = (
        f"[heartbeat-embedding-probe / {box_id}] EMBEDDING {status.upper()}\n"
        f"Reason: {reason}\n"
        ...
        f"Then re-run heartbeat-embedding-probe.py to confirm recovery."
    )
    if dry_run:
...
def probe(db_path: str, dry_run: bool = False) -> int:
    ...
    print(f"[heartbeat-embedding-probe] v{_SCRIPT_VERSION}  box={box_id}  db={db_path}")
    ...
    kw_hits  = _keyword_hits(db, PROBE_PHRASE)
    ...
    if not dry_run:
        db.execute(
            ...
            (
                ...
                coverage,
                PROBE_PHRASE_SEMANTIC, sem_hits,
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(
        description="Fleet embedding health probe for Skill 32 Command Center."
    )
    ap.add_argument(
        "--db", default=None,
