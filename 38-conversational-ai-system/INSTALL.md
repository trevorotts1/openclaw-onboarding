...
- MEMORY.md gets ONE compact pointer block (marker `<!-- BEGIN skill-38 memory-rules-pointer -->`) to the full design rules 6-44, which ship as `references/memory-design-rules.md` (+ the per-rule specs in `protocols/`). The rule corpus is NEVER pasted into MEMORY.md; the writer also self-heals legacy pasted-corpus blocks left by earlier versions.
...
./06-append-memory-rules.sh          # MEMORY.md design-rule POINTER (rules live in references/)
...
#   chain, 30-voice-phone-setup-wizard, 31-generate-workflow-visual,
#   32-verify-model-failover-support, 33-runtime-tool-gating-prover). Each is
#   idempotent; the two 22-/24- numbers ship two scripts each. See INSTRUCTIONS.md
#   for the interactive steps between them.
```
