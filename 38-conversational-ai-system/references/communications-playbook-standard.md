...
**Agent behavior at the START of a build (don't skip — full detail in
`protocols/conversation-workflows-protocol.md` §I.1a, §I.1b, §I.2):**

1. **Offer a personal TRIGGER WORD on the client's FIRST playbook build.** Before the brainstorm, the agent
   offers to set a personal trigger word — explained like **"Alexa" / "Hey Siri"**: a word/phrase that
   instantly tells the AI the client wants to build a playbook (e.g. *"Playbook time!"*). The agent asks for
   it, confirms it back, and **remembers it** (persists it to the client's `USER.md` + the
   `conversation-workflows/registry.md` `trigger-word` header) so future builds recognize it. On later builds,
   recognizing the trigger word starts the flow. The standard *"Help me build a [purpose] playbook"* phrasing
   always works too — the trigger word is an additional shortcut, never a replacement.
2. **Present the "I Do / You Do" overview.** So the client knows responsibilities + that a good playbook
   takes **~15-30 minutes**: (1) YOU trigger it → (2) I (AI) DO ask a few quick brainstorm questions using
   what I already know (NOT a 50-question interrogation) → (3) YOU answer (goal, audience, channel, offer,
   tone) → (4) I DO draft the full playbook + flow for approval → (5) YOU review/tweak → (6) I DO finalize +
   store (`conversation-workflows/` mirrored to Notion) + build the matching Workflow AI prompt wired to your
   Convert and Flow account → (7) I DO wire the actions (create tags, update calendar, create/book
   appointments) → (8) YOU approve and we go live.
3. **Brainstorm to land the PERFECT playbook (the agent's role) — give the client the "things to think
   about":** the **goal** (book a call / recover a sale / get a review / answer an FAQ), **who it's for**,
   the **channel(s)**, the **offer/hook**, the **tone/brand voice**, **timing & follow-up cadence**, and the
   **"win" action** (booked / replied / tagged / purchased). Reassure: *"if you're unsure, that's what I'm
   here to brainstorm."* Only ask the gaps the agent cannot infer — this list is the compass, not a
   questionnaire to read aloud.
