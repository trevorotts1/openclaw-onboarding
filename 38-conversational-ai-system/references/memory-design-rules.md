# Skill 38 — Conversational AI System: design rules 6-44 (canonical deep reference)

> **This file is the CANONICAL, FULL text of Skill 38's numbered design rules.**
> It is a Teach-Yourself-Protocol DEEP FILE. It is **never** pasted into a box's
> `MEMORY.md`. `scripts/06-append-memory-rules.sh` writes exactly ONE compact
> pointer block into `MEMORY.md` (marker `<!-- BEGIN skill-38 memory-rules-pointer -->`)
> that points here, and self-heals any legacy pasted-corpus blocks it finds.
>
> **Why:** pasting ~40,000 characters of rules into `MEMORY.md` re-bills the whole
> corpus on every single turn and, because the old writer keyed idempotency on a
> version-stamped marker string, every marker rename re-appended the entire corpus
> a second time. The rules did not change — only where they live.
>
> **Per-rule deep specs** live in `protocols/*.md` (each rule names its own protocol
> file). This file is the index of record for the rule text itself.
>
> The `<!-- BEGIN/END skill-38 … -->` comments below are retained verbatim so the
> QC gates can address each rule group by its historical marker name. They have no
> runtime meaning in this file.
