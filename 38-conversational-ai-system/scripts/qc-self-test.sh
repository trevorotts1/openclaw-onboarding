...
if [ ! -f "$ST" ]; then
  ...
  grep -qE 'hooks/\$\{?HOOK_NAME' "$ST" || grep -qE '/hooks/' "$ST" || \
    MISSING+=('24-self-test-hook.sh must POST to its OWN public hook URL (/hooks/<HOOK_NAME>)')
  ...
  grep -q '"channel":"sms"' "$ST" || MISSING+=('24-self-test-hook.sh synthetic body must be channel=sms')
  ...
  grep -qi 'conversations/messages' "$ST" || \
    MISSING+=('24-self-test-hook.sh must call the GHL Conversations API (conversations/messages) for the real send')
  ...
  grep -q 'selfTestPassed' "$ST" || MISSING+=('24-self-test-hook.sh must write the selfTestPassed state field')
  grep -qE 'selfTestPassed.*false|"selfTestPassed" "false"' "$ST" || \
    MISSING+=('24-self-test-hook.sh must set selfTestPassed=false on failure')
fi
...
[ -f "$QCC" ] && grep -q '24-self-test-hook' "$QCC" || \
  MISSING+=('scripts/11-run-qc-checklist.sh must reference scripts/24-self-test-hook.sh')
