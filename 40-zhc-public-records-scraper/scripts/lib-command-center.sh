#!/usr/bin/env bash
# lib-command-center.sh -- Skill 40 ZHC Public Records Scraper
#
# Best-effort Command Center (mission-control Kanban) reflection, mirroring Skill
# 41's fail-soft health-gated helper pattern. A bulk records scan is real work; if
# the operator runs the Command Center, this helper moves the scan's card across
# columns so the board reflects progress. It is a SILENT NO-OP when the Command
# Center is unreachable (client boxes may not run it) and NEVER fatal -- it never
# blocks a records query, never writes the CC database (HTTP only), and never
# prints the auth token.
#
# Source it, do not execute it:
#   source "$(dirname "${BASH_SOURCE[0]}")/lib-command-center.sh"
#
# Contract (identical shape to Skill 41's lib-command-center.sh):
...
# cc_available -- health probe. Returns curl's status; prints nothing. Short
# timeout so a missing Command Center never stalls a records scan.
cc_available() {
...
#   The bearer token is passed ONLY inside the Authorization header; never echoed.
cc_move_task() {
  ...
  if ! cc_status_valid "$status"; then
    echo "[skill 40][cc] bad status: '$status' (not a Kanban column) -- card not moved" >&2
    return 2
  fi

  if [ "$status" = "done" ]; then
    echo "[skill 40][cc] refusing 'done': review->done is set by the Command Center QC scorer, not the scraper" >&2
    return 3
  fi
  ...
  if ! out="$(curl "${args[@]}" "$url" 2>/dev/null)"; then
    echo "[skill 40][cc] WARNING: PATCH ${url} transport error -- card not moved (non-fatal)" >&2
    return 0
  fi

  http_code="${out##*$'\n'}"
  if [[ "$http_code" =~ ^2[0-9][0-9]$ ]]; then
    echo "[skill 40][cc] task ${task_id} -> ${status} (HTTP ${http_code})" >&2
  else
    echo "[skill 40][cc] WARNING: task ${task_id} -> ${status} failed (HTTP ${http_code}) -- non-fatal" >&2
  fi
  return 0
}
