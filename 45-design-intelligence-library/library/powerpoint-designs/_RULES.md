...
|---|---|---|
| Drafts / contact sheets | 1K | GPT-Image 2.5, Nano Banana 2, Wan 2.7 |
| Standard screen deck (DEFAULT) | 2K | All (Seedream `basic`) |
| Projection / print / premium | 4K | GPT-Image 2.5, Nano Banana 2, Seedream `high` — NOT Wan 2.7 |
...
- Text strategy decision per generation: (a) AI renders the text in-image (use exact-text quoting, GPT-Image 2.5 / Nano Banana 2), or (b) generate the background/imagery only and text is added in PowerPoint later. Record the chosen strategy.
- **ARCHITECTURE LAW FOR WEBINAR/AUDIENCE DECKS:** text-in-image is THE rule for webinar and audience decks. Strategy (a) is the required path; strategy (b) background-only + PowerPoint text-overlay is a per-element fallback only, permitted after 2 failed renders of a text-bearing slide or for a screenshot/data chip the renderer would garble. A deck assembled primarily from bare backgrounds + overlay text boxes is an AUTO-FAIL at final QC. Do NOT mark strategy (b) as a default or preferred option for any audience or webinar deck.
