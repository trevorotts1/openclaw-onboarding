 ...
 *  10. GET /kv-read requires Authorization: Bearer <KVREAD_TOKEN> (Worker secret)
 *
 ...
 *   KIE_CALLBACK_HMAC_KEY  -- shared box<->Worker key: derives callback validator (fix D)
 *   KVREAD_TOKEN           -- shared box<->Worker bearer token for /kv-read auth (fix B)
 *
 ...
 *   s=<callbackValidator> -- HMAC-SHA256(clientSlug + ":" + submitId, KIE_CALLBACK_HMAC_KEY)
 *                           Nothing secret -- safe in logs (fix D)
 *   h=<perTaskSecretHmac> -- HMAC-SHA256(perTaskSecret, KIE_CALLBACK_HMAC_KEY)
 *                           A hash of the secret, not the secret (fix C). The Worker
 *                           stores this in KV; on /kv-read the box sends the preimage
 *                           (raw perTaskSecret) and the Worker recomputes + compares.
 *
 ...
 *
 * Security (fixes B + C):
 *   B. Requires Authorization: Bearer <KVREAD_TOKEN>; returns 401 if missing/wrong.
 *   C. Box sends raw perTaskSecret as &p=<preimage>. Worker recomputes
 *      HMAC-SHA256(preimage, KIE_CALLBACK_HMAC_KEY) in constant time and compares
 *      against stored perTaskSecretHmac. Returns 403 on mismatch.
 *      The response body NEVER includes perTaskSecretHmac or any secret.
 *
...
async function handleKvRead(request, url, env) {
  // Fix B: authenticate with KVREAD_TOKEN bearer token
  const kvReadToken = env.KVREAD_TOKEN || '';
  if (!kvReadToken) {
    console.error('KIE_KV: KVREAD_TOKEN secret not configured -- refusing all /kv-read requests');
  ...
  const tokenValid = await timingSafeEqual(
    new TextEncoder().encode(kvReadToken),
  ...
  const perTaskSecretPreimage = url.searchParams.get('p'); // Fix C: box sends the preimage
  ...
  // Fix C: validate perTaskSecretPreimage against stored HMAC in constant time
  const callbackHmacKey = env.KIE_CALLBACK_HMAC_KEY || '';
  if (!callbackHmacKey) {
    console.error('KIE_KV: KIE_CALLBACK_HMAC_KEY secret not configured');
    return new Response(JSON.stringify({ error: 'server misconfiguration' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } });
  }

  const expectedHmac = await hmacHex(perTaskSecretPreimage, callbackHmacKey);
...
export default {
  async fetch(request, env, ctx) {
    ...
    // --- Fix D: verify the callback validator (s=) in constant time ---
    // Expected: HMAC-SHA256(clientSlug + ":" + submitId, KIE_CALLBACK_HMAC_KEY)
    // This is unique per task (submitId is 128-bit random), keyed by operator secret.
    // Nothing in s= is the raw perTaskSecret, so nothing secret traverses Kie logs.
    const callbackHmacKey = env.KIE_CALLBACK_HMAC_KEY;
    if (!callbackHmacKey) {
    ...
    const expectedValidator = await hmacHex(`${clientSlug}:${submitId}`, callbackHmacKey);
