...
SN=48
REQ=["Top_39_Suggested_Ad_Angles","Facebook_Headline_and_Primary_Text_Ad_Copy_Writer","Facebook_Targeting_Intelligence"]
SUP=[]
RMAP={"Top_39_Suggested_Ad_Angles":"angles","Facebook_Headline_and_Primary_Text_Ad_Copy_Writer":"copy","Facebook_Targeting_Intelligence":"audience"}
...
def import_handoff(dd):
 ...
 for i in target.get("inputs",[]):
  ...
  ds.append(dict(deliverable=dv,file=fn,sha256=i.get('sha256',''),content_path=str(dd/fn) if fn else '',role=RMAP.get(dv,'unknown')))
 ...
 return dict(adapter='aa_import_handoff',adapter_skill=SN,intake_type='avatar-alchemist-handoff',client_label=ho.get('client_label',''),purpose=target.get('purpose',''),skill_name=target.get('skill_name',''),verified=True,source_docs=ds,notes=['AA handoff.'],source_handoff_sha256=ho.get('source_manifest_sha256',''),source_certificate_sha256=ho.get('source_certificate_sha256','')),[]
def _rep(rr,es,op):
 if rr and not es:
  ...
  roles=sorted({d['role'] for d in rr.get('source_docs',[])})
  print(f'PASS:Skill 48 imported - roles:{roles}.',file=sys.stderr);return 0
 print(f'FAIL:{len(es)} violations.',file=sys.stderr)
 for m in es:print(f'  VIOLATION [AF-HI-48] {m}',file=sys.stderr)
 return 2
def main(argv):
