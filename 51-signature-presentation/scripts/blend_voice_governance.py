...
def prove_against_pin() -> dict:
    """F05 — compare the CURRENT on-disk sacred files to the golden pin file
    scripts/sacred-structure-hashes.json. Fail-closed: a missing/unreadable pin,
    a missing sacred file (None hash), or ANY hash mismatch is a FAIL.

    This is the runtime enforcer the pin file never had — it was generated by
    --hash-structure and asserted only inside a unit test; verify.sh and --prove
    both ran without ever reading it."""
    pin_path = _SKILL_ROOT / "scripts" / "sacred-structure-hashes.json"
    result = {"pin_file": str(pin_path), "pin_found": False,
              "pass": False, "mismatched": {}, "missing": []}
    if not pin_path.is_file():
        result["error"] = "pin file not found"
        return result
    try:
        raw = json.loads(pin_path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        result["error"] = f"pin file unreadable: {exc!r}"
        return result
    if not isinstance(raw, dict):
        result["error"] = "pin file is not a JSON object"
        return result
    pinned = {k: v for k, v in raw.items() if not k.startswith("_")}
    if not pinned:
        result["error"] = "pin file declares no hashes"
        return result
    result["pin_found"] = True

    current = structural_fixture_hashes()
    for rel in sorted(set(pinned) | set(current)):
        cur = current.get(rel)
        pin = pinned.get(rel)
        if pin is None:
            # On disk but NOT pinned: an unpinned new/renamed sacred file.
            result["mismatched"][rel] = {"pinned": None, "current": cur}
            continue
        if cur is None:
            result["missing"].append(rel)
            continue
        if cur != pin:
            result["mismatched"][rel] = {"pinned": pin, "current": cur}
    result["checked"] = len(set(pinned) | set(current))
    result["pass"] = not result["missing"] and not result["mismatched"]
    return result
...
def main(argv: list) -> int:
    ...
    ap.add_argument("--prove-pin", action="store_true",
                    help="(F05) fail-closed comparison of the sacred files against "
                         "scripts/sacred-structure-hashes.json; nonzero exit on any "
                         "drift, missing file, or missing pin")
