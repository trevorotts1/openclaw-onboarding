# Book Writer — deliverable index

**The Quiet Authority: How the Best New Leaders Trade Control for Trust** — the locked GATE-1 title and subtitle for this book, byte-exact.

Everything below is a LOCAL labeled deliverable (no n8n / Airtable / Google / Gmail /
Slack / GHL). See PROCESS-CERTIFICATE.json for the signed provenance.

- `00-INDEX.md`
- `30_Day_Challenge-Marcus_Halloway.md`
- `APPROVED-OUTLINE.md`
- `APPROVED-TITLE.txt`
- `Avatar_Document-Marcus_Halloway.md`
- `Book_Blurb_and_Chapter_Titles-Marcus_Halloway.md`
- `Book_Cover_Prompt.md`
- `MANIFEST.json`
- `PROCESS-CERTIFICATE.json`
- `PROCESS-CERTIFICATE.md`
- `Suggested_Titles-Marcus_Halloway.md`
- `The_Quiet_Authority-Manuscript.md`
- `Tone_Communication_Style_Analysis-Marcus_Halloway.md`
- `chapters/ch01.md`
- `chapters/ch02.md`
- `chapters/ch03.md`
- `chapters/ch04.md`
- `chapters/ch05.md`
- `chapters/ch06.md`
- `chapters/ch07.md`
- `chapters/ch08.md`
- `chapters/ch09.md`
- `chapters/ch10.md`
- `chapters/ch11.md`
- `chapters/ch12.md`
