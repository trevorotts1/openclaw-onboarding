...
## `ingest_poller.py` + `capability_probe.py` (U12)

The box ingest poller polls `GET /api/media/<answerId>` (the media.js pollJob
contract), pulls completed jobs, hands them to `bridge/media_textractor.py`
(U13) for transcription, and stages the output for the GHL write-back seam.
Mirrors preflight.sh's fail-closed discipline.
