...
done by **deterministic Python**, so there is **no formatter model tier** in
`assets/model-map.template.json` and no concrete model id is baked anywhere.
This removes five LLM calls (and their per-call cost/latency) with zero loss of
fidelity.
