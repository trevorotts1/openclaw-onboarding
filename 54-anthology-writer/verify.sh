...
if [ "$e2e_rc" -ne 0 ] && [ ! -f "$DTMP/delivery/PROCESS-CERTIFICATE.json" ]; then
    [ "$QUIET" -eq 0 ] && printf '  [PASS] seeded short chapter blocks the run; NO certificate issued (rc=%s)\n' "$e2e_rc"
else
    failed_checks+=("seeded short chapter blocks the run")
    [ "$QUIET" -eq 0 ] && printf '  [FAIL] seeded defect did not block the run / a certificate leaked (rc=%s)\n' "$e2e_rc"
    fails=$((fails + 1))
fi
rm -rf "$DTMP"
...
[ "$QUIET" -eq 0 ] && echo "  -- ENGINE-PIN hash pin (AF-AW-HASH-PIN) --"
ENFORCE_FILES=(
