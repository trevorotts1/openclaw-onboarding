...
(the two-show convention in the binding section above), and the publish step
(Step 15) never uses one fixed channel for both modes: it passes the channel
selected BY MODE as the payload's podcast_id, and every downstream consumer
keys on exactly that value.
