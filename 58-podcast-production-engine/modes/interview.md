...
Two-show target (pointer; SKILL.md section "The two-show model" is the authority): every client runs TWO shows under BlackCEO's single Podbean host account, and this mode publishes ONLY to the client's INTERVIEW show, the guest-system channel that carries the SHUA lead-generation lane. Channel selection: interview resolves the client's interview-show channel (env PODBEAN_PODCAST_ID when the interview show is the box default, else that show's own PODBEAN_PODCAST_ID_<SHOW_SLUG> variable, for example PODBEAN_PODCAST_ID_SOFT_GIRL_ERA); the payload's podcast_id carries that channel, and the operator's podcast_publish_roster holds one row per show (same email plus last_name, distinct podbean_channel_id, each good_standing=YES) with the n8n gates selecting the row whose channel matches the payload podcast_id. The personal show belongs to Personal Podcast Style; see modes/personal.md.
