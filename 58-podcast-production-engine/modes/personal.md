...
Two-show target (pointer; SKILL.md section "The two-show model" is the authority): every client runs TWO shows under BlackCEO's single Podbean host account, and this mode publishes ONLY to the client's PERSONAL show, the solo-episode channel. Channel selection: personal resolves the client's personal-show channel (env PODBEAN_PODCAST_ID when the personal show is the box default, else that show's own PODBEAN_PODCAST_ID_<SHOW_SLUG> variable); the payload's podcast_id carries that channel, and the operator's podcast_publish_roster holds one row per show (same email plus last_name, distinct podbean_channel_id, each good_standing=YES) with the n8n gates selecting the row whose channel matches the payload podcast_id. The interview show belongs to Interview Style; see modes/interview.md.

Selection: the Production Mode is carried on the intake payload and read at Step 1 (Ingest). Personal survey submissions use the dedicated selector contact.select_your_presentation_style_personal_podcast. Its Counter Intuitive and Passionate values are authoritative and must not fall back to contact.podcast_survey_writing_style, which may be stale from an Interview submission on the same contact. Generic upstream payloads may carry style directly. Resolving this mode sets the default preset to solo unless the payload explicitly names a different output-type preset (see config/presets.json).
