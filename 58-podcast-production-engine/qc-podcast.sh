...
# -- onb-4: optional n8n deployment drift probe -------------------------------
# Runs only when N8N_API_URL and N8N_API_KEY are both set. An unreachable
# instance (exit 2) is a WARN, never a FAIL. DRIFT (exit 1) is a hard FAIL.
# Skipped with a clear log line when either env var is absent.
N8N_DRIFT_SCRIPT="$SKILL_DIR/scripts/verify-n8n-deploy.py"
if [ -n "${N8N_API_URL:-}" ] && [ -n "${N8N_API_KEY:-}" ]; then
  if [ -f "$N8N_DRIFT_SCRIPT" ]; then
    echo ""
    echo "--- n8n deployment drift probe (onb-4) ---"
    set +u
    DRIFT_RC=0
    DRIFT_OUT=$(python3 "$N8N_DRIFT_SCRIPT" --config-dir "$SKILL_DIR/config/n8n" 2>&1) || DRIFT_RC=$?
    set -u
    printf '%s\n' "$DRIFT_OUT"
    if [ "$DRIFT_RC" -eq 0 ]; then
      green "  . PASS -- n8n deployment drift: MATCH"; PASS=$((PASS+1))
    elif [ "$DRIFT_RC" -eq 2 ]; then
      yellow "  .WARN -- n8n deployment drift: instance unreachable (non-fatal)"; WARN=$((WARN+1))
    else
      red "  . FAIL -- n8n deployment drift: DRIFT detected"; FAIL=$((FAIL+1))
    fi
  else
    yellow "  .WARN -- verify-n8n-deploy.py not found at $N8N_DRIFT_SCRIPT"; WARN=$((WARN+1))
  fi
else
  echo ""
