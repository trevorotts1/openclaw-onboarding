...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # full mode also requires the media-provider creds (unit 1.7): KIE/FISH
        # keys + the client reference_id. Provide them so this alias-resolution
        # case tests only the two-store precedence, not media-provider presence.
        engine_two_store[0][1]["KIE_API_KEY"] = "test-kie-key"
        engine_two_store[0][1]["FISH_AUDIO_API_KEY"] = "test-fish-key"
        engine_two_store[0][1]["FISH_AUDIO_REFERENCE_ID"] = "test-ref-id"
        g = CredentialGate(
