...
# Podcast Production Engine — Podbean publish+verify end-to-end integration test.
#
# WHAT THIS IS
#   A GATED integration test that exercises the full create+verify+delete cycle
#   against the live Podbean API using a Channel-scoped access token. It creates a
#   draft episode, verifies it exists in the episode listing, and then deletes it
#   so nothing is left behind. It NEVER publishes a live episode and NEVER touches
#   a production channel without explicit operator opt-in.
#
# SAFETY — EXPLICIT OPT-IN REQUIRED (this test NEVER runs unattended)
#   Set PODBEAN_INTEGRATION_TEST=1 in the environment, OR pass --run as the first
#   argument. Without one of these, the script prints a SKIPPED message and exits 0
#   immediately (gate green for automation that has not opted in).
#
#   The episode it creates is always status=draft (never visible publicly) and is
#   deleted before the script exits, whether the test passes or fails.
#
...
#
# USAGE
#   PODBEAN_INTEGRATION_TEST=1 bash podbean-integration-test.sh
#   bash podbean-integration-test.sh --run
#
# EXIT CODES
...
# including signals (INT/TERM) and failures.
CREATED_EPISODE_ID=""
