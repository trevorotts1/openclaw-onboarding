...
| D9 | Single-client hardcoding (pipeline and stage identifiers in nodes) | REPAIR -- the registry binds per anthology per client at provisioning; the standard pipeline auto-provisioned with the client's own token | anthology_registry.py + provision-anthology-client.sh |
