#!/usr/bin/env bash
# 59-anthology-engine/scripts/scan-no-client-identifiers.sh
# ----------------------------------------------------------------------------
# MERGE-GATE STATIC SCAN #3 of 4 (Unit W2.8). Proves NO client identifier or
# client personally identifiable information (PII) is present in the scanned
# tree. Enforces "zero client PII in any repo" (SPEC / CHECKLIST Part C items 2,
# 5, 10, 23 fleet-wide identifier grep). This IS a product enforcement tool:
# pattern matching is the mechanism, by design.
#
# WHAT IT CATCHES:
...
#                  "hardcoded test inbox" defect the engine kills)
