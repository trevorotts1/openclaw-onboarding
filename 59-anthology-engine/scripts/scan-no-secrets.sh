#!/usr/bin/env bash
# 59-anthology-engine/scripts/scan-no-secrets.sh
# ----------------------------------------------------------------------------
# MERGE-GATE STATIC SCAN #1 of 4 (Unit W2.8). Proves NO secret VALUE is present
# anywhere in the scanned tree. This IS a product enforcement tool: pattern
# matching is the mechanism, by design (the doctrine that forbids grep-for-
# judgment is about human reasoning, not about the guardrail's own detector).
#
...
#                  cover-node defect the engine retires)
...
#
# SCOPE (choose one):
#   (default)      the engine skill dir (this script's parent) -- the build's home
#   --changed      only files changed vs the base ref (BEST for a branch merge
#                  gate: high signal, zero pre-existing fleet noise)
#   --fleet        the whole git top-level (the Wave 6 fleet-wide pass)
