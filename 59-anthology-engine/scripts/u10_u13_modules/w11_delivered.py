#!/usr/bin/env python3
# =============================================================================
# SKILL 59 — ANTHOLOGY ENGINE :: u10_u13_modules/w11_delivered.py
# (U10-U13 tooling)
# W11 DELIVERED TEMPLATE MODULE — the OFFLINE JSON/Python data generator
# for the "Anthology: Book Delivered" tag-triggered notification workflow
# (contract row workflows.release_notifications, trigger tag
# anthology-delivered). The workflow is EMAIL + SMS: a producer-branded
# email carrying the finished manuscript's PDF (view) link plus editable
# Google Doc (edit) link, and a link-only SMS (one warm sentence plus ONE
# link).
# -----------------------------------------------------------------------------
...
#
# WHAT THIS OWNS
#   1. THE W11 TEMPLATE LAW. The delivered workflow is the eleventh of the
#      tag->notification workflows in contract
...
#      Convert and Flow build rail. It fires at S9 book delivery: the
#      assembly cockpit stamps the anthology-delivered tag on EVERY contact
#      itself after the producer's final-manuscript sign-off (gate_engine.py
#      GATE_RELEASE_SLUG doctrine — the assembly gates s9_ready / s9_producer
#      key on an anthology_id, not a single contact, so they never fire the
#      per-contact producer-gate release bus; the cockpit is the sole tag
#      writer here, caf_delivery.py add-tag). THIS module ships the workflow
...
#      {{ contact.first_name }}. It carries the S9 milestone congratulations
#      (the contract row note: "S9 all-hands book delivery, you are a
#      published author"). It carries the standing instruction, verbatim
#      from copy_law per_stage_copy.standing_instruction ("The PDF is yours
#      to view. The Google Doc is the one you edit, and it is the version we
#      use."). It links the finished manuscript pair: the PDF (view) link
#      and the Google Doc (edit) link from the matching field-map
#      deliverable_fields contact custom fields (field-map.json
#      deliverable_fields.manuscript doc_url + pdf_url), in the contract
#      row's own field order — the SAME fields the contract row names in
#      email_link_fields (PDF first, Doc second).
#   3. THE STAGE-FORM LINK LAW. The email ALSO carries the stage form link:
#      the Convert and Flow hosted-form URL
#      <forms_base_url>/widget/form/<form_id>?anthology_id=<id>&stage=
#      delivered with the anthology_id and stage query params PREFILLED from
#      the contact (anthology_id from {{ contact.anthology_active_id }} —
#      the G3 law: the query key is EXACTLY anthology_id, never
#      anthology_active_id — and stage prefilled as "delivered" so the
#      universal hidden-field contract (contact_id / anthology_id / stage)
#      re-stamps the delivered state on form entry). The stage token
#      "delivered" is the TERMINAL cursor of anthology_state.py STAGE_CURSORS
#      (the S9 sign-off moves every member participant to the "delivered"
#      cursor, then the cockpit stamps the tag); intake_router.py
#      classify_stage accepts a known-cursor token as a self-describing
#      per-stage form when the participant is at that cursor, so a
#      delivered-state form routes only for delivered participants. The form
...
#      per-anthology form binding — the template never hardcodes a
#      per-client domain or form id.
#   4. THE SMS LAW. The SMS is link-only per copy_law sms_shape: one warm
#      sentence plus ONE link. The one link is the finished manuscript's PDF
#      (view) link, {{ contact.anthology_manuscript_pdf_url }} — the
#      contract row's sms_link_field, byte-exact (the delivered row is the
#      S8-family exception: the SMS carries the PDF, not the Doc, because
#      the book is a published artifact for reading). No second link, no
#      call to action beyond the book.
...
#      contract (the W11 law would be unverifiable)
...
#   w11_delivered.py plan            # offline: the W11 template law
#   w11_delivered.py render          # offline: the generated template
#   w11_delivered.py self-test       # offline golden + attack battery
#
...
# =============================================================================
"""w11_delivered.py — offline data generator for the Anthology: Book
Delivered (W11) tag->notification workflow template: email + SMS under the
...
# workflows.release_notifications (name "Anthology: Book Delivered",
# trigger_tag anthology-delivered).
WORKFLOW_NAME = "Anthology: Book Delivered"
TRIGGER_TAG = "anthology-delivered"

# The S9 assembly producer sign-off that leads to the tag (gate_engine.py
# APPROVAL_GATES "s9_producer"; the assembly cockpit stamps
# anthology-delivered on every contact itself — the assembly gates key on an
# anthology_id, not a single contact, so they never fire the per-contact
# producer-gate release bus). GATE_CURSOR is the TERMINAL ledger cursor the
# S9 sign-off moves every member participant to (anthology_state.py
# STAGE_CURSORS "delivered").
PRODUCER_GATE = "s9_producer"
GATE_CURSOR = "delivered"

# The stage token the stage-form link prefills. The universal hidden-field
# contract re-stamps stage on form entry (contract forms
# universal_hidden_fields contact_id / anthology_id / stage). The delivered
# workflow fires at the TERMINAL cursor, so the stage token IS the terminal
# cursor "delivered" — a known STAGE_CURSORS vocabulary entry, accepted by
# intake_router.py classify_stage as a self-describing per-stage form for a
# participant at that cursor.
STAGE = "delivered"
...
# The W11 stage's deliverable pair is the finished MANUSCRIPT — S9's own
# compiled full text, delivered once at book delivery (field-map.json
# deliverable_fields.manuscript doc_url + pdf_url; stage_s8_deliver.py
# resolves "anthology_manuscript" to the "manuscript" deliverable slot).
# --------------------------------------------------------------------------- #
# The contact custom-field merges for the W11 stage's deliverable pair. The
...
MANUSCRIPT_PDF = "{{ contact.anthology_manuscript_pdf_url }}"
MANUSCRIPT_DOC = "{{ contact.anthology_manuscript_doc_url }}"

# The contract row's link fields, spelled BYTE-EXACT as the contract carries
# them (workflows.release_notifications row "Anthology: Book Delivered" —
# email_link_fields use the compact merge form "{{contact.anthology_...}}",
# with NO inner spaces; the contract is the byte-authority for these, so the
# template mirrors it exactly). The manuscript pair: PDF (view) first, then
# Google Doc (edit), in the contract row's own field order.
EMAIL_LINK_FIELDS = (
    "{{contact.anthology_manuscript_pdf_url}}",
    "{{contact.anthology_manuscript_doc_url}}",
)

# The contract row's sms_link_field, byte-exact: the finished manuscript's
# PDF (view) link — the one link the link-only SMS carries. (The delivered
# row is the S8-family exception: the SMS carries the PDF, not the Doc,
# because the book is a published artifact for reading, not editing.)
SMS_LINK_FIELD = "{{contact.anthology_manuscript_pdf_url}}"

# The contract row's bare link keys, resolved against field-map.json
# deliverable_fields (manuscript doc_url + pdf_url pair). The self-test
# asserts every key is declared there — a renamed key on either side is a
# drift, never a blind pass (the copy_qc_workflows.py
# AF-AE-COPY-FIELD-DRIFT law).
MANUSCRIPT_PDF_KEY = "anthology_manuscript_pdf_url"
MANUSCRIPT_DOC_KEY = "anthology_manuscript_doc_url"
LINK_BARE_KEYS = (MANUSCRIPT_PDF_KEY, MANUSCRIPT_DOC_KEY)
...
# The S9 milestone congratulations (the contract row note: "S9 all-hands
# book delivery, you are a published author").
MILESTONE_NOTE = ("S9 all-hands book delivery, you are a published author")
...
# placeholders, never a real value). The W11 template carries them so the
...
# --------------------------------------------------------------------------- #
# The W11 template law (what the self-test enforces).
# --------------------------------------------------------------------------- #
W11_LAW = {
    ...
    "email": {
        ...
        "greeting": "{{ contact.first_name }}, your book is delivered.",
    ...
    "sms": {
        ...
        "body": "Your book is delivered, and you are a published author. "
                "View it here: " + SMS_LINK_FIELD,
    },
    "never_a_real_token": {
...
def _stage_form_link():
    ...
    <forms_base_url>/widget/form/<form_id>?anthology_id=<active>&stage=
    delivered. The anthology_id value rides the contact's OWN active-
    anthology merge (the G3 law: the query key is EXACTLY anthology_id, never
    anthology_active_id; the template never holds a concrete id). The stage
    token is the TERMINAL ledger cursor "delivered" (anthology_state.py
    STAGE_CURSORS), accepted by intake_router.py classify_stage as a
    self-describing per-stage form for a participant at that cursor."""
...
def workflow_payload():
    """The W11 workflow template as a data object (the Python face of the
    ...
    email_body = (
        ...
        "Congratulations. Your book is delivered, and you are a published "
        "author. From your first chapter to this finished manuscript, our "
        "editors have worked alongside you at every step.\n\n"
        "View your book PDF here:\n" + MANUSCRIPT_PDF + "\n\n"
        "Edit your manuscript in the Google Doc here:\n"
        + MANUSCRIPT_DOC + "\n\n"
        + STANDING_INSTRUCTION + "\n\n"
        "If you have any final requests, open the delivery form here:\n"
        + stage_form + "\n\n"
        "Warm regards,\n" + SIGN_OFF)
    sms_body = ("Your book is delivered, and you are a published author. "
                "View it here: " + SMS_LINK_FIELD)
    return {
        "workflow_name": WORKFLOW_NAME,
        "trigger_tag": TRIGGER_TAG,
        ...
        "note": "offline template only — a REAL location write must ride the "
                ...
                "workflow) before it fires live; the tag is stamped by the "
                "assembly cockpit on every contact at S9 delivery, not by "
                "the per-contact producer-gate bus",
    }
...
# against the W11 law. Every law is asserted on the GENERATED payload, so a
...
def _contract_row(contract):
    """The contract's W11 row, or None when the contract does not carry it."""
...
def self_test(out=None) -> int:
    ...
    try:
        ...
        out.write("[w11-delivered] STOP: %s\n" % exc)
        return EX_STOP

    fails = []

    # (1) contract-row law: the W11 row must exist and carry the exact tag.
    ...
    # (2) field-map law: every W11 link key must be declared in the
    ...
    # bare field keys ("manuscript", ...); the template's link fields
    # reference the contact-level keys, so the bare KEY is matched against
    # the map's declared values' field keys (the "..._pdf_url" /
    # "..._doc_url" names), exactly as copy_qc_workflows resolves them.
    declared = set()
    ...
    link_count = 0
    for link in (MANUSCRIPT_PDF, MANUSCRIPT_DOC):
    ...
    if fails:
        for check, detail in fails:
            out.write("[w11-delivered] %s: %s\n" % (check, detail))
        out.write("[w11-delivered] self-test FAILED "
                  "(%d violation%s)\n" % (len(fails),
                                          "" if len(fails) == 1 else "s"))
        return EX_VIOLATION
    out.write("[w11-delivered] self-test PASS: W11 book-delivered "
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        prog="w11_delivered.py",
        description="W11 book-delivered notification workflow template "
    ...
    if args.cmd == "render":
        ...
        return EX_OK
    # plan: the offline plan — the W11 template law with its sources.
    print(json.dumps({
        ...
        "note": "offline plan only — synthetic merges, no network, no "
                ...
                "before it fires live; the tag is stamped by the assembly "
                "cockpit on every contact at S9 delivery",
