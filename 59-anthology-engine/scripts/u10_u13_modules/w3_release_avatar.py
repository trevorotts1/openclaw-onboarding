#!/usr/bin/env python3
# =============================================================================
# SKILL 59 — ANTHOLOGY ENGINE :: u10_u13_modules/w3_release_avatar.py
# (W3 template generator)
# ANTHOLOGY-RELEASE-AVATAR TEMPLATE — EMAIL + SMS (OFFLINE data generators).
# The ONLY client-facing copy the anthology-release-avatar tag may send:
#   trigger tag  anthology-release-avatar  (the §3 release bus; s1_producer
#                                          gate — GATE_RELEASE_SLUG
#                                          ["s1_producer"], the FIRST live
#                                          release slug of the contract)
#   actions      send-email + send-sms
#   email links  {{contact.anthology_avatar_pdf_url}} (PDF view) +
#                {{contact.anthology_avatar_doc_url}} (Google Doc edit)
#   sms link     {{contact.anthology_avatar_doc_url}}
# (contract rows: config/anthology-snapshot-contract.json
# workflows.release_notifications "Anthology Release: Avatar" + copy_law;
# field-map.json deliverable_fields.avatar doc_url / pdf_url).
...
#        token is "s1_gate" (the EXACT STAGE_CURSORS vocabulary of
...
#   ONE PDF VIEW + ONE DOC EDIT ..... each email carries the Avatar PDF
#        (view) link plus the Avatar Google Doc (edit) link, both from the
#        contact custom fields (field-map deliverable_fields avatar ->
...
#
# STDLIB ONLY. Importable by the u10_u13 package init and by the W3 assembler
...
"""W3 release-avatar template generator: EMAIL + SMS copy, OFFLINE and pure."""
...
WORKFLOW_NAME = "Anthology Release: Avatar"
TRIGGER_TAG = "anthology-release-avatar"
...
# gate that follows the Avatar stage.
STAGE_VOCABULARY = (
...
DEFAULT_STAGE = "s1_gate"

# Field-map deliverable slots for the avatar deliverable
# (field-map.json deliverable_fields.avatar): the contact custom fields the
# generated email/SMS reference.
PDF_LINK_MERGE = "{{contact.anthology_avatar_pdf_url}}"
DOC_LINK_MERGE = "{{contact.anthology_avatar_doc_url}}"
...
# The client-clean deliverable label of the Avatar stage — the SAME phrase
# the sanctioned nudge templates carry (nudge_send.GATE_META["s1_producer"]
# "author profile", label_checker.py): never a stage code or an internal name.
DELIVERABLE_LABEL = "author profile"
...
def email_subject(anthology_name=ANTHOLOGY_NAME_MERGE):
    """Subject line: the author profile deliverable is ready.
    ...
    return _assert_copy_law(
        "Your author profile for %s is ready" % anthology_name,
        "email_subject",
    )
...
def email_body(anthology_name=ANTHOLOGY_NAME_MERGE,
    ...
    """Render the release-avatar EMAIL body (plain text).

    Slots (all GHL merges by default): the author's first name, the
    anthology name, the Avatar PDF view link, the Avatar Google Doc edit
    link, and the review form link. The sign-off is "The Editors" (or the
    ...
    parts.append("")
    parts.append("Your author profile for %s is ready." % anthology_name)
    parts.append("")
...
def sms_body(doc_link=DOC_LINK_MERGE, first_name=FIRST_NAME_MERGE):
    """Render the release-avatar SMS body: ONE warm sentence plus ONE link
    (the sms_shape law; the SMS carries the doc link only, never the PDF).
    No sign-off — the SMS shape law has no room for one."""
    text = "%s, your author profile is ready to review. Here is your " \
           "Google Doc: %s" % (first_name, doc_link)
...
def render_email(sign_off=SIGN_OFF_EDITORS,
                 ...
                 stage=DEFAULT_STAGE,
                 forms_base="https://link.msgsndr.com"):
    """The complete EMAIL payload of the anthology-release-avatar workflow:
    subject, body, producer-branded from_name, reply_to, and the per-stage
    link set (Avatar PDF view + Avatar Doc edit + the pre-filled review form
...
def plan():
    print("W3 TEMPLATE  %s" % WORKFLOW_NAME)
    print("trigger tag: %s  (s1_producer §3 release bus)" % TRIGGER_TAG)
...
def _self_test():
    ...
    assert "&%s=s1_gate" % STAGE_KEY in link, "default stage token missing"
    ...
    assert "?anthology_id=ANTH_TEST&stage=s1_gate" in preview["stage_form_link"]
    ...
    _attack("ai-in-sms", lambda: sms_body(doc_link=DOC_LINK_MERGE,
                                          first_name="AI"))
    print("w3_release_avatar self-test: OK "
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="W3 template generator: anthology-release-avatar EMAIL + SMS "
    ...
    try:
        ...
        sys.stderr.write("w3_release_avatar: law violation: %s\n" % exc)
        return 2
    except Exception as exc:
        sys.stderr.write("w3_release_avatar: unexpected error: %s\n" % exc)
        return 1
