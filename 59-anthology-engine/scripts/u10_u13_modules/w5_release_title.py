#!/usr/bin/env python3
# =============================================================================
# SKILL 59 — ANTHOLOGY ENGINE :: u10_u13_modules/w5_release_title.py
# (W5 template generator)
# ANTHOLOGY-RELEASE-TITLE TEMPLATE — EMAIL + SMS (OFFLINE data generators).
# The ONLY client-facing copy the anthology-release-title tag may send:
#   trigger tag  anthology-release-title  (the §3 release bus; the S3 TITLE
#                                          stage delivers the Titles Doc plus
#                                          PDF and stamps the TITLE LOCK one-way,
#                                          then the participant picks title and
#                                          subtitle on the token page)
#   actions      send-email + send-sms
#   email links  {{contact.anthology_titles_pdf_url}} (PDF view) +
#                {{contact.anthology_titles_doc_url}} (Google Doc edit)
#   sms link     {{contact.anthology_titles_doc_url}}
# (contract rows: config/anthology-snapshot-contract.json field-map
# deliverable_fields.titles doc_url / pdf_url + copy_law; forms_check.py
# FORM_ID_BY_SLUG "title-select"). This module is a TEMPLATE GENERATOR, never
# a messenger: it renders the exact email/sms payloads and the stage review
# link OFFLINE — zero network, zero credentials, nothing ever sent. A send
# path (nudge_send.py / caf delivery) fills the rendered slots at send time;
# this module never touches a token, an env secret, or the wire.
# -----------------------------------------------------------------------------
...
#        pre-fill the form's HIDDEN fields client-side (the title-select form
#        carries EXACTLY the two routing keys anthology_id + stage — never
#        contact_id, it is only ever opened from an ALREADY-resolved
#        participant token page; prefill_verifier.py owns the verifier). The
#        default form is the title-select form (pin
#        UgiiSoZsA4vyqOVfO5fi — the Title Fire trigger AND the S3
#        title-and-subtitle link in "Release: Titles", the form the
#        participant picks title and subtitle in) and the default stage token
#        is "s3_gate" (the EXACT STAGE_CURSORS vocabulary of
...
#   ONE PDF VIEW + ONE DOC EDIT ..... each email carries the Titles PDF
#        (view) link plus the Titles Google Doc (edit) link, both from the
#        contact custom fields (field-map deliverable_fields.titles ->
...
#
# STDLIB ONLY. Importable by the u10_u13 package init and by the W5 assembler
...
"""W5 release-title template generator: EMAIL + SMS copy, OFFLINE and pure."""
...
# The §3 release bus event this template serves (the S3 TITLE stage's
# release; the release bus is fired by the stage runner, not by a
# producer-approve gate — the s3_selection gate is the participant token
# page, and the TITLE LOCK is stamped one-way by anthology_state.py
# record-approval).
WORKFLOW_NAME = "Anthology Release: Title"
TRIGGER_TAG = "anthology-release-title"

# The title-select form — the Title Fire trigger AND the S3 title-and-subtitle
# link in "Release: Titles" (forms_check.FORM_ID_BY_SLUG["title-select"] — the
# SAME pin form_spec_loader.py and title_select_builder.py ship against; the
# form carries EXACTLY the two routing keys anthology_id + stage, never
# contact_id).
STAGE_FORM_SLUG = "title-select"
STAGE_FORM_ID = "UgiiSoZsA4vyqOVfO5fi"
...
# gate that follows the Title stage (the participant token page where the
# title and subtitle pick happens).
STAGE_VOCABULARY = (
...
# (?anthology_id=<minted>&stage=<stage>; the title-select form carries the
# TWO routing keys anthology_id + stage, never the intake contact_id).
...
def email_subject(anthology_name=ANTHOLOGY_NAME_MERGE):
    """Subject line: the title-and-subtitle selection is ready. ``anthology_name``
    is the GHL merge slot by default; a caller that renders an offline preview
    may pass a literal name (preview values are test values, never secrets)."""
    return _assert_copy_law(
        "Your titles for %s are ready" % anthology_name, "email_subject"
    )
...
def email_body(anthology_name=ANTHOLOGY_NAME_MERGE,
    ...
    """Render the release-title EMAIL body (plain text).

    Slots (all GHL merges by default): the author's first name, the
    anthology name, the Titles PDF view link, the Titles Google Doc edit link,
    and the selection form link. The sign-off is "The Editors" (or the
    producer merge when the operator pins it). The selection link is optional
    ...
    parts.append("")
    parts.append("Your titles for %s are ready." % anthology_name)
    parts.append("")
    ...
    if stage_form_link:
        parts.append("")
        parts.append("Your chosen title and subtitle will be the title of "
                     "your book, so please take a look and make your "
                     "selection here:")
...
def sms_body(doc_link=DOC_LINK_MERGE, first_name=FIRST_NAME_MERGE):
    """Render the release-title SMS body: ONE warm sentence plus ONE link
    (the sms_shape law; the SMS carries the doc link only, never the PDF).
    No sign-off — the SMS shape law has no room for one."""
    text = "%s, your titles are ready to review. Here is your Google Doc: %s" % (
        first_name, doc_link
    )
...
def stage_form_link(form_id=STAGE_FORM_ID,
    ...
    """Build the stage selection form link — the U08 pre-fill law:
...
def render_email(sign_off=SIGN_OFF_EDITORS,
                 ...
                 stage=DEFAULT_STAGE,
                 forms_base="https://link.msgsndr.com"):
    """The complete EMAIL payload of the anthology-release-title workflow:
    subject, body, producer-branded from_name, reply_to, and the per-stage
    link set (Titles PDF view + Titles Doc edit + the pre-filled selection
    ...
    payload = {
        ...
        "body": email_body(
            sign_off=sign_off, stage_form_link=link, producer=producer,
        ),
        "from_name": producer,
...
def plan():
    print("W5 TEMPLATE  %s" % WORKFLOW_NAME)
    print("trigger tag: %s  (the S3 TITLE stage §3 release bus; TITLE LOCK "
          "one-way, participant title-and-subtitle pick)" % TRIGGER_TAG)
    print("channels:    EMAIL + SMS")
    print("email links: %s (PDF view) + %s (Doc edit)"
          % (PDF_LINK_MERGE, DOC_LINK_MERGE))
...
def _self_test():
    ...
    assert sms["body"].count(DOC_LINK_MERGE) == 1 and "http" not in sms["body"], (
        "SMS must carry exactly ONE link (the doc link)")
    ...
    _attack("ai-in-sms", lambda: sms_body(doc_link=DOC_LINK_MERGE,
                                          first_name="AI"))
    print("w5_release_title self-test: OK "
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="W5 template generator: anthology-release-title EMAIL + SMS "
    ...
    ap.add_argument("--form-id", default=STAGE_FORM_ID,
                    help="stage form id (default the title-select pin)")
    ...
    ap.add_argument("--sign-off", choices=(SIGN_OFF_EDITORS, "producer"),
                    default="editors",
    ...
    try:
        ...
        sys.stderr.write("w5_release_title: law violation: %s\n" % exc)
        return 2
    except Exception as exc:
        sys.stderr.write("w5_release_title: unexpected error: %s\n" % exc)
        return 1
