#!/usr/bin/env python3
# =============================================================================
# SKILL 59 — ANTHOLOGY ENGINE :: u10_u13_modules/w8_release_rewrite.py
# (U10-U13 tooling)
# W8 RELEASE-REWRITE TEMPLATE MODULE — the OFFLINE JSON/Python data generator
# for the "Anthology Release: Rewrite" tag-triggered notification workflow
# (contract row workflows.release_notifications, trigger tag
# anthology-release-rewrite). The workflow is EMAIL + SMS: a producer-branded
# email carrying that stage's PDF (view) links plus editable Google Doc (edit)
# links, and a link-only SMS (one warm sentence plus ONE link).
# -----------------------------------------------------------------------------
...
#
# WHAT THIS OWNS
#   1. THE W8 TEMPLATE LAW. The release-rewrite workflow is the eighth of the
#      eight tag->notification workflows in contract
...
#      Convert and Flow build rail. It fires when the producer approves at the
#      s6_producer gate (gate_engine.py GATE_RELEASE_SLUG "s6_producer" ->
#      "anthology-release-rewrite"; GATE_BY_CURSOR "s6_rewrite", release-only,
#      no cursor edge) and the engine stamps that slug on the participant's
#      Convert and Flow contact (caf_delivery.py add-tag, the sole tag
#      writer). THIS module ships the workflow template the build rail /
#      operator consumes: a pure data generator with two faces — Python
#      (workflow_payload()) and JSON (render_payload()).
...
#      use."). It links BOTH preserved rewrite revision pairs (PRD Gap G10:
#      the base chapter pair is never overwritten; the first editors' rewrite
#      lands in rewrite1, the second in rewrite2): the two PDF (view) links
#      and the two Google Doc (edit) links from the matching field-map
#      deliverable_fields contact custom fields (field-map.json
#      deliverable_fields rewrite1 / rewrite2 doc_url + pdf_url pairs), in
#      the contract row's own field order — the SAME fields the contract row
#      names in email_link_fields. The body shows the rewrites-used count as
#      the {{ contact.anthology_rewrite_count }} merge with the "of 2" budget
#      wording, per the contract row note ("shows rewrites-used count ... of
#      2; links both preserved revision pairs (G10)").
#   3. THE STAGE-FORM LINK LAW. The email ALSO carries the stage form link:
#      the Convert and Flow hosted-form URL
#      <forms_base_url>/widget/form/<form_id>?anthology_id=<id>&stage=s6 with
...
#      and stage prefilled as "s6" so the router re-stamps the universal
#      hidden-field contract on gate re-entry). The form base is the fleet
#      GHL/LeadConnector hosted-form domain (anthology_book.py
#      DEFAULT_FORMS_BASE, currently https://link.msgsndr.com) and the form
#      id is the stage gate form id; both stay overridable per box via config
#      intake.forms_base_url / the per-anthology form binding — the template
#      never hardcodes a per-client domain or form id.
#   4. THE SMS LAW. The SMS is link-only per copy_law sms_shape: one warm
#      sentence plus ONE link. The one link is the first rewrite's Google Doc
#      (edit) link, {{ contact.anthology_chapter_rewrite1_doc_url }} — the
#      contract row's sms_link_field, byte-exact. No second link, no call to
#      action beyond the doc.
...
#      contract (the W8 law would be unverifiable)
...
#   w8_release_rewrite.py plan            # offline: the W8 template law
#   w8_release_rewrite.py render          # offline: the generated template
#   w8_release_rewrite.py self-test       # offline golden + attack battery
#
...
# =============================================================================
"""w8_release_rewrite.py — offline data generator for the Anthology Release:
Rewrite (W8) tag->notification workflow template: email + SMS under the copy
law ("editors", never "AI"; zero em-dashes; sign-off "The Editors")."""
...
# workflows.release_notifications (name "Anthology Release: Rewrite",
# trigger_tag anthology-release-rewrite).
WORKFLOW_NAME = "Anthology Release: Rewrite"
TRIGGER_TAG = "anthology-release-rewrite"

# The producer gate that fires this slug (gate_engine.py GATE_RELEASE_SLUG;
# contract tags row producer_gate s6_producer).
PRODUCER_GATE = "s6_producer"
GATE_CURSOR = "s6_rewrite"
...
# universal_hidden_fields contact_id / anthology_id / stage); the rewrite
# re-enters the pipeline through the s6 stage token.
STAGE = "s6"
...
# The W8 stage's deliverable pairs: a chapter rewrite is a version of the
# chapter deliverable; the base chapter pair is NEVER overwritten (PRD Gap
# G10) — the first editors' rewrite lands in rewrite1, the second in
# rewrite2, so the base chapter and any earlier rewrite stay intact. The
# rewrite SLOT is chosen at delivery time from the participant's
# rewrite_count, never a static alias (caf_delivery.py
# FieldMap.deliverable_for_rewrite()).
# --------------------------------------------------------------------------- #
# The contact custom-field merges for the W8 stage's deliverable pairs. The
...
REWRITE1_PDF = "{{ contact.anthology_chapter_rewrite1_pdf_url }}"
REWRITE1_DOC = "{{ contact.anthology_chapter_rewrite1_doc_url }}"
REWRITE2_PDF = "{{ contact.anthology_chapter_rewrite2_pdf_url }}"
REWRITE2_DOC = "{{ contact.anthology_chapter_rewrite2_doc_url }}"

# The contract row's link fields, spelled BYTE-EXACT as the contract carries
# them (workflows.release_notifications row "Anthology Release: Rewrite" —
# email_link_fields / sms_link_field use the compact merge form
# "{{contact.anthology_...}}", with NO inner spaces; the contract is the
# byte-authority for these, so the template mirrors it exactly). The two
# preserved revision pairs — rewrite1 PDF, rewrite1 Doc, rewrite2 PDF,
# rewrite2 Doc.
EMAIL_LINK_FIELDS = (
    "{{contact.anthology_chapter_rewrite1_pdf_url}}",
    "{{contact.anthology_chapter_rewrite1_doc_url}}",
    "{{contact.anthology_chapter_rewrite2_pdf_url}}",
    "{{contact.anthology_chapter_rewrite2_doc_url}}",
)

# The contract row's sms_link_field, byte-exact: the first rewrite's Google
# Doc (edit) link — the one link the link-only SMS carries.
SMS_LINK_FIELD = "{{contact.anthology_chapter_rewrite1_doc_url}}"

# The contract row's bare link keys, resolved against field-map.json
# deliverable_fields (rewrite1 / rewrite2 doc_url + pdf_url pairs). The
# self-test asserts every key is declared there — a renamed key on either
# side is a drift, never a blind pass (the copy_qc_workflows.py
# AF-AE-COPY-FIELD-DRIFT law).
REWRITE1_PDF_KEY = "anthology_chapter_rewrite1_pdf_url"
REWRITE1_DOC_KEY = "anthology_chapter_rewrite1_doc_url"
REWRITE2_PDF_KEY = "anthology_chapter_rewrite2_pdf_url"
REWRITE2_DOC_KEY = "anthology_chapter_rewrite2_doc_url"
LINK_BARE_KEYS = (REWRITE1_PDF_KEY, REWRITE1_DOC_KEY,
                  REWRITE2_PDF_KEY, REWRITE2_DOC_KEY)
...
# contract row note: "shows rewrites-used count {{contact.anthology_rewrite_count}}
# of 2").
...
# placeholders, never a real value). The W8 template carries them so the
...
# --------------------------------------------------------------------------- #
# The W8 template law (what the self-test enforces).
# --------------------------------------------------------------------------- #
W8_LAW = {
    ...
    "email": {
        ...
        "subject": "Your revised chapter is ready to review",
        "greeting": "{{ contact.first_name }}, your revised chapter is ready.",
        "standing_instruction": STANDING_INSTRUCTION,
        "rewrite_budget_note": "You have used "
                               + REWRITE_COUNT_MERGE
                               + " of " + str(REWRITE_BUDGET) + " rewrites.",
    ...
    "sms": {
        ...
        "body": "Your revised chapter is ready to review here: "
                + SMS_LINK_FIELD,
    },
    "never_a_real_token": {
...
def _stage_form_link():
    ...
    <forms_base_url>/widget/form/<form_id>?anthology_id=<active>&stage=s6.
...
def workflow_payload():
    """The W8 workflow template as a data object (the Python face of the
    ...
    email_body = (
        ...
        "Your revised chapter is ready. Our editors have completed their "
        "revision, and we would love your review.\n\n"
        "View the revised chapter PDF here:\n" + REWRITE1_PDF + "\n"
        + REWRITE2_PDF + "\n\n"
        "Edit the revised chapter in the Google Doc here:\n"
        + REWRITE1_DOC + "\n" + REWRITE2_DOC + "\n\n"
        ...
        "review form here:\n" + stage_form + "\n\n"
        "Warm regards,\n" + SIGN_OFF)
    sms_body = ("Your revised chapter is ready to review here: " + SMS_LINK_FIELD)
    return {
        "workflow_name": WORKFLOW_NAME,
        "trigger_tag": TRIGGER_TAG,
        "producer_gate": PRODUCER_GATE,
...
# against the W8 law. Every law is asserted on the GENERATED payload, so a
...
def _contract_row(contract):
    """The contract's W8 row, or None when the contract does not carry it."""
...
def self_test(out=None) -> int:
    ...
    try:
        ...
        out.write("[w8-release-rewrite] STOP: %s\n" % exc)
        return EX_STOP

    fails = []

    # (1) contract-row law: the W8 row must exist and carry the exact tag.
    ...
    # (2) field-map law: every W8 link key must be declared in the
    ...
    # bare field keys ("rewrite1", "rewrite2", ...); the template's link
    # fields reference the contact-level keys, so the bare KEY is matched
    # against the map's declared values' field keys (the "..._pdf_url" /
    ...
    link_count = 0
    for link in (REWRITE1_PDF, REWRITE1_DOC, REWRITE2_PDF, REWRITE2_DOC):
    ...
    if fails:
        for check, detail in fails:
            out.write("[w8-release-rewrite] %s: %s\n" % (check, detail))
        out.write("[w8-release-rewrite] self-test FAILED "
                  "(%d violation%s)\n" % (len(fails),
                                          "" if len(fails) == 1 else "s"))
        return EX_VIOLATION
    out.write("[w8-release-rewrite] self-test PASS: W8 release-rewrite "
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        prog="w8_release_rewrite.py",
        description="W8 release-rewrite notification workflow template "
    ...
    if args.cmd == "render":
        ...
        return EX_OK
    # plan: the offline plan — the W8 template law with its sources.
    print(json.dumps({
