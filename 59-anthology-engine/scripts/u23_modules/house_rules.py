#!/usr/bin/env python3
# =============================================================================
# SKILL 59 — ANTHOLOGY ENGINE :: u23_modules/house_rules.py  (U23 tooling)
...
# the U23 SMS provisioner (provision_sms_phone.py), its extension helpers
# (phone_lister / provision_action / sms_verifier / attack_sms_failed /
# golden_has_phone), and the family's batteries import, so a law can NEVER
# drift between the modules that enforce it — exactly the
# delta_reporter.py single-implementation doctrine (a law read once, in one
# module) that the U03 / U05 / U06 / U07 families already practice (their
# house_rules.py is this module's packaged sibling; this module's authority
# is the manifest itself, exactly as the U03 / U05 siblings mirror, with the
# family's own rows pinned against the family authority docs_u23 as the U07
# sibling pins its docs_u07).
#
# WHAT THIS OWNS
#   1. THE BROWSER UA LAW (CF 1010). services.leadconnectorhq.com and
#      backend.leadconnectorhq.com are Cloudflare-fronted and 403s urllib's
#      default "Python-urllib/x.y" User-Agent at the WAF edge (CF error
#      1010) BEFORE the request ever reaches Convert and Flow (W0.6 / GK-09:
#      the proven failure mode, and the proven-live fix string ported
#      byte-for-byte from the Podcast gate). CAF_BROWSER_UA below is PORTED
#      BYTE-FOR-BYTE from anthology_registry.CAF_BROWSER_UA (the house
#      pattern every adapter rides through reg.CafClient /
#      reg.InternalRailClient / reg._internal_request_headers) so this
#      module's own copy cannot drift — and the offline self-test pins the
#      two strings byte-equal. The UA is NOT a secret: it is a public,
#      per-request header string carried on every request, exactly as the
#      registry sends it. The rule it enforces is absolute: ANY module in
#      this package that talks to GoHighLevel / Convert and Flow MUST send a
#      browser User-Agent on EVERY request — the U23 live surfaces
#      (phone_lister / provision_action / sms_verifier and the
#      provision_sms_phone.py provisioner) ride the proven public rail
#      (GET /phones/numbers, POST /phones/numbers, POST
#      /conversations/messages/outbound) and depend on this law.
...
#      set AND SAME order — and REFUSES any deviation (exit 4, the
#      AF-AE-HASH-PIN family in the manifest's own audit language: a drifted
#      enforcement set is a tamper). The engine's AF-AE-* families and the
#      U03 AF-AE-TEMPLATE-* family all ride this one table.
...
#      manifest-mirror table; a family row is NEVER inserted into the
#      manifest-mirror table while its manifest status is PENDING. Every
#      real family row is pinned per row against the manifest: EITHER
...
# code set that no longer matches the manifest, a family subset that no
# longer matches docs_u23, a non-AF-shaped code, a row whose manifest
# status no longer matches its declaration, or an empty table is a REFUSAL
# (HouseRulesError / exit 4) — never a silent pass, never a fabricated
# success.
#
...
# other u23_modules: sys.path.insert to scripts/ then
# `import anthology_registry as reg` for its canonical constants, and the
# family authority docs_u23 is imported as the packaged sibling exactly as
# the U07 sibling imports docs_u07 (a missing authority is a loud import
# failure, never a silent fallback — a family law that cannot be verified
# is never a blind skip).
...
# U23 WRITE ACTION (provisioning a phone number / sending an SMS) requires
# --execute (Trevor-gated, per the u23 package-init doctrine and
# provision_sms_phone.py's WRITE_ACTION law) — this constants module carries
...
# The U23 family authority — docs_u23.AF_CODES, imported as the packaged
# sibling exactly as the U07 family imports docs_u07. A missing authority is
# a loud import failure, never a silent fallback.
_U23_DIR = Path(__file__).resolve().parent
if str(_U23_DIR) not in sys.path:
    sys.path.insert(0, str(_U23_DIR))
import docs_u23 as _u23docs  # noqa: E402
...
# Cloudflare edge fronting services.leadconnectorhq.com / the internal rail).
...
# row the manifest has since stamped, an enforcement row that leaked into
# the manifest, or a family row in the manifest-mirror table — is a tamper,
# never silent (exit 4).
# ---------------------------------------------------------------------------
# The family subset is DERIVED from docs_u23.AF_CODES at import — a family
# code is never hand-typed in this module, so it cannot drift from the
# authority. The defensive filter drops a malformed row from the derived
# subset; the strict per-row gate (_u23_family_codes, defined below) and the
# self-test catch any such row — never a silent pass.
AF_U23_CODES = tuple(
    c[0] for c in _u23docs.af_codes()
    if isinstance(c, tuple) and c and isinstance(c[0], str) and c[0])
AF_U23_CODES_SET = frozenset(AF_U23_CODES)

# The U23 family rows PENDING in ENGINE-MANIFEST.json — the U06 / U07
# sibling pattern: a family row the family authority declares but the
# manifest has not yet stamped. As of 2026-08-11 every operational U23
# family row is PENDING (the manifest's autofails carry no U23 row yet).
# The self-test refuses a declared-PENDING row that the manifest has since
# stamped (a stale mirror is a tamper, never silent) and refuses a real row
# that is NEITHER stamped NOR declared PENDING NOR an enforcement row
# (drift). Pinned by the self-test against both authorities — a literal
# here can never drift.
AF_U23_PENDING = (
    "AF-AE-PROVPHONE-NO-EXECUTE",
    "AF-AE-PROVPHONE-READ-REFUSED",
    "AF-AE-PROVPHONE-CREATE-REFUSED",
    "AF-AE-PROVPHONE-VERIFY-REFUSED",
    "AF-AE-PROVPHONE-VERIFY-STALLED",
    "AF-AE-PROVPHONE-READBACK-MISMATCH",
    "AF-AE-PROVACTION-NO-EXECUTE",
    "AF-AE-PROVACTION-READ-REFUSED",
    "AF-AE-PROVACTION-CREATE-REFUSED",
    "AF-AE-PROVACTION-READBACK-MISMATCH",
    "AF-AE-PHONELIST-NO-EXECUTE",
    "AF-AE-PHONELIST-READ-REFUSED",
    "AF-AE-PHONELIST-CREATE-REFUSED",
    "AF-AE-SMSVER-NO-EXECUTE",
    "AF-AE-SMSVER-SEND-REFUSED",
    "AF-AE-SMSVER-NO-SID",
)
...
def _u23_family_codes() -> list:
    """The U23 family's OWN autofail rows from docs_u23.AF_CODES (the
    family's autofail authority, exactly as the U07 sibling mirrors its
    docs_u07): every (code, exit, meaning) row, in the authority's exact
    order. Fail-closed: an authority whose rows lost their code field, or
    an authority with no row at all, is a REFUSAL (HouseRulesError) — a
    family law that cannot be verified is never a blind skip."""
    rows = _u23docs.af_codes()
    codes = []
    for row in rows:
        code = row[0] if isinstance(row, tuple) and row else None
        if not isinstance(code, str) or not code:
            raise HouseRulesError(
                "docs_u23.AF_CODES carries a row with no code string -- the "
                "family AF law is unverifiable")
        codes.append(code)
    if not codes:
        raise HouseRulesError(
            "docs_u23.AF_CODES carries no autofail row -- the family AF "
            "law is unverifiable")
    return codes
...
def plan(*, out=None) -> int:
    ...
    sort_keys): the four laws with their sources and byte counts."""
    out = out or sys.stdout
    _check_laws(out=out)
    payload = {
        "contract": "anthology-engine-house-rules",
        "family": "U23",
        "u23_family_codes": list(AF_U23_CODES),
        "laws": {
            "browser_ua": {
            ...
            "u23_family": {
                ...
                "count": len(AF_U23_CODES),
                "codes": list(AF_U23_CODES),
                "pending": list(AF_U23_PENDING),
                "enforcement": list(AF_U23_ENFORCEMENT_CODES),
                "source": "docs_u23.AF_CODES (22 rows; the U23 family "
                          "authority -- the 16 operational rows declared "
                          "PENDING in the manifest as of 2026-08-11)",
            },
        },
    }
...
def self_test(*, out=None) -> int:
    ...
    byte-exact against ENGINE-MANIFEST.json's autofails, pins the family
    subset byte-exact against docs_u23.AF_CODES (SAME set AND SAME order),
    pins every family row's manifest status per row (stamped vs PENDING vs
    enforcement), and proves every attack fixture is REFUSED. A tamper NEVER
    masquerades as exit 1 -- it is exit 4 (AF-AE-HASH-PIN family: a drifted
    enforcement set)."""
    out = out or sys.stdout
    failures = []
    ...
    # ---- the golden U23 family rows: docs_u23 mirror, byte-exact ----------
    try:
        u23_codes = _u23_family_codes()
    except HouseRulesError as exc:
        failures.append("docs_u23 read: %s" % exc)
        u23_codes = None
    if u23_codes is not None:
        if sorted(AF_U23_CODES) != sorted(u23_codes):
            failures.append(
                "the family subset AF_U23_CODES drifted from "
                "docs_u23.AF_CODES: missing=%s extra=%s"
                % (sorted(set(u23_codes) - set(AF_U23_CODES)),
                   sorted(set(AF_U23_CODES) - set(u23_codes))))
        if list(AF_U23_CODES) != list(u23_codes):
            failures.append(
                "the family subset AF_U23_CODES order drifted from "
                "docs_u23.AF_CODES")
        if set(AF_U23_CODES) & {AE_DEPS_MISSING}:
            failures.append(
                "the family subset AF_U23_CODES absorbed a manifest-only "
                "row")
        if any("*" in code for code in AF_U23_CODES):
            failures.append(
                "the family subset carries a wildcard-shaped row -- "
                "docs_u23.AF_CODES declares real rows only")
        # Per-row membership: every pending row and every enforcement row
        # must exist in the family subset; a row may never be both.
        for pending in AF_U23_PENDING:
            if pending not in AF_U23_CODES:
                failures.append(
                    "the declared-PENDING row %r is missing from "
                    "AF_U23_CODES" % pending)
            if pending in AF_U23_ENFORCEMENT_CODES:
                failures.append(
                    "the row %r is declared both PENDING and an "
                    "enforcement code" % pending)
        for enf in AF_U23_ENFORCEMENT_CODES:
            if enf not in AF_U23_CODES:
                failures.append(
                    "the enforcement row %r is missing from AF_U23_CODES"
                    % enf)
            if enf in AF_U23_PENDING:
                failures.append(
                    "the enforcement row %r must never be declared PENDING"
                    % enf)
        # Per-row manifest status: every family row must be EITHER stamped
        # in the manifest OR declared PENDING OR an enforcement code. A row
        # that is none of the three drifted from its authority.
        if manifest_codes is not None:
            for code in AF_U23_CODES:
                if (code not in AF_U23_PENDING
                        and code not in AF_U23_ENFORCEMENT_CODES
                        and code not in manifest_codes):
                    failures.append(
                        "the U23 family row %r is neither declared PENDING "
                        "nor an enforcement code nor stamped in "
                        "ENGINE-MANIFEST.json -- the row drifted from its "
                        "authority" % code)
    if manifest_codes is not None:
        # A PENDING row the manifest has since stamped is a stale mirror.
        # An enforcement row in the manifest is a leak. Both are tamper,
        # never silent.
        for code in AF_U23_CODES:
            if code in manifest_codes:
                if code in AF_U23_PENDING:
                    failures.append(
                        "the U23 family row %s was stamped in "
                        "ENGINE-MANIFEST.json but is still declared PENDING "
                        "in AF_U23_PENDING -- a stale mirror is a tamper, "
                        "never silent" % code)
                if code in AF_U23_ENFORCEMENT_CODES:
                    failures.append(
                        "the enforcement row %s leaked into "
                        "ENGINE-MANIFEST.json's autofails -- enforcement "
                        "rows are the family's own self-test language, "
                        "never manifest rows" % code)
    ...
    for name, value in (("AF_CODES", AF_CODES),
                        ...
                        ("AF_U23_CODES", AF_U23_CODES),
                        ("AF_U23_CODES_SET", AF_U23_CODES_SET),
                        ("AF_U23_PENDING", AF_U23_PENDING),
                        ("AF_U23_ENFORCEMENT_CODES",
                         AF_U23_ENFORCEMENT_CODES),
    ...
    # Family shape: every family row must pass the house AF shape gate.
    for code in AF_U23_CODES:
        if not _af_shape_ok(code):
            failures.append("attack not refused: family row %r fails the "
                            "house AF shape gate" % code)
    # The family subset must never carry a foreign, lookalike, wildcard, or
    # shared-manifest row.
    for foreign in ("AF-AE-ANTHROPIC", "AF-AE-HASH-PIN",
                    "AF-AE-READBACK-MISMATCH", "AF-AE-PROVPHONE",
                    "AF-AE-PHONELIST", "AF-AE-PROVACTION", "AF-AE-SMSVER",
                    "AF-AE-PROVPHONE-*", "AF-AE-PROVPHONE-NO-EXECUTE-EXTRA",
                    "AF-AE-READBACK-MISMATCH-EXTRA", "AE_DEPS_MISSING"):
        if foreign in AF_U23_CODES:
            failures.append("attack not refused: foreign row %r polluted "
                            "the U23 family subset" % foreign)
    # The manifest-mirror table must never carry a family row whose
    # manifest status is PENDING or enforcement (the shared
    # AF-AE-READBACK-MISMATCH row legitimately lives in the mirror table,
    # which mirrors the manifest byte-exact -- the membership discipline is
    # enforced per row above, against the manifest itself).
    try:
