...
def test_field_map_all_free_text_is_large_text():
    ...
    assert len(inv) == 23, "expected 23 provisioning keys, got %d" % len(inv)
    assert fm["provisioning"]["total_keys"] == 23
    for row in inv:
        assert row["data_type"] == "LARGE_TEXT", \
            "%s must be LARGE_TEXT (G11), got %s" % (row["intended_key"], row["data_type"])
    # SINGLE_OPTIONS fields (the review decision + cover choice) are intentionally
    # NOT free-text link fields and must NOT appear in this map.
    keys = {row["intended_key"] for row in inv}
    for forbidden in ("contact.anthology_review_decision", "contact.anthology_cover_choice"):
        assert forbidden not in keys, "%s is SINGLE_OPTIONS and must not be in field-map" % forbidden
...
def test_field_map_inventory_matches_deliverable_contract():
    ...
    assert contract == inv_keys, "provisioning inventory drifted from deliverable/control keys"
...
def test_fresh_location_provisions_large_text():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert len(fake.fields) == 23, "fresh location should create 23 fields, got %d" % len(fake.fields)
        types = {f["dataType"] for f in fake.fields.values()}
        assert types == {"LARGE_TEXT"}, "fresh-location create must be LARGE_TEXT, got %s" % types
        # the map stamped every row LARGE_TEXT
