...
contract and value-free doctrine -- per Skill 60 SKILL.md's "Reuse before rebuild"
table. This guard is the static half of Skill 60's own doctrine point 7: "NOTHING
...
model-id / SDK / endpoint VALUES ship in any Skill 60 runtime file (the sentinel, the
ledger, the alert router, the baseline manager, the config catalogs), and it is built
to fold every future Skill 60 surface into a later full pass via extra path arguments
...
Exit codes (Skill 60 merge-gate; house convention for the edge cases, mirrored
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Static Anthropic-identifier gate over the shipped ZHC Early Warning "
                    "System (Skill 60) file set, extensible to future scan targets via "
