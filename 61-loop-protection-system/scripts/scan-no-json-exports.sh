#!/usr/bin/env bash
# 61-loop-protection-system/scripts/scan-no-json-exports.sh
# ----------------------------------------------------------------------------
# MERGE-GATE STATIC SCAN #2 of 4 for Loop Protection System (Skill 61),
# adapted from Skill 59's four-scanner merge-gate family per Skill 61 SKILL.md's
# "Reuse before rebuild" table (same static-scan mechanics, same 0/1/2/3/4 exit
# contract, same value-free doctrine). Skill 61 ships CONFIG json (thresholds,
...
# EXIT CODES (Skill 61 merge-gate; guard family, mirrored from Skill 59's
...
# workflow asset. Skill 61 ships no such asset today (its config/ dir holds only
...
# sanctioned Skill 61 n8n asset at this exact location would not false-positive.
