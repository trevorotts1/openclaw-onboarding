...
# ---------------------------------------------------------------------------
class _PersistentContextLauncher:
    """D6-safe browser stand-in for the category checks below.

    The fleet no-visible-window doctrine (guard-agent-browser-managed section 5,
    the 2026-06-23 orphan-descriptor incident) forbids a bare Playwright
    ``launch()`` anywhere under a skill directory: every browser must come up
    HEADLESS via ``launch_persistent_context(..., headless=True)`` (the same
    canonical pattern 06-ghl-install-pages/tools/ghl_iframe_drag.py uses). This
    adapter presents the exact ``new_context(**opts)`` surface the five
    ``_check_*`` helpers already call, but each call opens its OWN isolated,
    headless persistent context under an ephemeral per-check profile directory —
    behaviour-identical to the previous one-browser/five-contexts flow (every
    check was already fully isolated in its own context), with no bare
    ``launch()`` token. ``close()`` tears down any context a helper left open on
    an early raise, preserving the original ``finally: browser.close()`` safety
    net.
    """

    def __init__(self, playwright: Any, profile_root: Path) -> None:
        self._pw = playwright
        self._root = Path(profile_root)
        self._contexts: List[Any] = []
        self._n = 0

    def new_context(self, **opts: Any) -> Any:
        self._n += 1
        udd = self._root / f"profile-{self._n}"
        udd.mkdir(parents=True, exist_ok=True)
        ctx = self._pw.chromium.launch_persistent_context(str(udd), headless=True, **opts)
...
def evaluate(run_dir: Path, *, port: Optional[int] = None, server_timeout: float = SERVER_READY_TIMEOUT_SECONDS) -> Tuple[bool, str]:
    ...
    try:
        with sync_playwright() as p:
            browser = _PersistentContextLauncher(p, run_dir / "browser-qc-profiles")
            try:
