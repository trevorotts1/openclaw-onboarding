#!/usr/bin/env bash
# wire.sh — Skill 63 (Agnes Image): PERFORM the core-file updates.
#
# WHY THIS EXISTS (the defect it replaces)
#   Skill 63 shipped no installer, so its CORE_UPDATES.md was consumed by the
...
#   `63-agnes-image/agnes-image-full.md`, a ROOTLESS relative path that resolves
#   against whatever directory the agent happens to be in, so the reference could
#   never be opened reliably.
#
...
#   sentinel `<!-- skill:63-agnes-image:core-update-applied -->` so the generic
...
#   only ever touches its OWN `skill:63-agnes-image:*` markers, and it only ever ADDS
...
SKILL_SLUG="63-agnes-image"
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
REFERENCE_REL="agnes-image-full.md"
...
if [ -f "$SKILL_DIR/$REFERENCE_REL" ] && [ "$SKILL_DIR" != "$SKILL_DEST" ]; then
  ...
  if [ ! -f "$REF_DEST" ] || [ "$SKILL_DIR/$REFERENCE_REL" -nt "$REF_DEST" ]; then
    cp "$SKILL_DIR/$REFERENCE_REL" "$REF_DEST" 2>/dev/null \
      && echo "[skill 63] installed reference -> $REF_DEST"
  fi
fi

AGENTS_BODY="## Agnes Image 2.1 Flash
- Synchronous text-to-image + image-to-image on Agnes AI. Key: AGNES_AI_API_KEY (existing fleet credential; referenced, NEVER printed).
...
write_block() { # <file> <target-key> <body>
  ...
  [ -f "$file" ] || touch "$file" 2>/dev/null || { echo "[skill 63] cannot create $file — skipping"; return 0; }
...
PY
  ...
  if cmp -s "$file" "$tmp"; then
    rm -f "$tmp"
    echo "[skill 63] $(basename "$file"): block '$target' already current — no change, no backup"
    return 0
  fi
  cp -p "$file" "$file.bak-skill63-$(date -u +%Y%m%dT%H%M%SZ)"
  cat "$tmp" > "$file"
  rm -f "$tmp"
  CHANGED=1
  echo "[skill 63] $(basename "$file"): wrote block '$target' (replace-in-place)"
}
...
if ! grep -qF "$SENTINEL" "$WS/AGENTS.md" 2>/dev/null; then
  ...
  echo "[skill 63] stamped $SENTINEL"
fi

echo "[skill 63] core-file wiring complete (workspace: $WS; reference: $REF_DEST; changed=$CHANGED)"
exit 0
