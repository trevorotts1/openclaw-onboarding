#!/usr/bin/env bash
# wire.sh — Skill 67 (KIE Video): PERFORM the core-file updates.
#
...
#   `<!-- skill:67-kie-video:core-update-applied -->` so the generic merger
...
#   only ever touches its OWN `skill:67-kie-video:*` markers, and it only ever ADDS
  ...
  if [ -f "$SKILL_DIR/models.json" ]; then
    if [ ! -f "$SKILL_DEST/models.json" ] || [ "$SKILL_DIR/models.json" -nt "$SKILL_DEST/models.json" ]; then
      cp "$SKILL_DIR/models.json" "$SKILL_DEST/models.json" 2>/dev/null \
        && echo "[skill 67] installed registry -> $SKILL_DEST/models.json"
    fi
  fi
    ...
    for f in "$REFERENCES_SRC"/*; do
      ...
      if [ ! -f "$SKILL_DEST/references/$base" ] || [ "$f" -nt "$SKILL_DEST/references/$base" ]; then
        cp "$f" "$SKILL_DEST/references/$base" 2>/dev/null \
          && echo "[skill 67] installed reference -> $SKILL_DEST/references/$base"
      fi
    done
  fi
fi
...
TOOLS_BODY="## KIE Video API (Skill 67)
...
MEMORY_BODY="## KIE Video (67) — installed
...
write_block() { # <file> <target-key> <body>
  ...
  [ -f "$file" ] || touch "$file" 2>/dev/null || { echo "[skill 67] cannot create $file — skipping"; return 0; }
...
PY
  ...
  if cmp -s "$file" "$tmp"; then
    rm -f "$tmp"
    echo "[skill 67] $(basename "$file"): block '$target' already current — no change, no backup"
    return 0
  fi
  cp -p "$file" "$file.bak-skill67-$(date -u +%Y%m%dT%H%M%SZ)"
  cat "$tmp" > "$file"
  rm -f "$tmp"
  CHANGED=1
  echo "[skill 67] $(basename "$file"): wrote block '$target' (replace-in-place)"
}
...
if ! grep -qF "$SENTINEL" "$WS/AGENTS.md" 2>/dev/null; then
  ...
  echo "[skill 67] stamped $SENTINEL"
fi

echo "[skill 67] core-file wiring complete (workspace: $WS; reference: $REF_DEST; changed=$CHANGED)"
exit 0
