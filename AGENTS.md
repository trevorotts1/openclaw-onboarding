...
| N16 | **Persona governance on EVERY non-mechanical task.** Every dispatch that isn't a pure mechanical operation (file copy, version check) runs `persona-selector-v2.py` first; the resolver consumes the pinned persona via Hop 10 (`task_pinned` → `sticky_assignment` → `agent_settings` cascade). **PERSONA-INVENTORY HARD RULE:** any "what / how many / which / new personas do I have?" question is answered ONLY from the SET file `persona-categories.json` (the matcher's ground truth). Inventory answers from a conversational cache (which can be frozen at a stale snapshot) produced the "no new personas since March" false answer in June 2026 — the SET is the canonical source, period. | `direct-to-agent-install.md` Hard Rules + this file + `22-…/INSTRUCTIONS.md` | `intelligence-resolver.ts` (dashboard) |
...
| N29 | **Shared core files (Zero-Human-Workforce file model).** On every box, ALL of that account's agents + sub-agents SHARE the box's ONE canonical `AGENTS.md` / `TOOLS.md` / `USER.md` via a **verified real-file copy** (not a symlink, not hand-duplicated) — re-copied on every `install.sh` / `update-skills.sh` run, so drift is bounded to one roll. Per-agent `IDENTITY.md` / `SOUL.md` / `MEMORY.md` / `HEARTBEAT.md` stay each agent's OWN real files. The copy SOURCE is ALWAYS the LOCAL box's own canonical (the default agent workspace resolved from THIS box's `openclaw.json`) — NEVER a hardcoded or cross-box/cross-account path (co-mingling guard, N0). Nested workflow agents (`*/workflows/*/agents/*`) are EXEMPT. Real files are backed up (`*.bak-unify-<ts>`) + unique content preserved additively into the agent's own `IDENTITY.md` before copying. **Amended 2026-09-21:** backups are BOUNDED — only the `$UNIFY_BAK_KEEP` newest (default 3) are kept per target, oldest deleted first. They were unbounded: one full-size copy per target per roll, forever (measured live: 25,604 `AGENTS.md.bak-unify-*` files / 4.3 GB on one box, written daily since 2026-06-23, disk at 95%). Only a target's OWN `.bak-unify-<ts>` siblings are ever pruned. Idempotent. **Amended 2026-07-31:** the runtime's workspace-root boundary guard rejects any symlink whose realpath resolves outside the agent's own workspace, reporting the file missing and injecting a ~107-char stub with no error anywhere — do not restore symlinks. | This file (Shared Core Files section) + [`docs/SHARED-CORE-FILES.md`](docs/SHARED-CORE-FILES.md) | `link_shared_core_files()` in `install.sh` (Step 10a) + `update-skills.sh`; QC check 9.9 in `scripts/qc-system-integrity.sh` — real file, byte-identical to canonical (a lingering symlink is a FAIL) |
...
2. **TIER 2 — OpenRouter open-source** (open-weight vendors: DeepSeek, Moonshot/Kimi, Qwen, Z-AI/GLM, Xiaomi, Llama, Mistral, Gemma, MiniMax M3 — **MiniMax M2 is BANNED everywhere; the "MiniMax" allowance here means M3 only**; NOT proprietary OpenAI/Anthropic/Gemini-pro routes)
3. **TIER 3 — OpenRouter `google/gemini-3.5-flash`** — explicit universal last-resort safety rung; fires only when Tier 1 and Tier 2 both fail and credits are live. This supersedes the former `*:free` wording — gemini-3.5-flash is the named model at this rung, not an anonymous free slot. The `openrouter/free` literal and any bare "free" default remain REJECTED by the gate.
...
## GHL Platform Identity + Credential Doctrine

<!-- GHL_IDENTITY_V1 -->
**GHL = Convert & Flow = Go High Level — one platform, many names, one credential.**

- The "API key" in GHL Settings > Integrations > Private Integrations IS the `pit-` Private Integration Token (PIT). There is no separate API key; the PIT is the API key.
- `app.convertandflow.com`, `app.gohighlevel.com`, and `leadconnectorhq.com` (`services.leadconnectorhq.com` for the API) are the same platform. White-label domain does not change the backend.
- The LOCATION PIT resolves from 11 canonical alias names (see `TERMINOLOGY.md` for the full set). `GOHIGHLEVEL_API_KEY` is the preferred name. Every resolver in every skill must scan all 11; never read only one alias.
- AGENCY PITs (`GOHIGHLEVEL_AGENCY_PIT`, etc.) are a SEPARATE credential — they 401 on media and location-scoped calls. Never merge them into the Location PIT set.
- The Firebase refresh token (`GOHIGHLEVEL_FIREBASE_REFRESH_TOKEN`) is a THIRD, separate credential for browser-session auth in Skill 06/44 — it is NOT part of the PIT alias set and must never be confused with the PIT.
