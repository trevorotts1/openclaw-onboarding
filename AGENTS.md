...
| N16 | **Persona governance on EVERY non-mechanical task.** Every dispatch that isn't a pure mechanical operation (file copy, version check) runs `persona-selector-v2.py` first; the resolver consumes the pinned persona via Hop 10 (`task_pinned` → `sticky_assignment` → `agent_settings` cascade). **PERSONA-INVENTORY HARD RULE:** any "what / how many / which / new personas do I have?" question is answered ONLY from the SET file `persona-categories.json` (the matcher's ground truth) — **NEVER from `qmd`/the `coaching-personas` collection** (a conversational cache that can be frozen at a stale snapshot; it produced the "no new personas since March" false answer in June 2026). The pipeline keeps qmd repointed at the canonical store (`reconcile_qmd_persona_index`), but the SET is canonical for counts. | `direct-to-agent-install.md` Hard Rules + this file + `22-…/INSTRUCTIONS.md` | `intelligence-resolver.ts` (dashboard) |
