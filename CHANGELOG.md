...
**Standalone cluster SOP library added under `presentations/sops/` (19 documents).** Cross-role doctrine, each with purpose, the hard rule, the enforcement check, pass/fail examples drawn from the actual Corey and Lyric decks, and an escalation path:
...
- Pitch-Craft (4, authored net-new from lyric-principles.md + transcript-spec.md): SOP-PITCH-01 slow-drop process (anchor early, spread beats, earned reason + added value per drop, late final), SOP-PITCH-02 value-stack and promises (itemized stack summed past the anchor before the cheapest price; promises before the first number), SOP-PITCH-03 re-pitch (mandatory 4-to-7 slide post-price recap), SOP-PITCH-04 Wall of Wins (real-client homage about 5 slides before the offer, never a child future-pace).
