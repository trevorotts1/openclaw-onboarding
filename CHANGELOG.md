## [v16.1.10]  -  2026-06-29  -  feat: PRESENTATION DEPARTMENT INTAKE now offers the quick-vs-in-depth CHOICE first, then asks ONE question at a time. The entry agent used to free-form a six-question batch (Topic + Audience + Tone + Deadline + Existing assets + Content, "give me whatever you have got and I will get moving") because the choice-first + one-at-a-time discipline lived only inside two buried sub-specialist files. It is now promoted into the UNIVERSAL doctrine root and every first-contact artifact, reordered so the CHOICE comes FIRST, and locked with a named auto-fail + CI guard. INTAKE conversation only -- nothing touches the image-prompt floor (9000-18000 chars), build_deck.py, run_signature_deck.py, or the build pipeline.

### Risk: low -- additive doctrine + first-contact wording + a new INTAKE-scoped auto-fail and CI test. No build phase, gate, renderer, or prompt-floor is changed.

### What changed (choice FIRST, then one question per message)

- **PRIMARY doctrine root** `universal-sops/CLIENT-WEBINAR-DECK-SOP.md`: new binding section **0.5 CLIENT INTAKE CONVERSATION CONTRACT** (read before section 1). On any net-new deck request the FIRST response is a one-line warm acknowledgment + the quick-vs-in-depth CHOICE (canonical wording), then strictly ONE intake question per message (reflect each answer back, then the next). Names **AF-INTAKE-BATCH** and bans the exact screenshot batch anti-pattern verbatim. Sections 1-9 unchanged.
- **Canonical choice prompt** (em-dash-free, identical on every box): "Love this. Let's shape it together, and you set the pace. I can do this two ways: the QUICK way, where I ask you about 5 key questions, one at a time, and we lock a solid plan fast; or the IN-DEPTH way, where we go a little deeper, still one question at a time, so the deck is tailored exactly to your audience and offer. Which would you like, quick or in-depth? You can switch or stop whenever you want."
- **Brainstorming Buddy (ROLE-17)** `brainstorming-buddy-presentations.md` v2.1 -> v2.2: the warm acknowledgment + mode offer is now the FIRST move; SOP 9.0's six mandatory no-guess fields (representation mix + grounded content can NEVER be defaulted) are folded INTO the chosen path one at a time, never an upfront batch. `intake_state.json` (mode_offered / mode_chosen / awaiting_answer) gates cadence by state, not by the model remembering. The sops mirror is regenerated to include SOP 9.0 (was missing).
- **Cross-department generators**: the choice-first + one-at-a-time ordering goes into `_brainstorming-buddy-template.md` (regenerates the same experience for video, graphics, marketing, web, app) and the wording into `how-to-use-this-department.template.md`; `generate_how_to_use_docs.py` regenerated all 32 committed how-to-use guides.
- **First-contact artifacts** aligned: the welcome script message (`send-presentation-dept-welcome.sh`), the entry persona Slate (`IDENTITY.md`/`SOUL.md`, a FIRST-CONTACT INTAKE rule scoping "BUILD, never route" to dispatched tasks), the Director front-door (`director-of-presentations.md` + sops mirror: dispatch ROLE-22/ROLE-17, never improvise a batch), `00-START-HERE.md`, ROLE-22 first-time onboarding, and the data-driven question sets (`_brainstorming-buddy-question-banks.json`).
- **AF-INTAKE-BATCH** registered in `presentation-slide-craft/MASTER-QC-AUTOFAIL-RULESET.md` Section 5 (+ the sops mirror) and `PIPELINE-MANIFEST.json` (manifest_version 19 -> 20, `enforced_by: qc_check`, stage INTAKE, py_symbol null). It is scoped to the INTAKE conversation only: sync_check.py / gate_integrity_check.py treat it as out of build_deck scope, so it never gates the build. The QC/Healer intake-trace scan trips it when a single assistant turn emits two or more intake questions before an owner answer.

### TESTS / CI GUARD

New `tests/unit/presentation-intake-conversation.test.sh` + `.github/workflows/presentation-intake-conversation-guard.yml` assert, across every conversation-governing artifact, that the choice is present (quick AND in-depth, plus a real choice question on the canonical-prompt artifacts), the one-at-a-time rule is present, AF-INTAKE-BATCH is named, and no OWNER-FACING message regresses to a batch (the screenshot phrase and the old "ask a few quick questions" framing are banned). `test-presentation-dept-welcome.sh` TEST 5 extended (now wired into CI) to assert the choice + one-at-a-time wording in the resolved welcome message. Proven to FAIL on a regressed welcome (7 assertions trip). Presentations lockstep (sync_check / test_preflight / gate_integrity / doctrine_residual), the content manifest, and the version-consistency 10-marker set all stay green.
