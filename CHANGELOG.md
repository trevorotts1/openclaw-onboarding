## [v16.1.11]  -  2026-06-29  -  feat(presentations): PROCESS-INTEGRITY — make every guardrail MANDATORY, model-appropriate, and INDEPENDENTLY VERIFIED (one door → declare plan → validate+report each step → no-skip proof → always on the board). Closes the fleet-wide failure modes a flagship-deck run surfaced (a 2h41m run that produced nothing): voluntary/opt-in enforcement, a weak self-grading QC model, and gameable self-attested verification.

### Risk: medium — additive + fail-closed where it counts, fail-soft where it must. New runner methods + a manifest bump (v20 → v21) + new scripts; the canonical render path inside build_deck.py is UNTOUCHED. No existing gate weakened; the 9,000-char prompt floor is untouched. The front-door interceptor ships in the documented FALLBACK mode (see FIX B).

### What changed (by fix)

- **FIX A — model tiering (SAFE FLOOR, repo-only).** `shared-utils/dept-model-suitability.json`: added `quality-control` at tier **heavy** (it was absent → fell to the mid/text default, a tier BELOW the producer it grades). `build-workforce.py`: new `enforce_producer_qc_distinctness()` post-pass guarantees the QC dept agent resolves a DIFFERENT heavy model from the Presentations producer (from the box's OWN heavy inventory; never a hardcoded pin, never Anthropic), or — when only one heavy model exists — keeps it with a recorded reasoning-configuration-independence flag. Forced-high-thinking is realized via heavy-tier (thinking=high) model resolution + a safe dept-default metadata record, NOT a raw per-agent `thinking` key (that key is a documented `openclaw config validate` landmine). NO per-box sweep; NO client box changed.
- **FIX B — front-door deck-build interceptor (FALLBACK mode per OQ-1).** New `hooks/deck-build-guard.sh` denies raw `python3 working/*.py`, a direct `build_deck.py`/`run_signature_deck.py` call, an ad-hoc python-pptx assembler, a local Pillow slide canvas, and a direct kie createTask; only `presentation-canonical-entry.sh` is allowed; also denies a build while the intake ledger is not complete. **OQ-1 verified against the live repo/gateway:** OpenClaw exposes NO openclaw.json exec command-gate (`tools.exec` is security/ask only; `agents.defaults.tools.exec` is invalid; the per-agent AgentEntry.tools schema cannot pattern-match commands). The genuinely-supported pre-exec gate is the Claude-Code `hooks.PreToolUse` mechanism (the guard ships in that protocol + a standalone exec-wrapper), so NO unsupported `preExec` key is written to the agent config (that would break `config validate`). The always-on FALLBACK enforcement is `presentation-canonical-entry.sh` GATE 2 (bypass-scan) + the new GATE 0 (intake-complete). `TOOLS.md` now names the canonical entry as the one tool.
- **FIX C — end-of-process no-skip proof.** New `prove-deck.py` walks `declared_plan.json` + the attestation chain and FAILS (exit 9) on any skipped / out-of-order / empty-sha / substance-failed / unreported step; on a full pass it writes `PROCESS-CERTIFICATE.json/.md` + a `process_certificate_sha`. The only bypass for a step is a logged, VERIFIABLE owner_skip_approval (real timestamp + owner message id; self-authored / midnight-placeholder approvals are rejected — FM-4).
- **FIX D — intake turn-gate state machine.** New ordered `deck-intake-questions.json` (the quick-vs-in-depth CHOICE first, then the six mandatory fields in order, then scope, then a session budget) + `deck-intake-driver.py` (the DRIVER returns one question per turn — `--next`/`--answer`/`--budget`/`--confirm`/`--complete`; the agent cannot emit the next question). The client hears ONE seamless voice (no routing/relay chatter). Build is denied until `intake_ledger.json` is complete. Folded the universal "Client Intake Conversation" section into `CLIENT-WEBINAR-DECK-SOP.md`; the Brainstorming Buddy + Director roles reference the driver as authoritative.
- **FIX E — declared plan + per-step validate-and-report + report-gated, sha-mandatory attestation.** `run_signature_deck.py`: `declare_plan()` (writes `declared_plan.json` + one up-front client message), `emit_client_report(start|done)` via `openclaw message send` (NEVER the raw Telegram API; records `gateway_msg_id`), `attest_phase_verified()` (existence AND substance AND a non-empty verified `artifact_sha` AND a confirmed start+done report pair before any attest), and a watchdog registered at run-begin over all long_running phases (closes the deep-research silent gap). Manifest **v20 → v21**: `client_report` on every phase, `long_running`/`heartbeat_minutes` on the long phases, and 4 new autofails — `AF-PHASE-REPORT-START`, `AF-PHASE-REPORT-DONE`, `AF-PROCESS-INTEGRITY`, `AF-CC-UNREGISTERED`.
- **FIX F — real substance verifiers.** New `phase_verifiers.py` reuses the existing engine checkers (research-cited + claims + a minimum-evidence gate that rejects an UNVERIFIED fact-validation ledger — FM-5; intelligence/pitch arc-ORDER for copy — FM-2; prompt-QC; vision image-QC; deck harmony). The runner's attestation now requires existence AND substance.
- **FIX G — CC deliverable registration (producer side).** New fail-soft `cc_board.py` (mirror of the verified Skill-48 pattern: stdlib urllib, HMAC + Bearer, env-driven, NEVER raises): job-start `POST /api/tasks/ingest` + phase-boundary `PATCH`, records `task_id` in `process_manifest.json`. `AF-CC-UNREGISTERED` is fail-CLOSED on never-attempted, fail-SOFT on transport. CC API url+token entries added to `connection-manifest.json` (verify-wiring asserts them).
- **FIX H — interview-complete precondition (report-don't-build).** `32-command-center-setup/run-full-install.sh` gains an EARLY `interviewComplete` precondition (reports `commandCenterStatus=interview-pending`, exits 0 clean; `--update-only` exempt; complements — does not duplicate — the existing ZHE Phase-7z acceptance gate). `materialize-dept-agents.sh` independently refuses to scaffold pre-interview (exit 0, no mutation; `--dry-run` exempt). `resume-workforce-build.sh` upgrades its silent not-complete exits to ONE throttled (24h) report, cleared on completion. Self-documenting rule lines in both SKILL.md files + PREREQS.json. The [ALREADY SHIPPED] `build-workforce.py` exit-87 fail-close is NOT re-added.
- **FIX I — version pin + drift fixes + CI meta-gate.** Fixed the 5,000 → 9,000 char-floor drift in `presentation-canonical-entry.sh`; added `CANONICAL-RENDERER-PIN.sha256` (activates GATE 3 fleet-wide; CI recomputes + verifies it so a stale pin fails the build); removed the stale "Agent W3 / REGISTRATION PENDING" prose in `SOP-PRIORITY-02` (those gates are live); extended the CI meta-gate (Guard A) to `runner`-enforced codes (must be cited in `run_signature_deck.py`/`prove-deck.py`) and to any code declaring a `check_script` (file + symbol must exist).
- **ALSO — onboarding half of the dispatch-furnace fix.** Both seeders already build from the same Skill-23 `departments.json` + `canonical_slug` (the legacy→canonical alias map). New `provision-dept-runtime-dirs.sh` (idempotent, fail-soft) ensures a `<stateDir>/agents/dept-<slug>/agent` runtime dir per dept agent and delegates any NULL/forbidden-model agent to the existing `repair-model-sovereignty.sh` (box's own models, never Anthropic) so dispatch is never model-blocked.

### TESTS / CI GUARD

New `test_process_integrity.py` (run by presentations-lockstep CI): no-skip proof (empty-sha / skipped / out-of-order / self-authored-skip FAIL; clean chain + valid owner-skip PASS), one-question-per-turn intake selftest, mandatory verified artifact_sha, producer!=QC distinctness, fail-soft CC board (fail-closed never-attempted / fail-soft transport), substance verifiers (FM-5 unverified-ledger reject), single-source/no-duplicate department slugs. `sync_check.py` IN SYNC (manifest_version 21, 133 autofails, 35 roles); `test_preflight.py` ALL PASS; Guard A OK (extended to runner + check_script codes); Guard B CLEAN; renderer pin verified. No client names in changed files. Box user, not root.

### What is intentionally NOT changed

The render path inside `build_deck.py` is untouched. No per-box sweep and no client box was modified (FIX A is repo-only; the model-tier reconciliation sweep is a separate held per-box pass). The Command Center app changes (ingest resilience, the `process_certificate_sha` done-gate, the duplicate-task reaper, the `model_id`-population fix) are OTHER-REPO dependencies tracked in `blackceo-command-center`, not implemented here.
