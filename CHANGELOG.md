...
- **Made the documented count true:** the suite is **43 tests collected** (T-1..T-9 + T-N1..T-N6 + rollback idempotency + T-PRE-1/T-PRE-4 + T-N5 + graduation + the company_kpis schema-drift suite). The README and the CI workflow (`.github/workflows/full-funnel-pipeline.yml`) state 43, and CI now **asserts the collected count == 43** so the docs cannot drift. The CI rubric-scorecard-gate runs the harness with the REAL persona selector and also scores the committed live-run evidence with the same canonical scorer.
