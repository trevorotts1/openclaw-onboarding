## [v16.1.6]  -  2026-06-29  -  fix(skill-32): unify the Command Center pm2 app name so an install/update can never create two competing CC processes on :4000 (the fleet-wide port-4000 crash-loop + multi-hour gateway-outage root cause)

### What changed

The Skill-32 dashboard installer (`32-command-center-setup/scripts/run-full-install.sh`, Phase 6) and the Command Center repo used **different pm2 app names for the same Next.js board**, so an install/update on a box already running the board under the OTHER name could leave TWO pm2 apps bound to :4000, whose `next start` children then mutually killed each other through cc-start.sh's orphan-port killer — an endless restart loop (47k+ restarts) that wedged the gateway on a client box.

- **Root cause.** This installer historically only ever did `pm2 restart` / `pm2 delete` / `pm2 start --name blackceo-command-center`. But the CC repo's `ecosystem.config.cjs` (what `pm2 resurrect` restores at box boot), `scripts/watchdog-cc.sh` self-heal, both `scripts/install/*-bootstrap.sh` scripts, and `package.json` all name the app **`mission-control`**. On a box whose CC was already named `mission-control`, the installer's `pm2 restart blackceo-command-center` failed, fell through to a fresh `pm2 start blackceo-command-center`, and the box ended up with `mission-control` AND `blackceo-command-center` both on :4000.

- **Canonical name chosen: `mission-control`** (NOT the divergent `blackceo-command-center` this installer used). Rationale — least-disruptive + self-healing: every persistent/autonomous fleet path already converges on `mission-control` (ecosystem.config.cjs / resurrect, watchdog-cc.sh self-heal via `pm2 start ecosystem.config.cjs`, both bootstrap scripts, package.json), and the CC health analyzer `pm2-analyze-cc.py` is name-agnostic. The crashing box was itself running `mission-control`. Choosing `mission-control` is a single-file installer change; choosing `blackceo-command-center` would have required rewriting ~6 CC-repo files (ecosystem, package.json, both bootstrap scripts incl. their reconcile logic, two docs) and would have re-introduced divergence vs. the bootstrap scripts.

- **The fix — idempotent + reconciling Phase 6.** New helpers `cc_reconcile_pm2_names` / `cc_pm2_start_canonical` / `cc_apply_canonical_pm2`. On BOTH the full-install and `--update-only` paths the installer now (1) `pm2 delete`s every non-canonical alias (`blackceo-command-center`, `command-center`) AND any stale canonical app, then (2) starts exactly ONE canonical `mission-control` from the CC repo's `ecosystem.config.cjs` — so the install/update path uses the SAME single app definition (canonical name + circuit-breaker + the hardened cc-start.sh launcher) as every other tool, instead of a divergent `npm start` definition that also lacked the `min_uptime`/`max_restarts` circuit-breaker. Falls back to `npm start` under the canonical name only if `ecosystem.config.cjs` is absent from the checkout. `pm2 save` after, so the resurrect dump is clean. Phase 7 log hints updated to `pm2 logs mission-control`.

- **PROOF (before/after).** A deterministic simulation drives the REAL extracted Phase-6 helpers against a faithful mock `pm2`: the OLD logic reproduces the 2-app port-4000 fight on a `mission-control` box; the NEW logic converges EVERY box-state — `{mission-control}`, `{mission-control, blackceo-command-center}` (the dup), `{blackceo-command-center}`-only, `{command-center}`-only, and a clean box — to exactly one `mission-control` with no second app, and is idempotent across repeated runs.

- The CC-repo half ships separately (blackceo-command-center v4.55.3): documents `mission-control` as the fleet-canonical pm2 app name in `ecosystem.config.cjs`, and hardens `scripts/cc-start.sh`'s orphan-port killer to exclude its own pid/supervisor (defence-in-depth — with one canonical app there is no sibling CC to fight).

Existing Phase-6/7 contract preserved (`db:seed`, `phase=6c` board sync, `prove-zhe` gate all unchanged). `bash -n` clean. Box user, not root. No client names in changed files.
