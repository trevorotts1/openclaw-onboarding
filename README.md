...
> **Version:** see `/version` - this repo at v10.15.25.
...
>
> **NOTE (v10.15.25) — Skill 38 Round-2 backlog shipped (advertising rolled forward with it).** This release ships the Skill 38 Round-2 backlog and rolls the advertised version forward in the SAME merge, so there is no advertised-vs-shipped drift window. Live state: **Skill 38 (Conversational AI System) → v1.5.12** (its own `38-conversational-ai-system/skill-version.txt`, intentionally independent of the repo version line), **40 numbered skill folders (37 active + 3 archived: 13, 33, 34)**, with **Skill 39 — Real Estate Playbook & Property Intelligence** and **Skill 40 — ZHC Public Records Scraper** present in the catalog. **Round-2 adds 6 NEW advanced features, ALL default-OFF (opt-in; the installer never flips one ON): F21 Multi-Tenant Agent Isolation (the agency tier), F17 Customer Segmentation Awareness, F15 Proactive Outreach Campaigns, F16 A/B Testing of Reply Variants, F14 Voice / Phone Integration, F18 Webhook Chaining** — each with its own protocol, QC gate + negative test, PII-free log, and AGENTS.md/cron wiring. Per-release version history lives in [CHANGELOG.md](CHANGELOG.md). VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
...
**Current Version: v10.15.25** — See [CHANGELOG.md](CHANGELOG.md) for the full per-release history.

This repo is **Mac-only**. The Hostinger Docker VPS installer lives at https://github.com/trevorotts1/openclaw-onboarding-vps.

This repo contains **40 numbered skill folders (01 through 40)** — 37 active plus 3 archived (13, 33, 34) — plus an install script and update script. See the [Skill Inventory](#skill-inventory-folder-names) below for the full live list.
