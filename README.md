...
> **Version:** see `/version` - this repo at v10.15.14.
...
>
> **NOTE (v10.15.14):** **Round-3 cross-repo reconciliation + roadmap spec committed; Skill 38 → v1.5.3.** Canonical alignment of the Skill 38 surface between the Mac and VPS onboarding repos (protocols, install/QC scripts, INSTRUCTIONS, INSTALL, and the QC-static CI workflow), plus the conversational-AI strategic roadmap spec (`references/conversational-ai-strategic-roadmap.md`) committed as part of the canonical surface. `scripts/` self-counts re-verified 51 → 54 (`25-seed-round3-feature-files`, `qc-backend-ready`, `qc-feature-logs`). VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
> **NOTE (v10.15.14):** **Skill 38 v1.5.2 (Round-3 Queue-A) + two NEW universal verticals.** Skill 38 ships its Round-3 Queue-A feature pack: the ZHC tag-prefix rule, F50 two-tier aggression, F44 detour-and-return interrupts, F45 geo-qualification, F46 CRM field write/create, F47 inline smart-FAQ, and F49 ZHC Pixel (per-client visitor-signal pixel + Pixel Concierge); the three QC-enforced standards now ship and are machine-checked — the Communication-Playbook standard, the GHL Raw-Body-JSON standard, and the Notion Client-Doc standard. **NEW Skill 39 — Real Estate Playbook & Property Intelligence** and **NEW Skill 40 — ZHC Public Records Scraper** join the catalog as two new universal verticals. VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
> **NOTE (v10.15.14):** Skill 38 → v1.4.21. **Workflow-AI standardization + exhaustive Build-with-AI Custom Webhook:** the Build-with-AI prompt now specifies the Custom Webhook end-to-end (URL → headers/Bearer token → Raw Body JSON → Allow-Re-entry) so the workflow SHAPE is built consistently every time. 60-year-old-friendly verification rewrite: every step is plain-English with literal copy code blocks. New client self-test section + AI backend self-test (`38-conversational-ai-system/scripts/{12,24}-self-test-hook.sh`) so the client can prove the loop works without an engineer. Heavily-enforced Notion doc delivery (the client reference doc is mandatory and machine-checked). VPS-vs-Mac install considerations documented inline. GHL API quick-reference preloaded into the client's TOOLS.md: all channels send via one `/conversations/messages` endpoint, plus calendars/appointments/invoices and the required scopes. Channel-mirroring reply: mirror the inbound channel as the message `type`, send by `contactId` (`conversationId` is read-only). Full UNIVERSAL personal-data scrub across the repo + a new `qc-no-personal-data` gate that blocks any client/personal data from shipping. VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
> **NOTE (v10.15.14):** Skill 38 → v1.4.13. **v1.4.13 (v1.4.11 install-script bug fixes + MANDATORY manual Custom-Webhook fill):** PART A fixes 4 install-script bugs verified on a live 2026.5.27 box — the Model Wizard no longer writes the invalid `agents.defaults.async/.batch` keys (config-validate failure) and its hooks-merge `jq` no longer uses the jq-1.7-rejected `.hooks //= {};` form; `04-register-crons.sh` registers crons via `openclaw cron add` (the `cron.jobs` JSON block doesn't validate on 2026.5.27); `02`/`03` read the master-files pointer instead of `source`-ing a directory; `12` resolves the skill path dynamically (no hardcoded legacy path). New `qc-config-keys.sh` gate (+ CI step) catches any of these reappearing. PART B makes the manual Custom-Webhook fill MANDATORY and explicit: the generated client reference sheet now LEADS with the copy-paste values (URL → Bearer token → Raw Body JSON → manual fill steps → Workflow-AI prompt), and the workflow-AI/SMS/verification docs spell out that Build-with-AI builds only the workflow SHAPE and will NOT fill the Custom Webhook URL/headers/Raw Body — the client must paste them by hand and verify every field is non-empty before publishing (machine-enforced by `qc-reference-sheet.sh --require-manual-fill`).
>
> **NOTE (v10.15.14):** Skill 38 → v1.4.1. **v1.4.1 (Conversation Playbook Builder enhancement — the differentiator):** Step 9.20 reframed as an explicit 3-PART build (Workflow AI instruction set + manual-build fallback + verification checklist; the conversation playbook registered in `registry.md`; the friendly brainstorm trigger), plus MEMORY.md builder design rules 15-19 (Terminology, No-GHL-API, 3-PART Build, Brainstorm-Not-50-Questions, Mac Env) and Mac env handling that searches BOTH `~/clawd/secrets/.env` and `~/.openclaw/.env`. Content-only; no version files touched. **v1.4.0 (GHL Build-with-AI hardening + calendar-sync):** new authoritative reference `38-conversational-ai-system/references/GHL-INBOUND-AND-PLAYBOOKS.md` (4-token disambiguation, one-tunnel-many-hooks, copy-paste **Build-with-AI prompt** template + post-build verification checklist, Reusable Tunnel Values rule, JSON one-value-per-key rule, verified channel→type enum, Conversations + Calendar recipes, first-playbook = appointment booking). Playbook fixes: `deliver: false` on reply hooks (true breaks replies), `openclaw cron add` CLI (cron.jobs JSON doesn't validate on 2026.5.27), Build-with-AI primary / manual fallback, Mac interactive-sudo `cloudflared` blocker flagged, `GHL_PRIVATE_INTEGRATION_TOKEN` + `Version: 2021-04-15` standardized. New `38-conversational-ai-system/scripts/skill38-calendar-sync.sh` (weekly Sunday cron maintains the GHL calendar table in TOOLS.md). VPS and Mac sequences remain intentionally independent.
>
...
Skills (especially the Skill 22 book-to-persona pipeline and Skill 35 social-media-planner) had legacy documentation that defaulted heavy-reasoning calls to OpenRouter even when Ollama Cloud Kimi / DeepSeek-pro was available. Ollama Cloud is subscription-billed (cheap) while OpenRouter is per-token (expensive), so the wrong default was costing real money on every install.

The `shared-utils/select_model.py` selector was already correctly ordered Ollama-first. v10.1.0 fixes the stale skill docs (SKILL.md tables, INSTALL.md verify steps, QC.md checklists, agent prompts) to match. Clients with Ollama Cloud configured will now correctly route through it for heavy reasoning; OpenRouter only fires as fallback when Ollama Cloud is unavailable.
...
v9.7.7 added a `rotate_all_devices_to_full_scopes()` helper that, per the OpenClaw docs, was actually creating new pending scope upgrade requests. Those pending requests blocked the gateway from sending Telegram messages during install — exactly the failure mode the rotation was meant to prevent. Self-inflicted deadlock.

v10.0.1 removes the rotation entirely, plus the two scope-approval helpers that were trying to clean up after it. Existing clients all have `operator.write` on their paired device (that's what their daily Telegram requires), so `openclaw message send` just works directly. No rotation, no approval, no retries, no user action.

See CHANGELOG.md for the full root-cause analysis and removed-code list.
