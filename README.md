...
> **Version:** see `/version` - this repo at v10.15.34.
...
>
> **NOTE (v10.15.34) — BIG PROJECT MODE: universal AGENTS.md standard.** Ships `BIG-PROJECT-MODE.md` (the client-universal large-build standard: orchestrator pastes the shared document byte-identical atop every worker, warm-up then fan-out, skinny orchestrator, independent numeric QC) and extends `apply-fleet-standards.sh` to idempotently append a compact `## BIG PROJECT MODE` section to every agent's active-workspace `AGENTS.md` on install AND update. Per-release version history lives in [CHANGELOG.md](CHANGELOG.md). VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
...
**Current Version: v10.15.34** — See [CHANGELOG.md](CHANGELOG.md) for the full per-release history.
