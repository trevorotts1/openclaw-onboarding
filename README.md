...
> **Version:** see `/version` - this repo at v10.15.15.
...
>
> **NOTE (v10.15.15) — version-surface reconcile.** This release rolls the advertised version forward so it matches the shipped tree; no skill content changed. Live state: **Skill 38 (Conversational AI System) → v1.5.6** (its own `38-conversational-ai-system/skill-version.txt`, intentionally independent of the repo version line), **40 numbered skill folders (37 active + 3 archived: 13, 33, 34)**, with **Skill 39 — Real Estate Playbook & Property Intelligence** and **Skill 40 — ZHC Public Records Scraper** present in the catalog. Includes the Round-3 cross-repo reconciliation + roadmap spec and the F45/F46/F47 + ZHC Tag-Prefix QC fix wave that landed after the v10.15.14 tag. Per-release version history lives in [CHANGELOG.md](CHANGELOG.md). VPS (10.16.x) and Mac (10.15.x) sequences remain intentionally independent.
>
...
| 38-conversational-ai-system | **Conversational AI System (v1.5.6)** — the conversational AI BRAIN on top of skill 29 (GHL Convert and Flow). 39 protocols (sales brain, intelligent follow-up, dual-mode customer service + support, typed knowledge bases, intelligent routing, weekly + monthly self-tuning, model version freshness, ZHC tag-prefix, F45 geo-qualification, F46 CRM field write, F47 inline smart-FAQ, F49 ZHC Pixel, etc.). 8 customer journey templates. 55 idempotent OS-aware scripts. 19 references. Sunday 2am + Saturday 11pm + 1st-of-month crons. Skills 05/10/19/29 required as prerequisites. Its `skill-version.txt` versions independently of the repo line. |
