...
- "Start onboarding"
- "Install the 41 active skills"

**Your task:** Install all 41 active skills using the 5-Wave Parallel Orchestration below.
...
Tell user: "Onboarding triggered. I will install 41 active skills using [5-Wave Parallel / Sequential] strategy. Checking for existing installations..."
...
- After all 41 active skills: final complete index
...
- **Avatar Intelligence Engine (Skill 52)** — `52-avatar-intelligence/`. Marketing (the Brand Positioning Specialist, routed by the Chief Marketing Officer) turns ONE completed brand-intake interview into the full **brand-intelligence package** — 40 generators across 7 subsystems (Avatar Core, Awareness, Bios, Tone, a 13-set Facebook Ad system, Booking Bots, Landing/Hero) → 16 named deliverables (37 documents). A Book/Brand version selector runs FIRST: `version=brand` runs the 40-stage pipeline; `version=book` routes to the separate Avatar Alchemist Book skill (53) or parks fail-closed `book-skill-not-available`. Built through the ONE sanctioned front door `52-avatar-intelligence/entry.sh` (deps → bypass-scan → hash-pin → nonce) then the foreman `52-avatar-intelligence/scripts/aa_director.py`; every SACRED count/floor is MEASURED by fail-closed, model-free provers (`52-avatar-intelligence/scripts/aa_*.py`) — self-reported counts ignored — with a signed provenance certificate issued only on a full 40/40 pass (no certificate = not done). Delivery is a labeled LOCAL bundle in `~/Downloads/` (no n8n / Airtable / Google Drive / Slack / Gmail). Cross-linked with, but NEVER merged into, **Skill 55** (routing: a standalone master-brain bio → Skill 55; a full brand-intelligence package → Skill 52). Shared procedure: `universal-sops/avatar-craft/`.
...
5. Report progress: "[X] of 41 active skills installed. Currently on: [skill name]."

DO NOT REMOVE this block until all 41 active skills are installed or attempted.
```
...
"Onboarding progress: [X] of 41 active skills installed. Currently on: [skill name]. [Y] skills remaining."

### Completion

When all 41 active skills are installed:
...
5. If a gateway restart is needed for any changes to take effect, tell the user:
   "All 41 active skills are installed. To activate the changes, please type /restart in Telegram."
...
**ZERO TOLERANCE: The onboarding does not end until all 41 active skills have been attempted. If you stop early, you failed.**
...
| **Final** | After ALL 41 active skills complete | Complete system index |
...
4. **Write ONBOARDING COMPLETE to MEMORY.md**
   - Add entry: "OpenClaw onboarding completed on [date]. All 41 active skills installed."
...
If `sessions_spawn` doesn't work, install 41 active skills one at a time:
...
The OpenClaw onboarding uses a **5-WAVE PARALLEL STRATEGY** to install 41 active skills efficiently.
...
"Onboarding in progress: [X] of 41 active skills complete. Currently on: [skill name]."

---

## FINAL STEP - SET UP WEEKLY AUTO-UPDATE (Agent Runs This)

After all 41 active skills are installed, run this as the final step.
...
After all 41 active skills are installed, run the final Gemini Engine indexing:
...
Then write to MEMORY.md: "ONBOARDING COMPLETE - [date] - All 41 active skills processed"
