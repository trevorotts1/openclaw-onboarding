...
For n8n, require workflows named create/append/weekly. Each contains actual `workflow_id`, `active: true`, successful `execution_id` and `execution_result: "success"`, `source_repo_path`, `source_file_sha256`, `normalization_proof`, `source_definition`, `deployed_definition`, and `expected_sha256`. Capture definitions through the repository's deterministic versioned import/readback normalization; canonical source/deployed bytes must match their expected digest. Never improvise broad field deletion to hide differences. The normalization proof adds `source_releases`, `source_sha256` of the actual released repo file and `canonical_sha256` of the final artifact; retain raw exports, the normalizer command/version, credentials-binding check and declared server-field exclusions in its captured artifacts. Credentials values must stay out of committed source/logs; reference-binding evidence must still prove the correct client.
...
The suite contains a deliberately synthetic complete fixture, corruption cases and a real temporary Git-history test. It rejects false-green nested proofs, unknown repositories, forged diff/SHA, omitted F02, wrong/lightweight tags, missing releases, skipped required CI, stale reviews, old installed builds, rollback, inactive workflows, missing browser evidence, pending workers/queues and path escape. The injected provider is a Python testing seam; the CLI has no offline-pass or mock-provider switch.
