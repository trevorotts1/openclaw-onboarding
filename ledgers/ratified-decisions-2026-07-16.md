...
**Where this decision lives, and why it has two names.** The `D-U74` entry above (this same file, "Unit 74 is UNBLOCKED / BUILDABLE") already flagged this exact question as U74's own **co-located** decision, naming it **GK-D4** — *"which podcast pipeline is canonical"* — and recorded that U74's own spec requires it to be "asked and answered BEFORE the run starts." Separately, U74's ledger row (`ledgers/skill6-blended-persona-kanban-v2-2026-07-13.md`, id U74) reads: *"[GK-12] (n8n + ONB, P1) Canonicalize the podcast pipeline per **D19** (kill the double-publish risk)."* This file's own "Correction to the 'genuinely waiting on Trevor' list" section (above) independently lists **D19** among the fourteen master-spec decisions still formally unratified. Comparing the two texts side by side — "which podcast pipeline is canonical" (GK-D4) and "canonicalize the podcast pipeline... kill the double-publish risk" (D19) — they are the same question under two labels: GK-D4 is the GK-track's own in-unit name for it, D19 is the master spec's global decision number for it. **This entry ratifies both labels together, as one decision, and closes both.**
...
**Standing rule reaffirmed, unchanged by this ruling:** per `D-U65` above, the Podbean OAuth plaintext credentials on both `BqRLOn8TP1wPaAzn` and `COfgxe6HXRcWOleV` remain **NEVER-PRINT, NEVER-VAULT, NEVER-ROTATE** — permanent and closed. Deactivating `COfgxe6HXRcWOleV`'s trigger does not touch, read, relocate, or "fix" that credential; it changes only the workflow's `active` flag. This ruling is about which workflow is allowed to **fire**, never about the credential inside either one.

---

## Third pass — 2026-07-16 (GK-D4/D19 closure)

The **GK-D4 (D19)** ruling above was appended on branch `chore/ratified-decisions-2026-07-16-d12-d4` (tip `84c0ead3` at the time of this pass), unblocking U74/GK-12. Not merged by this pass — lands via the standing one-merge-writer-per-repo serial train, same as the two passes above it.

**Evidence discipline for this pass:** every load-bearing claim about the two n8n workflows (node structure, node types, `active`/`updatedAt` metadata, execution history and status) was read live this pass via `mcp__n8n-mcp__n8n_get_workflow` (`mode=structure` and `mode=minimal` only) and `mcp__n8n-mcp__n8n_executions` (`action=list`) — no Code/Set/HTTP-Request node body was fetched or printed on either workflow, and no credential value was read, printed, or referenced beyond its name. This entry was recorded **before** the deactivation it authorizes was executed, per the standing instruction to record ratification first.
...
**Attestation, stated precisely, matching this file's own standard.** This ruling is recorded here on the **coordinating agent's attestation**: it states Trevor was presented with the round-2 judge ticket's `owedLegs` choice (path A — author real, paid prompt-authoring work as a separate scoped unit; path B — an explicit operator ruling descoping the receipt for a fixture that intentionally never renders) and ruled path B, descoping the receipt, dated 2026-07-16. The agent writing this ledger entry (and updating `ledgers/evidence/U87-GK-25/README.md` to match) did not witness that exchange, has no direct channel to Trevor, and cannot independently verify it — the same standard already applied to D15's and GK-D4/D19's provenance trails above. If this attribution is wrong, Trevor corrects it in one line, and this note is what makes the error findable. Every other claim in this entry (the judge's category scores and blocker-closure findings, the fixture's own README text, the CI and authorship facts below) **was** independently re-derived this pass from primary source — the round-2 judge ticket itself, and PR #599 at head `09d8140f2144b0a45746cfacd48304897bf92f3c` — not relayed as fact from a sub-agent's self-description.
...
## Fourth pass — 2026-07-16 (GK-25/U87 prompt-floor descope)

The **GK-25 (U87)** ruling above was appended on branch `fix/gk25-u87-golden-quest-real-run` (PR #599, draft, head `09d8140f2144b0a45746cfacd48304897bf92f3c` at the time this entry was appended), alongside the matching update to `ledgers/evidence/U87-GK-25/README.md`. Not merged by this pass — lands via the standing one-merge-writer-per-repo serial train, same as the three passes above it.

**Base-content note.** This file did not yet exist on `origin/main` or on PR #599's branch when this pass began — `ledgers/ratified-decisions-2026-07-16.md` was created and carried forward on a separate, still-unmerged branch, `chore/ratified-decisions-2026-07-16-d12-d4` (tip `41d2d1f9` at the time of this pass), which this pass never fetched into a working checkout, reset, or otherwise touched. To keep this file's continuity as the one permanent record rather than forking a second, divergent copy of it, this pass's base content (everything above this section) was taken read-only from that branch's tip via `git show <branch>:<path>` and this entry appended on top. Reconciling the two branches' independent appends into one committed history on `main` is the serial merge-writer's job at actual merge time, per this file's own standing practice.

**Evidence discipline for this pass:** every load-bearing claim about U87/GK-25 (blocker-closure findings, category scores, CI counts, `owedLegs`) was read directly from the round-2 judge ticket, not relayed from a sub-agent's self-description. The CI count was independently re-verified this pass, not merely quoted from the ticket, via four separate methods against PR #599's exact head SHA — authenticated paginated `gh api` check-runs, `gh pr checks`, GraphQL `statusCheckRollup`, and unauthenticated REST via `curl` — all four agree: 24 total check-runs, 23 success, 1 skipped (`G1 — version file change requires a matching annotated tag`, correctly skipped on a draft), 0 failures. Author/committer/trailers were independently re-verified structurally this pass via `git log --format='%an|%cn|%(trailers)'` on a fresh scratch clone (never the shared tracked tree): `Trevor Otts <trevor@blackceo.com>` for both author and committer on the one commit ahead of `main`, zero trailers. No live box was mutated by this pass. `prove-deck.py` (write-capable) was not run at all this pass — no fixture content under `51-signature-presentation/examples/golden-quest/` was touched, only ledger/evidence prose.
