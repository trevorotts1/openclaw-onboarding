...
**Leg tag (required for `scripts/ledger-truth-gate.py` / `unit-status.sh` to gate a row
mechanically instead of falling through to a human-review-required failure):** every
unit whose ONLY leg is a live n8n-API/execution proof (no `openclaw-onboarding` or
`blackceo-command-center` repo commit backs it -- true of every unit in this file except
the HYBRID/repo-leg ones, which carry their own `(ONB, ...)`/`(both, ...)` tag instead)
MUST open its `desc` cell with a `(live, P#)` tag, same grammar as any other ledger's
`(ONB, P#)` / `(both, P#)` / `(doc, P#)` leg tag (see `shared-utils/ledger_reconciler_
core.py`'s `LEG_TAG_RE`). This is what tells the gate's tooling this row has no repo leg
to independently git-check -- the SAME zero-leg category already used elsewhere (e.g. the
kanban ledger's `(doc, P#)` rows), not a new or looser one. A row without a recognized
leg tag reads as `mode=unknown` and can never pass the gate mechanically, no matter how
true the claim is.

**Ids in this file are NOT globally unique across ledgers** -- this file's `U2`/`U4`/
`U5`/`U8` (etc.) are this campaign's own numbering, unrelated to any other ledger's same-
numbered rows (the kanban ledger has its own, different U2/U4/U5/U8). `unit-status.sh`
resolves this correctly ONLY when it is told the SPECIFIC ledger file to read via
`--ledger`; `scripts/ledger-truth-gate.py` always does this per-candidate (scoped to the
exact file a row was diffed out of), so this is safe from that call site. Anyone running
`./unit-status.sh <id>` by hand against this file's units must pass `--ledger
ledgers/skill58-podbean-proxy-2026-07-16.md` explicitly, or risk reading a different
campaign's same-numbered row instead of this one's.
...
| U2 | (live, P1) Create + seed `podcast_publish_roster` data table (6 cols, all good_standing=YES, + OPERATOR TEST row) | — | pending | GREENFIELD. Fresh `GET /api/v1/data-tables` HTTP 200 → 6 tables exist; `podcast_publish_roster` is ABSENT. Reuse pattern: `snapshot_provision_ledger` (FCb16RvM2l7f4HKl). | 2026-07-16T12:35Z |
...
| U4 | (live, P1) Snapshot workflow JSON, then add webhook header auth (`Podcast Publish Gate`, `X-Podcast-Publish-Token`) | — | pending | GREENFIELD — **LIVE SECURITY HOLE OPEN AND CARRYING REAL TRAFFIC**. Fresh API re-read of `TkL0rn2SH3q32SeB` webhook node: `authentication` key ABSENT, `credentials: NONE`, `path: podbean-publish`, `httpMethod: POST`. Webhook is publicly postable. Pattern source `aN6MrIJ4zLeKS047` still ACTIVE (2 nodes). **SEE THE LIVE-CALLER FINDING BELOW — execution 91115 published a real episode at 2026-07-16T12:38:08Z, DURING this bootstrap. U4 will 401 that caller unless it is provisioned first. This is now the central input to D1.** | 2026-07-16T12:41Z |
| U5 | (live, P1) Good-standing + identity gate (roster lookup; downstream uses roster's `effective_channel_id`) | — | pending | GREENFIELD. Fresh re-read: ZERO dataTable nodes in the 24-node graph. No roster lookup exists. | 2026-07-16T12:35Z |
...
| U8 | (live, P1) Server-side idempotency via `podcast_publish_ledger` | — | pending | GREENFIELD. Depends on U3 (table absent). ZERO dataTable nodes live. | 2026-07-16T12:35Z |
