...
| U21 | (ONB + live, P1) Release ripple + truth-gate close-out (ancestry + tags + ZERO integrity alarms + fresh n8n re-reads) | [Sonnet 5 x1] release-ripple + truth-gate close-out U21 + [Fable 5 x1] G3 retarget + honest pre-merge status | verified (ONB leg; live leg = bounded fresh re-read, not a full U1-U20 re-audit) | **ONB LEG (pre-merge record, kept for the audit trail; closed by the MERGE-WRITER FOLLOW-UP below):** Version bump `v20.0.68` -> `v20.0.70` via `scripts/bump-version.sh v20.0.70` — renumbered from v20.0.69 pre-tag: `06-ghl-install-pages/skill-version.txt` was already pre-bumped to v20.0.69 on `main` by commit ee1dc5a4 (U24's own G3-compliance bump), so a v20.0.69 roll produced no diff in that marker while this PR changes other files under `06-ghl-install-pages/`, tripping CI gate G3's diff-presence check; retargeted one patch forward per the 0348b9b3 precedent (see CHANGELOG v20.0.70; those two reference SHAs are deliberately NOT backtick-quoted here so this ledger's citation-extraction tooling never mistakes them for this unit's own evidence). All 11 tracked markers agree (`--check` exit 0 both before and after), CHANGELOG.md v20.0.70 entry added, README.md's two tracked version lines rolled in lockstep. **RETRACTION of this row's earlier `GATE RESULT: PASS` claim:** that pre-merge gate run had omitted `ledger-truth-gate.py`'s mandatory `--cc-dir` argument and errored — no PASS was ever actually produced; the claim is withdrawn, not carried forward. A correctly-invoked pre-merge run returns verdict=UNKNOWN for this row's own then-`verified` claim, and honestly must: this ledger's header defines `verified` for repo legs as merge-commit-is-ancestor-of-`origin/main` + annotated tag on the remote, and neither fact can exist before this very PR merges — chicken-and-egg by construction for the terminal close-out unit; the gate refusing the premature claim is the gate working, not a defect. This branch (`closeout/s58-u21-podcast-terminal`) is pushed as PR #654 against `main`; the flip to `verified` plus the merge-commit SHA, ancestor confirmation, and the annotated `v20.0.70` tag's `git cat-file -t == tag` proof are appended to this row the instant the merge-writer step completes, per this ledger's own append-evidence convention — never claimed here in advance of the actual merge. **MERGE-WRITER FOLLOW-UP (2026-07-20T02:54:31Z) — merge landed; ONB leg is now a real GIT state; row flipped to `verified`:** PR #654 (https://github.com/trevorotts1/openclaw-onboarding/pull/654, branch closeout/s58-u21-podcast-terminal, tip `9aae3a54`) merged into `origin/main` via 2-parent merge commit `e718a019`; `git merge-base --is-ancestor` on the merge commit vs `origin/main` = true (fresh fetch this pass). PR #654's final check rollup on the tip: 53 SUCCESS + 3 SKIPPED = 0 failures — including the two checks red on the previous push, "G3 — skill content change requires skill-version.txt bump" (cleared by the v20.0.70 retarget) and the ledger-truth gate "Reject ledger rows claiming verified/done that git + check-run history cannot prove" (cleared by holding this row at `in_progress` until this follow-up). Annotated tag `v20.0.70` cut ON the merge commit and pushed: local `git cat-file -t v20.0.70` prints `tag` (annotated object, not lightweight), and `git ls-remote origin v20.0.70` resolves to a tag object whose SHA differs from the merge commit's — the remote holds the annotated object itself. **LIVE LEG (bounded, disclosed scope):** a fresh, read-only re-check this pass, NOT a re-run of every individual U1-U20 live claim (which would mean re-firing publishes and creating more untracked test episodes on the operator's own channel for no proof-value gain) — (a) `n8n_health_check`: instance reachable, `status:"ok"`. (b) `podcast_publish_roster` (id `UWjpksxU2b6TjKow`) re-read: still exactly 6 columns, same schema; 31/31 rows still `good_standing:"YES"`, unchanged from the 2026-07-18 structure-mode read recorded on this ledger's U2 row — no name, email, or channel id reproduced here, aggregate counts only, per this ledger's own no-client-data convention. (c) Main publish workflow `TkL0rn2SH3q32SeB` (minimal-mode, no parameters/credentials read): `active:true`, `updatedAt:"2026-07-16T14:29:30.615Z"` — byte-identical to the value already on record in this ledger's U4 row, proving zero writes have touched this single-writer-serialized workflow since that check, i.e. nothing silently drifted underneath U4-U12's evidence. **Disclosed gap, not hidden:** this bounded check does NOT re-fire every prior unit's own live assertion (U1, U3, U6-U13's individual live claims stand on their own already-captured evidence, not re-proven here) — it is a drift/tamper check on the shared substrate (roster table + production workflow), not an exhaustive re-audit. No client names, no emails, no channel ids, no secret values read or printed. | 2026-07-20T02:54:31Z |
