...
| U5 | Good-standing + identity gate (roster lookup; downstream uses roster's `effective_channel_id`) | [Sonnet 5 x1] partial live-execution verification U5 | pending | PARTIAL LIVE-EXECUTION PROOF 2026-07-18 — real progress beyond the prior parameter-only pass, still NOT sufficient to close this row against spec Section 5 U5 accept clauses (a)-(d). (a) unchanged, parameter/topology-confirmed by the prior pass. (b) requires THREE live refusal executions (one per reason: `identity_unknown`, `identity_mismatch`, `not_in_good_standing`), each with ZERO Podbean nodes run — this pass fired and confirmed only ONE of the three. A live POST (correct auth header, full contract-v2 shape, an email/last-name pair matching no roster row) returned HTTP 403 `{"ok":false,"reason":"identity_unknown"}` — execution `92225` (2026-07-18T05:34:11Z, 16/16 nodes: Webhook→Guard→Idempotency chain→Standing Gate Normalize/Lookup/Determine Verdict→IF Standing Identity Gate Passed (false)→Gmail Standing Identity Gate Refused Notification→Respond Standing Identity Gate Refused→Idempotency Mark Refused (Standing Gate)) — zero OAuth/Media-Preflight/download/upload/Publish nodes ran. `identity_mismatch` and `not_in_good_standing` were NOT fired this pass. (c) CONFIRMED: a live POST using the OPERATOR TEST roster row's own on-file identity + its own channel id passed the standing gate — execution `92226` (2026-07-18T05:34:12Z, 20/20 nodes, correctly reached `Media Preflight` before being refused there for unreachable test media, deliberately used so no real Podbean call would occur). The `Standing Gate — Determine Verdict` node's own output was read directly (filtered execution read, not inferred): `standing_gate_verdict:"pass"`, `roster_channel_id`, `payload_channel_id`, and `effective_channel_id` all equal to the roster row's own channel id (the pre-authorized OPERATOR test channel; no other channel appeared anywhere in this output) — proving downstream routing is sourced from the roster row, exactly as clause (c) requires. (d) fail-closed (table unreachable → refuse) was NOT tested this pass; the spec's own U5 Do-text explicitly requires never breaking the live table to prove it, and building a scratch-clone table for this was out of this pass's authorized scope. Row stays `pending`: 2 of 3 refusal reasons and the fail-closed clause remain unproven by live execution. | 2026-07-18T05:36:50Z |
