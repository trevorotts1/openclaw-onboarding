...
| U4 | Snapshot workflow JSON, then add webhook header auth (`Podcast Publish Gate`, `X-Podcast-Publish-Token`) | [Sonnet 5 x1] full-mode parameter re-read U4 | pending | PARTIAL PARAMETER CONFIRMATION 2026-07-18, NOT sufficient to close this row — see spec `skill58-podbean-server-side-publish-SPEC-v1-2026-07-16.md` Section 5 U4 accept clauses (a)-(d): clause (a) IS a parameter fact and IS now confirmed by FULL-MODE `n8n_get_workflow(TkL0rn2SH3q32SeB, mode=full)`: node `Webhook — Receive Podcast Payload` has `parameters.authentication = "headerAuth"`, `credentials.httpHeaderAuth = {id: "8HTB7khC7fDcRVhN", name: "Podcast Publish Gate"}` (name matches exactly). Clauses (b) and (c) require LIVE webhook curls (POST without the header expect 401/403; POST with the header expect the execution to reach the guard) — no webhook test calls were authorized or fired this pass. The literal header KEY NAME (part of clause (a)'s spirit) is additionally unobtainable via n8n's API in ANY mode: `n8n_manage_credentials(action=get, id=8HTB7khC7fDcRVhN)` returned only `{id, name, type, isManaged, isGlobal, createdAt, updatedAt}` — no `data` object at all, ever, for any credential (platform behavior, not a scope artifact); the webhook node's own parameters carry no header-name field for `headerAuth` type. Corroborating (not independent-proof) same-repo evidence: `58-podcast-production-engine/scripts/podbean_publish.sh` documents (line 126) `PODBEAN_PUBLISH_TOKEN shared header token for the proxy (X-Podcast-Publish-Token)` and constructs (line 372) `header = "X-Podcast-Publish-Token: %s"`; U6's already-verified row recorded 21 live requests against this same webhook returning HTTP 422, never 401, consistent with that name. Row stays `pending`: this unit's own accept criteria require live-execution proof that a read-only parameter pass cannot supply, regardless of mode. | 2026-07-18T05:19:46Z |
| U5 | Good-standing + identity gate (roster lookup; downstream uses roster's `effective_channel_id`) | [Sonnet 5 x1] full-mode parameter re-read U5 | pending | PARTIAL PARAMETER CONFIRMATION 2026-07-18, NOT sufficient to close this row — see spec Section 5 U5 accept clauses (a)-(d): clause (a) (node graph ordered webhook→guard→ledger-check→gate→timestamp) IS a parameter/topology fact and IS now confirmed by FULL-MODE `n8n_get_workflow(TkL0rn2SH3q32SeB, mode=full)` connections: `Webhook → Guard → IF Entry Guard Passed → Idempotency Lookup By Key → IF Idempotency Already Completed → IF Idempotency In Flight → Idempotency Upsert Row Received → Idempotency Mark In Flight → Standing Gate Normalize Lookup Keys → Standing Gate Roster Lookup By Email → Standing Gate Determine Verdict → IF Standing Identity Gate Passed → Media Preflight ... → Compute Timestamp`, matching the required order exactly. Table-target and expression facts also parameter-confirmed: node `Standing Gate — Roster Lookup By Email` (`dataTable`) has `dataTableId = {mode:"id", value:"UWjpksxU2b6TjKow"}` — the same id U2's row confirms as `podcast_publish_roster`, filtered on `keyName:"email"`. Node `Standing Gate — Determine Verdict` (code) sets `effective_channel_id` from the matched roster row's `podbean_channel_id` ONLY on verdict `pass`. Downstream expression use of `effective_channel_id` confirmed in `Podbean OAuth — Get Access Token` (body param `podcast_id` = `{{ $('Standing Gate — Determine Verdict').item.json.effective_channel_id }}`) and `Set Config — Consolidate Workflow Variables` (identical expression) — every downstream Podbean call routes on the roster's channel id, never the raw payload. Clauses (b), (c), (d) all require LIVE refusal/passing executions and a fail-closed live test — none fired this pass (no webhook test calls authorized). Row stays `pending`: this unit's own accept criteria require live-execution proof a read-only parameter pass cannot supply. | 2026-07-18T05:19:46Z |
...
| U8 | Server-side idempotency via `podcast_publish_ledger` | [Sonnet 5 x1] full-mode parameter re-read U8 | pending | PARTIAL PARAMETER CONFIRMATION 2026-07-18, NOT sufficient to close this row — see spec Section 5 U8 accept clause: firing the SAME `idempotency_key` twice live, confirming exactly ONE Podbean episode exists (+1 not +2), the second response is `idempotent_replay:true`, and ledger rows show the transitions — entirely a live-execution proof; no webhook test calls were authorized or fired this pass. What FULL-MODE `n8n_get_workflow(TkL0rn2SH3q32SeB, mode=full)` DID parameter-confirm this pass: all 7 `dataTable` nodes in the Idempotency chain carry the identical `dataTableId = {mode:"id", value:"3anOzegbKtLcgVud"}` — the exact table id U3's row confirms as `podcast_publish_ledger` (8 columns, empty at creation) — `Idempotency — Lookup By Key` (filter `idempotency_key eq`), `Idempotency — Upsert Row Received` (upsert, sets `status:"received"`), `Idempotency — Mark In Flight` (update, `status:"in_flight"`), `Idempotency — Mark Refused (Standing Gate)` (update, `status:"refused"`), `Idempotency — Mark Refused (Media Preflight)` (update, `status:"refused"`), `Idempotency — Mark Completed` (update, `status:"completed"`, sets `permalink_url`/`episode_number`/`completed_at`), `Idempotency — Mark Failed` (update, `status:"failed"`). No node in this chain targets any other table id — the WIRING is correct-shaped, but this is parameter evidence, not the live double-fire proof the spec clause requires. Row stays `pending`. | 2026-07-18T05:19:46Z |
