...
- [ ] W0.6 LIVE-VERIFY the Kie.ai TEXT-TO-IMAGE PORTRAIT endpoint at 1024x1536 with GPT-Image-2.5 (NOT the 16:9 presentation recipe; verify the portrait endpoint shape fresh) plus the Skill 46 callback relay; DIRECT Drive and Docs API access with the operator's EXISTING service account (clawd/google-api.js pattern, GOOGLE_IMPERSONATE_USER label) including view-only sharing AND reachability of the existing shared delivery root (PRD 3.7); the deterministic HTML-to-PDF toolchain with a 14-point-floor render probe. PENDING
...
- [ ] W1.14 Cover module (cover_render.py): pin aw-11, Kie.ai GPT-Image-2.5 portrait 1024x1536 via the W0.6-verified endpoint, Skills 07/46 callback handling with bounded re-poll, Drive landing. PENDING
