...
def test_shipped_python_file_count_matches_the_documented_56():
    """CHECKLIST.md's reconciliation note states 56 shipped Python files in
    ...
    assert len(py_files) == 56, (
        f"59-anthology-engine/ now ships {len(py_files)} .py files, not the 56 "
        "CHECKLIST.md's reconciliation note documents -- update the note"
    )
    assert "56 shipped Python files" in CHECKLIST.read_text()
