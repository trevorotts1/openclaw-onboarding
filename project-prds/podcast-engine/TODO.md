...
|---|---|---|---|---|---|---|
| 2026-07-06 | W2.12 T1 to T9 webhook verification suite | Opus 4.8 (webhook-verification-suite) | DONE (branch pushed) | feat/podcast-webhook-verification-suite | self-QC 9.3 | Authored executable 58-podcast-production-engine/scripts/verify-t1-t9.sh: T1 to T9 plus TM (method) and TB (body cap). Drives real HTTP, observes the intake ledger, quarantine dir, and operator alert log; writes a machine-readable results record to <state-dir>/verify/ and a history line; exit codes 0/1/2/3 for the provision gate. Observed, never assumed. Proven on this box against a faithful mock gateway: healthy=all 11 PASS exit 0; cleanup removes only its own test/needs_input/quarantine records (ledger empties); --loopback-only=exit 2 (T9 SKIP); a broken auth-bypass gateway correctly FAILS T1 to T3 exit 1. shellcheck clean, bash -n clean, zero em dashes, zero fences, zero model ids, secret never printed, portable bash. Additive: touched only the one script plus this row and the SESSION-LOG row. |
