...
| 2026-07-06 | W1.17 webhook-fixtures-tests | Opus 4.8 exec | DONE (branch) | feat/podcast-webhook-fixtures-tests | self-QC 9.0 | Added 58-podcast-production-engine/tests/webhook/ (spec_reference oracle, three upstream family fixtures plus interview/tenant/missing fixtures, four pytest files, conftest, runner, README). 33 pytest cases green: three-family meaning-mapping to one canonical, job-key collision and divergence, idempotency (one record plus duplicate ack on identical redelivery, new job on one-answer change, cross-family dedup, exclusive-create race, failed-retry gating, _test), tenant quarantine (T7), needs_input gate (T8). Independent-check role: same tests bind to the shipped W1.16 mapper via PODCAST_WEBHOOK_SUT or a podcast_webhook_sut adapter; proven to catch a divergent mapper. Hard-rule scans clean: valid JSON, zero em/en dashes, zero triple backticks in JSON/scripts, zero Anthropic tokens, no MCP/model/secret use, no client-facing GHL name. Only tests/webhook/ touched; shared files untouched (Merge phase owns those). |
