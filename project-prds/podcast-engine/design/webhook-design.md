...
## 1. Mechanism decision: BOTH surfaces, each doing the job it can actually do

> **AMENDED (ISSUE-02 / ISSUE-03).** The original decision below picked the Webhooks plugin as the sole inbound mechanism. Shipping it proved that choice incomplete, and the gap was silent: the plugin route accepts ONLY a `{"action":"create_flow", ...}` envelope, while every upstream sender this engine actually has (a Convert and Flow workflow webhook action, Make.com, n8n) posts a FLAT body. A flat body on that route returns HTTP 400 `action: Invalid discriminator value. Expected 'create_flow'`, and even a well-formed envelope only CREATES a queued flow: nothing in the plugin dispatches the bound session, so the flow sits queued forever. The observable failure was the worst one the engine has: intake succeeds, the ledger says `received`, the dashboard says Received, and no episode is ever produced.
>
> **The amended decision: the two surfaces split by role, and both are registered per client by `register-podcast-hook.sh`.**
>
> | Surface | Endpoint | Who calls it | What it carries |
> |---|---|---|---|
> | Gateway hook mapping (the TRIGGER) | `POST /hooks/podcast-intake-<client-slug>` | the ONE upstream survey sender | the flat survey body, with `Authorization: Bearer <box hooks token>` |
> | Webhooks plugin route (the CONTROL surface) | `POST /plugins/webhooks/podcast-intake-<client-slug>` | the podcast agent, from inside its own turn | the TaskFlow actions: create_flow, run_task, get_flow, resume_flow, finish_flow, fail_flow, set_waiting |
>
> The mapping is `hooks.mappings[]` id `podcast-intake-<slug>`, `match.path` `podcast-intake-<slug>`, `action: agent`, `agentId` the podcast department agent, `sessionKey` `podcast:intake:<slug>`, `sessionMode: persistent`, `wakeMode: now`, `deliver: false`, `allowUnsafeExternalContent: false`, with a deterministic `messageTemplate` that runs `webhook/intake_handler.py --mode trigger-flow` and then `podcast_step_driver.py next`. The whole request body reaches that template as `{{.}}` (verified against the installed resolver; `{{payload}}` renders empty). The gateway answers the POST as soon as the agent run is ADMITTED, not when it completes, so a long episode turn never holds the upstream open.
>
> The fallback conditions in the table below are therefore no longer "fallback" conditions: conditions (c), (d) and (e) are now the permanent, enforced shape of the mapping, and the durability the plugin provides is retained because the flow is still a durable managed TaskFlow. What DID survive from the original decision is its warning, so it is restated as a live trade-off rather than a rejection: `hooks.token` is ONE box-wide token, so any sender holding it can reach every hook endpoint on that box. This engine bounds that with `hooks.allowedAgentIds` (the podcast agent) and `hooks.allowedSessionKeyPrefixes` (the `podcast:` namespace, plus the `hook:` prefix the gateway itself requires when `hooks.defaultSessionKey` is unset), keeps the per-route SecretRef per-client and unshared on the plugin surface, and points `hooks.token` at the SAME env label as that SecretRef so one rotation covers both. `register-podcast-hook.sh` never overwrites a `hooks.token` another integration already set. `/hooks/wake` remains never appropriate for intake.

**Original decision (superseded in part; kept for the reasoning that still holds): use the OpenClaw Webhooks plugin (`POST /plugins/webhooks/<routeId>`) as the standard inbound mechanism on every client box.**
...
1. **Delivery:** upstream POSTs the flat survey body to the client's public URL `https://<client-hooks-host>/hooks/podcast-intake-<client-slug>` with `Authorization: Bearer <box hooks token>`; Cloudflare Tunnel carries it to loopback 127.0.0.1:18789; the gateway hooks ingress authenticates the bearer token and matches the request path against the `podcast-intake-<client-slug>` hook mapping, which dispatches ONE turn of the podcast department agent in session `podcast:intake:<client-slug>` and answers the POST as soon as that run is admitted. (The plugin route's own SecretRef authenticates the agent's in-turn TaskFlow calls on the control surface; it is not what authenticates the upstream sender.)
...
2. Run `58-podcast-production-engine/scripts/register-podcast-hook.sh --client-slug <client-slug>` as the runtime user, never root. It writes BOTH surfaces in one idempotent pass: the plugin route (id `podcast-intake-<client-slug>`, sessionKey `podcast:intake:<client-slug>`, SecretRef per Section 2) and the gateway hook mapping that actually triggers production (Section 1). It merges `hooks.allowedAgentIds` and `hooks.allowedSessionKeyPrefixes` without dropping a pre-existing entry, enables the hooks ingress, sets `hooks.token` to `${PODCAST_INTAKE_HOOK_SECRET}` only when the box has none, validates the merged config against the INSTALLED OpenClaw schema (schema drift between releases is a known trap), and restores its own backup if validation fails. Prove it with `register-podcast-hook.sh --verify --client-slug <client-slug>`.
...
5. Configure the ONE upstream sender the client actually uses (GoHighLevel workflow webhook action, Make.com, or n8n): method POST; URL `https://<client-hooks-host>/hooks/podcast-intake-<client-slug>` (NOT `/plugins/webhooks/...`, which rejects a flat body with `action: Invalid discriminator value. Expected 'create_flow'`); header `Authorization: Bearer <the box hooks token>` (value pasted from the credential store, never from chat; the gateway reads the token from `Authorization: Bearer` or `x-openclaw-token` and from nowhere else, so a payload-field or query-string secret is rejected); FLAT JSON body carrying the survey fields plus `contact_id`, `location_id`, `podcast_id`, mode and style, with no `action` wrapper. Capture one real sample payload from this pipeline and add it to the mapper's test fixtures.
