...
ROUTE_HELPER_SH
  ...
  # Idempotent V2 stamp with V1 auto-migration. Re-stamp when V2 is missing OR a
  # stray V1 co-exists; otherwise no-op (helper was still refreshed above).
  if grep -qF "$PRES_REFLEX_V2_MARKER" "$AGENTS_FILE" && ! grep -qF "$PRES_REFLEX_V1_MARKER" "$AGENTS_FILE"; then
    _log "L1: PRESENTATION_ROUTING_REFLEX_V2 already present in $AGENTS_FILE — no-op (helper refreshed)"
  else
    grep -qE 'PRESENTATION_ROUTING_REFLEX_V[12]' "$AGENTS_FILE" && _strip_pres_reflex "$AGENTS_FILE" || true
...
PRES_REFLEX_V2
    ...
    ORIGINAL_REFLEX_CONTENT="$(cat "$AGENTS_FILE")"
    { cat "$PRES_REFLEX_RENDERED"; printf '\n'; printf '%s' "$ORIGINAL_REFLEX_CONTENT"; } > "$AGENTS_FILE"
    rm -f "$PRES_REFLEX_TMPL" "$PRES_REFLEX_RENDERED"
    _log "L1: PRESENTATION_ROUTING_REFLEX_V2 stamped at absolute top of $AGENTS_FILE"
  fi
fi
