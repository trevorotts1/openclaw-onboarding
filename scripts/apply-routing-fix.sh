...
# TRIGGER-BEFORE-THINKING gate: on a presentation / deck / PowerPoint / slide /
# keynote request the CEO's VERY FIRST action must route the task to the
# `presentations` department on the Command Center. It MUST sit ABOVE
# ROLE_DISCIPLINE_V1 + CEO_ROUTING_NO_LOOPHOLES_V1 (both stamped at the top above),
# so it is stamped LAST here as an absolute-top prepend and becomes the file's
# first block.
#
# P1-4 FIX (V1→V2): V1 told the CEO to POST a BARE (unauthenticated) curl to
...
#   NO same-origin exemption (no Origin header). V2 has the CEO run a stamped,
...
# (4b-REFLEX) — the ROUTE_HELPER_SH and PRES_REFLEX_V2 heredocs are BYTE-IDENTICAL
# twins.
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  if grep -qE 'PRESENTATION_ROUTING_REFLEX_V[12]' "$AGENTS_FILE" 2>/dev/null; then
    _strip_pres_reflex "$AGENTS_FILE"
    _log "L1: presentations dept ABSENT — REMOVED previously-stamped reflex block from $AGENTS_FILE (P2-1)"
  else
    _log "L1: presentations dept ABSENT — NOT stamping reflex block (P2-1)"
  fi
else
...
ROUTE_HELPER_SH
  ...
  _log "L1: stamped signed route helper → $PRES_REFLEX_HELPER_PATH (chmod 700)"

  # Idempotent BY CONTENT (same convention as RESCUE_ESCALATION_BOXNAME_V2 in
  # apply-fleet-standards.sh): marker PRESENCE alone is NOT sufficient — a
  # template body edited in place without a version bump (as happened when the
  # chat-id MANDATORY text below was added) left every already-V2-stamped box
  # frozen at the first-stamped body forever. Render the template fresh every
  # run, then compare it byte-for-byte against whatever is currently stamped;
  # re-stamp on ANY diff (including a stray V1, or no marker at all),
  # no-op only on a true content match.
  PRES_REFLEX_TMPL="$(mktemp)"
...
PRES_REFLEX_V2
  ...
  PRES_REFLEX_VERDICT="$(python3 - "$AGENTS_FILE" "$PRES_REFLEX_RENDERED" <<'PRESCMP_PY'
import os, re, sys
...
PRESCMP_PY
  ...
  case "$PRES_REFLEX_VERDICT" in
    noop)
      _log "L1: PRESENTATION_ROUTING_REFLEX_V2 already current in $AGENTS_FILE — no-op (helper refreshed)" ;;
    restamped|migrated|stamped)
      _log "L1: PRESENTATION_ROUTING_REFLEX_V2 ($PRES_REFLEX_VERDICT — content changed) at absolute top of $AGENTS_FILE" ;;
    *)
      _log "L1: PRESENTATION_ROUTING_REFLEX_V2 stamp FAILED for $AGENTS_FILE (verdict=$PRES_REFLEX_VERDICT) — next roll retries" ;;
  esac
  rm -f "$PRES_REFLEX_TMPL" "$PRES_REFLEX_RENDERED"
fi

if [ "$OC_ROOT" = "/data/.openclaw" ] && [ "$DRY_RUN" = "0" ]; then
