...
# CONTRACT (scripts/ghl-mcp-pin-digest.sh, "CONTRACT FOR scripts/ghl-mcp-vet-pin.sh"):
# this tool MUST NOT reimplement the digest algorithm. It writes the tuple
# fields, then asks the primitive for the digest, then writes that. Honoured
# literally below -- there is no sha256 of the tuple anywhere in this file.
PRIMITIVE="$SELF_DIR/ghl-mcp-pin-digest.sh"
CHECKER="$SELF_DIR/ghl-mcp-check-pin-digest.sh"
[ -f "$PRIMITIVE" ] || envdie "scripts/ghl-mcp-pin-digest.sh is missing — it owns the canonical digest algorithm and this tool will not reimplement it."
[ -f "$CHECKER" ]   || envdie "scripts/ghl-mcp-check-pin-digest.sh is missing — this tool self-checks through it before writing anything."
...
# ── Write the tuple fields IN PLACE ─────────────────────────────────────────
# Each field is rewritten where it already lives, preserving the file's
# structure and the prose that documents each value. The alternative -- moving
# every field into one machine-owned block -- would have thrown away the
# explanation attached to each one, and a pin file nobody can read is a pin file
# nobody checks.
#
# Replaces the LAST `^KEY="…"` assignment, which is the one a `.`-source would
# actually take, and which is what both the primitive and the policy gate read.
# Refuses outright if a key is absent: silently appending a field to a file this
# tool does not understand is how a digest ends up binding a value that no
# consumer reads.
set_field() {
  local file="$1" key="$2" val="$3"
  grep -q "^${key}=\"" "$file" || die "pin file has no ${key}=\"…\" line — refusing to invent one."
  awk -v key="$key" -v val="$val" '
    { lines[NR] = $0; if ($0 ~ "^" key "=\"") last = NR }
    END {
      for (i = 1; i <= NR; i++) {
        if (i == last) {
          # Preserve any trailing ` # comment` that documents the value.
          tail = lines[i]
          sub("^" key "=\"[^\"]*\"", "", tail)
          print key "=\"" val "\"" tail
        } else {
...
WORKPIN="$WORK/pin.env"
cat "$PIN_FILE" > "$WORKPIN"
for _kv in \
  "GHL_MCP_VETTED_COMMIT=$FULL_SHA" \
  "GHL_MCP_PIN_VETTED_VERDICT=$VERDICT_UP" \
  "GHL_MCP_PIN_VETTED_ON=$TODAY" \
  "GHL_MCP_DEPS_LOCK_SHA256=$LOCK_SHA256" \
  "GHL_MCP_REPO_URL=$MIRROR_URL"; do
  _k="${_kv%%=*}"; _v="${_kv#*=}"
  set_field "$WORKPIN" "$_k" "$_v" > "$WORK/pin.next" || die "could not set $_k"
  cat "$WORK/pin.next" > "$WORKPIN"
done
# BY is set separately: it is the one field that may contain '=' characters.
set_field "$WORKPIN" GHL_MCP_PIN_VETTED_BY "$VET_BY" > "$WORK/pin.next" || die "could not set GHL_MCP_PIN_VETTED_BY"
cat "$WORK/pin.next" > "$WORKPIN"

# Clear the digest BEFORE computing, so a stale value can never be hashed into
# the payload by accident. (The primitive ignores the recorded digest when it
# computes, so this is belt-and-braces -- but the belt costs one line.)
set_field "$WORKPIN" GHL_MCP_PIN_VETTED_DIGEST "" > "$WORK/pin.next" || die "could not clear the digest field"
cat "$WORK/pin.next" > "$WORKPIN"

DIGEST="$(bash "$PRIMITIVE" compute "$WORKPIN")" || die "the digest primitive failed"
case "$DIGEST" in
...
set_field "$WORKPIN" GHL_MCP_PIN_VETTED_DIGEST "$DIGEST" > "$SEALED" || die "could not write the digest"
...
sync_fallbacks() {
  local target="$1" f full changed src
  for f in $FALLBACK_FILES; do
    full="$REPO_ROOT/$f"
    [ -f "$full" ] || { echo "   (absent: $f)"; continue; }
    changed=""
    # CRITICAL: only ever rewrite a fallback that ALREADY HOLDS A VALUE.
    #
    # An EMPTY default -- `${GHL_MCP_VETTED_COMMIT:-}` -- is not a fallback
    # waiting to be filled in. It is a deliberate fail-closed decision: v21.6.0
    # deleted the silent hardcoded commit so that a box which cannot read the
    # pin file REFUSES (STATUS: PIN_UNVERIFIED) instead of quietly building
    # whatever constant happened to be baked into the script. A vetting tool
    # that "helpfully" refills it would silently reinstate the exact bypass the
    # gate exists to close, and would do it on every pin bump forever.
    #
    # Hence the 40-char / non-empty patterns below rather than `*`, which
    # matches the empty string and would have done precisely that.
    if grep -Eq '^[[:space:]]*GHL_MCP_VETTED_COMMIT="\$\{GHL_MCP_VETTED_COMMIT:-[0-9a-f]{40}\}"' "$full"; then
      sed "s|^\([[:space:]]*GHL_MCP_VETTED_COMMIT=\"\${GHL_MCP_VETTED_COMMIT:-\)[0-9a-f]\{40\}\(}\"\)|\1${FULL_SHA}\2|" \
        "$full" > "$WORK/fb.tmp" && cat "$WORK/fb.tmp" > "$target/$(basename "$f").out"
      changed="commit"
    fi
    if grep -Eq '^[[:space:]]*GHL_MCP_REPO_URL="\$\{GHL_MCP_REPO_URL:-[^}]+\}"' "$full"; then
      src="$full"
      [ -f "$target/$(basename "$f").out" ] && src="$target/$(basename "$f").out"
      sed "s|^\([[:space:]]*GHL_MCP_REPO_URL=\"\${GHL_MCP_REPO_URL:-\)[^}]\{1,\}\(}\"\)|\1${MIRROR_URL}\2|" \
        "$src" > "$WORK/fb2.tmp" && cat "$WORK/fb2.tmp" > "$target/$(basename "$f").out"
      changed="${changed:+$changed+}repo_url"
    fi
    if [ -z "$changed" ]; then
      echo "   $f — no populated fallback constant (fail-closed); nothing to keep in lockstep"
    else
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  hr
    grep -E '^(GHL_MCP_VETTED_COMMIT|GHL_MCP_PIN_VETTED_(VERDICT|ON|BY|DIGEST)|GHL_MCP_DEPS_LOCK_SHA256|GHL_MCP_REPO_URL)=' "$SEALED"
  exit 0
fi
...
hr
grep -E '^(GHL_MCP_VETTED_COMMIT|GHL_MCP_PIN_VETTED_(VERDICT|ON|BY|DIGEST)|GHL_MCP_DEPS_LOCK_SHA256|GHL_MCP_REPO_URL)=' "$PIN_FILE"
hr
