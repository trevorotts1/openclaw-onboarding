#!/usr/bin/env bash
# =============================================================================
# scripts/loop-protection-canary.sh
# -----------------------------------------------------------------------------
# THE OPERATOR-BOX CANARY for the furnace / loop protection pair
#   Skill 60 (ZHC Early Warning System) + Skill 61 (Loop Protection System).
#
# This is the ONE box that proves the machinery FIRST (SKILL.md law 8:
# CANARY, THEN HOLD). Trevor runs it on HIS box only. It never touches a client
# box, never arms without a 7-day burn-in, and never fires a cron off-box.
#
...
#
# FLEET ROLLOUT IS SEPARATE and comes AFTER this canary passes: flip
...
#
# USAGE:
#   loop-protection-canary.sh install [--skills-dir DIR] [--no-cron]
#   loop-protection-canary.sh verify  [--skills-dir DIR]
#   loop-protection-canary.sh status  [--skills-dir DIR]
#   loop-protection-canary.sh arm     [--yes] [--force]
#   loop-protection-canary.sh disarm
#   loop-protection-canary.sh runbook            # print the full runbook
#   loop-protection-canary.sh --self-test        # offline, sandboxed
...
cmd_install() {
    ...
    _note "operator canary install: Skill 60 + 61 (role=operator, UNGATED, DRY_RUN observe-only)..."
...
try: print(len(json.load(sys.stdin).get("findings",[])))
    ...
    echo "== Loop Protection canary status =="
    echo "  skills dir     : $SD"
...
cmd_arm() {
    ...
    if [ "$role" != "operator" ] && [ "$FORCE" -ne 1 ]; then
        _err "this ledger's role is '$role', not 'operator'. The canary arms the OPERATOR box only."
...
cmd_runbook() {
    cat <<RB
============================================================================
 LOOP PROTECTION CANARY RUNBOOK  (operator box only — Trevor runs this)
============================================================================
 ...
 the updater but HELD fleet-wide (rollout.json). This canary proves them on
 the operator box FIRST (SKILL.md law 8: CANARY, THEN HOLD).

 STEP 1 - INSTALL (idempotent, DRY_RUN observe-only, never arms):
     bash scripts/loop-protection-canary.sh install
   ...
   On VPS run inside the container as the box user:
     docker exec -u node <ctr> bash /data/.openclaw/skills/.../loop-protection-canary.sh install

 STEP 2 - VERIFY (failable, fully offline drill batteries):
     bash scripts/loop-protection-canary.sh verify

 STEP 3 - BURN IN 7 DAYS, then review the observe-only ledger:
     bash scripts/loop-protection-canary.sh status

 STEP 4 - ARM Tier-1 auto-fix (only after burn-in; requires --yes):
     bash scripts/loop-protection-canary.sh arm --yes
   Revert any time:
     bash scripts/loop-protection-canary.sh disarm

 AFTER THE CANARY PASSES — FLEET ROLLOUT (separate, operator-timed, ONE batch):
...
# --------------------------------------------------------------------------- #
self_test() {
    echo "$TAG self-test: sandboxed canary (offline, no cron)"
