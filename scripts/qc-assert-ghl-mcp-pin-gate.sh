...
#      not this gate's.
#   3. Pin-file resolver sync, which is TWO independent invariants and needs
#      both -- neither implies the other:
#      3a. DELIVERY (qc-assert-pin-delivery-paths.sh, which owns that
#          cross-reference): every path any consumer searches is a path an
#          installer actually populates. Without it the pin is a repo-only
#          document -- exactly what shipped in v21.5.0.
#      3b. COVERAGE (qc-assert-ghl-mcp-pin-resolvers.py, which owns that one):
#          every path any consumer searches is ALSO searched by the digest
#          checker. If the checker's list is narrower than a consumer's, the
#          gate and the box read DIFFERENT FILES -- the gate passes on a record
#          the box never sees while the box runs whatever it did find. A gate
#          that validates a different file from the one that gets executed is
#          not a gate.
#      Both consumers' lists being installer-delivered says nothing about
#      whether the checker searches them, so 3a cannot subsume 3b.
...
RESOLVERS_DELIVERY="$REPO_ROOT/scripts/qc-assert-pin-delivery-paths.sh"
RESOLVERS_COVERAGE="$REPO_ROOT/scripts/qc-assert-ghl-mcp-pin-resolvers.py"
...
# ── 3a. Resolver DELIVERY sync ───────────────────────────────────────────────
if [ -f "$RESOLVERS_DELIVERY" ]; then
  if RES_OUT="$(bash "$RESOLVERS_DELIVERY" 2>&1)"; then
    _pass "every consumer's pin-file search path is a path an installer populates"
  else
    _fail "pin-file resolver/delivery drift — a consumer searches a path no installer writes, so the pin never reaches the box:"
    printf '%s\n' "$RES_OUT" | sed 's/^/        /' >&2
  fi
else
  _fail "scripts/qc-assert-pin-delivery-paths.sh is missing — resolver/delivery sync unverified."
fi

# ── 3b. Resolver COVERAGE sync (checker ⊇ every consumer) ────────────────────
# Distinct from 3a and NOT implied by it: 3a proves the paths are delivered,
# this proves the thing that JUDGES the record can see every file a box might
# execute. A narrower checker list means the gate validates one file while the
# box runs another.
if [ -f "$RESOLVERS_COVERAGE" ]; then
  if command -v python3 >/dev/null 2>&1; then
    if RES_OUT="$(python3 "$RESOLVERS_COVERAGE" 2>&1)"; then
      _pass "every consumer's pin-file search path is also searched by the digest checker"
    else
      _fail "pin-file resolver coverage drift — the gate could validate a different file from the one a box reads:"
      printf '%s\n' "$RES_OUT" | sed 's/^/        /' >&2
    fi
  else
    _stop "python3 is not on PATH — resolver COVERAGE sync was NOT verified. This is not a pass."
  fi
else
  _fail "scripts/qc-assert-ghl-mcp-pin-resolvers.py is missing — resolver coverage sync unverified."
fi
