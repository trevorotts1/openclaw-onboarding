...
function create(options={}){
 ...
 const state={id:fixtureId+'_'+(++copySequence),appProperties:{skill35_legacy_brand:body.brandName,skill35_legacy_email:body.clientEmail,skill35_legacy_state:options.state??'initializing'},sheets:initialSheets};
 const result=graph(creator,'Legacy Create Hook',options.body??body,(name,p,prior)=>{
  ...
  if(p.method==='GET'){const url=new URL(p.url);if(name==='Read Legacy Permissions')assert.equal(url.pathname,'/drive/v3/files/'+state.id+'/permissions');else if(name==='Read New Copy Tabs')assert.equal(url.pathname,'/v4/spreadsheets/'+state.id);else throw Error('Unapproved legacy GET or discovery request');}
  if(p.method!=='GET')writes.push({name,url:p.url,payload});
  switch(name){
   case 'Copy Legacy Template':assert(p.url.includes('__LEGACY_TEMPLATE_ID__/copy'));state.appProperties=payload.appProperties;if(options.failAfterCopy)throw Error('Copy response lost');return {id:state.id,mimeType};
...
test('New legacy copy clears every tab before sharing then returns document-only receipt',()=>{const r=create();assert.equal(r.status,200);assert.equal(r.body.company_registration_verified,false);assert.equal(r.body.publication_verified,false);assert.equal(r.body.sheetId,r.state.id);assert(!JSON.stringify(r.state.sheets).includes('SAMPLE'));assert(r.trace.indexOf('Mark Legacy Formatted')<r.trace.indexOf('Share Legacy Copy'))});
test('Identical legacy requests always create distinct fresh copies',()=>{const first=create(),second=create();assert.equal(first.status,200);assert.equal(second.status,200);assert.notEqual(first.body.sheetId,second.body.sheetId);for(const r of [first,second]){assert.equal(r.body.deduped,false);assert.match(r.body.retry_policy,/reconcile_execution_before_retry/);assert.equal(r.trace.filter(n=>n==='Copy Legacy Template').length,1)}});
test('Brand and email cannot look up or disclose any existing document',()=>{const serialized=JSON.stringify(creator);assert(!serialized.includes('Read Legacy Copies'));assert(!serialized.includes('Resolve Legacy Copy'));assert(!serialized.includes('Legacy Copy Exists'));assert(!serialized.includes('appProperties has'));assert(!serialized.includes('/files?q='));for(const node of creator.nodes.filter(n=>n.type.endsWith('.httpRequest')&&n.parameters.method==='GET'))assert(!/^https:\/\/www\.googleapis\.com\/drive\/v3\/files[?/]?$/.test(node.parameters.url));const r=create({files:[{id:'unrelated_existing_document',mimeType}],existing:true,state:'ready'});assert.equal(r.status,200);assert.notEqual(r.body.sheetId,'unrelated_existing_document');assert(r.trace.includes('Copy Legacy Template'));assert(!r.trace.some(n=>n.includes('Copies')))});
test('Fresh copied document with any non-owner access refuses destructive clear',()=>{for(const permission of [{type:'anyone',role:'writer'},{type:'user',role:'reader'},{type:'group',role:'writer'}]){const r=create({permissions:[permission]});assert.equal(r.status,422);assert.deepEqual(r.writes.map(w=>w.name),['Copy Legacy Template']);assert(JSON.stringify(r.state.sheets).includes('SAMPLE'))}});
test('Lost copy response fails once and does not run automatic recovery lookup',()=>{const r=create({failAfterCopy:true});assert.equal(r.status,422);assert.equal(r.body.success,false);assert.equal(r.writes.length,1);assert.equal(r.trace.filter(n=>n==='Copy Legacy Template').length,1);assert(!r.trace.includes('Read Legacy Permissions'))});
...
test('Caller-supplied existing ID or recovery identity never changes fresh-copy semantics',()=>{for(const extra of [{sheetId:'existing_document'},{recovery_token:'pretend_recovery_token'},{existing:true}]){const r=create({body:{...body,...extra}});assert.equal(r.status,422);assert.equal(r.writes.length,0)}});
test('Mixed modern create and injected input are rejected before lookup',()=>{for(const b of [{...body,company_id:null},{...body,templateSheetId:'other'}, {...body,brandName:'line\nbreak'}, {...body,clientEmail:'bad'}, {...body,brandName:'x'.repeat(200)}]){const r=create({body:b});assert.equal(r.status,422);assert.equal(r.writes.length,0);assert(!r.trace.includes('Copy Legacy Template'))}});
