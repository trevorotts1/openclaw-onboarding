...
class Compatibility(unittest.TestCase):
    ...
    def test_security_floor_and_resolver(self):
        ...
        self.assertEqual(compat['commandCenter']['minVersion'],'v7.1.2')
        self.assertEqual(resolve_cc_tag(compat),'v7.1.2')
        ...
        guard.assert_cc_package({'version':'7.1.2'})
        compat['commandCenter']['pinnedTag']=None
        self.assertEqual(resolve_cc_tag(compat,['v7.0.0','v7.1.2']),'v7.1.2')
    ...
    def test_cli_node_and_checkout_fail_closed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            package.write_text('{"version":"7.1.2"}')
            env={**os.environ,'PATH':str(directory)}
            for node_version,cc_version,expected in [('v22.12.0','7.1.2',1),('v22.13.0','7.0.0',1),('v22.13.0','7.1.0',1),('v22.13.0','7.1.1',1),('v22.13.0','7.1.2',0)]:
    ...
    def test_fleet_blocks_before_fetch_on_old_node_and_before_deploy_on_old_checkout(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(guard,'check_node',side_effect=ValueError('unsupported Node')), patch.object(runner.subprocess,'run') as run:
                runner.step_pull_cc({'cc_dir':directory},'v7.1.2',result,False)
