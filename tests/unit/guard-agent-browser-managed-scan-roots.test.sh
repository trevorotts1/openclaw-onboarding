...
# Area 2 / B.2 negative-fixture proof.
#
# Before this fix, scripts/guard-agent-browser-managed.sh's MANAGED-ONLY (1) and
...
# or 03-agent-browser/ passed the guard SILENTLY (guard exit 0). This test
# proves the guard now catches it in BOTH newly-scanned roots.
#
...
echo ""
echo "--- (A) baseline: minimal tree (incl. empty 41-*/03-* roots) passes ---"
...
echo ""
echo "--- (B) planted unmanaged spawn in 41-* AND 03-* -> guard MUST FAIL on both ---"
...
B_EXIT=$?
if [[ "$B_EXIT" -ne 0 ]]; then
  pass "(B) guard FAILS (exit $B_EXIT) with both fixtures planted"
else
  fail "(B) guard did NOT bite with unmanaged spawns planted in 41-*/03-* (exit 0 -- the guard is still blind to these roots)"
fi
...
C_EXIT=$?
if [[ "$C_EXIT" -eq 0 ]]; then
  pass "(C) guard PASSES again after fixtures removed (fixtures didn't leave residue)"
else
