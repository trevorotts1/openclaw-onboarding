...
echo "$out" | grep -qi "5th member" && pass "triad message names the 5th member (embedded asset)" \
    || fail "triad message did not name the 5th member"
...
grep -qi "asset_rebuild_required=true" "$SB/e.out" && pass "provision names asset_rebuild_required=true as the skip reason" \
    || fail "provision skip reason did not name asset_rebuild_required"
case "${_PIDX_SKIP_WARNINGS:-}" in
    *asset_rebuild_required=true*) pass "reason surfaced in _PIDX_SKIP_WARNINGS accumulator" ;;
