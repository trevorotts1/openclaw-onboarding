...
# U120 fleet rollout -- proves the SOP library skip gate verifies canonical
# membership by identifier, not bare count.
set -uo pipefail
...
{"asset":"sops-library-v2.jsonl.gz","release_tag":"fixture-tag","sha256":"$sha","canonical_sop_count":$CANON,"probe_sop_ids":["sop_fixture_sop_000"]}
EOF
}
...
[ "$BEFORE_A" = "$CANON" ] && ok "FAIL-BEFORE: box has $BEFORE_A rows (>= canonical $CANON) -- ALL non-library ids" || bad "expected $CANON, got $BEFORE_A"
...
[ "$AFTER_A" -gt "$BEFORE_A" ] && ok "PASS-AFTER: box GAINED rows ($BEFORE_A -> $AFTER_A)" || bad "box did not gain any rows"
grep -q "canonical-members" "$A/out.log" && ok "log: mentions the membership count" || bad "no membership count in log"
grep -q "non-library source" "$A/out.log" && ok "log: identifies non-library source" || bad "no non-library source message"
...
[ "$FULL_B" = "$EXPECT_B" ] && ok "canonical box: $FULL_B rows = $LIB_RECORDS library + $STARTER_ROWS starters" || bad "expected $EXPECT_B, got $FULL_B"
[ "$MEMBER_FULL" = "$CANON" ] && ok "canonical box: $MEMBER_FULL membership rows = canonical $CANON" || bad "expected $CANON, got $MEMBER_FULL"
[ "$RC_B" = "0" ] && ok "canonical box: ingest exits 0" || bad "rc=$RC_B"
[ ! -f "$B/bin/.curl-was-called" ] && ok "canonical box: NO download (skip gate)" || bad "downloaded anyway"
[ "$SUM_BEFORE" = "$SUM_AFTER" ] && ok "canonical box: DB UNCHANGED" || bad "DB was modified"
grep -q "membership verified" "$B/out.log" && ok "canonical box: membership verified" || bad "no membership verification"
...
[ "$RC_C" = "0" ] && ok "force mode: ingest exits 0" || bad "rc=$RC_C"
[ "$AFTER_C" -gt "$BEFORE_C" ] && ok "force mode: library downloaded" || bad "rows did not increase"
...
[ "$RC_D" = "0" ] && ok "under-populated: ingest exits 0" || bad "rc=$RC_D"
[ "$AFTER_D" = "$EXPECT_D" ] && ok "under-populated: reached $EXPECT_D rows (was $BEFORE_D)" || bad "expected $EXPECT_D, got $AFTER_D"
