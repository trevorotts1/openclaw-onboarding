...
## 9. CAF RELEASE-TEMPLATE FAMILY (scripts/u10_u13_workflows/)

The thirteen JSON documents in `59-anthology-engine/scripts/u10_u13_workflows/`
(seats 1..13, generated by `build_anthology_workflows.py`) are the Convert-and-Flow
template DATA consumed by the Skill 44 caf build rail
(`config/anthology-snapshot-contract.json` workflows.release_notifications +
tag_to_notification). They are NOT n8n or GHL workflow JSON exports and must never
be imported as such; `scan-no-json-exports.sh` enforces this over the directory.

Seat numbers are FILENAME SEATS, not pipeline chronology: the release-outline seat
13 fires BEFORE the release-chapter seat 06 in the participant journey. Read the
per-seat `trigger` field, never the number, for firing order.

Lifecycle statuses (`status` field per file):

| Status | Meaning |
|---|---|
| LIVE | Generated template is wired into the live snapshot contract and fires today. |
| WIRED-AHEAD | Template generated and validated ahead of need; not yet firing in the live journey. |
| DOCTRINE | Documented contract only; the seat is described but intentionally not built. |
| PRODUCER-NOTIFY | Producer-facing notification seat (email-only by family law); rides its own trigger. |
| OWNED-ELSEWHERE | The seat's real template lives outside this family (seat 12 intake fire is owned by the U02/U05 tooling's live webhook-to-route mapping). |
