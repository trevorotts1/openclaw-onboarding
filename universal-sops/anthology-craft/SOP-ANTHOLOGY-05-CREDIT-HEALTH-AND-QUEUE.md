...
**Enforcement pointer:** the smoke test plus `guard-cron-inventory.py`, together this SOP's enforcement pointer: `59-anthology-engine/scripts/anthology-smoke-test.py` (self-metering at or under one cent per run, the funded-reachability probe, the hold-queue age tick and drain trigger) AND `59-anthology-engine/scripts/guard-cron-inventory.py` (exactly one recurring engine job, daily cadence, no heartbeat, and a churn sweep for a departed producer's leftover jobs), with founder notices routed only through `alert-dedup.py`.
