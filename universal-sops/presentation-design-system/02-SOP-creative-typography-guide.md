...
> Grounds the deck's typography in real type practice so it is a DESIGNED system, not a default one. The forensic reference deck used one device (a black headline with a single accent word) on about 40 of 45 slides: no weight ladder, no expressive display type, no per-slide treatment. This guide encodes the weight ladder, expressive display rules, and hierarchy that the Typography Architect locks and the prompt writer renders.
...
| The same single-device look (black headline + one accent word) is used on more than 70% of slides | AUTO-FAIL (deck-level): the reference-case cookie-cutter pattern |
...
**FAIL (reference failure case, deck-wide):** essentially one device, a black headline with a single accent word, reused on about 40 of 45 slides. No weight ladder. No expressive display. The only good idea (the gold anchor numeral on one slide) was not carried to the later price beats, which landed flat. Verdict: deck-level AUTO-FAIL (single-device over 70%) plus per-slide FAIL on the flat price beats.

**PASS (gold-standard reference proof):** a locked Montserrat weight ladder (BLACK headlines, ExtraBold sub-heads, Bold labels, Medium/Italic body), hero numbers in metallic gold gradient, the live price glowing in raspberry-pink, dead prices double-struck in gold, per-word color emphasis ("EVER" in gold, "RIGHT NOW" underlined), and giant faint watermark numerals on section reveals. Every price beat carried the gold treatment, not just one. Verdict: PASS.
...
- The gold-standard reference design proof (the locked Montserrat weight ladder, gold gradient, glow, strikethrough, watermark numerals, per-word emphasis) is the empirical model this guide encodes.
