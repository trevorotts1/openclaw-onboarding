# Presentation Design System SOP Cluster (`universal-sops/presentation-design-system/`)
...
This craft cluster (`universal-sops/presentation-design-system/`) is the execution playbook for the skill(s) below. A specialist reaches for it when the client's plain-language request matches any of these intents — the client never has to name the skill or type its slash command. Source of truth: `23-ai-workforce-blueprint/skill-department-map.json` (Layer D).

| Skill | Reach for this craft when the client says… |
|---|---|
| **45** design-intelligence-library | "match this brand style" · "on-brand images" · "a style card" · "generate images in my brand look" |
