...
**Status:** DRAFT for overhaul - POPULATE + WIRE existing skill 45; do not rebuild it
...
1. Defines the **seeding step** that fills the empty library by running a reference deck (the gold-standard reference run, and any client reference deck) through the analyzer. (Closes GAP a.)
...
### 4.1 Bootstrap seed - run the gold-standard reference run through the analyzer (one-time, fleet baseline)

Run ONCE per box during onboarding so every client starts with at least one production-status reference style.

**Source (the verified gold standard, the 75-slide reference prompt set, NOT any text-free abstract PNG deck):** the proven 75-slide reference prompt set provided to the box at onboarding (a structured per-slide spec). Do NOT seed from a text-free abstract tech deck even if one is present alongside it.
...
|---|---|---|
| Trigger fired but INDEX.md empty (check 1) | CDO schedules a New-Client Calibration Run (SOP-DIU-613) to analyze the client's reference deck, OR seeds from the gold-standard reference bootstrap (§4.1) if the client has no reference. Do NOT proceed to prompts with an invented style on a style-match deck. | Director of Presentations + CDO |
