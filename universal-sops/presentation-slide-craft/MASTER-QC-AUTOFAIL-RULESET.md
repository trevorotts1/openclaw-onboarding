...
**COVERAGE, STATED HONESTLY (U051, updated by FIX 18).** A row in this table means the rule is DECLARED. It does not mean the rule is MEASURED. Of the 27 numbered craft triggers in SOP-SLIDE-01/02/03/04 plus the 4 craft-prefixed codes that belong to other batteries — 31 rows in total — 20 carry a resolving enforcer (the FIX 15 ten plus the FIX 18 five, enumerated in their Detection cells), 2 are enforced under another code's name and say so in their Detection cell, 1 is owned by a unit not yet merged, 6 are WARNING-with-disposition heuristics (they may warn, never claim a deterministic auto-fail; each holds the owning QC phase — P1Q-COPY-QC raises AF-WARNING-HOLD at attestation, non-crash, resumable — until corrected or acknowledged per-rule per-slide via working/qc/craft-warning-dispositions.json, with the pending hold persisted in working/qc/craft-warnings.json), and 2 remain decided by a human (marked HUMAN VERDICT REQUIRED in their Detection cell — AF-OBI-4 and AF-OBI: an independent reviewer's pass/fail is recorded in working/qc/human_craft_verdicts.json and a missing verdict blocks the relevant QC attestation). Checksum: 20 + 2 + 1 + 6 + 2 = 31. A Detection cell that begins HUMAN VERDICT REQUIRED is a promise that a person — not the builder — reads the slide; a cell that says the rule warns promises exactly that and no more — a warning HOLDS the QC phase until dispositioned, it never auto-fails the scan.

| Code | Stage | Level | Trigger (one line) | Detection |
| AF-SLIDE-CRAFT-LOADER | Preflight | DECK | slide_craft.py not importable next to run, or run_all_checks raised (FIX 15 enforcer loader) | build_deck._chk_slide_craft: importable + run_all_checks smoke prior to render; any load/raise = fail (waivable only via owner token per slide_craft_waivers.json) |
| AF-CRAFT-JUDGEMENT-LOADER | Preflight | DECK | craft_judgement.py not importable next to run, or run_all_checks raised (FIX 18 craft-judgement disposition loader) | build_deck._chk_slide_craft: importable + run_all_checks smoke prior to render; any load/raise = fail (waivable only via owner token per slide_craft_waivers.json) |
|---|---|---|---|---|
| AF-HOOK-1 | 1Q/5/6 | DECK | hook on > 4 slides | count fuzzy-matched hook occurrences across deck |
| AF-HOOK-2 | 1Q/5/6 | slide | hook in a footer/band position | craft_judgement.check_hook_footer (FIX 18, blocking via build_deck._chk_slide_craft): the slide's DECLARED `layout` string is the evidence — a slide whose layout declaration names footer/band/lower-third/caption-band placement while carrying the canonical hook (intake.json hook key) fails. The trigger text above is preserved: the prompt half remains AF-HOOK's job; this check owns the declared-layout half. Defers when no hook or no slides.json exists. Owner-token waiver applies (slide_craft_waivers.json). |
| AF-HOOK-3 | 1Q | DECK | zero dedicated hook slides | ENFORCED VIA AF-NO-HOOK-REFRAIN — intelligence_engines_check._check_hook_refrain_copy fails at fewer than 3 hook-carrying slides, which contains 0. No separate enforcer; a second one would double-fire. |
| AF-HOOK-4 | 1Q/5 | slide | hook printed 2+ times on one slide | per-slide hook occurrence count >= 2 |
| AF-HOOK-5 | 1Q/5/6 | slide | hook mutated/extended/reworded | build_deck._chk_hook_verbatim → slide_craft.check_hook_verbatim: char-exact match of the canonical hook (intake.json hook key) in every copy block; near-variants caught via difflib.SequenceMatcher at threshold 0.82 (prompt_gate.py OCR_MATCH_RATIO). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-HOOK-6 | 5/6 | slide | hook misspelled/garbled on render | COVERED BY AF-SPELLING (U047, not yet merged) — hook misspelled/garbled on render is slide_geometry.check_spelling restricted to the hook line. |
| AF-HOOK-7 | 1Q/5 | slide | signature quote conflated with main hook | craft_judgement.check_hook_signature_quote (FIX 18, blocking via build_deck._chk_slide_craft): a slide carrying the canonical hook that is either DECLARED a signature-quote slide (arc_allocation slot boolean/label) or also contains the intake signature_quote text fails — the original trigger (conflation) preserved, now measured from declared evidence instead of a missing schema label. Defers without a hook/quote; waiver applies. |
| AF-AUD-1 | 1Q/5 | slide | speaker SAY line on the face | WARNING (FIX 18) — craft_judgement computes_warnings: the literal SAY:/Speaker:/You-say: directive patterns warn per slide (first-person phrasing is deliberately NOT a pattern — that defect remains U012's producer gap). HOLDS THE QC PHASE: an unacknowledged warning blocks P1Q-COPY-QC attestation (run_signature_deck.attest_phase exits 2, AF-WARNING-HOLD, non-crash, resumable) until corrected or individually acknowledged — one record, EXACTLY {rule_code: AF-AUD-1, slide_ids, run_id, reviewer, rationale, decision, captured_at, owner_signature} in working/qc/craft-warning-dispositions.json, one disposition covers ONE rule and its named slides, never a blanket ack (the pending hold is persisted in working/qc/craft-warnings.json). The mechanical scan (qc_check) stays exit-neutral. |
| AF-AUD-2 | 1Q/5 | slide | internal pitch doctrine as caption | WARNING (FIX 18) — craft_judgement computes_warnings: literal doctrine tokens (AF-code names, "auto-fail code", "QC gate", the named doctrine phrases) inside a caption/footer/band/note line warn; paraphrase detection remains out of scope (the SOP's Section 4.3 principle restated in fresh words is a human read). HOLDS THE QC PHASE (AF-WARNING-HOLD at P1Q-COPY-QC attestation, same mechanism as AF-AUD-1) until corrected or acknowledged per-rule per-slide in craft-warning-dispositions.json. Never exit-1 from the scan. |
| AF-AUD-3 | 1Q/5 | slide | image-narration caption | WARNING (FIX 18) — craft_judgement computes_warnings: "[IMAGE: … shows/depicts/…]"-class literal narration captions warn. Cross-modal semantics (whether the image ACTUALLY shows it) remain senior human/vision work — the warning names the pattern, never claims the semantic verdict. HOLDS THE QC PHASE (AF-WARNING-HOLD at P1Q-COPY-QC attestation) until corrected or acknowledged. Never exit-1 from the scan. |
| AF-AUD-4 | 1Q/5 | slide | meta-telegraph / "webinar" / technique label | build_deck._chk_aud_meta_tokens → slide_craft.check_aud_meta_tokens: case-insensitive substring match of the literal word 'webinar' and the named format/technique-announcement lines (SOP-SLIDE-02 §2.4/§3). LITERAL TOKENS ONLY: the check matches the word "webinar" and the named announcement lines. An unlisted meta line is not detected and remains HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-AUD-5 | 1Q/5 | slide | credential/justification dump | build_deck._chk_aud_credentials → slide_craft.check_aud_credentials: credential-marker substring match in NON-HEADLINE copy blocks only (SOP-SLIDE-02 §3 AUD-5). copy[0] is exempt — a headline naming a credential is a design choice the doctrine does not ban. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-PLACEHOLDER (AF-AUD-6) | 5/6 | slide (blocks FINAL) | bracket/placeholder token on a rendered slide | build_deck._chk_aud_placeholder_render → slide_craft.check_aud_placeholder_render: \[[^\]]*\] regex + token substrings on OCR'd rendered text (renders/slide-*.ocr.json). RENDERED, not copy — SOP-SLIDE-02 §2.6 permits placeholders at the copy stage. Defers when OCR is unavailable. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-OBI-1 | 1Q/5 | slide | > 3 text blocks | build_deck._chk_obi_text_blocks → slide_craft.check_obi_text_blocks: count of non-empty copy blocks per slide; ceiling 3 (SOP-SLIDE-01 §2.3). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-OBI-2 | 1Q | slide | headline > 9 words | build_deck._chk_obi_headline_words → slide_craft.check_obi_headline_words: exact word count of slides.json copy[0]; ceiling 9 (SOP-SLIDE-01 §2.4). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-OBI-3 | 1Q | slide | 2+ core ideas | WARNING (FIX 18) — craft_judgement computes_warnings: two headlines on one slide whose word-set jaccard < 0.3 warns (same transparency the retired heuristic had), but the warning explicitly names itself a heuristic and HOLDS THE QC PHASE (AF-WARNING-HOLD at P1Q-COPY-QC attestation) only until corrected or acknowledged per-rule per-slide in craft-warning-dispositions.json; the QC Specialist's own distinct-claim read (SOP-SLIDE-01 §3) decides. Never exit-1 from the scan. |
| AF-OBI-4 | 1Q | slide | full value trio on one slide | HUMAN VERDICT REQUIRED (FIX 18) — NO keyword proxy exists or will exist (a marker list would fire on any three-noun headline). A relevant QC phase (P1Q-COPY-QC / P-PROMPT-QC) cannot attest (run_signature_deck.attest_phase exits 2) until working/qc/human_craft_verdicts.json carries a pass/fail verdict for AF-OBI-4 by an independent reviewer (builder identities rejected) with rationale; a "fail" verdict blocks outright. Never presented as measured automation. |
| AF-OBI-5 | 1Q | slide | bulleted pain list | WARNING (FIX 18) — craft_judgement computes_warnings: three or more pain-marked bullets warn (literal pain-token match — the same honesty limit the retired scanner had: pain-vs-feature is semantic and stays human). HOLDS THE QC PHASE (AF-WARNING-HOLD at P1Q-COPY-QC attestation) until corrected or acknowledged per-rule per-slide in craft-warning-dispositions.json. Never exit-1 from the scan. |
| AF-OBI-6 | 5 | slide | comparison table > 2 rows | craft_judgement.check_comparison_rows (FIX 18, blocking via build_deck._chk_slide_craft): the deck's copy IS its text face — the pipeline renders copy verbatim (AF-OVERLAY-DELIVERED) — so pipe-table data rows (header separator excluded) per slides.json copy block are countable deterministically; > 2 data rows on one slide fails, naming the slide. A declared comparison_rows > 2 in arc_allocation also fails. Glyph analysis on pixels is NOT claimed; a rendered-text-layer check remains AF-PLACEHOLDER's class. Waiver applies. |
| AF-DEN-1 | 1Q/6 | DECK | price beats < 8 slides apart | build_deck._chk_den_ladder_gaps → slide_craft.check_den_ladder_gaps: minimum 8-slide gap between adjacent price-beat rungs (SOP-SLIDE-04 §2.1). Client's client_requested_slide_count wins — message says re-space, never lengthen. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-2 | 1Q/6 | DECK | anchor outside 25-45% depth | build_deck._chk_den_anchor_depth → slide_craft.check_den_anchor_depth: anchor slide position / total slides in [25%, 45%] (SOP-SLIDE-04 §3 DEN-2 trigger row; §2.2 prose 30-40% is the TARGET, not the trigger — divergence recorded). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-3 | 1Q/6 | DECK | DROP with no BUILDUP before it | craft_judgement.check_drop_buildup (FIX 18, blocking via build_deck._chk_slide_craft): the offset-window objection above is retired by reading ORDINALS from the Director's own allocation — arc_allocation.json slots carry slide + arc_section; a DROP-tagged slide whose immediately preceding ordinal (slide-1) is not a BUILDUP-tagged slot fails, naming the drop slide. Defers when no DROP is tagged (pitchless deck) or no arc_allocation exists. Waiver applies. |
| AF-DEN-4 | 1Q/6 | DECK | no value-stack slide before Drop 1 | build_deck._chk_den_stack_before_drop → slide_craft.check_den_stack_before_drop: min(ARC-marker offset of VALUE_STACK tags) < min(offset of DROP tags). ORDER ONLY: the check proves a value-stack beat precedes the first DROP. It does NOT prove the stack is itemized or that its total exceeds the anchor (SOP-SLIDE-04 §2.4) — those remain HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-5 | 1Q/6 | DECK | no promises beat before anchor | ENFORCED VIA AF-PRICE-BEFORE-PROMISE — pitch_engines_check.chk_promise_before_price already requires a PROMISE beat before the first price beat, and ANCHOR is in its price-token set. |
| AF-DEN-6 | 1Q/6 | DECK | Wall of Wins not 4-6 before offer | craft_judgement.check_wall_of_wins (FIX 18, blocking via build_deck._chk_slide_craft): the WALL_OF_WINS ordinal comes from arc_allocation.json slot tags (not a price_ladder rung — that gap is closed the other way), the offer slide from the first OFFER/ANCHOR/DROP/FINAL-tagged slot or price_ladder rung; gap outside [4, 6] fails with the computed gap named. Defers without a WOW slot or any offer slot. Waiver applies. |
| AF-DEN-7 | 1Q/6 | DECK | no 4-7 slide re-pitch after FINAL | build_deck._chk_den_repitch_block → slide_craft.check_den_repitch_block: total_slides - final_rung.target_slide in [4, 7] (SOP-SLIDE-04 §2.7). COUNT ONLY: the check proves 4-7 slides follow the FINAL price. It does NOT prove they recap the stack, restate the promises or reset the urgency — those remain HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-8 | 1Q/6 | DECK | section below its slide floor | WARNING (FIX 18) — the §2.8 objection stands: no scaling function exists and §5.0's client_requested_slide_count override is honored (craft_judgement suppresses the warning entirely when the client fixed the count). Present-tagged sections in arc_allocation with slot counts below the QC-side floor table warn with the count and floor named, AND the warning attributes the section's OWN slide ordinals so the per-warning hold can be individually acknowledged (an empty slide_ids disposition is refused fail-closed and clears nothing) — advisory, HOLDS THE QC PHASE (AF-WARNING-HOLD at P1Q-COPY-QC attestation) until corrected or acknowledged. Never exit-1, never treated as a computable auto-fail gate. |
...
| AF-WEBINAR-SIZE | 8.92 | DECK | webinar video absent, zero-byte, not a real MP4 (no 'ftyp' box), or over the size gate (> 500MB GHL v3 video-upload ceiling, hard fail > 900MB) | build_webinar_video.gate_size (P9.6-WEBINAR-VIDEO executor, Feature L2-G): ffprobe the rendered mp4, fail loud above WEBINAR_TARGET_MAX_BYTES (500MB) and hard-fail above WEBINAR_HARD_MAX_BYTES (900MB); the phase's runner-side verifier (phase_verifiers._verify_webinar_video) additionally enforces exists / non-empty / ftyp-magic / timing-track at attestation time |
| AF-WEBINAR-INTRO | 8.54 | DECK | the webinarized speech audio (P9-SPEECH-WEBINAR-INTRO, Feature L2-H) is absent, zero-byte, not a valid MP3, or was produced WITHOUT the host framing: the executor must pass --webinar-intro-outro to synthesize_full_speech.py so a ~5-min WELCOME section is spoken before Slide 1 and a ~2-min CHAT Q&A + ~3-min CRESCENDO CLOSE after the last slide (generated by webinar_intro_outro.py, Fish-tagged; the on-disk UNTAGGED deck is never modified) | synthesize_full_speech --webinar-intro-outro (P9-SPEECH-WEBINAR-INTRO executor, Feature L2-H): the manifest-authored executor cmd carries --webinar-intro-outro so the framing words count in the duration gate (the audio really is longer); the phase's runner-side verifier (phase_verifiers._verify_webinarized_speech) additionally enforces the mp3-exists / non-empty / MP3-validity-probe half AND the framing-structure half (webinar_intro_outro.build_framing_markdown carries WELCOME/QNA/CLOSE sections in word band) at attestation time — a plain deck-only audio renamed as the webinar audio is a defect |
| AF-WORKBOOK-PROMPT-NO-CONTENT | 8.25 | DECK | workbook page prompt (P8.25-WORKBOOK, Feature L2-D redesign) carries the wireframe directive ("BACKGROUND ONLY", "NO text", "NO labels", "NO words") OR is missing any of that page's content_strings[] verbatim | workbook_mapper._assert_content_in_prompt (P8.25-WORKBOOK executor, Feature L2-D redesign): rejects the prompt pre-submit (fail-closed — no wireframe page is ever rendered). The wireframe-language scan is a literal ban; the verbatim check mirrors build_deck's AF-P-VERBATIM. |
| AF-WORKBOOK-EMPTY | 8.25 | DECK | a rendered workbook page's OCR read-back finds < 100% of that page's content_strings[] (a bare wireframe page has zero content to find) | workbook_builder.verify_content (P8.25-WORKBOOK executor, Feature L2-D redesign): render each page to pixels and OCR-read back every content string; any page under 100% FAILS and the phase does not attest. |
| AF-WORKBOOK-BOTH | 8.25 | DECK | the dual-PDF workbook contract is broken: either the regular (*-WORKBOOK.pdf) or the fillable (*-WORKBOOK-FILLABLE.pdf) is missing / zero-byte / fails pypdf read-back (pages + fields + /NeedAppearances true) | phase_verifiers._verify_workbook (P8.25-WORKBOOK runner-side substance verifier, Feature L2-D redesign): both PDFs must exist, be non-trivially sized, and pypdf must read the expected pages/fields; the fillable must read /NeedAppearances true. |
| AF-CAPACITY-UNMEASURED | 0 (pre-dispatch) | DECK | the engine was dispatched, or an execution plan was built, without a capacity measurement for THIS client's account — the probe returned PARKED (provider detected, plan never declared) or FAILED (a declared capacity_override.json is unusable), so no dispatchable number exists. Cap table, exact: ollama-cloud $20/month = 3 concurrent agents, ollama-cloud $100/month = 8, deepseek-direct v4-pro = 500, deepseek-direct v4-flash = 2500. The $100/month row is 8 and NOT the raw seat maximum of 10 ON PURPOSE: operator ruling 2026-09-04, restated verbatim 2026-09-07 -- "ollama 20 dollar plan is 3 agents max ollama 100 a month plan is 8" and "i said 8 so that 2 are left over for other work". The seat allows ten; the presentation job takes eight of them on BOTH axes (width and rate) so the client keeps two slots for whatever else runs on the same Ollama account. capacity.CAP_TABLE[(ollama-cloud, $100/month)] = 8 and governor.PLAN_TIER_RPS["$100/month"] = 8.0 are the two places that number lives, and governor.PLAN_TIER_RPS["10"] = 8.0 maps the ten-slot plan onto the same reserve. "Correcting" this row back to 10 does not fix a typo -- it silently spends the operator's reserve; an unknown provider or plan collapses to DEFAULT_CONSERVATIVE = 3 and is NEVER guessed upward | presentation_job/capacity.py::probe detects provider+plan read-only, and it resolves PER PROVIDER -- never client-wide. Step 1, establish WHICH provider is being asked about: (a) the provider the caller names (the routed provider, for a per-route width), else (b) the 9Router configuration (which provider the primary model routes to), else (c) the OpenClaw agent model configuration (provider namespace prefix on the primary model), else (d) the provider a declared capacity_override.json names, when nothing else identified one. Step 2, resolve THAT provider's plan, first hit wins: its model slug, else its own locked entry in resource_profile.json (THE store of interview answers), else the capacity_override.json sub-record for that same provider, else a NO_CAP_PROVIDERS hit → UNBOUNDED; a cap-table provider with no plan OF ITS OWN → emit the one-time interview question and PARK; nothing found at all → DEFAULT_CONSERVATIVE plus a loud UNDETERMINED line. The ask-once gate is the resource_profile.json lock for that provider, NOT the existence of an override file: the interview answer is never projected into a client-wide capacity_override.json, because one provider's plan answer sitting in a client-wide file is exactly what capped every OTHER provider's routes (a two-provider client can have no single correct global answer). capacity_override.json survives only as an operator self-throttle -- a declared max_concurrent that can LOWER, never raise, and only for the provider the record names. No step ever reads a credential VALUE. The teeth: launcher.dispatch calls the probe BEFORE any engine process is created and returns DISPATCH_CAPACITY_REFUSED (CLI exit 3) when the number is absent, and execution_plan.build_execution_plan raises CapacityUnmeasured rather than substituting a constant — wave width is min(ready phases, measured available), so a 5-phase independent wave on a measured 3 dispatches 3 then 2 |
...
| AF-PROMPT-NAME | Phase 3 (Prompt Authoring) | slide | a slide prompt file in working/prompts/ has a NON-canonical name (prompt_gate scan_prompt_dir / prompt_dir_problems verdict) | R3 canonical family = exactly 2 or 3 digits: slide-01..slide-99, slide-100..slide-999, plus the -prompt variants; a 1-digit (slide-1.txt) or 4+ digit (slide-0100.txt, slide-1000.txt) name fails. R3 overlay: build_deck._canonical_prompt_dir_problems re-judges the shared verdict, keeping the failure only when a flagged name's ordinal FIELD is still non-canonical; a message whose only offender is now-canonical (e.g. slide-100.txt) is dropped |
| AF-PROMPT-DUP-FILE | Phase 3 (Prompt Authoring) | slide | two or more prompt files in working/prompts/ target the SAME slide ordinal (prompt_gate scan_prompt_dir / prompt_dir_problems verdict; e.g. slide-01.txt vs slide-1.txt, slide-100.txt vs slide-0100.txt) | R3 passes duplicate verdicts through UNCHANGED: the %02d and %03d families are the SAME spelling for ordinals 100+ and target the same slide for ordinals 1-99, so a collision is ALWAYS same-ordinal — exactly one canonical file per slide stays the rule at any digit width (build_deck._canonical_prompt_dir_problems) |
...
| AF-QC-PLACEHOLDER | QC | DECK | A QC report for one of the four QC phases (copy / prompt / typography / shift) is a placeholder or sub-floor \u2014 missing, <= 256 bytes (the 3-byte \"{}\" rubber-stamp), not valid JSON, or carrying fewer than 20 real per-slide verdicts. FIX-2 makes QC structurally unskippable + real-content-floor: a QC phase cannot attest without a real report | working/qc/{copy,prompt,typography,priority_shift}_qc_report.json: file exists, size > 256 bytes, valid JSON, >= 20 real per-slide verdicts (per_slide_scores / slides / results / items); any miss fails the QC phase attestation (AF-QC-PLACEHOLDER) |
| AF-QC-SKIP | QC | DECK | A phase-skip approval record (phase_skip_approvals.json) or owner_skip_approval token names one of the four QC phases (copy / prompt / typography / shift) \u2014 QC phases are STRUCTURALLY UNSKIPPABLE; no owner record can waive a QC phase | A skip record naming P1Q-COPY-QC / P-PROMPT-QC / P-TYPO-QC / P-SHIFT-QC is REFUSED by load_skip_approvals + check_phase_preconditions + the pre-delivery guard; the QC phase stays required (AF-QC-SKIP) |
...
| AF-OCR-ENGINE-MISSING | Phase-0 | DECK | No OCR engine (pytesseract / Pillow / the tesseract binary) is available in the exact interpreter/environment that will render this deck | ocr_engine_preflight(run_dir): MASTER-SPEC 7.4 — "the missing OCR dependency fails at minute zero, before any paid generation, not after 62 images." Probes prompt_gate.ocr_engine_diagnostic() in-process; HARD ABORTS before ANY phase is dispatched (run_signature_deck.phase0_preflight) and, defense-in-depth, before any render in build_deck.py's own direct-invocation Phase-0 block. Distinct from the postflight OCR-readback sidecar audit, which can only fire once a slide has already been rendered and paid for. Skipped only in --adhoc mode (forced empty api_key — never spends real credit regardless of OCR state) |
...
| AF-AGENT-ENV-MISSING | Phase0 | DECK | The presentation agent runtime env does not carry MC_API_TOKEN or MISSION_CONTROL_URL (FIX-14 / Error 8 / D-8) | run_signature_deck.py phase0_preflight via check_agent_env.py (live-process-first across the gateway service-env + secrets stores): missing label -> HARD ABORT exit 4 AF-AGENT-ENV-MISSING before any phase; repair = regenerate-gateway-env.sh + gateway restart |
| AF-AGENT-ENV-UNMANAGED | Phase0 | DECK | MC_API_TOKEN / MISSION_CONTROL_URL present in a store but NOT in OPENCLAW_SERVICE_MANAGED_ENV_KEYS (a regeneration will drop them — the 15-day regression) | run_signature_deck.py phase0_preflight via check_agent_env.py: label present but unmanaged -> HARD ABORT exit 4 AF-AGENT-ENV-UNMANAGED; repair = regenerate-gateway-env.sh + gateway restart |
| AF-AGENT-ENV-UNKNOWN | Phase0 | DECK | check_agent_env.py is not co-located beside run_signature_deck.py, so the FIX-14 regression guard cannot run | run_signature_deck.py phase0_preflight: missing probe module -> HARD ABORT exit 4 AF-AGENT-ENV-UNKNOWN (fail-closed; a missing probe must never read as PASS) |
| AF-TOOL-SCHEMA-LOOP | Phase0 | DECK | The model serialized tool args as a STRING instead of a JSON object (FIX-18 / Error 10 / D17: 12x args-must-be-object, 3x missing path in the E2E) and a tool hit 5 CONSECUTIVE malformed-args failures | tool_schema_validator.py returns a NORMALIZED schema hint (e.g. write(path: string, content: string) — path is REQUIRED, not file) on a malformed call so the model self-corrects in ONE turn; a tool's 5th consecutive failure writes an AF-TOOL-SCHEMA-LOOP event to working/checkpoints/tool_schema_events.json; run_signature_deck.py phase0_preflight HARD ABORTS exit 4 on a non-empty loop-event ledger before any phase |
| AF-OVERLAY-DELIVERED | 5/6/Postflight | slide | A delivered slide ships native on-slide text (or a pptx_text_overlays.json is present) instead of a single composed image (GPT-Image-2.5 default, or the legacy gpt-image-2 route for 3:1 / 1:3 / 9:21 per Ruling 6) | _chk_no_overlay(run_dir): fails if any eliminated pptx_text_overlays.json file is present OR the delivered PPTX carries any native (non-notes) on-slide text run; the native-overlay path is eliminated by construction; persistent garble re-prompts/re-seeds then escalates to a human, never a native overlay; strengthens AF-BAKED/AF-I14 (Decision 5C) |
| AF-CANONICAL-RENDER-BYPASS | 4/6/Postflight | DECK | The deck — or any slide PNG — was produced by a path other than the ONE canonical render surface build_deck.py + run_signature_deck.py kie.ai path (GPT-Image-2.5 default; the legacy gpt-image-2 route is equally canonical for 3:1 / 1:3 / 9:21 per Ruling 6, 2026-09-09) | check_canonical_render_path(run_dir): SHARED CONTRACT (FIX 2 + FIX 9). Fires when process_manifest.json carries no phase=='render' attestation, when a hand-rolled renderer/assembler in working/*.py (a direct kie createTask, a 2048x1152 slide-canvas builder, or a native add_textbox assembler) produced or could produce the pixels, or when any slide cannot be tied to a canonical kie taskId / recordInfo trace. MAY be skipped ONLY by an explicit, logged owner/founder approval token in process_manifest.json (owner_skip_approval) — never silently, never by an agent's own choice |
| AF-LOCAL-CANVAS | 4/5/6 | slide | A slide image was fabricated LOCALLY instead of baked by kie.ai (GPT-Image-2.5 default, or the legacy gpt-image-2 route for 3:1 / 1:3 / 9:21 per Ruling 6) | check_canonical_render_path(run_dir): SHARED CONTRACT (FIX 2 + FIX 9). Fires when a working/*.py script constructs a 2048x1152 (or full-bleed) slide canvas via PIL Image.new / ImageDraw, when a slide PNG falls below the PLACEHOLDER_MIN_BYTES (51,200-byte) kie-bake floor, or when a pure-typography hook slide carries no real kie taskId. The ONLY PIL step permitted is the locked-logo composite (SOP-IMG-05). Strengthens AF-BAKED/AF-I14. MAY be skipped ONLY by a logged owner_skip_approval token |
...
| AF-OCR-READBACK | Postflight | DECK | A rendered slide's baked text was never read back against its approved copy (missing sidecar or checked:false), or the readback found a mismatch (matched:false) | check_ocr_readback(run_dir): U027 postflight gate. Every rendered PNG must carry a renders/<slide>.ocr.json sidecar written by _record_ocr_readback()/prompt_gate.ocr_readback(). Missing/unparseable sidecar = FAIL; checked:false (OCR engine never ran against the render) = FAIL and NEVER waivable, no owner_skip_approval token can waive it; checked:true + matched:false = FAIL, waivable ONLY by a logged owner_skip_approval token for AF-OCR-READBACK in process_manifest.json. Defers ONLY pre-render (no rendered PNGs yet) |
...
| AF-RESEARCH-REACHES-RENDER | 4/Render | DECK | Research that the weave gate validated in slides_copy.md never reached the RENDER COPY — the deck renders WITHOUT the approved research | _chk_research_reaches_render(run_dir, slides_path): reads the ACTUAL positional slides.json the renderer will bake (H3, the same file _load_slide_copy_map feeds the verbatim/prompt gates) and FAILS when a research-mapped non-exempt slide's anchor token is absent from that slide's render copy[]. This closes the divergence where slides_copy.md passes AF-RESEARCH-WEAVE but slides.json (the renderer's only copy source) carries no research. Defers pre-map/pre-copy (owned upstream); hook/pure-type/transition exempt slides excluded |
...
| AF-OBI | 1Q | slide | Generic One-Big-Idea violation catch-all — the OBI dimension label for any unclassified multi-idea defect; specific violations are carried by AF-OBI-1..6 sub-codes; this base code labels the dimension in routeback work orders and engine-name tables | HUMAN VERDICT REQUIRED (FIX 18) — still not a machine rule (its trigger self-describes as a label); a relevant QC attestation cannot pass until working/qc/human_craft_verdicts.json carries an independent reviewer's pass/fail for AF-OBI alongside AF-OBI-4 (builder/self verdicts rejected, fail-closed). Never presented as measured automation. |
...
| AF-CC-UNREGISTERED | Process/Closeout | DECK | Closeout with no cc_task_id and no logged deliverable-register attempt (fail-closed on never-attempted; fail-soft on transport) | build_deck._chk_cc_registered: reads working/checkpoints/process_manifest.json; fails if cc_task_id absent AND cc_register_attempted absent. Symbol: _chk_cc_registered. Enforced_by: build_deck. |
| AF-CC-UNVERIFIED | Process/Closeout | DECK | Closeout where CC registration could NOT be confirmed consistent: cc_register_attempted=True but no cc_task_id (transport/partial failure or outage), or a cc_task_id whose cc_registration linkage tag does not match (not provably produced by a real cc_board.ingest_deck_task round-trip). NOTE (honesty fix, 2026-08-18): the tag is a same-file consistency check, not a cryptographic proof — a match is a hint, never authorization; no consumer treats it otherwise. Could-not-confirm-consistency NEVER counts as verified — the gate fails with an explicit honest message; a genuine Command Center outage is surfaced truthfully, never read as verified (fail-soft) | build_deck._chk_cc_registered: "VERIFIED" (gate pass) only when cc_task_id + matching cc_registration linkage tag; cc_register_attempted=True with no matching tag -> AF-CC-UNVERIFIED fail; neither field -> AF-CC-UNREGISTERED fail-closed. Symbol: _chk_cc_registered / _cc_registration_linkage_matches. Enforced_by: build_deck. |
| AF-SP-TYPE-UNDECLARED | Intake | DECK | Signature Presentation: SP signals present (an sp_intake.json / a set Signature frame / a frame-selection question / a 'signature presentation' request) but intake.json does not declare deck_type==signature_presentation (a signature deck built through the generic path by omitting the magic word) | build_deck._chk_sp_claim -> prove_sp_routing.evaluate_run_dir. Runs for EVERY deck (does NOT defer) — the router that guards the three deferring SP gates |
| AF-SP-8Q-MISSING | Intake | DECK | Signature Presentation: any of the 8 Questions (q1..q8, especially q7 the offer question) is missing or empty | build_deck._chk_sp_intake -> prove_sp_intake.evaluate. DEFERS (no-op) unless intake.json deck_type==signature_presentation |
| AF-SP-8Q-SPLIT | Intake | DECK | Signature Presentation: the 8 Questions + the frame-selection question were not delivered as ONE message block | build_deck._chk_sp_intake -> prove_sp_intake. DEFERS unless deck_type==signature_presentation |
| AF-INTAKE-BATCH | Intake | DECK | Signature Presentation intake CONVERSATION: two or more bank questions dumped in one assistant turn (BATCH-IN-TURN), two or more unmatched questions in one turn (BATCH-BY-QMARKS), no quick-vs-in-depth choice in the opening assistant turn (NO-CHOICE-OPENER), or the banned batch anti-pattern phrase (BANNED-PHRASE). An absent/unparseable working/interview/intake_transcript.json is fail-closed | build_deck._chk_sp_intake_trace -> intake_trace_check.scan_transcript. Proves the conversation that produced the record, where AF-SP-8Q-* prove the record itself. DEFERS unless deck_type==signature_presentation |
| AF-SP-FRAME-UNSET | Intake | DECK | Signature Presentation: no frame-selection question, or signature_frame not one of rulebook/vault/quest/original | build_deck._chk_sp_intake. DEFERS unless deck_type==signature_presentation |
| AF-SP-TYPE-MISMATCH | Intake | DECK | Signature Presentation: the sp_intake record declares a deck_type other than signature_presentation | build_deck._chk_sp_intake. DEFERS unless deck_type==signature_presentation |
| AF-SP-OFFER-UNDECLARED | Intake | DECK | Signature Presentation: q7 offer/product name(s) not carried into the offer_token_ledger | build_deck._chk_sp_intake. DEFERS unless deck_type==signature_presentation |
| AF-SP-SLIDE-FLOOR | Structure | DECK | Signature Presentation: fewer than 100 slides with no logged client-exact override | build_deck._chk_sp_structure -> prove_sp_structure.verify. DEFERS unless deck_type==signature_presentation |
| AF-SP-PHASE-RANGE | Structure | DECK | Signature Presentation: a phase under its floor (avatar 11 / story 13 / teaching 36 / pitch 40) | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
| AF-SP-PHASE-ORDER | Structure | DECK | Signature Presentation: phases not contiguous-from-slide-1 in order avatar-story-teaching-pitch (or invalid slide/phase) | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
| AF-SP-PHASE-LABEL | Structure | DECK | Signature Presentation: a phase carries no label slide (its name + purpose) | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
| AF-SP-IMG-SUGGESTION | Structure | slide | Signature Presentation: a slide has an empty / whitespace-only suggested_image | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
| AF-SP-CASESTUDY-CAP | Structure | DECK | Signature Presentation: more than 2 CASE_STUDY-tagged slides (cap 2, floor 1), or a slide missing its tags array | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
| AF-SP-TEACH-STEPS | Structure | DECK | Signature Presentation: the teaching phase does not carry 3 to 7 teaching steps | build_deck._chk_sp_structure. DEFERS unless deck_type==signature_presentation |
...
| AF-U-SALES-BUILD | 8.75 | DECK | the upsell sales page (P-U-SALES-BUILD) was elected (pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes') but working/sales-checkout/html/sales.html is absent/empty, or working/sales-checkout/build_receipt.json carries no real preview_urls/funnel_id (placeholder or fabricated receipt) | sales_checkout_builder.verify_push_receipt (P-U-SALES-BUILD executor, corrected 2026-08-19 after C2's script landed with no --mode flag: one invocation builds both pages): asserts sales.html exists and the shared build_receipt.json is a real, non-placeholder GHL push receipt. DEFERS (exit 0, writes nothing new) unless intake.json pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes'; a logged 'no' waiver (SALES_CHECKOUT_DECLINED_REASON, verbatim, non-empty) gates the phase OUT entirely — silence is never consent. |
| AF-U-CHECKOUT-BUILD | 8.76 | DECK | the upsell checkout page (P-U-CHECKOUT-BUILD) was elected (same flag as AF-U-SALES-BUILD) but working/sales-checkout/html/checkout.html is absent/empty, or the shared build_receipt.json fails verify_push_receipt | sales_checkout_builder.verify_push_receipt (P-U-CHECKOUT-BUILD executor; re-invokes the SAME script as P-U-SALES-BUILD with --skip-design, which per its own docstring spends no kie.ai money, purely to give the checkout page its own tracked gate). DEFERS (exit 0) unless intake.json pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes'; same waiver gate as AF-U-SALES-BUILD (one flag covers both pages). |
| AF-U-FORM-CHECKOUT | 8.77 | DECK | STATUS (2026-08-19, confirmed by full-file search of sales_checkout_builder.py, independently corroborated by SALES-CHECKOUT-BUILDER-SOP.md's own audit): the checkout page's payment/lead-capture form has NO dedicated implementation yet anywhere in the department — this code currently fires on the SAME placeholder-receipt condition as AF-U-CHECKOUT-BUILD and does not yet independently prove a form/payment field was wired | sales_checkout_builder.verify_push_receipt (P-U-FORM-CHECKOUT executor, interim placeholder: re-invokes the same script with --skip-design, no kie.ai spend). DEFERS (exit 0) unless intake.json pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes'. MUST be re-scoped to a real form/payment-field check once a follow-up unit implements SALES-CHECKOUT-BUILDER-SOP.md §4 "THE CHECKOUT FORM". |
| AF-U-GHL-SALES | 6.2 | DECK | PRES-011: the elected sales+checkout funnel install (P-U-GHL-SALES) has no verified install proof -- working/sales-checkout/ghl_build_receipt.json absent, schema/status wrong, run or location unbound (stale/foreign/WRONG_LOCATION), inputs changed since install, or no adapter remote IDs (funnel_id/page_ids) with matching-location remote readback and ops-ledger provenance (execution_id). A plan-emitted/model-authored receipt is pending_external_execution, never complete | ghl_external_installer.validate_receipt + phase_verifiers._verify_ghl_sales_install (P-U-GHL-SALES executor: python3 scripts/ghl_external_installer.py --run-dir {run_dir} --phase P-U-GHL-SALES, Skill 06 page-install). DEFERS unless pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes'; a logged 'no' waiver gates the phase OUT entirely. |
| AF-U-GHL-VSL | 6.4 | DECK | PRES-011: the elected VSL funnel install (P-U-GHL-VSL) has no verified install proof -- working/vsl/ghl_build_receipt.json absent, schema/status wrong, run or location unbound, inputs changed since install, or no adapter funnel/page IDs with matching-location remote readback and ops-ledger provenance. Reuses the verified sales funnel_id when this run already pushed sales+checkout; never a second funnel for one client | ghl_external_installer.validate_receipt + phase_verifiers._verify_ghl_vsl_install (P-U-GHL-VSL executor: python3 scripts/ghl_external_installer.py --run-dir {run_dir} --phase P-U-GHL-VSL, Skill 06 page-install). DEFERS unless pre_presentation_capture.WANT_VSL_PAGE == 'yes'; a logged 'no' waiver gates the phase OUT entirely. |
| AF-U-FORM-GATE | 5.6 | DECK | PRES-011: the elected VSL gate form+workflow (P-U-FORM-GATE) has no verified install proof -- ecosystem/gate-form.json or workflows/gate-workflow.json absent, or either without adapter form_id/workflow_id, matching-location readback, run/input binding and ops-ledger provenance (SKILL44_WIDGET -> FORM per vsl-gate-form-spec.json; workflow DRAFT-only). Partial state (form created, workflow missing) resumes without duplicating | ghl_external_installer.validate_receipt + phase_verifiers._verify_form_gate_install (P-U-FORM-GATE executor: python3 scripts/ghl_external_installer.py --run-dir {run_dir} --phase P-U-FORM-GATE, Skill 44 form/workflow). DEFERS unless pre_presentation_capture.WANT_VSL_PAGE == 'yes'; a logged 'no' waiver gates the phase OUT entirely. |
| AF-U-VSL-BUILD | 8.93 | DECK | the upsell VSL page (P-U-VSL-BUILD) was elected (pre_presentation_capture.WANT_VSL_PAGE == 'yes') but working/vsl/html/vsl.html is absent/empty, verify_video_dependency() cannot resolve the P9.6-produced working/delivery/{deck_slug}-WEBINAR.mp4 (AF-VSL-NO-VIDEO in the executor's own exit-code contract), or working/vsl/build_receipt.json fails verify_push_receipt | vsl_builder.verify_video_dependency + vsl_builder.verify_push_receipt (P-U-VSL-BUILD executor): resolves the REAL P9.6 webinar video path via build_webinar_video.py's own WEBINAR_FILENAME_TEMPLATE (never a literal string), fails closed if absent, and asserts vsl.html + a real, non-placeholder GHL push receipt. Order 8.93, strictly after P9.6-WEBINAR-VIDEO (8.92). DEFERS (exit 0) unless intake.json pre_presentation_capture.WANT_VSL_PAGE == 'yes'; a logged 'no' waiver (VSL_PAGE_DECLINED_REASON, verbatim, non-empty) gates the phase OUT entirely. |
...
**Producing-role requirement (Slide Image Creator):** Every non-pure-typography slide must specify a real generated raster (Kie / GPT-Image-2.5 by default, or Kie's legacy gpt-image-2 route for slides pinned at 3:1 / 1:3 / 9:21 per Ruling 6) at >=1920px on the long edge, full-bleed or designed-zone, sourced from the Category E grounded anchor. Decorative icon-font glyphs, single-color clip-art PNGs <=256px, and emoji-as-iconography are forbidden as slide content art.
...
2. **Model pin held on a converter run:** the latest `process_manifest.json` render record `model_used` == `intake.json model_pin` with no undocumented fallback (re-uses AF-MODEL-SOVEREIGNTY logic). The GPT-Image-2.5 pin applies to converter runs exactly as it applies to regular runs — including the legacy gpt-image-2 exception for slides pinned at 3:1 / 1:3 / 9:21 per Ruling 6, which is not a parity violation.
...
**Failure message:** `AF-CONVERTER-PARITY: DECK FAIL -- converter-origin deck (source_brief_origin: content-to-presentation-architect) failed the runtime parity gate. Failed conditions: {list}. A converter deck must use the identical pipeline as a regular build: canonical renderer + the client's pinned model (GPT-Image-2.5 default, or the legacy gpt-image-2 route for 3:1 / 1:3 / 9:21 per Ruling 6) + real vision QC + Phase -0.5 Research Brief (Category E + F) + propagated persuasion variables. Correct the failing conditions and re-run QC.`
...
---
## slide-craft
The `slide_craft` module provides deterministic enforcers for craft rules that are arithmetic — text-block counts, word counts, substring scans, fuzzy-match comparisons, slide-number gaps, and depth-percentage checks. All ten public checks ship in warn-mode (Rule 3.5) and defer on missing inputs. The rules that remain purely human are marked HUMAN VERDICT REQUIRED in the Section-5 table above and gate run_signature_deck.attest_phase via craft_judgement.attestation_blocker. `sop_ref` values for craft codes point here.
