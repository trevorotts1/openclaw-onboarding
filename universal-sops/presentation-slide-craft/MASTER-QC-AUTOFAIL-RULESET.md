...
| AF-HOOK-2 | 1Q/5/6 | slide | hook in a footer/band position | HUMAN JUDGEMENT — the prompt half is already enforced as AF-HOOK; the render half's named input TEXT_ANCHOR cannot exist (slides.schema.json is additionalProperties:false with six properties). |
...
| AF-HOOK-5 | 1Q/5/6 | slide | hook mutated/extended/reworded | build_deck._chk_hook_verbatim → slide_craft.check_hook_verbatim: char-exact compare of every copy occurrence against the canonical hook (intake.json hook key); near-variants caught by difflib.SequenceMatcher >= 0.82 (prompt_gate.py OCR_MATCH_RATIO). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-HOOK-6 | 5/6 | slide | hook misspelled/garbled on render | COVERED BY AF-SPELLING (U047, not yet merged) — spell/glyph check on rendered hook line, owned by U047's slide_geometry.check_spelling. |
| AF-HOOK-7 | 1Q/5 | slide | signature quote conflated with main hook | HUMAN JUDGEMENT — "main hook present on the signature-quote slide" needs a signature-quote label; no such label exists in slides.schema.json, in the one arc_allocation.json, or in _arc_tags_in_order's token vocabulary. |
| AF-AUD-1 | 1Q/5 | slide | speaker SAY line on the face | HUMAN JUDGEMENT — presenter-speech phrasing is judgement; the SOP's own markers include first-person we/I which fires on any first-person headline. The exact alternative (SAY-tag set intersection) needs working/copy/audience_say_tags.json — a whole-repository census found zero such files and zero readers. Producer gap, not a checker gap. |
| AF-AUD-2 | 1Q/5 | slide | internal pitch doctrine as caption | HUMAN JUDGEMENT — "restates a Section 4.3 principle" is paraphrase detection; no computable test exists. |
| AF-AUD-3 | 1Q/5 | slide | image-narration caption | HUMAN JUDGEMENT — "caption describes what the image already shows" is cross-modal semantics; the same vision work U047 marked senior. |
| AF-AUD-4 | 1Q/5 | slide | meta-telegraph / "webinar" / technique label | build_deck._chk_aud_meta_tokens → slide_craft.check_aud_meta_tokens: LITERAL TOKENS ONLY — matches the word "webinar" and the named announcement lines case-insensitively. An unlisted meta line is not detected and remains HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-AUD-5 | 1Q/5 | slide | credential/justification dump | build_deck._chk_aud_credentials → slide_craft.check_aud_credentials: substring scan of non-headline copy blocks for credential markers (licensed, clinical, years in, certified, credential, accredited). Headline (copy[0]) is exempt. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-PLACEHOLDER (AF-AUD-6) | 5/6 | slide (blocks FINAL) | bracket/placeholder token on a rendered slide | build_deck._chk_aud_placeholder_render → slide_craft.check_aud_placeholder_render: same enforcer as AF-AUD-6 — one check, two manifest rows. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-OBI-1 | 1Q/5 | slide | > 3 text blocks | build_deck._chk_obi_text_blocks → slide_craft.check_obi_text_blocks: count of non-empty copy blocks per slide in slides.json; ceiling 3 (SOP-SLIDE-01 §2.3). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
...
| AF-OBI-4 | 1Q | slide | full value trio on one slide | HUMAN JUDGEMENT — three parallel named values (alliterated trio) is a literary judgement; a marker list would fire on any three-noun headline. |
| AF-OBI-5 | 1Q | slide | bulleted pain list | HUMAN JUDGEMENT — distinguishing a pain from a feature is semantic; no computable test exists for 2+ distinct pain statements. |
| AF-OBI-6 | 5 | slide | comparison table > 2 rows | HUMAN JUDGEMENT — rendered contrast-row count > 2 requires table structure from a baked image; falls into the same glyph-analysis class U047 carved out as senior. |
| AF-DEN-1 | 1Q/6 | DECK | price beats < 8 slides apart | build_deck._chk_den_ladder_gaps → slide_craft.check_den_ladder_gaps: adjacent price_ladder.json rung gap computed from target_slide; floor 8 slides (SOP-SLIDE-04 §2.1). Respects client_requested_slide_count. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-2 | 1Q/6 | DECK | anchor outside 25-45% depth | build_deck._chk_den_anchor_depth → slide_craft.check_den_anchor_depth: anchor.target_slide / total slides; band 25-45% (SOP-SLIDE-04 §3 DEN-2 trigger row, not §2.2 prose 30-40%). Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-3 | 1Q/6 | DECK | DROP with no BUILDUP before it | HUMAN JUDGEMENT — "DROP with no BUILDUP immediately before it" needs adjacency; _arc_tags_in_order returns character offsets and chk_cadence:135-138 calls its own window mapping "crude" and an "approximation". An enforcer here returns a confident wrong number. |
| AF-DEN-4 | 1Q/6 | DECK | no value-stack slide before Drop 1 | build_deck._chk_den_stack_before_drop → slide_craft.check_den_stack_before_drop: ORDER ONLY — proves a value-stack ARC beat precedes the first DROP beat via marker offsets. It does NOT prove the stack is itemized or that its total exceeds the anchor (SOP-SLIDE-04 §2.4) — those remain HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
...
| AF-DEN-6 | 1Q/6 | DECK | Wall of Wins not 4-6 before offer | HUMAN JUDGEMENT — needs a slide gap (4-6) for the Wall of Wins, which carries no price_ladder.json rung and therefore no target_slide. |
| AF-DEN-7 | 1Q/6 | DECK | no 4-7 slide re-pitch after FINAL | build_deck._chk_den_repitch_block → slide_craft.check_den_repitch_block: COUNT ONLY — proves 4-7 slides follow the FINAL price via total_slides - final_rung.target_slide. It does NOT prove they recap the stack, restate the promises or reset the urgency — those remain HUMAN JUDGEMENT. Warn-mode until PRESENTATION_SLIDE_CRAFT_ENFORCE=1. |
| AF-DEN-8 | 1Q/6 | DECK | section below its slide floor | HUMAN JUDGEMENT — SOP-SLIDE-04 §2.8 floors are declared "scaled to deck length" with no scaling function given, and §5.0 makes them non-binding whenever the client fixed the count. A gate with no computable threshold and a doctrinal override is not a gate. |
