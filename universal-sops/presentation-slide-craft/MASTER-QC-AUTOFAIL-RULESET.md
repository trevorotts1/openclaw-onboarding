...
| AF-PHASE-REPORT-START | Runner | DECK | A long_running / milestone phase was dispatched with no emitted client START report | run_signature_deck.py emit_client_report(start): sends ONE message via `openclaw message send` (never the Telegram API) and records its gateway_msg_id in process_manifest.json client_reports[]; the runner refuses to advance a report-required phase whose start report is missing. Closes the deep-research silent gap (Decision 3D) |
| AF-PHASE-REPORT-DONE | Runner | DECK | A report-required phase attested with no emitted client DONE report, or the done report preceded a passing substance verifier | run_signature_deck.py attest_phase() is INVALID without a confirmed start+done client_reports pair; a failed phase_verifiers substance check BLOCKS the done report. 'Done' means the client was told AND the artifact is real, never just a file on disk (Decision 3D) |
| AF-PROCESS-INTEGRITY | Runner/Closeout | DECK | The end-of-process no-skip proof found a skipped, out-of-order, empty-sha, substance-failed, or unreported declared step | prove-deck.py walks declared_plan.json + the attestation chain: artifact_sha is mandatory + verified before any attest (FM-3); self-authored / placeholder-timestamp skip approvals are rejected (FM-4); a full pass writes PROCESS-CERTIFICATE.json (process_certificate_sha); only bypass is a logged verifiable owner_skip_approval. Exit 9 on any failure (Decision 3D) |
| AF-CC-UNREGISTERED | Closeout | DECK | Closeout reached with no Command Center registration receipt for the deck | scripts/cc_board.py chk_cc_registered(run_dir): process_manifest.json must carry a cc_registration receipt (task_id + attempted:true); the fail-soft producer boards the deck at job-start (POST /api/tasks/ingest, department_slug:presentations) and PATCHes phase boundaries. FAIL-CLOSED on never-attempted (boarding is a deterministic postflight step, not an agent choice), FAIL-SOFT on transport (Decision 3D) |
