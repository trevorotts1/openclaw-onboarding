...
**COVERAGE, STATED HONESTLY (U051).** A row in this table means the rule is DECLARED. It does not mean the rule is MEASURED. Of the 27 numbered craft triggers in SOP-SLIDE-01/02/03/04 plus the 4 craft-prefixed codes that belong to other batteries — 31 rows in total — 15 carry a resolving enforcer, 2 are enforced under another code's name and say so in their Detection cell, 1 is owned by a unit not yet merged, and 13 are decided by a human and are marked HUMAN JUDGEMENT in their Detection cell. Checksum: 15 + 2 + 1 + 13 = 31. A Detection cell that begins HUMAN JUDGEMENT is a promise that a person reads the slide, not a promise that a machine does.
...
| AF-HOOK-2 | 1Q/5/6 | slide | hook in a footer/band position | HUMAN JUDGEMENT — the render half needs TEXT_ANCHOR, which cannot exist: slides.schema.json is additionalProperties:false with six properties. The prompt half is already enforced as AF-HOOK. |
...
| AF-HOOK-7 | 1Q/5 | slide | signature quote conflated with main hook | HUMAN JUDGEMENT — main hook present on a signature-quote slide; no such label exists in slides.schema.json, in the one arc_allocation.json, or in _arc_tags_in_order's token vocabulary. |
| AF-AUD-1 | 1Q/5 | slide | speaker SAY line on the face | HUMAN JUDGEMENT — presenter-speech phrasing is judgement; the SOP's own markers include first-person we/I, which fires on any first-person headline. The exact route needs audience_say_tags.json which does not exist (producer gap, U012). |
| AF-AUD-2 | 1Q/5 | slide | internal pitch doctrine as caption | HUMAN JUDGEMENT — restates a Section 4.3 principle; paraphrase detection. |
| AF-AUD-3 | 1Q/5 | slide | image-narration caption | HUMAN JUDGEMENT — caption describes what the image already shows; cross-modal semantics, the same vision work U047 marked senior. |
...
| AF-OBI-3 | 1Q | slide | 2+ core ideas | HUMAN JUDGEMENT — distinct-claim count >= 2; SOP-SLIDE-01 §3 assigns this to the QC Specialist ("the QC agent identifies the count of distinct claims"). No enforcer exists and none is honest. |
| AF-OBI-4 | 1Q | slide | full value trio on one slide | HUMAN JUDGEMENT — alliterated-value-trio; alliteration-plus-parallelism is a literary judgement; a marker list would fire on any three-noun headline. |
| AF-OBI-5 | 1Q | slide | bulleted pain list | HUMAN JUDGEMENT — pain-statements count >= 2; distinguishing a pain from a feature is semantic. |
| AF-OBI-6 | 5 | slide | comparison table > 2 rows | HUMAN JUDGEMENT — contrast-row count via glyph analysis on baked renders; AF-OVERLAY-DELIVERED guarantees there is no text layer. Same glyph-analysis class U047 carved out as senior. |
...
| AF-DEN-3 | 1Q/6 | DECK | DROP with no BUILDUP before it | HUMAN JUDGEMENT — DROP with no BUILDUP immediately before it; _arc_tags_in_order returns character offsets, not slide numbers, and chk_cadence:135-138 calls its own window mapping "crude" and an "approximation". An enforcer here returns a confident wrong number. |
...
| AF-DEN-6 | 1Q/6 | DECK | Wall of Wins not 4-6 before offer | HUMAN JUDGEMENT — Wall of Wins 4-6 slides before the offer; the Wall of Wins carries no price_ladder.json rung and therefore no target_slide exists for it at all. |
...
| AF-DEN-8 | 1Q/6 | DECK | section below its slide floor | HUMAN JUDGEMENT — SOP-SLIDE-04 §2.8 floors are declared "scaled to deck length" with no scaling function given, and §5.0 makes them non-binding whenever client_requested_slide_count is set. A gate with no computable threshold and a doctrinal override is not a gate. |
...
---
## slide-craft
The `slide_craft` module provides deterministic enforcers for craft rules that are arithmetic — text-block counts, word counts, substring scans, fuzzy-match comparisons, slide-number gaps, and depth-percentage checks. All ten public checks ship in warn-mode (Rule 3.5) and defer on missing inputs. The rules that remain human judgement are marked HUMAN JUDGEMENT in the Section-5 table above. `sop_ref` values for craft codes point here.
