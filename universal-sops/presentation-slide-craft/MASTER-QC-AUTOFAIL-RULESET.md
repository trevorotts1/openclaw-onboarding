...
| AF-U-FORM-CHECKOUT | 8.77 | DECK | PRES-025: the elected checkout form (P-U-FORM-CHECKOUT) has no real form proof -- working/sales-checkout/checkout_form.json absent/stale, page CTA audit failing (#/empty/javascript: or no order form), scope mismatch (wrong location), or mode contract unmet (lead-capture: status=complete + form_id + workflow_id + one-contact/one-enrollment proof; sandbox: product/amount/currency + success/cancel routes, zero real charge). Phase-scoped blockers (MISSING_OFFER / MISSING_MERCHANT / WRONG_LOCATION / UNSUPPORTED_PAYMENT) soft-checkpoint as NOTE, never deck failure | checkout_form_builder.main + phase_verifiers._verify_upsell_form_checkout (P-U-FORM-CHECKOUT executor). DEFERS (exit 0) unless intake.json pre_presentation_capture.WANT_SALES_CHECKOUT == 'yes'; a logged 'no' waiver gates the phase OUT entirely -- silence is never consent. |
