...
| AF-CAPACITY-UNMEASURED | 0 (pre-dispatch) | DECK | the engine was dispatched, or an execution plan was built, without a capacity measurement for THIS client's account — the probe returned PARKED (provider detected, plan never declared) or FAILED (a declared capacity_override.json is unusable), so no dispatchable number exists. Cap table, exact: ollama-cloud $20/month = 3 concurrent agents, ollama-cloud $100/month = 10, deepseek-direct v4-pro = 500, deepseek-direct v4-flash = 2500; an unknown provider or plan collapses to DEFAULT_CONSERVATIVE = 3 and is NEVER guessed upward (the cap table IS the governor's resource profile, presentation_job/governor.py, and wave width is governed by it) | presentation_job/capacity.py::probe detects provider+plan read-only, in order: (a) capacity_override.json in the department config dir, (b) the 9Router configuration (which provider the primary model routes to), (c) the OpenClaw agent model configuration (provider namespace prefix on the primary model), (d) provider known but plan unknown → emit the one-time interview question and PARK, persisting the answer to capacity_override.json so it is asked ONCE, (e) nothing found → DEFAULT_CONSERVATIVE plus a loud UNDETERMINED line. No step ever reads a credential VALUE. The teeth: launcher.dispatch calls the probe BEFORE any engine process is created and returns DISPATCH_CAPACITY_REFUSED (CLI exit 3) when the number is absent, and execution_plan.build_execution_plan raises CapacityUnmeasured rather than substituting a constant — wave width is min(ready phases, measured available), so a 5-phase independent wave on a measured 3 dispatches 3 then 2 |
...
| AF-SPEECH-PACING | Phase 9 (Speech) | DECK | Fix 97 hard pacing band: |speech words/target_wpm − target_talk_minutes| ÷ target_talk_minutes exceeds the band — the speech does not fill the requested duration at the run's own target pace (target_wpm from intake.json, default 140; the legacy wide 120-140 wpm bracket is advisory under this band) | _speech_pacing_deviation <= SPEECH_PACING_BAND (0.10) at build_deck speech gate; leg 1 AF-SPEECH-SHORT word floor unchanged |
| AF-RENDER-EMPTY | Phase P8-ASSEMBLE | DECK | closeout reached with NO rendered slide PNGs — zero renders, never assemble | renders/ has >=1 slide PNG at closeout; zero fails the deck |
| AF-RENDER-COMPLETE | Phase P8-ASSEMBLE | DECK | render gate failed before full slide output; partial decks refused | full slide set present before assembly |
...
| AF-OVERLAY-DELIVERED | 5/6/Postflight | slide | A delivered slide ships native on-slide text (or the eliminated legacy overlay-ledger file is present) instead of a single composed gpt-image-2 image | _chk_no_overlay(run_dir): fails if any eliminated legacy overlay-ledger file (the retired native-text-overlay ledger; its former name is recorded in the RETIRED appendix of pptx-assembly-specialist.md) is present OR the delivered PPTX carries any native (non-notes) on-slide text run; the native-overlay path is eliminated by construction; persistent garble re-prompts/re-seeds then escalates to a human, never a native overlay; strengthens AF-BAKED/AF-I14 (Decision 5C) |
| AF-CANONICAL-RENDER-BYPASS | 4/6/Postflight | DECK | The deck — or any slide PNG — was produced by a path other than the ONE canonical render surface build_deck.py + run_signature_deck.py kie.ai gpt-image-2 path | check_canonical_render_path(run_dir): SHARED CONTRACT (FIX 2 + FIX 9). Fires when process_manifest.json carries no phase=='render' attestation, when a hand-rolled renderer/assembler in working/*.py (a direct kie createTask, a 2048x1152 slide-canvas builder, or a native PowerPoint text-box overlay assembler) produced or could produce the pixels, or when any slide cannot be tied to a canonical kie taskId / recordInfo trace. MAY be skipped ONLY by an explicit, logged owner/founder approval token in process_manifest.json (owner_skip_approval) — never silently, never by an agent's own choice |
...
| AF-IMAGE-GROUNDING-PARK | Image-QC/closeout | DECK | the image-grounding-steward verdict is missing or failing at closeout: working/qc/image_qc_report.json carries NO image_grounding sub-verdict ({pass:true, reviewed_by:<independent identity>}), the verdict marks pass!=true, or its reviewed_by is a builder/self identity — the run PARKS for human vision review (working/qc/human-review/) instead of shipping. FIX 92 registers the FIX 16 sub-verdict (emitted by phase_verifiers._verify_image_grounding at P-IMAGE-QC) as a closeout gate so an un-parked deck proves its grounding verdict exists; the companion scene-grounding/anchor-parity findings ride the same verifier as AF-IMAGE-GROUNDING. Defers (pass) only when the run has no prompts/anchors to judge and no report at all — at closeout, after render+assembly, the park is binding | build_deck._chk_image_grounding_verdict → phase_verifiers._verify_image_grounding (the SAME sub-verifier the P-IMAGE-QC attestation runs, so closeout and attestation cannot drift; enforced_by closeout_gate, py_symbol _chk_image_grounding_verdict). PRESENTATION_FIX16_QC_WIRING=0 (the documented FIX 16 rollback) also rolls the wrapper back to defer |
| AF-CASTING-PARK | Prompt-QC/closeout | DECK | the representation-casting-director verdict is missing or failing at closeout: working/qc/prompt_qc_report.json carries NO representation_casting sub-verdict ({pass:true, reviewed_by:<independent identity>}), the verdict marks pass!=true, its reviewed_by is a builder/self identity, or the verifier's cast-parity checks fail (an invented demographic default, or a declared representation_mix no people-bearing prompt honors — the verifier's own AF-CASTING / AF-CASTING-MIX-PARITY findings) — the run PARKS for human review (working/qc/human-review/) instead of shipping. FIX 92 registers the FIX 16 sub-verdict (emitted by phase_verifiers._verify_representation_casting at P-PROMPT-QC) as a closeout gate. Defers (pass) only when the client declined representation_mix capture and no forbidden default is present | build_deck._chk_representation_casting_verdict → phase_verifiers._verify_representation_casting (the SAME sub-verifier the P-PROMPT-QC attestation runs; enforced_by closeout_gate, py_symbol _chk_representation_casting_verdict). PRESENTATION_FIX16_QC_WIRING=0 also rolls the wrapper back to defer |
