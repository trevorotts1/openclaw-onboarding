...
| AF-CC-UNVERIFIED | Process/Closeout | DECK | Closeout where CC registration could NOT be confirmed self-consistent: cc_register_attempted=True but no cc_task_id (transport/partial failure or outage), or a cc_task_id whose cc_registration linkage tag does not match (corrupted/stale/hand-edited manifest). NOTE (fakehmac fix, 2026-08-18): the tag is a same-file sha256 consistency check, not a cryptographic signature — no secret is available to this offline gate, so a MATCH is a hint only (internal consistency), never proof that a live cc_board.ingest_deck_task round-trip occurred. Could-not-confirm-consistency NEVER counts as verified — the gate fails with an explicit honest message; a genuine Command Center outage is surfaced truthfully, never read as verified (fail-soft) | build_deck._chk_cc_registered: passes ("") only when cc_task_id + a matching cc_registration linkage tag; cc_register_attempted=True with no match -> AF-CC-UNVERIFIED fail; neither field -> AF-CC-UNREGISTERED fail-closed. Symbol: _chk_cc_registered / _cc_registration_linkage_matches. Enforced_by: build_deck. |
