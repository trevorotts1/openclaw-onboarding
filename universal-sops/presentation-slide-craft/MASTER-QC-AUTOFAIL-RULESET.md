...
| AF-CC-UNVERIFIED | Process/Closeout | DECK | Closeout where CC registration could NOT be verified: cc_register_attempted=True but no cc_task_id (transport/partial failure or outage), or a cc_task_id whose cc_registration HMAC receipt does not verify (not provably produced by a real cc_board.ingest_deck_task round-trip). Could-not-verify NEVER counts as verified — the gate fails with an explicit honest message; a genuine Command Center outage is surfaced truthfully, never read as verified (fail-soft) | build_deck._chk_cc_registered: VERIFIED only when cc_task_id + verifying cc_registration HMAC; cc_register_attempted=True with no proof -> AF-CC-UNVERIFIED fail; neither field -> AF-CC-UNREGISTERED fail-closed. Symbol: _chk_cc_registered. Enforced_by: build_deck. |
