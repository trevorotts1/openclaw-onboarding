...
6. **The Wall of Wins sits about 5 slides before the final offer** (4 to 6 acceptable), with a build-up run between, never jammed 2 slides against it. (reference run: s68 -> s73.) See the Wall of Wins SOP in the offer cluster for its content rules; this SOP enforces only its SPACING.
7. **A 4-to-7-slide re-pitch block follows the FINAL price** (recap the stack, restate the promises, reset the urgency), before the send-off. (reference run: s74-75.) A deck that closes on a plain thank-you with no re-pitch fails.
...
**FAIL (reference failure case ladder):** anchor s32, Drop 1 s34 (gap 2), Drop 2 s37 (gap 3), final s43 (gap 6) -> DEN-1 fails all three gaps; DEN-2 fails (anchor at 71% depth).
**PASS (gold-standard reference model):** anchor at ~one-third, then gaps of 11, 16, 14, 8 across the back two-thirds.

**FAIL (reference failure case):** no itemized value stack (slide 39 only re-asserts $[ANCHOR] without summing components) -> DEN-4. No promises slide before the anchor -> DEN-5. No re-pitch after the final price (closes on a moment slide + thank-you) -> DEN-7.
...
**FAIL (reference failure case):** Wall of Wins (s40) 3 slides before the offer (s43) -> DEN-6.
**PASS:** Wall of Wins ~5 slides before the offer with a build-up run between.

**FAIL (reference failure case overall):** deck too thin to space the beats (the operator: "needs roughly 8-10 MORE slides") -> DEN-8 across teaching and offer sections.
...
**0. THE CLIENT'S EXPLICIT SLIDE COUNT IS A FIXED LENGTH AND OVERRIDES THESE DENSITY FLOORS.** When the client stated an exact number (`client_requested_slide_count`), the deck length is FIXED at that number and honored EXACTLY (`AF-SLIDE-COUNT-EXACT`). The ladder, the 8-slide gap, the Wall-of-Wins spacing, and the section floors must be fit WITHIN the client's chosen length — the Director does NOT add slides past the client's number to satisfy a gap, and NEVER asks the client to accept a different number. If the full slow-drop arc genuinely cannot fit the client's chosen length, that is surfaced to the client as an ARC trade-off (e.g. fewer ladder rungs, a tighter build) for the client to decide — the count itself is never silently changed. Items 2 and 5 below ("lengthen the deck" / "increase the total slide count") apply ONLY when the client gave no explicit count.
